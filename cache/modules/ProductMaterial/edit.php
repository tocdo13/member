<script type="text/javascript">
	var materials = <?php echo String::array2js($this->map['materials']);?>;
</script>
    ...
    <form name="EditProductMaterialForm" method="post" >
        <div class="content">
        	...
        	<table cellpadding="15" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
        		<tr>
                	<td width="70%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><?php echo Portal::language('product_limit');?></td>
                    <td width="30%" align="right" nowrap="nowrap" style="padding-right: 30px;">
        				...
        				<?php if(User::can_add(false,ANY_CATEGORY)){?><a href="<?php echo Url::build_current(array('type'));?>"  class="w3-btn w3-green w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"><?php echo Portal::language('back');?></a><?php }?>
                    </td>
                </tr>
            </table>
            
            <fieldset style="width: 475px;" >
                <legend class="title"><?php echo Portal::language('product');?></legend>
                <?php if(Url::get('cmd')=='edit'){ ?>
                <input type="button" onclick="if(confirm('Are you sure to delete?')) location='<?php echo Url::build_current(array('cmd'=>'remove_all','product_id','product_name','portal_id'));?>'" value="<?php echo Portal::language('delete_material');?>"/>
                <?php }?>
            ...
            <div>
                ...
                <span id="mi_material_product_all_elems">
                    <span class="multi-input-header" style="width:150px;"><?php echo Portal::language('material_id');?></span>
                    <span class="multi-input-header" style="width:200px;"><?php echo Portal::language('material_name');?></span>
                    <span class="multi-input-header" style="width:100px;"><?php echo Portal::language('norm_quantity');?></span>
                ...
                <input class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; margin-top: 5px;" type="button" value="<?php echo Portal::language('add_item');?>" onclick="mi_add_new_row('mi_material_product');$('material_id_'+input_count).focus();myAutocomplete(input_count);"/>
            </div>
        </div>
...
<script type="text/javascript">
product_id_list = <?php echo $this->map['product_id_list_js'];?>;
product_name_list = <?php echo $this->map['product_name_list_js'];?>;

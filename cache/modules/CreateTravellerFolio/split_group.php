...
<form method="post" name="CreateTravellerFolioForm">
    <table width="99%" style="margin:auto " class="invoice">
        <!---<div style="text-align:center; color:#36F; font-size:16px; font-weight:bold; margin-top:0px; text-transform: uppercase;"><?php echo Portal::language('create_folio');?> <?php echo Portal::language('group');?></div>--->
        <tr>
            <td style="float: left;">
            ...
            <td style="float: right;">
                ...
                <select  onchange="OpenFolio(this.value,1);" style="float: right; margin-right: 10px; color: red ; font-weight: bold; padding: 5px; border: 1px solid red; width: 170px; ">
                    <option value="" style="color: red ; font-weight: bold; line-height: 25px;"><?php echo Portal::language('bill_folio');?></option>
                    <?php if(isset($this->map['folio']) and is_array($this->map['folio'])){ foreach($this->map['folio'] as $key3=>&$item3){if($key3!='current'){$this->map['folio']['current'] = &$item3;?>
                        <?php if((int)(abs($this->map['folio']['current']['total']-$this->map['folio']['current']['amount'])) < 1000){ ?>
                            <?php if(isset($this->map['folio']['current']['folio_code'])){?>
                            <option value="<?php echo $this->map['folio']['current']['id'];?>,<?php echo $this->map['folio']['current']['traveller_id'];?>" style="color: red ; line-height: 25px;"> - <?php echo 'No.F'.str_pad($this->map['folio']['current']['folio_code'],6,"0",STR_PAD_LEFT);?></option>
                            <?php } else{?>
                            <option value="<?php echo $this->map['folio']['current']['id'];?>,<?php echo $this->map['folio']['current']['traveller_id'];?>" style="color: red ; line-height: 25px;"> - <?php echo 'Ref'.str_pad($this->map['folio']['current']['id'],6,"0",STR_PAD_LEFT);?></option>
                            <?php } ?>
                        <?php }?>
                ...
                <select  onchange="OpenFolio(this.value,0);" style="float: right; margin-right: 10px; color: green ; font-weight: bold;  padding: 5px; border: 1px solid green; width: 170px;">
                    <option value="" style="color: green ; font-weight: bold; line-height: 25px;"><?php echo Portal::language('preview_order');?></option>
                    <?php if(isset($this->map['folio']) and is_array($this->map['folio'])){ foreach($this->map['folio'] as $key4=>&$item4){if($key4!='current'){$this->map['folio']['current'] = &$item4;?>
                        <?php if((int)(abs($this->map['folio']['current']['total']-$this->map['folio']['current']['amount'])) >= 1000){ ?>
                            <?php if(isset($this->map['folio']['current']['folio_code'])){?>
                            <option value="<?php echo $this->map['folio']['current']['id'];?>,<?php echo $this->map['folio']['current']['traveller_id'];?>" style="color: green ; line-height: 25px;"> - <?php echo 'No.F'.str_pad($this->map['folio']['current']['folio_code'],6,"0",STR_PAD_LEFT);?></option>
                            <?php } else{?>
                            <option value="<?php echo $this->map['folio']['current']['id'];?>,<?php echo $this->map['folio']['current']['traveller_id'];?>" style="color: green ; line-height: 25px;"> - <?php echo 'Ref'.str_pad($this->map['folio']['current']['id'],6,"0",STR_PAD_LEFT);?></option>
                            <?php }?>
                        <?php }?> 
                    <?php }}unset($this->map['folio']['current']);} ?>
                </select>
	...
	<tr id="title">
        <td width="50%">
            <?php echo Portal::language('code');?>:<span id="rr_id" style="text-align:left; color:#0A3680;"> <?php echo (Url::get('add_payment')?Url::get('add_payment'):(Url::get('id')?Url::get('id'):''));?></span>
        ...
        <td width="50%"><span style="text-align:left; color:#0A3680;">
            ...
            <?php if($this->map['folio']['current']['folio_code']){?>
                <?php if(Url::get('folio_id') && $this->map['folio']['current']['id']==Url::get('folio_id')){echo Portal::language('folio').'_'.$this->map['folio']['current']['folio_code'];}?> 
            <?php } else {?>
           ...
           <?php }}unset($this->map['folio']['current']);} ?>
                ...
                <a href="#" class="w3-btn w3-indigo w3-hover-indigo w3-hover-text-white" name="add_payment" type="button" id="add_payment" value="" style="float:right;display:none;margin-right: 5px; text-transform: uppercase; text-decoration: none;" ><?php echo Portal::language('add');?></a>
                <input name="save" type="button" style="float:right;margin-right: 5px; text-transform: uppercase; text-decoration: none;" id="save" value="<?php echo Portal::language('Save_and_stay');?>" class="w3-btn w3-blue w3-hover-blue w3-text-white w3-hover-text-white" onclick="SubmitForm('save');" />
                <input name="save_and_print" type="button" id="save_and_print" class="w3-btn w3-orange w3-hover-orange w3-text-white w3-hover-text-white" style="float:right;height:28px; margin-right: 5px; text-transform: uppercase; text-decoration: none;" value="<?php echo Portal::language('save_view');?>" onclick="SubmitForm('save_and_print');" />
                <?php } ?>
             ...
             <div id="check_add" style="display:none;position:absolute;top:auto;left:auto;border:1px solid #000000;padding:10px;text-align:center;background:#FFFF99;">
				<?php echo Portal::language('add_order_id');?>: <select  name="order_ids" id="order_ids"><?php
    ...
    <tr>
            ...
            <table style="width: 100%;">
                <tr>
                    <td style="border-right: 1px solid #cdcdcd;">
                        <?php
                            foreach($this->map['all_service'] as $val){
                          ?>
                         <input name="all_room_rate" value="<?php echo $val; ?>" type="button"  id="all_room_rate"  onclick="checkSplit('ALL','<?php echo $val; ?>','');" title="<?php echo Portal::language('pay_all_room_amount');?>" style="margin:2px 0;"/>
        ...
        <td style="vertical-align: bottom;">
            <div style="width: 100%; margin: 0px auto; float: left;">
                <span>&nbsp;&nbsp;<?php echo Portal::language('customer');?>: </span><?php echo $this->map['customer_name'];?>
                <input name="customer_id" type="hidden" id="customer_id" value="<?php echo $this->map['customer_id'];?>" />
                <input  name="traveller_folio" id="traveller_folio"  / type ="hidden" value="<?php echo String::html_normalize(URL::get('traveller_folio'));?>">                
            </div>
            <div style="width: 100%; margin: 0px auto; float: left;">
                <span style="float: left; width: 80px;">&nbsp;&nbsp;<?php echo Portal::language('select_traveller');?>: </span>
                <select  name="travellers_id" id="travellers_id" onchange="jQuery('#traveller_id').val(this.value);window.location='<?php echo 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'form.php?block_id=429&cmd=group_folio&id='.Url::get('id');?>&traveller_id='+this.value+'&customer_id='+jQuery('#customer_id').val();" style="width:120px; height: 23px; float: left;"><?php
                ...
                <a class="w3-button w3-gray w3-hover-gray" href="#" type="button" value="<?php echo Portal::language('change_traveller');?>" style="padding: 3px; height: 23px; margin-left: 2px; float: left; text-decoration: none;" onclick="jQuery('#change_traveller_folio').css('display','');"><i class="fa fa-refresh" style="font-size: 18px;"></i> <?php echo Portal::language('change_traveller');?></a>
            ...
            <div style="width: 100%; margin: 5px auto; float: left;">
                <span style="float: left;">
                    <span style="float: left; width: 80px;"> <?php echo Portal::language('member_code');?>: </span>
     ...
     <tr>
        <td style="vertical-align:top;">
        	<table border="1" width="100%" style="border:1px solid silver;margin:auto;" class="invoice" cellpadding="3">              
                <?php $type=''; $rr = 0;?> 
                <?php if(isset($this->map['folios']) and is_array($this->map['folios'])){ foreach($this->map['folios'] as $key6=>&$item6){if($key6!='current'){$this->map['folios']['current'] = &$item6;?>  
                	<?php if($rr != $this->map['folios']['current']['id']){?> 
                	<tr id="title_reservation_room_<?php echo $this->map['folios']['current']['id'];?>" style="background:#f1f1f1;">
                        <td width="400"><span><b><em><?php echo $this->map['folios']['current']['room_name'];?> </em></b></span></td>
                        <td width="30"><a href="#" name="reservation_room_<?php echo $this->map['folios']['current']['id'];?>" type="image" id="reservation_room_<?php echo $this->map['folios']['current']['id'];?>" onclick="checkSplit('<?php echo $this->map['folios']['current']['id'];?>','ALL_ROOM','');return false;" style="padding-left: 3px;"><i class="fa fa-arrow-circle-right w3-text-gray w3-hover-text-black" style="font-size: 23px;"></i></a>
                        <input type="checkbox" id="reservation_room_<?php echo $this->map['folios']['current']['id'];?>"/></td>
                    </tr>
                    <?php }?>    
                	<?php if(isset($this->map['folios']['current']['items']) and is_array($this->map['folios']['current']['items'])){ foreach($this->map['folios']['current']['items'] as $key7=>&$item7){if($key7!='current'){$this->map['folios']['current']['items']['current'] = &$item7;?>	
                <?php if($type != $this->map['folios']['current']['items']['current']['type'] || $rr != $this->map['folios']['current']['id']){ $rr = $this->map['folios']['current']['id']; $type = $this->map['folios']['current']['items']['current']['type']; ?>       
                        <tr id="title_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>">
                        	<td width="400">
                                <input name="add_item_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" type="image" id="add_item_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" 
                                    src="packages/hotel/skins/default/images/icons/add_item.gif" 
                                    onclick="ItemDetail('<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>');return false;" style="float:left;" />
                                    <span> &nbsp; &nbsp; <b><em><?php echo $this->map['folios']['current']['items']['current']['type'];?></em></b></span>
                            	<input name="total_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" id="total_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" value="" style="float:right; text-align:right; border:none;" align="right" /> <?php echo $this->map['folios']['current']['id'];?>
                            </td>
                            <td width="30">
                                <a href="#" name="all_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" type="image" id="all_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" 
                                onclick="jQuery('#All_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>').attr('checked',true);jQuery('.detail_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>').attr('checked',true);checkSplit('<?php echo $this->map['folios']['current']['id'];?>','<?php echo $this->map['folios']['current']['items']['current']['type'];?>','');return false;" 
                            ...
                            <input type="checkbox" id="ALL_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>"/></td>
                        </tr>
                <?php }?>
                        <tr style="display:none;"><td colspan="2"><input  name="foc_<?php echo $this->map['folios']['current']['id'];?>" value="<?php echo String::html_normalize(URL::get('foc_'.$this->map['folios']['current']['id'],$this->map['folios']['current']['foc']));?>" id="foc_<?php echo $this->map['folios']['current']['id'];?>"  type ="hidden"><input  name="foc_all_<?php echo $this->map['folios']['current']['id'];?>" value="<?php echo String::html_normalize(URL::get('foc_all_'.$this->map['folios']['current']['id'],$this->map['folios']['current']['foc_all']));?>" id="foc_all_<?php echo $this->map['folios']['current']['id'];?>"  type ="hidden"></td></tr>
                        <tr id="tr_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" class="<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>" style="display:none;" lang="<?php echo $this->map['folios']['current']['items']['current']['status'];?>">  
                        <td width="400">
                            <div class="item-body" style="width:100%;">	
                                <div class="date" id="date_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>"><?php echo $this->map['folios']['current']['items']['current']['date'];?></div>
                                <input type="hidden" id="full_date_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" value="<?php echo $this->map['folios']['current']['items']['current']['full_date'];?>"/>
                                <div class="description" id="description_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" style="width:150px !important;"><?php echo $this->map['folios']['current']['items']['current']['description'];?></div>
                                <?php if($this->map['folios']['current']['items']['current']['type']=='LAUNDRY'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=laundry_invoice&cmd=edit&id='.$this->map['folios']['current']['items']['current']['id'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#LD_'.$this->map['folios']['current']['items']['current']['position'].'</a>';
															}else if($this->map['folios']['current']['items']['current']['type']=='MINIBAR'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=minibar_invoice&cmd=edit&id='.$this->map['folios']['current']['items']['current']['id'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#MN_'.$this->map['folios']['current']['items']['current']['position'].'</a>';
															}else if($this->map['folios']['current']['items']['current']['type']=='EQUIPMENT'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=equipment_invoice&cmd=edit&id='.$this->map['folios']['current']['items']['current']['id'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#EQ_'.$this->map['folios']['current']['items']['current']['position'].'</a>';
															}else if($this->map['folios']['current']['items']['current']['type']=='BAR'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=touch_bar_restaurant&cmd=detail&'.md5('act').'='.md5('print_bill').'&'.md5('preview').'=1&id='.$this->map['folios']['current']['items']['current']['id'].$this->map['folios']['current']['items']['current']['link'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#'.$this->map['folios']['current']['items']['current']['code'].'</a>';
															}else if($this->map['folios']['current']['items']['current']['type']=='TICKET'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=ticket_invoice_group&cmd=bill&'.md5('act').'='.md5('print').'&id='.$this->map['folios']['current']['items']['current']['id'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#'.$this->map['folios']['current']['items']['current']['id'].'</a>';
															}else if($this->map['folios']['current']['items']['current']['type']=='EXTRA_SERVICE'){
																$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=extra_service_invoice&cmd=view_receipt&id='.$this->map['folios']['current']['items']['current']['ex_id'];
																echo '<a target="_blank" href="'.$her.'" style="float:left">#'.$this->map['folios']['current']['items']['current']['ex_bill'].'</a>';
															}//else if($this->map['folios']['current']['items']['current']['type']=='MASSAGE'){
															//	$her = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=massage_daily_summary&cmd=invoice&room_id='.$this->map['folios']['current']['items']['current']['room_id'].'&id='.$this->map['folios']['current']['items']['current']['id'];
															//	echo '<a target="_blank" href="'.$her.'" style="float:left">#'.$this->map['folios']['current']['items']['current']['id'].'</a>';
															//}	?>           
                                <div class="amount" style="float:right;"><?php echo (System::Display_number(System::calculate_number($this->map['folios']['current']['items']['current']['net_amount'])));?><input name="amount_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" type="text" id="amount_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" value="<?php echo $this->map['folios']['current']['items']['current']['net_amount'];?>" style="border:none;font-size:11px; text-align:center;float:right; display:none;" readonly="readonly" class="amount" lang="<?php echo $this->map['folios']['current']['items']['current']['service_rate'];?>" alt="<?php echo $this->map['folios']['current']['items']['current']['tax_rate'];?>" /> </div>
                            </div>
                        </td>
                        <td width="30"><a href="#" name="split_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" type="image" id="split_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" onclick="jQuery('#detail_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>').attr('checked',true);checkSplit('<?php echo $this->map['folios']['current']['id'];?>','<?php echo $this->map['folios']['current']['items']['current']['type'];?>','<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>');return false;" style="padding-left: 3px;" lang="<?php echo $this->map['folios']['current']['items']['current']['percent'];?>" ><i class="fa fa-arrow-circle-right w3-text-gray w3-hover-text-black" style="font-size: 23px;"></i></a>
                        <input type="checkbox" id="<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['items']['current']['id'];?>" lang="<?php echo $this->map['folios']['current']['items']['current']['type'];?>" alt="<?php echo $this->map['folios']['current']['items']['current']['id'];?>" value="<?php echo $this->map['folios']['current']['items']['current']['amount'];?>" name="<?php echo $this->map['folios']['current']['items']['current']['id'];?>" class="detail_<?php echo $this->map['folios']['current']['items']['current']['type'];?>_<?php echo $this->map['folios']['current']['id'];?>"/></td>
                        </tr>
                <?php }}unset($this->map['folios']['current']['items']['current']);} ?>
             <?php }}unset($this->map['folios']['current']);} ?>           
            </table> 
        </td>
        <td style="vertical-align:top;">
        	 <input  name="all_type" id="all_type" value="ROOM,ROOM_SERVICE,SERVICE,MINIBAR,LAUNDRY,EQUIPMENT,BAR,EXTRA_SERVICE,TELEPHONE,MASSAGE,DISCOUNT,DEPOSIT,DEPOSIT_GROUP,TICKET,KARAOKE,VE,PACKAGE" style="width:300px;"/ type ="hidden" value="<?php echo String::html_normalize(URL::get('all_type'));?>">
            ...
            <input  name="arr_ids" value="" id="arr_ids" / type ="hidden" value="<?php echo String::html_normalize(URL::get('arr_ids'));?>">
...
<tr id="tr_total_vat" style="display:none;">
            ...
            <div id="payment_bound">
           		<fieldset>
            	<legend><b><i><?php echo Portal::language('payment');?></i></b></legend>
                <div id="payment_method_bound">
                    <b><em><?php echo Portal::language('balance');?>:&nbsp; </em></b><input  name="balance" id="balance" value="0" style="border:none; width:100px; height:25px; color: red; text-align: left;;" readonly="readonly" / type ="text" value="<?php echo String::html_normalize(URL::get('balance'));?>">
                    <?php if((User::can_add(false,ANY_CATEGORY) and !isset($_REQUEST['folio_id'])) OR (User::can_edit(false,ANY_CATEGORY) and isset($_REQUEST['folio_id']) and $_REQUEST['check_payment']==0 ) OR (User::can_delete(false,ANY_CATEGORY) and isset($_REQUEST['folio_id']) and $_REQUEST['check_payment']==1) OR (User::can_admin(false,ANY_CATEGORY) and isset($_REQUEST['folio_id'])) ){ ?>
                        <a href="#" class="w3-btn w3-lime w3-hover-lime" name="payment" type="button" id="payment" value="<?php echo Portal::language('payment');?>" onclick="SubmitForm('payment');" style="height:34px; border: 1px solid lime; text-transform: uppercase; text-decoration: none;"><i class="fa fa-dollar" style="font-size: 18px;"></i> <span><?php echo Portal::language('payment');?></span></a>
                    <?php }?>    
                ...
                <a target="_blank" href="<?php echo $hef;?>&portal_id=<?php echo PORTAL_ID;?>" class="w3-btn w3-red w3-hover-red" style="text-transform: uppercase; text-decoration: none; height: 34px; float: right;" ><i class="fa fa-eye" style="font-size: 18px;"></i> <!---<input  type="button" id="view_invoice_#xxxx#" title="<?php echo Portal::language('view_order');?>" style="float:right;"/>---><?php echo Portal::language('view_folio');?></a>
                <?php }?>
                </div>
          		</fieldset>
          </div>  
...
<script>
    var traveller_folio_check =  <?php echo $this->map['traveller_folio_check'];?> ;
	...
	for(j in items_js){
		...
		for(var k in items_js[j]['items']){
            ...
            if(items['type']=='DEPOSIT_GROUP')
            {
                jQuery('#total_'+items['type']+'_'+items_js[j]['id']).val(number_format(total[''+items['type']+'_'+j+'']));
            }
    ...
    function DrawSplitOrder(){
    	jQuery('#table_split').html('<tr style="text-align:center;"><td style="width:50%;"><b><?php echo Portal::language('detail');?></b></td><td style="width:20%;"><b><?php echo Portal::language('percent');?></b></td><td style="width:25%;"><b><?php echo Portal::language('amount');?></b></td><td>&nbsp;</td></tr>');
	...
	function UpdateTotal(){
        ...
        else
        {
            ...
            if(to_numeric(jQuery('#total_payment').val()) < 0){
                jQuery('#payment span').html('<?php echo Portal::language('refun');?>');
    		}else{
    			jQuery('#payment span').html('<?php echo Portal::language('payment');?>');
    		}
        }

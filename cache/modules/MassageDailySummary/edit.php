...
<script type="text/javascript">
	var staff_arr = <?php echo String::array2js($this->map['staffs']);?>;
    var product_array = <?php echo String::array2js($this->map['products']);?>;
	var travellers = <?php echo $this->map['travellers'];?>;
</script>
<span style="display:none">
	<span id="mi_staff_group_sample">
		<div id="mi_staff_group_#xxxx#" style="width:100%;text-align:left;">
			...
			<span class="multi-input">
                <select   name="mi_staff_group[#xxxx#][staff_id]" id="staff_id_#xxxx#"><?php echo $this->map['staff_options'];?></select>
            </span>
			<span class="multi-input">
                <select   name="mi_staff_group[#xxxx#][room_id]" id="room_id_#xxxx#" onchange="check_room(this,this.value);" class="staff" index="#xxxx#" ><?php echo $this->map['room_options_staff'];?></select>
            </span>
			<span class="multi-input">
...
<span style="display:none">
	<span id="mi_product_group_sample">
		<div id="mi_product_group_#xxxx#" style="width:100%;text-align:left;" >
			...
			<span class="multi-input">
                <select   name="mi_product_group[#xxxx#][room_id]" id="room_id_#xxxx#" style="width:105px;" class="product" onchange="add_room_staff(this.value);"><?php echo $this->map['room_options'];?></select>
            </span>
			<span class="multi-input">
			...
			<span class="multi-input">
                <select   name="mi_product_group[#xxxx#][status]" id="status_#xxxx#" style="width:85px;font-size:11px;height:22px;" onchange="updateTime('#xxxx#');"><?php echo $this->map['status_options'];?></select>
            </span>
			<span class="multi-input">
			...
			<span class="multi-input DeleteInput_#xxxx#">
                <img src="<?php echo Portal::template('core');?>/images/buttons/delete.gif" onclick="if(!confirm('<?php echo Portal::language('are_you_sure');?>')){return false};mi_delete_row($('mi_product_group_'+#xxxx#),'mi_product_group','#xxxx#','product_');delete_staff(#xxxx#);DeleteInput(#xxxx#);if(document.all)event.returnValue=false; else return false;" style="cursor:pointer;"/>
            </span>
            <br clear="all" /><!---->
		</div>
	</span> 
...
<form  name="MassageEditForm" method="post">
	<table cellpadding="15" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
		<tr>
        	<td width="60%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><?php echo $this->map['title'];?></td>
            <td align="right" nowrap="nowrap" style="width: 40%; padding-right: 30px;">
            	<?php if(User::can_add(false,ANY_CATEGORY)){?><input name="save" type="submit" value="<?php echo Portal::language('Save_and_close');?>" class="w3-btn w3-orange w3-text-white" onclick="check_select_room();jQuery('.button-medium-save').css('display','none');" style="text-transform: uppercase; margin-right: 5px;"/><?php }?>
                <?php if(User::can_add(false,ANY_CATEGORY)){?><input name="save_stay" type="submit" value="<?php echo Portal::language('Save_stay');?>" class="w3-btn w3-blue w3-text-white" onclick="check_select_room();jQuery('.button-medium-save').css('display','none');" style="text-transform: uppercase; margin-right: 5px;"/><?php }?>
				<a href="<?php echo Url::build_current();?>" class="w3-btn w3-green w3-text-white" style="text-transform: uppercase; margin-right: 5px; text-decoration: none;"><?php echo Portal::language('back');?></a>
                <?php if(Url::get('cmd')!='add' and User::can_delete(false,ANY_CATEGORY)){?><a onclick="return check_delete();" href="<?php echo Url::build_current(array('cmd'=>'delete','id'));?>" class="w3-btn w3-red w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"><?php echo Portal::language('delete');?></a><?php }?>
            </td>
        </tr>
    </table>
	<div class="content">
		...
		<fieldset>
			<legend class="title"><?php echo Portal::language('used_products_services');?></legend>
			<span id="mi_product_group_all_elems" style="text-align:left;">
				<span>
					<span class="multi-input-header" style="width:105px;float:left;"><?php echo Portal::language('room');?></span>
					<span class="multi-input-header" style="width:122px;float:left;"><?php echo Portal::language('code');?></span>
					<span class="multi-input-header" style="width:120px;float:left;"><?php echo Portal::language('name');?></span>
					<span class="multi-input-header" style="width:90px;float:left;"><?php echo Portal::language('price');?></span>
                    <span class="multi-input-header" style="width:110px;float:left;"><?php echo Portal::language('use_time');?></span>
					<span class="multi-input-header" style="width:50px;float:left;"><?php echo Portal::language('no_of_people');?></span>
					<span class="multi-input-header" style="width:80px;float:left;"><?php echo Portal::language('amount');?></span>
					<span class="multi-input-header" style="width:85px;float:left;"><?php echo Portal::language('status');?></span>
					<span class="multi-input-header" style="width:126px;float:left;"><?php echo Portal::language('time_in');?></span>
					<span class="multi-input-header" style="width:126px;float:left;"><?php echo Portal::language('time_out');?></span>
				</span>
                <br clear="all" />
			</span>
			<input class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; margin-top: 5px;" type="button" value="<?php echo Portal::language('add_service');?>" onclick="mi_add_new_row('mi_product_group');productAutoComplete();jQuery('#time_in_hour_'+input_count).mask('99:99');jQuery('#time_out_hour_'+input_count).mask('99:99');show_datepicker(input_count);jQuery('#code_'+input_count).ForceCodeOnly();"/>
		</fieldset>
        
		<fieldset>
			<legend class="title"><?php echo Portal::language('staffs');?></legend>
            <legend class="title"><?php echo Portal::language('only_choose_staff_in_room_selected');?></legend>
				<span id="mi_staff_group_all_elems">
				</span>
                <br clear="all" />
				<input class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; margin-top: 5px;" type="button" value="<?php echo Portal::language('add_staff');?>" onclick="mi_add_new_row('mi_staff_group');jQuery('#tip_'+input_count).FormatNumber();jQuery('#tip_'+input_count).ForceNumericOnly();"/>
		</fieldset>
        
        <fieldset>
        	<legend class="title"><?php echo Portal::language('general_information');?></legend>
        	<table style="width: 65%;" border="0" cellspacing="0" cellpadding="2">
				...
				{?><a class="w3-text-blue" href="#" onclick="window.open('?page=customer&action=select_customer&site=spa','customer')" style="text-decoration: none;"> <i class="fa fa-plus-square" style="font-size: 18px;"></i> <?php echo Portal::language('add_now');?></a>
				<?php
				}
				?>
                    </td>
                ...
                <tr valign="middle">
                    <td align="right" class="label"><?php echo Portal::language('guest_code');?></td>
                    <td>
                        <input  name="guest_id" id="guest_id" style="display:none;"/ type ="text" value="<?php echo String::html_normalize(URL::get('guest_id'));?>">
                        <input  name="code" id="code" onclick="window"/ type ="text" value="<?php echo String::html_normalize(URL::get('code'));?>">
                    ...
                    <td align="right"><?php echo Portal::language('guest_name');?></td>
                    <td align="left"><input  name="full_name" id="full_name"/ type ="text" value="<?php echo String::html_normalize(URL::get('full_name'));?>"></td>
                    <td colspan="2">
                ...
                <tr valign="middle">
                    <td align="right" class="label"><?php echo Portal::language('hotel_room');?></td>
                    <td>
                        <select  name="hotel_reservation_room_id" id="hotel_reservation_room_id" style="display: block; float:left;" ><?php
					if(isset($this->map['hotel_reservation_room_id_list']))
					{
						foreach($this->map['hotel_reservation_room_id_list'] as $key=>$value)
						{
                    ...
                    if(URL::get('hotel_reservation_room_id',isset($this->map['hotel_reservation_room_id'])?$this->map['hotel_reservation_room_id']:''))
                    echo "<script>$('hotel_reservation_room_id').value = \"".addslashes(URL::get('hotel_reservation_room_id',isset($this->map['hotel_reservation_room_id'])?$this->map['hotel_reservation_room_id']:''))."\";</script>";
                    ?>
	</select>
                        &nbsp;<span id="room_traveller_name" style="display: block; float:left; margin-left: 5px;" ><?php echo $this->map['room_traveller_name'];?></span><span style="margin-left: 7px;display: block; float:left;"><input name="pay_with_room" type="checkbox" id="pay_with_room" onclick="check_pay_with_room();" style="display:block; padding:10px;" <?php if($this->map['pay_with_room']==1){ echo 'checked';}?> title="Trả tiền về phòng" value="1" /></span>
                        <span id="payment_note" style="display:none ;"><?php echo Portal::language('da_thanh_toan_ve_phong');?></span>
                        <input  name="hotel_reservation_room_check" id="hotel_reservation_room_check" value="0" / type ="hidden" value="<?php echo String::html_normalize(URL::get('hotel_reservation_room_check'));?>">
                        <input  name="hotel_reservation_room_id_old" id="hotel_reservation_room_id_old" value="" / type ="hidden" value="<?php echo String::html_normalize(URL::get('hotel_reservation_room_id_old'));?>">
                    </td>
                    <td align="right"><span class="label"><?php echo Portal::language('discount');?></span></td>
                    <td align="left">
                        <input  name="discount_amount" id="discount_amount" onchange="updateTotalPayment();" onkeyup="check_discount_amount();" style="width:80px;" class="input_number"/ type ="text" value="<?php echo String::html_normalize(URL::get('discount_amount'));?>">
                        <input  name="discount" id="discount" onchange="updateTotalPayment();" onkeyup="check_discount1();" style="width:40px;" class="input_number"/ type ="text" value="<?php echo String::html_normalize(URL::get('discount'));?>">% 
                        <span><?php echo Portal::language('tax');?> <input name="tax" type="text" id="tax"  value ="<?php echo $this->map['tax'];?>" onchange="updateTotalPayment();" style="width:40px;"/>%</span>
                        <span><?php echo Portal::language('service_rate');?> <input name="service_rate" type="text" id="service_rate"  value ="<?php echo $this->map['service_rate'];?>" onchange="updateTotalPayment();" style="width:40px;"/>%</span>
                        <?php 
				if((Url::get('cmd')=='add'))
				{?>
                        <span><?php echo Portal::language('net_price');?><input  name="net_price" value="1" type="checkbox" id="net_price" <?php if(NET_PRICE_SPA==1){ echo 'checked';}?> onclick="change_net_price()"/></span>
                         <?php }else{ ?>
                        <span><?php echo Portal::language('net_price');?><input  name="net_price" value="1" type="checkbox" id="net_price" onclick="change_net_price()"/></span>
                ...
                <tr valign="middle">
                    <td align="right" class="label"><?php echo Portal::language('note');?></td>
                    <td colspan="3"><textarea  name="note" id="note" style="width:98%"><?php echo String::html_normalize(URL::get('note',''));?></textarea></td>
                </tr>
        	</table>
        </fieldset>
        ...
        <fieldset>
            <div style="float: right; padding-right: 100px;">
                <strong><?php echo Portal::language('total_payment');?>:</strong> : <input  name="total_amount" id="total_amount" readonly="true" style="width:100px;text-align:right;border:0px;border-bottom:1px solid #CCCCCC;font-weight:bold;color:#000000;"/ type ="text" value="<?php echo String::html_normalize(URL::get('total_amount'));?>"> <?php echo HOTEL_CURRENCY; ?>
		        <input  name="total_amount_old" id="total_amount_old" readonly="" value="" / type ="hidden" value="<?php echo String::html_normalize(URL::get('total_amount_old'));?>">
                <input name="total_amount_old_2" type="hidden" id="total_amount_old_2" readonly="" value="<?php echo $this->map['total_amount'];?>" />
                <?php if($this->map['pay_with_room']!=1){ ?>
                <input class="w3-btn w3-lime" style="text-transform: uppercase;" name="payment" type="button" id="payment" value="<?php echo Portal::language('payment');?>" onclick="openPayment();"/>
                <?php
                    }
                ?>
            </div>
        ...
        <fieldset style="display:none;">
            <legend class="title"><?php echo Portal::language('Pay_by');?></legend>
            ...
            <div class="currency-list"><span>&nbsp;</span>
                ...
                <tr>
                    ...
                    <td>
                    	...
                    	<span>
                            <?php echo $this->map['currencies']['current']['name'];?><br /><input  name="currency_<?php echo $this->map['currencies']['current']['id'];?>" id="currency_<?php echo $this->map['currencies']['current']['id'];?>" / type ="text" value="<?php echo String::html_normalize(URL::get('currency_'.$this->map['currencies']['current']['id']));?>">
                    	</span>
                    </td>
    ...
    <input  name="extra_charge" id="extra_charge" onchange="updateTotalPayment();" style="display:none;"/ type ="text" value="<?php echo String::html_normalize(URL::get('extra_charge'));?>">
...
<script type="text/javascript">
   <?php echo 'var block_id = '.Module::block_id().';';?>
    ...
    hotel_reservation_room_id = '<?php echo $this->map['hotel_reservation_room_id'];?>';
    jQuery('#hotel_reservation_room_id_old').val(hotel_reservation_room_id);

    var check_discount = <?php echo $this->map['check_discount'];?>;
    if(check_discount == 1)
    {
    ...
    if(cmd == 'edit')
    {
        var total_amount = <?php echo $this->map['total_amount'];?>;
        var amount_pay_with_room = <?php echo $this->map['amount_pay_with_room'];?>;
...
//        }
	...
	<?php 
				if((isset($this->map['hotel_reservation_room_id']) and $this->map['hotel_reservation_room_id']))
				{?>
	//$('room_traveller_name').innerHTML = travellers[<?php echo $this->map['hotel_reservation_room_id'];?>]['full_name'];
	...
	function updateTotalPayment(){
        ...
        if(discount_amount > total_payment){
            alert('<?php echo Portal::language('so_tien_khuyen_mai_lon_hon_so_tien_dich_vu');?>');
        ...
        {
            alert('<?php echo Portal::language('invalid_percent');?>');
        ...
        if(jQuery('#net_price').is(':checked'))//gia da co thue phi
        {
           <?php if(DISCOUNT_BEFORE_TAX == 1){?>//giam gia truoc thue phi
            ...
            <?php } else { ?>//giam gia sau thue phi
                ...
                var price_package =<?php echo $this->map['price_package']?>;
	...
	function getStaffFromCode(id,value){
		if(typeof(staff_arr[value])=='object'){
			...
			if(value){
				...
				$('full_name_'+id).value = '<?php echo Portal::language('staffs_does_not_exist');?>';
			}else{
				$('full_name_'+id).value = '';
			}
		}
	...
	function getProductFromCode(id,value){	 	   
	    if(typeof(product_array[value])=='object'){
			...
			if(value){
				...
				$('name_'+id).value = '<?php echo Portal::language('product_does_not_exist');?>';
			}else{
				$('name_'+id).value = '';
			}
		}
...
//        jQuery('#payment').css('display','none');
    ...
    var payment = <?php echo $this->map['payment'];?>;
    if(payment>0)
    {
        jQuery("#hotel_reservation_room_id").each(function(){
    ...
    function openPayment()
    {
        ...
        if (check_edit === 'no')
        {
            alert('<?php echo Portal::language('please_save_first');?>');
            return false;
        }
...
function check_delete()
{
    ...
    if(cf)
    {
        ...
        <?php if(DB::fetch('SELECT * FROM traveller_folio WHERE invoice_id =\''.Url::get('id').'\' AND type =\'MASSAGE\'')){?>
            check = true;
            notice = notice+'<?php echo Portal::language('da_tao_hoa_don_tai_phong_khach_san');?>';
        <?php }?>
        <?php if(DB::fetch('SELECT * FROM payment WHERE bill_id =\''.Url::get('id').'\' AND type =\'SPA\'')){?>
            check = true;
            notice = notice+'<?php echo Portal::language('da_thanh_toan');?>';
        <?php }?>
        if(check==true)
        {
            notice = notice+' '+'<?php echo Portal::language('khong_duoc_xoa');?>';
            alert(notice);
            return false;
        }
        else

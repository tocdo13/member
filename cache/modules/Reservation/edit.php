	...
	<span id="mi_reservation_room_sample">
		...
		<div id="reservation_room_bound_#xxxx#" class="reservation_room_bound">
			...
			<span class="multi-input" style="width: 70px;">
				...
				<a id="select_room_#xxxx#" onclick="Check_Availblity(#xxxx#,input_count);" style="cursor:pointer;" alt="move_room"><i class="fa fa-hotel w3-text-blue" style="font-size: 16px;"></i></a>
            ...
            <span class="multi-input">
					<input  name="mi_reservation_room[#xxxx#][child]" type="text" id="child_#xxxx#" style="width:47px;" AUTOCOMPLETE=OFF><i class="fa fa-child" style="font-size: 16px;"></i></span>
			...
			<!---<span class="multi-input" title="<?php echo Portal::language('select_rate');?>" style="width:20px;height:5px;">
                <img src="packages/core/skins/default/images/buttons/rate_list.gif" id="rate_list_#xxxx#" class="select-rate" alt="<?php echo Portal::language('select_rate');?>">
            </span>--->
            ...
            <span class="multi-input" style="display: none;">
					<select   name="mi_reservation_room[#xxxx#][early_checkin]" id="early_checkin_#xxxx#" class="early_checkin" title="<?php echo Portal::language('early_checkin');?>" style="width:48px;"><?php echo $this->map['verify_dayuse_options'];?></select>
            ...
            <span class="multi-input" style="display: none;">
					<select   name="mi_reservation_room[#xxxx#][late_checkout]" id="late_checkout_#xxxx#" class="late_checkout" title="<?php echo Portal::language('late_checkout');?>" style="width:50px;"><?php echo $this->map['verify_dayuse_options'];?></select>
			</span>
            <span style="display: none;">
					...
					onchange=" 
						...
						if(this.value=='CHECKIN')
                        {
                        	...
                        	if(!$('room_id_#xxxx#').value)
                            {
                            	...
                            	alert('<?php echo Portal::language('miss_room_number');?>');
								return false;
                            }
                        ...
                        {
                            jQuery('cancel_note_#xxxx#').css('display','');
                        }
            ...
            <span class="multi-input" id="cancel_charge_span_#xxxx#" style="display:none;">
					<input onclick="jQuery('div#wrapper').fadeIn('fast');jQuery('div#cancel_extra').fadeIn('fast'); getValue('cancel_charge_#xxxx#','0')" style="width:50px; text-align: right;" id="cancel_charge_#xxxx#" class="cancel_charge_#xxxx#" type="text" readonly=""/>
			</span>
			...
			<span class="multi-input" style="display: none;">
					<select   name="mi_reservation_room[#xxxx#][reservation_type_id]" style="width:45px;font-size:11px;" id="reservation_type_id_#xxxx#"><?php echo $this->map['reservation_type_options'];?></select>
			</span>
			<span class="multi-input">
					<input  name="mi_reservation_room[#xxxx#][confirm]" type="checkbox" id="confirm_#xxxx#" style="width:20px;font-size:11px;" value="1" class="checkbox"/>
			</span>
            <span class="multi-input">
                    <input  name="mi_reservation_room[#xxxx#][breakfast]" type="checkbox" id="breakfast_#xxxx#" style="width:18px;font-size:11px;" value="1" class="cb_bf"/>
            </span>
            <!-- Manh them allotment -->
            <span class="multi-input" style="text-align: center;<?php if(!USE_ALLOTMENT){ ?> display: none; <?php } ?>">
					<input  name="mi_reservation_room[#xxxx#][allotment]" type="checkbox" id="allotment_#xxxx#" style="width:20px;font-size:11px;" value="1" class="checkbox" onclick="SetupAllotment('#xxxx#');"/>
			</span>
            <!-- Manh them allotment -->
            ...
            <span class="multi-input">
                ...
                <?php echo $this->map['package_sale_options'];?>
                </select>
			</span>            
            <!--end giap.ln -->
            <!--giap.ln add key-->
                ...
                if(defined('IS_KEY') && IS_KEY==1)
                {
                    ...
                    switch(SERVER_KEY)
                    {
                        case IS_BETECH:
                        {
                            ...
                            <a id="btn_key_#xxxx#" title="<?php echo Portal::language('create_key_card');?>" target="_blank" onclick="window.open('?page=manager_key&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val()+ '&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                        ...
                        case IS_ADEL:
                        {
                            ?>
                            <span class="multi-input" style="width:20px;">
                                <a id="btn_key_#xxxx#" title="Tạo khóa từ" target="_blank" onclick="window.open('?page=manager_key_adel&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val() + '&portal=<?php echo PORTAL_ID; ?>')"><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                            </span>
                            <span class="multi-input" style="width:1px;">
                                <input class="items_key_hune" name="mi_reservation_room[#xxxx#][total_key]" type="text" id="total_key_#xxxx#" readonly="readonly"/>
                            </span>
                        ...
                        case IS_SALTO:
                        {
                            ?>
                            <span class="multi-input" style="width:20px;">
                                <a id="btn_key_#xxxx#" title="Tạo khóa từ" target="_blank" onclick="window.open('?page=manager_key_salto&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val() + '&portal=<?php echo PORTAL_ID; ?>')"><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                            </span>
                            <span class="multi-input" style="width:1px;">
                                <input class="items_key_hune" name="mi_reservation_room[#xxxx#][total_key]" type="text" id="total_key_#xxxx#" readonly="readonly"/>
                            </span>
                        ...
                        case IS_HUNERF:
                        {
                            ?>
                            <span class="multi-input" style="width:20px;">
                                <a id="btn_key_#xxxx#" title="Tạo khóa từ" target="_blank" onclick="window.open('?page=manager_key_hune&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val()+ '&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                            </span>
                            <span class="multi-input" style="width:1px;">
                                <input class="items_key_hune" name="mi_reservation_room[#xxxx#][total_key]" type="text" id="total_key_#xxxx#" readonly="readonly" />
                            </span>
                        ...
                        case IS_ORBITA:
                        {
                            ?>
                            <span class="multi-input" style="width:20px;">
                                <a id="btn_key_#xxxx#" title="Tạo khóa từ" target="_blank" onclick="window.open('?page=manager_key_orbita&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val()+ '&rr_id='+jQuery('#id_#xxxx#').val()+ '&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                            </span>
                            <span class="multi-input" style="width:1px;">
                                <input class="items_key_hune" name="mi_reservation_room[#xxxx#][total_key]" type="text" id="total_key_#xxxx#" readonly="readonly" />
                            </span>
                        ...
                        case IS_BQLOCK:
                        {
                            ?>
                            <span class="multi-input" style="width:20px;">
                                <a id="btn_key_#xxxx#" title="Tạo khóa từ" target="_blank" onclick="window.open('?page=manager_key_bqlock&cmd=create&resevation_room_id='+jQuery('#id_#xxxx#').val()+ '&rr_id='+jQuery('#id_#xxxx#').val()+ '&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tag w3-text-blue" style="font-size: 18px;"></i></a>
                            </span>
                            <span class="multi-input" style="width:1px;">
                                <input class="items_key_hune" name="mi_reservation_room[#xxxx#][total_key]" type="text" id="total_key_#xxxx#" readonly="readonly" />
                            </span>
            ...
            <!--end giap.ln-->
            <!---<span  class="multi-input">
                    <input  type="button" id="view_invoice_#xxxx#" onclick="viewInvoice('#xxxx#',true,false);" class="view-order-button" title="<?php echo Portal::language('view_order');?>"/>
            </span>--->
                                
            <span class="">            
					<a href="#" class="w3-button w3-cyan w3-hover-cyan w3-text-white" onclick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_UPDATE_TRAVELLER;?>&cmd=add_traveller&rr_id='+$('id_#xxxx#').value+'&r_id='+<?php echo Url::get('id');?>,Array('add_traveller_'+$('id_#xxxx#').value,'<?php echo Portal::language('add_guest');?>','20','110','1100','570'));" title="<?php echo Portal::language('add_guest');?>" style="width:100px; height: 20px; text-align: left; text-decoration: none; padding-top: 3px;"><?php echo Portal::language('add_guest');?></a>
			</span>                                
            <span class="multi-input trrrr" <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ ?> style="display: none;" <?php } ?> >
                    <?php if(User::can_view($this->get_module_id('CreateTravellerFolio'),ANY_CATEGORY)){ ?>
					<input  type="button" id="split_invoice_#xxxx#" class="w3-button w3-lime w3-hover-lime w3-text-white" onclick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_CREATE_FOLIO;?>&cmd=create_folio&rr_id='+$('id_#xxxx#').value+'&r_id='+<?php echo Url::get('id');?>,Array('folio_'+$('id_#xxxx#').value,'<?php echo Portal::language('create_folio');?>','80','210','1030','500'));" value="<?php echo Portal::language('folio');?>" title="<?php echo Portal::language('create_folio');?>" style="display:none; width:50px; height: 20px; margin-right: 5px; padding-top: 4px;"/>
                    <?php } ?>
			</span>
			...
			<span id="mi_reservation_room_sample_#xxxx#" style="display:none;">
                <div class="room-extra-info" id="room_extra_info_#xxxx#">
                    <div>
                         <span class="multi-input-extra">
                            ...
                            <span style="float: left; padding-right: 3px; padding-left: 5px;"><?php echo Portal::language('foc_room_charge');?></span>
                            ...
                            <span style="float: left; padding-right: 3px;"><?php echo Portal::language('foc_all');?></span>
                            <input  name="mi_reservation_room[#xxxx#][foc_all]" style="width:15px; margin-right: 5px;" value="1" type="checkbox" id="foc_all_#xxxx#" title="<?php echo Portal::language('foc_all_services');?>"/>
                            
                            <span style="float: left; height: 21px; border-left: 1px solid gray; margin-right: 5px;"></span>
                            
                            <span style="float: left; padding-right: 3px;"><?php echo Portal::language('discount_room_charge');?>(%)</span>
                            <input  name="mi_reservation_room[#xxxx#][discount_after_tax]" style="width:15px;display:none; " value="1" type="checkbox" id="discount_after_tax_#xxxx#" title="<?php echo Portal::language('discount_percent_after_tax');?>"/>
                            ...
                            <span style="float: left; padding-right: 3px;"><?php echo Portal::language('tax_rate_vat');?>(%)</span>
                            ...
                            <span style="float: left; padding-right: 3px;"><?php echo Portal::language('service_rate');?>(%)</span>
                            ...
                            <span style="float: left; padding-right: 3px;"><?php echo Portal::language('net_price');?></span>
                            <input  name="mi_reservation_room[#xxxx#][net_price]" class="net_price" style="width:15px; margin-right: 5px;" value="1" type="checkbox" id="net_price_#xxxx#" title="<?php echo Portal::language('net_price');?>"/>
                            
                            <span style="float: left; height: 21px; border-left: 1px solid gray; margin-right: 5px;"></span>
                            
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][deposit_button]" type="button" value="<?php echo Portal::language('deposit');?>" id="deposit_button_#xxxx#" onclick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_PAYMENT;?>&cmd=deposit&id='+$('id_#xxxx#').value+'&r_id=<?php echo Url::get('id');?>&type=RESERVATION&act=traveller&portal_id=<?php echo PORTAL_ID;?>',Array('deposit_'+$('id_#xxxx#').value,'<?php echo Portal::language('deposit');?>','80','210','1000','500'));" style="width:70px; height: 23px; text-align: center !important;"/>
                            ...
                            <span style="float: left; height: 21px; border-left: 1px solid gray; margin-right: 5px;"></span>
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][extra_service_button]" type="button" value="<?php echo Portal::language('extra_char_room');?>" id="extra_service_button_#xxxx#" onclick="getUrlExtraChargeRoom('#xxxx#');" style="width:100px; height: 23px;"/>
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][extra_service_button]" type="button" value="<?php echo Portal::language('extra_service');?>" id="extra_service_button_#xxxx#" onclick="getUrlExtraService('#xxxx#');" style="width:85px; height: 23px;"/>
                            <?php if(User::can_add($this->get_module_id('MinibarInvoice'),ANY_CATEGORY)){ ?>                            
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][minibar_button]" type="button" value="<?php echo Portal::language('Minibar');?>" id="minibar_button_#xxxx#" onclick="if(jQuery('#status_#xxxx#').val()!='CHECKIN'){ alert('<?php echo Portal::language('Only_check_in_room_avaiable');?>!') } else{ window.open('?page=minibar_invoice&cmd=add&reservation_room_id='+jQuery('#id_#xxxx#').val() + '&portal=<?php echo PORTAL_ID; ?>');}" style="width:85px; height: 23px;"/>
                            <?php
                                } 
                            ?>
                            <?php if(User::can_add($this->get_module_id('LaundryInvoice'),ANY_CATEGORY)){ ?>
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][laundry_button]" type="button" value="<?php echo Portal::language('Laundry');?>" id="laundry_button_#xxxx#" onclick="if(jQuery('#status_#xxxx#').val()!='CHECKIN'){ alert('<?php echo Portal::language('Only_check_in_room_avaiable');?>!') } else{window.open('?page=laundry_invoice&cmd=add&reservation_room_id='+jQuery('#id_#xxxx#').val() + '&portal=<?php echo PORTAL_ID; ?>');}" style="width:85px; height: 23px;"/>
                            <?php
                                } 
                            ?>
                            <?php if(User::can_add($this->get_module_id('EquipmentInvoice'),ANY_CATEGORY)){ ?>
                            <input class=" w3-gray w3-hover-orange" name="mi_reservation_room[#xxxx#][equipment_button]" type="button" value="<?php echo Portal::language('Equipment');?>" id="equipment_button_#xxxx#" onclick="if(jQuery('#status_#xxxx#').val()!='CHECKIN'){ alert('<?php echo Portal::language('Only_check_in_room_avaiable');?>!') } else{window.open('?page=equipment_invoice&cmd=add&reservation_room_id='+jQuery('#id_#xxxx#').val() + '&portal=<?php echo PORTAL_ID; ?>');}" style="width:85px; height: 23px;"/>
                            <?php
                                } 
                            ?>
                        </span>
                    ...
                    <div id="extra-service" style="padding:5px 0px 5px 0px;clear:both;">
                        ...
                        <legend class="title"><?php echo Portal::language('Extra_service_and_room_charge');?></legend>
	                    <span class="multi-input-extra">
                           <span class="label"><?php echo Portal::language('extra_service');?>:</span>
                                <span id="extra_service_#xxxx#">
                                
                                </span>
                           <br clear="all" />
                           <span class="label"><?php echo Portal::language('extra_room_charge');?>:</span>
		          ...
		          <div id="box_do_not_move_#xxxx#" style="clear: both; display: none;">
                        <span class="multi-input-extra">
                            ...
                            <span style="float: left;"><?php echo Portal::language('do_not_move');?></span>
                            <input  name="mi_reservation_room[#xxxx#][do_not_move]" class="do_not_move" style="width:15px; margin-right: 5px;" value="1" type="checkbox" id="do_not_move_#xxxx#" title="<?php echo Portal::language('do_not_move');?>"/>
                            <span style="float: left;"><?php echo Portal::language('user_do_not_move');?>: </span>
                            ...
                            <span style="float: left;"><?php echo Portal::language('note_do_not_move');?>: </span>
                    ...
                    <div style="clear: both; display: none;">
                        <span style="float: left; margin-left:20px;"><?php echo Portal::language('Cancel_note');?>: </span>
                        <input name="mi_reservation_room[#xxxx#][cancel_note]" type="text" id="cancel_note_#xxxx#" style="width: 300px; padding: 3px;"  />
                    </div>
                    <div id="extra_bed" style="padding:5px 0px 5px 0px;clear:both;display: none;">
                    	<fieldset style="margin-left:20px;">
                        <legend class="title"><?php echo Portal::language('Extra_bed_baby_cot');?></legend>
	                    <span class="multi-input-extra">
                           <span class="label"><?php echo Portal::language('extra_bed');?>:</span>
                           ...
                           <span class="label"><?php echo Portal::language('extra_bed_from_date');?>:</span>
                           ...
                           <span class="label"><?php echo Portal::language('extra_bed_to_date');?>:</span>
                           ...
                           <span class="label"><?php echo Portal::language('extra_bed_rate');?>:</span>
                        ...
                        <span class="multi-input-extra">
                            <span><?php echo Portal::language('price_schedule');?> <?php echo Portal::language('extra_bed');?></span><br />
					...
					<div id="price_schedule_bound_#xxxx#" style="display:none;float:left;width:100%;padding:5px 0px 5px 0px;margin:5px 0px 5px 0px;">
						<span class="multi-input-extra">
							 <span><strong>&nbsp;&nbsp;<?php echo Portal::language('price_schedule');?></strong></span><br />
							 <span id="price_schedule_#xxxx#"></span>
						</span>
					</div><br clear="all"/>
					<div id="payment_type_bound_#xxxx#" class="payment-type-bound" style="display:none;">
						<span class="multi-input-extra">
                              <span><strong><?php echo Portal::language('payment_type');?>:</strong>
                            ...
                            onchange="
								...
								">
                                <option value="CASH"><?php echo Portal::language('pay_now');?></option>
                                <option value="DEBIT"><?php echo Portal::language('debit');?></option>
								<option value="CREDIT_CARD"><?php echo Portal::language('credit_card');?></option>
                            </select>
							<span id="related_rr_#xxxx#" style="float:right;width:340px;"><?php echo Portal::language('pay_with_room');?> (<?php echo Portal::language('input_invoice_id');?>): <input  name="mi_reservation_room[#xxxx#][related_rr_id]" type="text" id="related_rr_id_#xxxx#" size="8" maxlength="11" style="float:right;font-size:11px;font-weight:bold;color:#FF0000;"></span>
							</span>
							 <div id="currencies_#xxxx#" class="currency-list" style="display:none;"><span>&nbsp;</span>
                               <?php $i=0;?>
								...
								<span>
							 	<?php echo $this->map['currencies']['current']['name'];?><br /><input name="mi_reservation_room[#xxxx#][currency_<?php echo $this->map['currencies']['current']['id'];?>]" type="text" id="currency_<?php echo $this->map['currencies']['current']['id'];?>_#xxxx#" style="width:65px;" onclick="this.value=number_format(to_numeric($('total_payment_#xxxx#').value)*<?php echo $this->map['currencies']['current']['exchange'];?>);">
								</span>
								<?php }}unset($this->map['currencies']['current']);} ?>
								<span><br />&nbsp;&nbsp;</span><span><?php echo Portal::language('total_payment');?><br /><strong><span><input name="mi_reservation_room[#xxxx#][total_payment]" type="text" id="total_payment_#xxxx#" readonly="readonly"  style="width:200px;border:0px;font-weight:bold;"></span></strong></span>
							 </div>
						</span>
					</div>
                    <br clear="all" />
                    <div style="display:none;">&nbsp;&nbsp;&nbsp;&nbsp;
                    	<span id="invoice_#xxxx#" style="display:none;" class="invoice-option">
                           	<input  onchange="updateUrl('#xxxx#');" type="checkbox" id="room_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('room_invoice');?> |
							<input  onchange="updateUrl('#xxxx#');" type="checkbox" id="hk_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('housekepping_invoice');?> |
							<input  onchange="updateUrl('#xxxx#');" type="checkbox" id="bar_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('bar_invoice');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="other_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('other_invoice');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="karaoke_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('karaoke_invoice');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="vend_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('vend_invoice');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="phone_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('phone_invoice');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="extra_service_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('extra_service_invoice');?> |
                            <?php
                if((1))//HAVE_MASSAGE
                {?>
                            <input onchange="updateUrl('#xxxx#');" type="checkbox" id="massage_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('massage_invoice');?> |
                <?php
                }
                ?>
                            <?php
                if((HAVE_TENNIS))
                {?>
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="tennis_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('tennis_invoice');?> |
				<?php
				}
				?>
							<?php
				if((HAVE_SWIMMING))
				{?>
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="swimming_pool_invoice_#xxxx#" checked="checked"/><?php echo Portal::language('swimming_pool_invoice');?> |
				<?php
				}
				?>
							<input  onchange="updateUrl('#xxxx#');" type="checkbox" id="included_deposit_#xxxx#" checked="checked"/><?php echo Portal::language('included_deposit');?> |
                            <input  onchange="updateUrl('#xxxx#');" type="checkbox" id="included_related_total_#xxxx#" checked="checked"/><?php echo Portal::language('included_related_total');?>
				...
				{?><!--<span style="float:left;width:10px;height:20px;"></span><a class="view-registration-form" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card'));?>&id='+$('id_#xxxx#').value)">[ <?php echo Portal::language('guest_registration_card');?> ]</a>-->
				<?php
				}
				?>
<?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><span style="float:left;width:10px;height:20px;"></span><a class="" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card_new'));?>&id='+$('id_#xxxx#').value)" style="text-decoration: none; margin-right: 20px;"><i class="fa fa-pencil-square" style="font-size: 18px;"></i> <?php echo Portal::language('reg_card');?></a>
				<?php
				}
				?>
                <?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><span style="float:left;width:10px;height:20px;"></span><a class="" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>4));?>&id='+$('id_#xxxx#').value)" style="text-decoration: none; margin-right: 20px;"><i class="fa fa-envelope-open" style="font-size: 18px;"></i> <?php echo Portal::language('guest_welcome_card');?></a>
				<?php
				}
				?>
                ...
                <?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><span style="float:left;width:10px;height:20px;"></span><a class="" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>5));?>&id='+$('id_#xxxx#').value)" style="text-decoration: none; margin-right: 20px;"><i class="fa fa-envelope-open-o" style="font-size: 18px;"></i> <?php echo Portal::language('guest_welcome_card_en');?></a>
				<?php
				}
				?>
					<?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><!--<span style="float:left;width:10px;height:20px;"></span><a class="view-registration-form" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>1));?>&id='+$('id_#xxxx#').value)">[ <?php echo Portal::language('registration_form');?> ]</a>-->
				<?php
				}
				?>
                <?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><!--<span style="float:left;width:10px;height:20px;"></span><a class="view-registration-form" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>3));?>&id='+$('id_#xxxx#').value)">[ <?php echo Portal::language('registration_form_vn');?> ]</a>-->
				<?php
				}
				?>
                <?php
				if((User::can_view(false,ANY_CATEGORY)))
				{?><!--<span style="float:left;width:10px;height:20px;"></span><a class="view-registration-form" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>6));?>&id='+$('id_#xxxx#').value)">[ <?php echo Portal::language('guest_registration_form');?> ]</a>-->
				<?php
				}
				?>
                <?php
                ...
                <br />
                 <div id="Remarks" style="padding:5px 0px 5px 0px;clear:both">
                        <span class="multi-input-extra">
                        		<span class="label"><?php echo Portal::language('Remarks_for_room');?></span>
                                <textarea  name="mi_reservation_room[#xxxx#][note]" id="note_#xxxx#" style="width: 80%;"><?php echo String::html_normalize(URL::get('mi_reservation_room[#xxxx#][note]',''));?></textarea>
                        </span>
                 </div>           
                 </div>
...
<input  name="is_change_room_status" id="is_change_room_status"/ type ="hidden" value="<?php echo String::html_normalize(URL::get('is_change_room_status'));?>">
...
<table cellspacing="0" cellpadding="5" width="100%" style="margin-top:10px;">
	<tr valign="top">
		<td align="left">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table-bound">
				<tr height="40">
					<td width="70%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><i class="fa fa-file-text w3-text-blue" style="font-size: 30px;"></i> <?php echo Portal::language('reservation_detail');?>: <strong style="font-size: 30px; color: red;"><?php echo $this->map['id'];?></strong></td>
                    <?php if($this->map['close_mice']==0){ ?>
                        <?php if(User::can_edit(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap" style="padding-right: 10px;"><input name="asign" type="button" value="<?php echo Portal::language('assign');?>" class="w3-button w3-yellow"   onclick="check_room_asign();"/></td><?php }?>
    					<?php if(User::can_edit(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap" style="padding-right: 10px;"><input name="save_button" type="button" value="<?php echo Portal::language('Save_and_close');?>" class="w3-button w3-orange w3-hover-orange" onclick="checkDirtyAll('save');" style="text-transform: uppercase;" /><input class="" name="save" id="save" type="checkbox" style="display: none;"/></td><?php }?>
                        <?php if(User::can_edit(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap" style="padding-right: 10px;"><input name="update_button" type="button" value="<?php echo Portal::language('Save_and_stay');?>" class="w3-button w3-blue w3-hover-blue" onclick="checkDirtyAll('update');"  style="text-transform: uppercase; "/><input name="update" id="update" type="checkbox" value="<?php echo Portal::language('Save_and_stay');?>" style="display: none;"/></td><?php }?>
    					<td width="1%" nowrap="nowrap" style="padding-right: 10px;"><a href="<?php echo Url::build('room_map')?>" class="w3-button w3-gray w3-hover-gray" onclick="if(!confirm('<?php echo Portal::language('are_you_sure_to_close');?> <?php echo Portal::language('if_you_close_all_datas_will_not_be_save');?>')){return false;}"  style="text-decoration: none; text-transform: uppercase;"><?php echo Portal::language('close');?></a></td>
                        <?php if(User::can_admin(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap" style="padding-right: 10px;"><a href="<?php echo Url::build_current(array('cmd'=>'delete','id'));?>" class="w3-button w3-red w3-hover-red" style="text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('delete');?></a></td><?php }?>
                    <?php } ?>
				</tr>
		  </table>
		</td>
	...
	<tr><td align="left">
			...
			<legend class="w3-white" style="width: 190px; height: 26px; padding: 5px 5px 0px 5px; border: 1px solid orange;"><?php echo Portal::language('general_information');?></legend>
                ...
                <tr valign="top">                 
                 ...
                 <td align="right"><?php echo Portal::language('booker');?></td>                 
                 <td style="padding-left:10px;">
                 <input  name="booker" id="booker" style="margin-bottom: 5px;width:170px; text-transform: uppercase; font-size: 13px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('booker'));?>">
                 </td>                 
                 <td align="right" nowrap>&nbsp;</td>
                 <td rowspan="4" width="40%"><span class="label"><?php echo Portal::language('note_for_tour_or_group');?></span>
                    ...
                    <tr valign="top">
                        <td align="right" nowrap>&nbsp;</td>
                        <td align="right"><?php echo Portal::language('customer');?></td>
                        <td align="left" style="padding-left:10px;">
                        <div style=" float: left;">
                        <input  name="customer_name" id="customer_name" onkeypress="Autocomplete();"  autocomplete="off" style="width:170px;margin-bottom: 5px;" / type ="text" value="<?php echo String::html_normalize(URL::get('customer_name'));?>"><!--Giap comment readonly="readonly"  class="readonly"-->
                        <input name="customer_id" type="text" id="customer_id" value="<?php echo $this->map['customer_id'];?>" class="hidden" />
                        <?php 
				if((User::can_edit(false,ANY_CATEGORY)))
				{?><a class="w3-text-blue" href="#" onclick="window.open('?page=customer&amp;action=select_customer&customer_id_of_kid='+jQuery('#customer_id').val(),'customer')" style="text-decoration: none;"> <i class="fa fa-plus-square" style="font-size: 18px;"></i> <?php echo Portal::language('add_now');?></a> &nbsp; <!---<a class="w3-text-blue" href="#" onClick="$('customer_name').value='';$('customer_id').value='';" style="cursor:hand;"><i class="fa fa-times-circle w3-text-red" style="font-size: 18px;"></i></a>--->
				<?php
				}
				?>
                        </div>                       
                        </td>
                        <td align="right"><?php echo Portal::language('phone_booker');?></td>
                        <td style="padding-left:10px;">
                        <input  name="phone_booker" id="phone_booker" style="margin-bottom: 5px;width:170px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('phone_booker'));?>">
                        </td>
                    ...
                    <tr valign="top" style="display: none;">
                        <td align="right" nowrap>&nbsp;</td>
                        <td align="right"><?php echo Portal::language('tour');?> / <?php echo Portal::language('group');?></td>
                        <td align="left" style="padding-left:10px;">
                            <input  name="tour_name" id="tour_name" style="width:170px;margin-bottom: 5px;" readonly="" class="readonly"/ type ="text" value="<?php echo String::html_normalize(URL::get('tour_name'));?>">
                            <input  name="tour_id" type="text" id="tour_id" value="<?php echo $this->map['tour_id'];?>" class="hidden"/>
                            <?php 
				if((User::can_edit(false,ANY_CATEGORY)))
				{?><a href="#" onclick="window.open('?page=tour&amp;action=select_tour','tour')"><i class="fa fa-search" style="font-size: 18px;"></i></a> <img width="15" src="packages/core/skins/default/images/buttons/delete.gif" onClick="$('tour_name').value='';$('tour_id').value=0;" style="cursor:hand;"/>
				<?php
				}
				?>
                        </td>
                    ...
                    <tr valign="top">
                      ...
                      <td align="right">
                      <label class="w3-text-blue" for="is_rate_code" style="margin-left: 10px;"><?php echo Portal::language('use_rate_code');?></label>
                      </td>
                      <td>
                      <input  name="is_rate_code" id="is_rate_code" onclick="using_rate_code(this);" style="margin-left: 10px;" / type ="checkbox" value="<?php echo String::html_normalize(URL::get('is_rate_code'));?>"> 
                      ...
                      ?>
                        ...
                        <td style="padding-left:10px;">
                        <input  name="email_booker" id="email_booker" style="margin-bottom: 5px;width:170px;"/ type ="email" value="<?php echo String::html_normalize(URL::get('email_booker'));?>">
                        </td>                        
                    </tr>
                    <tr valign="top" style="display:none;">
                        <td align="right" nowrap>&nbsp;</td>
                        <td align="right" height="20"><?php echo Portal::language('total_payment');?></td>
                        <td align="left" style="padding-left:10px;font-weight:bold;"><span id="total_payment"></span></td>
                    </tr>
                    <tr valign="top" style="display:none;">
                        <td align="right" nowrap>&nbsp;</td>
                        <td align="right"><?php echo Portal::language('payment');?></td>
                        <td align="left" style="padding-left:10px;">
                    ...
                    <tr valign="top">
                        <td align="right" nowrap>&nbsp;</td>
                        <td align="right"><?php echo Portal::language('payment_type1');?></td>
                        <td align="left" style="padding-left:10px;">
                            <select  name="payment_type1" id="payment_type1" style="width:170px;margin-bottom: 5px; height: 20px;"><?php
					if(isset($this->map['payment_type1_list']))
					{
						foreach($this->map['payment_type1_list'] as $key=>$value)
						{
                    ...
                    if(URL::get('payment_type1',isset($this->map['payment_type1'])?$this->map['payment_type1']:''))
                    echo "<script>$('payment_type1').value = \"".addslashes(URL::get('payment_type1',isset($this->map['payment_type1'])?$this->map['payment_type1']:''))."\";</script>";
                    ?>
	</select> 
                        </td>
                        <td align="right"><?php echo Portal::language('color_of_group');?></td>
                        <td align="left" style="padding-left:10px;">
                            <input  name="color" id="color" style="width:148px;margin-bottom: 5px;" / type ="text" value="<?php echo String::html_normalize(URL::get('color'));?>">
                            <span onclick="TCP.popup($('color'));" class="color-select-button" title="<?php echo Portal::language('select_color');?>"><img src="packages/core/skins/default/images/color_picker.gif" width="18"></span>
                            <?php if($this->map['color']){?><script type="text/javascript">jQuery('#color').css('background-color','<?php echo $this->map['color'];?>');jQuery('#color').css('color','<?php echo $this->map['color'];?>');</script><?php }?>
                        </td>
                        
                        <td bgcolor="#EFEFEF">
                            <?php if((User::can_edit(false,ANY_CATEGORY))){?>
                            <!--<input name="print" type="button" id="print" value="<?php echo Portal::language('preview_invoice');?>" class="button-medium-add" onclick="window.location='<?php echo Url::build_current(array('cmd'=>'print','id','r_r_id'));?>'" />-->
                            <?php } ?>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr valign="top" style="line-height: 22px;">
                        <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ ?>
                            ...
                            <td align="left" style="padding-left:10px;">
                                ...
                                <input value="<?php echo Portal::language('split');?> MICE" type="button" onclick="FunctionSplitMice('<?php echo $this->map['mice_reservation_id'];?>','REC','<?php echo $this->map['id'];?>');" style="font-weight: bold; padding: 3px 10px; margin-left: 20px;" />
                            </td>
                            <td align="center" colspan="2" style="color: blue;"><?php echo Portal::language('personnel_booking');?>: <?php echo $this->map['user_name'];?> ( <?php echo date("H:i d/m/Y",$this->map['time'] ); ?> )</td>
                            <td align="right" nowrap>&nbsp;</td>
                        <?php }else{ ?>
                            <td></td>
                            <td align="right"><input class="w3-button w3-gray w3-hover-lime" value="<?php echo Portal::language('add_mice');?>" type="button" onclick="FunctionAddMice('REC','<?php echo $this->map['id'];?>');" style="padding: 3px 10px;" /></td>
                            <td align="left" style="padding-left:10px;"><input class="w3-button w3-gray w3-hover-lime" value="<?php echo Portal::language('select');?> MICE" type="button" onclick="FunctionSelectMice('REC','<?php echo $this->map['id'];?>');" style="padding: 3px 10px;" /></td>
                            <td align="center" colspan="2" style="color: blue;"><?php echo Portal::language('personnel_booking');?>: <?php echo $this->map['user_name'];?> ( <?php echo date("H:i d/m/Y",$this->map['time']); ?> )</td>
                        <?php } ?>                                        
                            <td class="w3-margin-right" style="text-align: right;" colspan="3"><a class="w3-button w3-teal w3-hover-teal" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;" href="javascript:void(0);" onclick="window.open('?page=general_statement&id=<?php echo $_GET['id']; ?>')"><?php echo Portal::language('invoice_information');?></a>
                            <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ }else{ 
                            if(User::can_view($this->get_module_id('CreateTravellerFolio'),ANY_CATEGORY)){
                        ?> 
                        <a class="w3-button w3-indigo w3-hover-indigo" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;" href="#" onClick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_CREATE_FOLIO;?>&cmd=group_folio&id=<?php echo Url::get('id');?>&customer_id='+jQuery('#customer_id').val(),Array('group_folio_<?php echo Url::get('id');?>','<?php echo Portal::language('create_folio');?>','80','210','950','540'));" id="group_folio_a"><?php echo Portal::language('group_folio');?></a>
                         <?php } } ?></td>
                    </tr>
                ...
                <div style="border-top:1px solid #CCCCCC;margin-bottom:5px;display:none">
                    <div style="font-size:11px;border-top:1px solid #FFFFFF;padding-top:2px;">
                        <input  name="include_booked" type="checkbox" id="include_booked"/> <?php echo Portal::language('Include_booked');?> |
                        <input  type="checkbox" id="room_invoice" checked="checked"/><?php echo Portal::language('room_invoice');?> |
                        <input  type="checkbox" id="hk_invoice" checked="checked"/><?php echo Portal::language('housekepping_invoice');?> |
                        <input  type="checkbox" id="bar_invoice" checked="checked"/><?php echo Portal::language('bar_invoice');?> |
                        <input  type="checkbox" id="karaoke_invoice" checked="checked"/><?php echo Portal::language('karaoke_invoice');?> |
                        <input  type="checkbox" id="vend_invoice" checked="checked"/><?php echo Portal::language('vend_invoice');?> |
                        <input  type="checkbox" id="other_invoice" checked="checked"/><?php echo Portal::language('other_invoice');?> |
                        <input  type="checkbox" id="phone_invoice" checked="checked"/><?php echo Portal::language('phone_invoice');?> |
                        <input  type="checkbox" id="extra_service_invoice" checked="checked"/><?php echo Portal::language('extra_service_invoice');?> |
                        <!--HAVE_MASSAGE-->
                        <?php 
				if((1))
				{?>
                        <input  type="checkbox" id="massage_invoice" checked="checked"/><?php echo Portal::language('massage_invoice');?> |
				...
				if((HAVE_TENNIS))
				{?>
                        <input  type="checkbox" id="tennis_invoice" checked="checked"/><?php echo Portal::language('tennis_invoice');?> |
				...
				if((HAVE_SWIMMING))
				{?>
                        <input  type="checkbox" id="swimming_pool_invoice" checked="checked"/><?php echo Portal::language('swimming_pool_invoice');?> |
                        
				<?php
				}
				?>
                        <input  type="checkbox" id="included_deposit" checked="checked"/><?php echo Portal::language('included_deposit');?>
                    </div>
                </div>
                ...
                <div style="border-top:1px solid #CCCCCC;margin-top:5px;float:left;width:100%;">
                    <div style="font-size:11px;border-top:1px solid #FFFFFF;float:left;width:100%;text-align:center;padding-top:5px;">
                        <!---<a class="view-order-button" href="javascript:void(0);" onclick="window.open('?page=general_statement&id=<?php echo $_GET['id']; ?>')"><?php echo Portal::language('invoice_information');?></a>
                        <a class="view-order-button" href="javascript:void(0);" onclick="window.open('?page=reservation&cmd=consolidated_invoice&id=<?php echo $_GET['id']; ?>&portal=<?php echo PORTAL_ID; ?>')"><?php echo Portal::language('view_group_invoice');?></a>
                        <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ }else{ 
                            if(User::can_view($this->get_module_id('CreateTravellerFolio'),ANY_CATEGORY)){
                        ?> 
                        <a class="add-order-button" href="#" onClick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_CREATE_FOLIO;?>&cmd=group_folio&id=<?php echo Url::get('id');?>&customer_id='+jQuery('#customer_id').val(),Array('group_folio_<?php echo Url::get('id');?>','<?php echo Portal::language('create_folio');?>','80','210','950','540'));" id="group_folio_a"><?php echo Portal::language('group_folio');?></a>
                         <?php } } ?>--->
                        <!--<a class="print-order-button" href="#" onclick="window.open('?page=vat_invoice&cmd=entry&department=RECEPTION&type=GROUP&r_id=<?php //echo Url::get('id');?>');"><?php echo Portal::language('print_VAT_group_invoice');?></a>-->
                        <!--<a href="#" onclick="openGroupInvoice(false);" class="view-invoice-button"><?php echo Portal::language('view_tour_invoice');?></a> -->
                        <a class="w3-hover-text-red" target="_blank" href="<?php echo Url::build_current(array('cmd'=>'rooming_list','id'))?>" style="text-decoration: none; padding-right: 15px;"><i class="fa fa-address-book-o" style="font-size: 18px;"></i> <?php echo Portal::language('rooming_list');?></a>
                        <a class="w3-text-black w3-hover-text-red" href="#" onClick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_UPDATE_TRAVELLER;?>&cmd=group_traveller&r_id=<?php echo Url::get('id');?>&customer_id='+jQuery('#customer_id').val(),Array('group_traveller_<?php echo Url::get('id');?>','<?php echo Portal::language('add_traveller');?>','20','110','1100','570'));" id="group_traveller_a" style="text-decoration: none; padding-right: 15px;"><i class="fa fa-address-card-o" style="font-size: 18px;"></i> <?php echo Portal::language('add_traveller');?></a>
                        ...
                        <a class="w3-hover-text-red" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'group_registration','id'=>Url::get('id')));?>')" style="text-decoration: none; padding-right: 15px;"><i class="fa fa-pencil-square-o" style="font-size: 18px;"></i> <?php echo Portal::language('group_registration_card');?></a>
                        <a class="w3-hover-text-red" target="_blank" onclick="window.open('<?php echo Url::build_current(array('cmd'=>'guest_registration_card','form'=>8));?>&re_id=<?php echo $_GET['id']; ?>')" style="text-decoration: none; padding-right: 15px;"><i class="fa fa-coffee" style="font-size: 18px;"></i> <?php echo Portal::language('titket_breakfast_form');?></a>
                        <a class="w3-hover-text-red" target="_blank" onclick="window.open('<?php echo '?page=history_log&recode='.Url::get('id');?>')" style="text-decoration: none; "><i class="fa fa-spinner fa-spin" style="font-size: 18px;"></i> <?php echo Portal::language('log_recode');?></a>
                        </div>
                </div>
                <div id="wrapper" style="display:none; position: fixed; width:100%; height:100%; z-index:999; background: black; opacity: 0.5; top:0px; left:0px;">
                    
                </div>
                <div id="cancel_extra" style="z-index:9999;position: fixed; width:300px; height:285px; border-radius:10px 10px 0px 0px; background: white; top:30%; left:40%; display:none;">
                        <div style="background: #7979FF;padding-left:7px; width:100%;height:40px; border-radius:10px 10px 0px 0px; float:left; box-sizing: border-box;">
                            <h4 style="width: 90%; float: left; color: white;"><?php echo Portal::language('cancellation_note_details');?></h4>
                        </div>
                        <div id="cancel_input" style="padding-left: 10px;">
                            <br />
                            <div style="margin-top: 40px;"><?php echo Portal::language('do_you_want_to_cancel_this_booking');?>?</div>
                            <br />
                            <div style="width: 100%;">
                                <textarea  name="cancel_note" id="cancel_note" cancel_input="" target="" style="resize: none; width: 90%; margin: 5px; margin-top:15px; height: 100px;" placeholder="<?php echo Portal::language('reason_cancel');?>" position="3"><?php echo String::html_normalize(URL::get('cancel_note',''));?></textarea>
                            </div>
                            <br />
                            <button style="float: right; margin-right: 10px;" type="button" onclick="cancel_back();" type="button"><?php echo Portal::language('cancel');?></button>
                            <button style="float: right; margin-right: 10px;" type="button" onclick="setValue();"><?php echo Portal::language('save');?></button>
                        </div>
                </div>
        </fieldset>
		<fieldset style="background:#E5E5E4;margin-bottom:5px;">
			<legend class="w3-white w3-text-black" style="width: 190px; height: 26px; padding: 5px 5px 0px 5px; border: 1px solid orange;"><?php echo Portal::language('reservation_room');?>&nbsp;(<span style="color: red; font-size: 20px;" id="count_number_of_room"></span> <?php echo Portal::language('room');?>)</legend>
			<span id="mi_reservation_room_all_elems">
				<span>
                	...
                	<span class="multi-input-header" style="width:34px;"><?php echo Portal::language('index');?></span>
                    
					<span class="multi-input-header" style="width:70px;"><?php echo Portal::language('room_level');?></span>
					<span class="multi-input-header" style="width:70px;"><?php echo Portal::language('room');?></span>
					<span class="multi-input-header" style="width:38px;"><?php echo Portal::language('adult');?></span>
                    <span class="multi-input-header" style="width:58px;"><?php echo Portal::language('child');?></span>
                    <span class="multi-input-header" style="width:48px;"><?php echo Portal::language('child_5');?></span> <!-- trung add: tr.e<5 tuoi -->
					<span class="multi-input-header" style="width:65px;"><?php echo Portal::language('room_rate');?></span>
                    <span class="multi-input-header" style="width:65px;"><?php echo Portal::language('usd_price');?></span>
                    ...
                    <?php if(User::can_admin($this->get_module_id('ReceptionCommission'),ANY_CATEGORY)){ ?>
                    <span class="multi-input-header" style="width:42px;"><?php echo Portal::language('cms');?></span>
					<?php } ?>
					<span class="multi-input-header" style="width:45px;"><?php echo Portal::language('time_in');?></span>
                    <span class="multi-input-header" style="width:62px;"><?php echo Portal::language('arrival_date');?></span>
					<!--<span class="multi-input-header" style="width:16px;"><input  name="all_early_checkin" onclick="jQuery('.early_checkin').attr('checked',jQuery(this).attr('checked'));" / type ="checkbox" value="<?php echo String::html_normalize(URL::get('all_early_checkin'));?>"></span>-->
					<span class="multi-input-header" style="width:45px;"><?php echo Portal::language('time_out');?></span>					
                    <span class="multi-input-header" style="width:16px; display: none;" title="<?php echo Portal::language('early_arrival_time');?>">E.A</span>
					<span class="multi-input-header" style="width:62px;"><?php echo Portal::language('departure_date');?></span>
					<span class="multi-input-header" style="width:45px; display: none;" title="<?php echo Portal::language('early_checkin');?>">E.I</span>
                    <span class="multi-input-header" style="width:45px; display: none;" title="<?php echo Portal::language('late_checkout');?>">L.O</span>
					<span class="multi-input-header" style="width:80px;"><?php echo Portal::language('status');?></span>
					<span class="multi-input-header" style="width:40px; display: none;"><?php echo Portal::language('reservation_type');?></span>
					<span class="multi-input-header" style="width:20px;" title="<?php echo Portal::language('confirm');?>"><?php echo Portal::language('CF');?></span>
                    <span class="multi-input-header" style="width:20px;" title="<?php echo Portal::language('breakfast');?>"><?php echo Portal::language('BF');?></span>
                    <span class="multi-input-header" style="width:20px;<?php if(!USE_ALLOTMENT){ ?> display: none; <?php } ?>" title="<?php echo Portal::language('allotment');?>"><?php echo Portal::language('ALM');?></span>
					<span class="multi-input-header" style="width:40px;" title="<?php echo Portal::language('closed');?>"><i class="fa fa-user-times w3-text-red" style="font-size: 15px;"></i>
                    <!-- tieubinh add-->
                            ...
                            <span class="multi-input-header" style="width:20px;">
                                <?php
                                    switch(SERVER_KEY)
                                    {
                                        ...
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                            <?php
                                            break; 
                                        }
                                        case IS_ADEL:
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key_adel&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                            <?php 
                                            break;
                                        }
                                        case IS_SALTO:
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key_salto&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')"><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                            <?php 
                                            break;
                                        }
                                        case IS_HUNERF:
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key_hune&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                        ...
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key_orbita&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                            <?php
                                            break; 
                                        }
                                        case IS_BQLOCK:
                                        {
                                            ?>
                                            <a title="Tạo nhóm khóa" target="_blank" onclick="window.open('?page=manager_key_bqlock&cmd=create_group&reservation_id='+'<?php echo Url::get('id'); ?>&portal=<?php echo PORTAL_ID; ?>')" ><i class="fa fa-tags w3-text-red" style="font-size: 18px;"></i></a>
                                            <?php
                                            break; 
                                        }
                                        default:
                    ...
                    <!--Oanh add -->
                    <span id="hidden_room_cancel_span" style="display: none;" ><input type="checkbox" name="hidden_room_cancel" id="hidden_room_cancel" checked="checked" onclick="hiddenRoomCancel();"/><?php echo Portal::language('hidden_room_cancel');?></span>   
                    <!--End Oanh -->
					<br clear="all"/>
				</span>
			</span>
            <br clear="all"/>
           	<input type="button" value="<?php echo Portal::language('add_room');?>" onclick="var roomCount = to_numeric($('count_number_of_room').innerHTML);$('count_number_of_room').innerHTML=roomCount+1;mi_add_new_row('mi_reservation_room');updateStatusList(input_count);updateInput(input_count);AddInput(input_count);buildRateList(input_count);buildCommissionList(input_count);if($('index_'+input_count)){$('index_'+input_count).innerHTML = input_count;}" class="w3-button w3-gray w3-hover-cyan w3-hover-text-white"/>
            <br clear="all"/>
            <br clear="all"/>
<!---THAY DOI CHO GROUP--->
            ...
            <fieldset id="change_for_group_div" style="width: 99%; margin: 0px auto; background: #F2F4CE; border: 1px dashed #171717;">
                <legend style="font-weight: bold; text-transform: uppercase;"><?php echo Portal::language('change_for_group');?></legend>
                <table style="width: 100%;">
                    <tr style="background: #fac73e;">
                        <td style="width: 180px; padding-left: 40px;"><?php echo Portal::language('room_type');?></td>
                        <td style="width: 125px;"><?php echo Portal::language('adult');?></td>
                        <td style="width: 70px;"><?php echo Portal::language('price');?></td>
                        <td style="width: 65px;"><?php echo Portal::language('usd_price');?></td>
                        <td style="width: 40px;"><?php if(User::can_admin($this->get_module_id('ReceptionCommission'),ANY_CATEGORY)) {?><?php echo Portal::language('cms');?><?php }?></td>
                        <td style="width: 45px;"><?php echo Portal::language('time_in');?></td>
                        <td style="width: 65px;"><?php echo Portal::language('arrival_time');?></td>
                        <td style="width: 50px;"><?php echo Portal::language('time_out');?></td>
                        <td style="width: 65px;"><?php echo Portal::language('departure_time');?></td>
                        <td style="width: 30px; display: none;">E.I</td>
                        <td style="width: 30px; display: none;">L.O</td>
                        <td style="width: 77px;"><?php echo Portal::language('status');?></td>
                        <td style="width: 20px;"><?php echo Portal::language('CF');?></td>
                        <td><?php echo Portal::language('BF');?></td>
                    </tr>
                    <tr style="border-bottom: 1px solid #171717;">
                        <td style="width: 180px; padding-left: 40px; cursor: pointer;" onclick="fun_Check_Availblity(input_count,'<?php echo Url::get('cmd');?>');"><i class="fa fa-refresh w3-text-blue" style="font-size: 18px;"></i> <?php echo Portal::language('choose_room_type');?></td>
                        ...
                        <td style="width: 40px;"><?php if(User::can_admin($this->get_module_id('ReceptionCommission'),ANY_CATEGORY)) {?><select   name="change_all_commission_rate" id="change_all_commission_rate" onchange="changeAllCommissionRate(this.value);"><?php echo $this->map['commission_rate_options'];?></select><?php }?></td>
                        ...
                        <td style="width: 30px; display: none;"><select  name="all_early_checkin" style="width: 50px;" id="all_early_checkin"  onchange="fun_change_all_ei(this.value);"><?php
					if(isset($this->map['all_early_checkin_list']))
					{
						foreach($this->map['all_early_checkin_list'] as $key=>$value)
						{
							echo '<option value="'.$key.'"';
							echo '>'.$value.'</option>';
                    ...
                    if(URL::get('all_early_checkin',isset($this->map['all_early_checkin'])?$this->map['all_early_checkin']:''))
                    echo "<script>$('all_early_checkin').value = \"".addslashes(URL::get('all_early_checkin',isset($this->map['all_early_checkin'])?$this->map['all_early_checkin']:''))."\";</script>";
                    ?>
	<?php echo $this->map['verify_dayuse_options'];?></select></td>
                        <td style="width: 30px; display: none;"><select  name="all_late_checkout" style="width: 50px;" id="all_late_checkout" onchange="fun_change_all_lo(this.value);"><?php
					if(isset($this->map['all_late_checkout_list']))
					{
						foreach($this->map['all_late_checkout_list'] as $key=>$value)
						{
							echo '<option value="'.$key.'"';
							echo '>'.$value.'</option>';
                    ...
                    if(URL::get('all_late_checkout',isset($this->map['all_late_checkout'])?$this->map['all_late_checkout']:''))
                    echo "<script>$('all_late_checkout').value = \"".addslashes(URL::get('all_late_checkout',isset($this->map['all_late_checkout'])?$this->map['all_late_checkout']:''))."\";</script>";
                    ?>
	<?php echo $this->map['verify_dayuse_options'];?></select></td>
                        <td style="width: 77px;"><select  name="change_all_status" id="change_all_status" onchange="changeAllStatus(this.value,input_count);if(this.value=='CHECKOUT'){ <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ }else{ ?> check_folio(0,1); <?php } ?> };if(this.value=='CANCEL') { check_cancel_deposit(0,1);}" style="width: 80px; height: 21px;"><option value=""><?php echo Portal::language('status');?></option><option value="CHECKIN">CHECKIN</option><option value="CHECKOUT">CHECKOUT</option><option value="CANCEL">CANCEL</option><option value="BOOKED">BOOKED</option></select></td>
                    ...
                    <tr>
                        <td colspan="14" style="text-align: center; ">
                            <input class="w3-cyan w3-hover-cyan w3-text-white" name="deposit_for_group" type="button" id="deposit_for_group" style="border: 1px solid #dddddd; height: 28px; <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ echo "display: none;"; } ?>" value="<?php echo Portal::language('deposit_for_group');?>" onclick="openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_PAYMENT;?>&cmd=deposit&act=group&id=<?php echo Url::get('id');?>&type=RESERVATION&customer_id=<?php echo $this->map['customer_id'];?>&portal_id=<?php echo PORTAL_ID;?>',Array('deposit_group_<?php echo Url::get('id');?>','<?php echo Portal::language('deposit');?>','80','210','1000','500'));" />
                            <!-- manh -->
                            <?php if($this->map['deposit']>0){ ?>
                            <span><?php echo Portal::language('deposit_monney');?>: <?php echo System::display_number($this->map['deposit']);?> |</span> 
                            <?php }else{ echo ""; } ?>
                            <!-- end manh -->
                            |
                            <?php echo Portal::language('deposit_cut_of_date');?> : <input  name="deposit_cut_of_date" id="deposit_cut_of_date" style="width:65px;" / type ="text" value="<?php echo String::html_normalize(URL::get('deposit_cut_of_date'));?>"> 
                            |
                            <?php echo Portal::language('cut_off_day');?>: <input  name="cut_of_date" id="cut_of_date" style="width:65px;" class="date-input" / type ="text" value="<?php echo String::html_normalize(URL::get('cut_of_date'));?>"> 
                            | 
                            <?php echo Portal::language('net_price');?>: <input  name="change_all_net_price" id="change_all_net_price" onclick="fun_change_all_net();" / type ="checkbox" value="<?php echo String::html_normalize(URL::get('change_all_net_price'));?>"> 
                            |
                            <?php echo Portal::language('foc_group');?>: <input  name="foc_group" id="foc_group" onclick="func_chang_foc_group();" / type ="checkbox" value="<?php echo String::html_normalize(URL::get('foc_group'));?>">
                            |
                            <?php echo Portal::language('foc_all_group');?>: <input  name="foc_all_group" id="foc_all_group" onclick="func_chang_foc_all_group();" / type ="checkbox" value="<?php echo String::html_normalize(URL::get('foc_all_group'));?>">
                            | 
                            <?php echo Portal::language('discount_for_group');?>: <input type="text" id="discount_for_group" class="input_number" onkeyup="changeAllDiscount(this.value);" style="width:30px;"/>(%)
                            | 
                            <?php echo Portal::language('copy_room');?> <input  name="room_indexs" id="room_indexs" value="<?php echo Portal::language('index');?>1,<?php echo Portal::language('index');?>2" onclick="if(this.value=='<?php echo Portal::language('index');?>1,<?php echo Portal::language('index');?>2'){this.value='';}" onblur="if(this.value==''){this.value='<?php echo Portal::language('index');?>1,<?php echo Portal::language('index');?>2';}" type ="text" value="<?php echo String::html_normalize(URL::get('room_indexs'));?>" style="width:50px;" />  <?php echo Portal::language('quantity');?> 
                            <input  name="room_quantity" id="room_quantity" style="width:20px;" size="100" maxlength="3" type ="text" />
                            <input type="button" value="<?php echo Portal::language('copy');?>" onclick="copyRoom($('room_indexs').value,$('room_quantity').value,input_count)"/>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="14" style="text-align: center;">
                            <fieldset style="width: 100%; text-align: center;">
                                <legend style="text-transform: uppercase;"><?php echo Portal::language('change_price_in_date_on_group');?></legend>
                                <?php echo Portal::language('select_room_level');?>:
                                <select onchange="func_change_price_in_date();" id="room_level_group" name="room_level_group" style="height: 20px;"><?php echo $this->map['room_level_options'];?></select>
                                <?php echo Portal::language('select_date');?>:
                                <input onchange="func_change_price_in_date();" style="width: 80px;" id="date_group" name="date_group" type="text" />
                                <?php echo Portal::language('input_price');?>:
...
<div id="commission_list" class="room-rate-list" style="display:none;">
    <div>
        <?php echo Portal::language('commission_list');?>&nbsp;&nbsp;
        <a onclick="$('commission_list').style.display='none';"><img src="skins/default/images/close.JPG" title="<?php echo Portal::language('close');?>"/></a>
    </div>
...
<script>
var mice_reservation_id = '<?php echo $this->map['mice_reservation_id'];?>';
jQuery("#date_group").datepicker();
<?php
    if(Url::get('adddd_guest') and Url::get('adddd_guest') == 'yes')
    {
?>
    openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_UPDATE_TRAVELLER;?>&cmd=group_traveller&r_id=<?php echo Url::get('id');?>&customer_id='+jQuery('#customer_id').val(),Array('group_traveller_<?php echo Url::get('id');?>','<?php echo Portal::language('add_traveller');?>','20','110','990','570'));
<?php 
    } 
?>
...
var holidays = <?php echo $this->map['holidays'];?>;
...
jQuery(document).ready(function(){
    ...
    for(var i=101;i<=input_count;i++)//(cÃ¡i nÃ y giáº¥u nÃºt táº¡o hÃ³a Ä‘Æ¡n nhÃ³m phÃ²ng Ä‘Ã£ check out Ä‘á»‘i vá»›i tÃ i khoáº£n thÆ°á»ng)
    {
        if(jQuery('#status_'+i).val()=='CANCEL'){
            ...
            jQuery("#cancel_note_"+i).parent().css("display","");
        }
        if(jQuery('#status_'+i).val()=='CANCEL')
        {
                            ...
                            xmlhttp.onreadystatechange=function()
                            {
                                if (xmlhttp.readyState==4 && xmlhttp.status==200)
                                {
                                    ...
                                    for(var i in otbjs)
                                    {
                                        ...
                                        {
                                            $mess += '<?php echo Portal::language('room');?> '+$('room_name_'+otbjs[i]['index']).value+' <?php echo Portal::language('dirting');?>! \n';
    ...
    if(status_check=='CHECKIN' && room_id_check!='' && ( (!mi_reservation_room_arr) || (mi_reservation_room_arr && mi_reservation_room_arr[id_check]==undefined) || (mi_reservation_room_arr && mi_reservation_room_arr[id_check]!= undefined && mi_reservation_room_arr[id_check]['status']=='BOOKED') ))
    {
        ...
        xmlhttp.onreadystatechange=function()
        {
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
            {
                ...
                {
                    if(!messeger)
                        alert('<?php echo Portal::language('room');?> '+room_name_check+' <?php echo Portal::language('dirting');?>!');
...
function windowOpenUrlTraveller(rr_id,r_id,rt_id){
	openWindowUrl('http://<?php echo $_SERVER['HTTP_HOST'];?>/<?php echo Url::$root;?>form.php?block_id=<?php echo BLOCK_CREATE_FOLIO;?>&cmd=create_folio&rr_id='+rr_id+'&r_id='+r_id+'&traveller_id='+rt_id+'',Array('folio_traveller_'+rt_id+'','<?php echo Portal::language('create_folio');?>','80','210','950','500'));
}
...
function confirm_delete(index)
{
    ...
    {
        <?php if(User::can_delete(false,ANY_CATEGORY)){ ?>
        var con_ = confirm('<?php echo Portal::language('are_you_sure_to_delete_room');?>: '+room_name);
        if(con_)
        {
...
function check_cancel(value,target){
    ...
    function check_cancel_group(value){
        if(value=="CANCEL"){
            ...
            if(!boolen){
                alert("Có phòng hiện đang CHECKIN. Xin vui lòng kiểm tra lại !");
                return false;
            }
            jQuery("#cancel_charge").css('display','block');
            ...
            jQuery("#cancel_note").attr("target","group");      
        }
        else{
            jQuery("#cancel_charge").css('display','none');
            jQuery("span[cancel_span_target]").each(function(){
...
//end
// Start - Thanh - Nhap ly do cancel -------  
     
     function getValue(cancel_input_id){  
          jQuery("#cancel_note").attr("target","single");        
          jQuery("#cancel_note").attr("cancel_input",cancel_input_id);
          jQuery("#cancel_note").val(jQuery("#cancel_note_"+cancel_input_id).val());
                    
     }
     function setValue()
     {
          
          var target_temp = jQuery("#cancel_note").attr("target");
          var cancel_note = jQuery("#cancel_note").val();   
          var id_temp =  jQuery("#cancel_note").attr("cancel_input");
          
              if(target_temp=="single")
              {                            
                   if(cancel_note.trim()!="")
                   {
                      jQuery("#cancel_note_"+id_temp).val(jQuery("#cancel_note").val());  
                      jQuery("#cancel_note_"+id_temp).parent().css("display",'');
                   }
                   else
                   {
                        alert("Bắt buộc phải nhập lý do cancel");
                        var old_status = jQuery("#old_status_"+id_temp).val();
                        jQuery("#status_"+id_temp+" option[value="+old_status+"]").attr("selected","");
                        count_price(id_temp,false);
                        return false;
                   }   
              }
              else
              {             
                if(cancel_note.trim()!="")
                {  
                    jQuery("input[id^=cancel_note_]").each(function(){
                        if(jQuery(this).val()=="")
                        {
                            jQuery(this).val(cancel_note);
                ...
                {
                    alert("Bắt buộc phải nhập lý do cancel");
                    jQuery("input[id^=old_status_]").each(function(){
                        var this_val = jQuery(this).val();
                        if(this_val=="BOOKED")
                        {
                            var arr_temp = jQuery(this).attr("id");
                            arr_temp = arr_temp.split("_");
                            jQuery("#status_"+arr_temp[2]+" option[value="+this_val+"]").attr("selected","");
                            count_price(arr_temp[2],false);
                        }
                    });
                    jQuery("select#change_all_status option:first-child").attr("selected","");
                    return false;
                }                    
              }                            
          jQuery('div#wrapper').fadeOut('fast'); jQuery('div#cancel_extra').fadeOut('fast');                     
     }
...
function check_room_asign()
{
    ...
    jQuery.ajax({
					...
					success:function(html)
                    {
                        if(html=='error')
                        {
                            alert('<?php echo Portal::language('this_room_type_has_expired');?>');
                            return false;
                        }
                        else
                        {

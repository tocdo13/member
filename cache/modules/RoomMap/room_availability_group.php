<form method="post" name="HotelRoomAvailabilityForm">
        <div style="text-align: center;"><h3 style="text-transform: uppercase;"><?php echo Portal::language('change_room_level_group');?></h3></div>
        <div style="width: 600px; margin: 50px auto;">
            <fieldset style="width: 90%; margin: 10px auto;">
                <legend style="color: #555555;"><?php echo Portal::language('search');?></legend>
                <table>
                    <tr style="text-align: center;">
                        <td>
                            ...
                            <input  name="from_date" id="from_date" class="date-input" onchange="check_value_date(true);" style="padding: 5px;" / type ="text" value="<?php echo String::html_normalize(URL::get('from_date'));?>">
                        </td>
                        <td>
                            ...
                            <input  name="to_date" id="to_date" class="date-input" onchange="check_value_date(false);" style="padding: 5px;" / type ="text" value="<?php echo String::html_normalize(URL::get('to_date'));?>">
                        </td>
                        <td>
                            <input name="search_date" type="button" value="<?php echo Portal::language('search');?>" onclick="fun_check_submit();" style="padding: 5px;" />
                        </td>
                    </tr>
                </table>
            </fieldset>
            <?php 
        		...
        		<fieldset style="width: 90%; margin: 20px auto;">
        			<legend style="color: #555555;"><?php echo Portal::language('select_room_level');?></legend>
				...
				{?>
            			<div class="row-even">
                            ...
                            <a style="text-decoration: none; line-height: 30px; padding-left: 5px; color: blue;" href="#" style="color: #00b2f9;" onclick="selectRoomLevel(<?php echo Url::iget('object_id');?>,'<?php echo $this->map['room_levels']['current']['name'];?>',<?php echo $this->map['room_levels']['current']['id'];?>,<?php echo Url::iget('input_count');?>,<?php echo $this->map['room_levels']['current']['vacant_room'];?>)">
                ...
                if( (opener.document.getElementById("check_box_"+i).checked==true || chec_box_tick(inputCount)==true) && (opener.document.getElementById("status_"+i).value=='BOOKED' || cmd=='add') )
                {
                    ...
                    else
                    {
                        ...
                        <?php }elseif(CHANGE_ROOM_BOOKED=='ALWAY'){ ?>
                            opener.document.getElementById('price_'+i).value = number_format(room_levels[roomLevelId]['price'],2);
                            opener.document.getElementById('usd_price_'+i).value = number_format(room_levels[roomLevelId]['usd_price'],2);
                            //usd_price_'+index).value = number_format(room_levels[roomLevelId]['usd_price'],2);
                        <?php } ?>
                    }

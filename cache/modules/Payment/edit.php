<span style="display:none">
	<span id="mi_payment_sample">
		<div id="input_group_#xxxx#" style="position: relative;">
            ...
            <span class="multi-input">
				<select  name="mi_payment[#xxxx#][currency_id]" style="width:85px;" id="currency_id_#xxxx#" lang="#xxxx#" tabindex="3" onchange="UpdateAmount('#xxxx#');UpdateBalance('#xxxx#');"><?php echo $this->map['currency_options'];?></select>
			</span>
            <span class="multi-input">
				<select  name="mi_payment[#xxxx#][payment_type_id]" style="width:90px;" id="payment_type_id_#xxxx#" lang="#xxxx#" tabindex="3" onchange="selectPaymentType(this.value,#xxxx#,$('amount_#xxxx#').value);" class="payment_type"><?php echo $this->map['payment_type_options'];?></select>
			</span>
            <span class="multi-input">
				<select  name="mi_payment[#xxxx#][credit_card_id]" style="width:75px;" id="credit_card_id_#xxxx#" class="credit_card_id" lang="#xxxx#" tabindex="4" onchange="updateBankFee(this.value,'#xxxx#');"><?php echo $this->map['credit_card_options'];?></select>
			</span>
            <span class="multi-input">
...
<table cellspacing="0" cellpadding="5" width="99%" border="0">
	<tr valign="top">
		<td align="left">
            <table cellpadding="0" cellspacing="0" width="99%" border="0" bordercolor="#CCCCCC" class="table-bound">
                <tr height="40">
                    <td class="form-title payment" width="99%"><?php if(Url::get('cmd')=='deposit'){?><?php echo Portal::language('deposit_for');?> <?php echo $this->map['obj_payment'];?><?php }else{?><?php echo Portal::language('payment');?> <?php echo Portal::language('for');?>: <?php echo System::display_number($this->map['total_amount']).' '.HOTEL_CURRENCY; }?></td>
                    <?php 
                    ...
                    <td width="1%"><a href="<?php echo $her;?>&portal_id=<?php echo PORTAL_ID;?>" style="margin-right: 5px; text-transform: uppercase; text-decoration: none;" class="w3-btn w3-green"><?php echo Portal::language('back');?></a></td>
                    <?php }?>
                    <?php if(User::can_add(false,ANY_CATEGORY) && Url::get('cmd')=='deposit'){?><td width="1%"><a href="javascript:void(0)" onclick="checkPrint();" class="w3-btn w3-lime" style="margin-right: 5px; text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('print');?></a></td><?php }?>
					<?php if(!isset($_SESSION['check_payment'])){ ?>
                    <?php if(User::can_add(false,ANY_CATEGORY)){?><td width="1%"><a href="javascript:void(0)" onclick="checkSubmit('save');"  class="w3-btn w3-blue" style="margin-right: 5px; text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('Save_and_stay');?></a></td><?php }?>
                    <?php if(User::can_add(false,ANY_CATEGORY) and !Url::get('fast')){ ?><td width="1%"><a href="javascript:void(0)" onclick="checkSubmit('save_stay');"  class="w3-btn w3-orange" style="margin-right: 5px; text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('Save_and_close');?></a></td><?php }?>
                    <?php if((Url::get('type')=='RESERVATION' or Url::get('type')=='BILL_MICE') and Url::get('cmd')=='payment'){ if(User::can_add(false,ANY_CATEGORY)){?><td width="1%"><a href="javascript:void(0)" onclick="checkSubmit('save_and_view_folio');"  class="w3-btn w3-gray" style="margin-right: 5px; text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('save_view');?></a></td><?php }?>
                    <?php }} ?>
                </tr>
            </table>
            <div>
            ...
            if(isset($_SESSION['check_payment'])){
             ?>
             <div style="text-align: center;color:red;font-size:15px;padding:5px;"><?php echo Portal::language('show_check_payment');?></div>
             <?php   
            }
             ?>
            </div>
    ...
    <tr style="display: none;">
        <td><?php if(SETTING_POINT==1){ ?> <?php echo Portal::language('member_code');?>: <?php } ?><input  name="member_code" id="member_code" style="border: none;" readonly="" / type ="text" value="<?php echo String::html_normalize(URL::get('member_code'));?>"></td>
    </tr>
		...
		<div>
				<span id="mi_payment_all_elems">
					<span style="white-space:nowrap; float: left;">
						...
						<span style="padding-right:47px; padding-left: 5px;"><span style="width:95px;"><?php echo Portal::language('pmt_time');?></span></span>
                        <span style="padding-right:35px;"><span style="width:75px;"><?php echo Portal::language('amount_paid');?></span></span>
                        <span style="padding-right:40px;"><span style="width:85px;"><?php echo Portal::language('currency');?></span></span>
                        <span style="padding-right:10px;"><span style="width:90px;"><?php echo Portal::language('payment_type');?></span></span>
                        <span style="padding-right:30px;" id="header_credit_type"><span style="width:75px;" ><?php echo Portal::language('credit_type');?></span></span>
                        <span style="padding-right:20px;"><span style="width:120px;" id="bank_title"><?php echo Portal::language('bank_account');?>/<?php echo Portal::language('card_number');?></span></span>
                        <span style="padding-right:155px;"><span style="width:25px;"><?php echo Portal::language('description');?></span></span>                        
                        <span style="padding-right:10px;"><span style="width:50px;"><?php echo Portal::language('paid');?></span></span>
                        <span style="display: none;"><span style="width:50px;"><?php echo Portal::language('point');?></span></span>
						<span><span style="width:20px;"><img src="skins/default/images/spacer.gif"/></span></span>
					</span><br clear="all"/>
				</span>
				<div style="float:left;padding:5px 0px 5px 85px;font-weight:bold;" id="balance_div"><?php echo Portal::language('balance');?>: <span id="balance" style="color:#FF0000;"><?php echo System::display_number($this->map['total_amount']-$this->map['total_paid']);?></span><?php echo ' '.HOTEL_CURRENCY;?></div><br clear="all">
		</div>
		<div style="float:left; padding-left: 6px;"><input type="button" value="<?php echo Portal::language('add');?>" class="w3-btn w3-cyan w3-text-white" style="text-decoration: none; text-transform: uppercase;" onclick="mi_add_new_row('mi_payment');jQuery('#amount_'+input_count).ForceNumericOnly();jQuery('#amount_'+input_count).FormatNumber();$('exchange_rate_'+input_count).value = <?php echo $this->map['default_exchange_rate'];?>;UpdatePaymentType(jQuery('#payment_type_id_'+input_count).val(),input_count);"></div>
		</td>
	</tr>
	</table>
    <input name="action" id="action" value="" type="hidden" />
    <input  name="confirm_edit" value="1" / type ="hidden" value="<?php echo String::html_normalize(URL::get('confirm_edit'));?>">
    <input  name="total_amount" type ="hidden" id="t" value="<?php echo String::html_normalize(URL::get('total_amount',''.$this->map['total_amount']));?>">
	</td>
</tr>
<tr>
	<td style="font-size:12px;"><b><?php echo Portal::language('payment_note');?>:</b><br/>- <?php echo Portal::language('payment_note_1');?><br/>- <?php echo Portal::language('payment_note_2');?><br/>- <?php echo Portal::language('payment_note_3');?>
...
var currencies = <?php echo $this->map['currencies'];?>;
...
function checkPrint(){
	...
	if(index==''){
		alert('<?php echo Portal::language('no_item_selected');?>');
		return false;	
	}else{
		<?php $con = '';
...
function UpdateBalance(index){
	...
	if(cmd=='deposit'){
		...
		var total = <?php echo $this->map['total_amount'];?>;
		for(i=101;i<=input_count;i++){
			if($('currency_id_'+i) && $('currency_id_'+i).value){
		...
		if(index && total<0){
			//alert('<?php echo Portal::language('total_must_not_be_a_nagative_number');?>. <?php echo Portal::language('please_try_again');?>');
			//$('amount_'+index).value = 0;
			UpdateBalance();
			return;
		}
...
function checkSubmit(act)
{
    ...
    if(count_click==1)
    {
        ...
        if(check_zezo){
            alert('<?php echo Portal::language('Do_not_enter_an_amount_equal_to_zero');?> !');
            count_click = 0;
            return false;
        }
        ...
        else
        {
    		if(total>100 && cmd != 'deposit')
            {
    			...
    			if(firm)
                {
    				...
    				jQuery('#exchange_rate_'+input_count).val(<?php echo $this->map['default_exchange_rate'];?>);
                ...
                {
                    alert('<?php echo Portal::language('event_not_submit');?>!');
                    count_click = 0;
    				//jQuery('#action').val(act);
//    				EditPaymentForm.submit();	
    			}
...
credit_cards = <?php echo $this->map['credit_cards_js'];?>;

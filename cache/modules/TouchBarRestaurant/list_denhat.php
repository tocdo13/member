...
<script>
    ...
    var room_array={
        '':''
    <?php if(isset($this->map['reservation_traveller_list']) and is_array($this->map['reservation_traveller_list'])){ foreach($this->map['reservation_traveller_list'] as $key1=>&$item1){if($key1!='current'){$this->map['reservation_traveller_list']['current'] = &$item1;?>
    <?php 
				if((isset($this->map['reservation_traveller_list']['current']['reservation_room_id'])))
				{?>
        ,'<?php echo $this->map['reservation_traveller_list']['current']['id'];?>':{
            'id':'<?php echo $this->map['reservation_traveller_list']['current']['reservation_room_id'];?>',
            'name':'<?php echo addslashes($this->map['reservation_traveller_list']['current']['name'])?>'
        }
    
				<?php
				}
				?>
    <?php }}unset($this->map['reservation_traveller_list']['current']);} ?>
    }
    var vip_card={
        '':''
    <?php if(isset($this->map['vip_cards']) and is_array($this->map['vip_cards'])){ foreach($this->map['vip_cards'] as $key2=>&$item2){if($key2!='current'){$this->map['vip_cards']['current'] = &$item2;?>
        ,'<?php echo $this->map['vip_cards']['current']['code'];?>':{
            'code':'<?php echo $this->map['vip_cards']['current']['code'];?>',
            'name':'<?php echo addslashes($this->map['vip_cards']['current']['name'])?>',
            'discount_percent':'<?php echo $this->map['vip_cards']['current']['discount_percent'];?>'
        }
    <?php }}unset($this->map['vip_cards']['current']);} ?>
    }   
</script>
...
<div id="order_full_screen">
    <form name="TouchBarRestaurantForm" method="post">
        ...
        <input  name="act" id="act" value="" / type ="hidden" value="<?php echo String::html_normalize(URL::get('act'));?>">
        <div id="mask"></div>
        <div class="w3-row"><?php echo Form::$current->error_messages();?></div>
        <div class="w3-row">
            <div class="w3-quarter">
                <h3><?php echo $this->map['bar_name'];?></h3>
                <p><?php echo $this->map['table_name'];?></p>
                <p style="font-weight: normal;<?php 
				if((User::id()=='developer05'))
				{?> display: none;
				<?php
				}
				?>"><input  name="print_automatic_bill" type="checkbox" value="print_auto" id="print_automatic_bill" <?php 
				if(($this->map['print_automatic_bill']!=''))
				{?> checked="checked" 
				<?php
				}
				?>  /><?php echo Portal::language('print_automatic_bill');?></p>
            ...
            <div class="w3-threequarter" style="text-align: right;">
                <div style="float:left;padding-left:10px;width:150px;display:none;" id="restaurant_other">
                    <div align="right"><?php echo Portal::language('restaurant_menu');?>: <input  name="bar_name" id="bar_name" style="width:200px; border:none; font-size:14px; font-weight:bold;" readonly="readonly" / type ="text" value="<?php echo String::html_normalize(URL::get('bar_name'));?>"></div>
                    <div id="div_add_charge" style="text-align:left;"><?php echo Portal::language('add_charge');?>: <span id="add_charge"></span>%</div>
                </div>
                <?php if(User::can_add(false,ANY_CATEGORY)){?>
                    <input name="checkin" type="button" value="<?php echo Portal::language('table_checkin');?>" id="checkin" class="w3-button w3-border w3-amber w3-hover-amber" onclick="checkSubmit(this);" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;"/>
                    <input name="save" type="button" value="<?php echo Portal::language('save');?>" id="save" class="w3-button w3-border w3-blue w3-hover-blue" onclick="checkSubmit(this);" <?php if(!isset($this->map['status']) || $this->map['status']==''){echo 'style="display:none;"';}else{?>style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;"<?php } ?> />
                    <input name="save_stay" type="button" value="<?php echo Portal::language('save_and_stay');?>" id="save_stay" class="w3-button w3-border w3-blue w3-hover-blue" onclick="checkSubmit(this);" <?php if(!isset($this->map['status']) || $this->map['status']==''){echo 'style="display:none;"';}else{?>style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;"<?php } ?> />
                    <input name="booked" type="button" value="<?php echo Portal::language('table_booked');?>" id="booked" class="w3-button w3-border w3-blue w3-hover-blue" onclick="checkSubmit(this);" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;"/>
                    
                    <input name="back" type="button" value="<?php echo Portal::language('back');?>" class="w3-button w3-border w3-dark-grey w3-hover-dark-grey" onclick="window.location='<?php echo Url::build('table_map',array('bar_id'=>Session::get('bar_id'),'group'=>Url::get('bar_area_id')?Url::get('bar_area_id'):''));?>'" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;" />
                    <span id="dynamic_save_button_bound" style="margin-left: 10px;"></span>
                    <?php 
				if((Url::get('cmd')=='edit'))
				{?>
                        <?php 
				if((isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''))
				{?>
                        <p>
                            <a href="?page=mice_reservation&cmd=edit&id=<?php echo $this->map['mice_reservation_id'];?>" class="w3-text-pink"><?php echo Portal::language('mice');?> <?php echo $this->map['mice_reservation_id'];?></a>
                            <input value="<?php echo Portal::language('split');?> <?php echo Portal::language('mice');?>" type="button" onclick="FunctionSplitMice('<?php echo $this->map['mice_reservation_id'];?>','RES','<?php echo $this->map['id'];?>');" class="w3-button w3-border w3-white w3-hover-white w3-text-pink" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;" />
                        </p>
                         <?php }else{ ?>
                            <?php 
				if(($this->map['pay_with_room']!=1))
				{?>
                            <!--<p>
                                <input value="<?php echo Portal::language('add_mice');?>" type="button" onclick="FunctionAddMice('RES','<?php echo $this->map['id'];?>');" class="w3-button w3-border w3-white w3-hover-white w3-text-pink" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;" />
                                <input value="<?php echo Portal::language('select');?> <?php echo Portal::language('mice');?>" type="button" onclick="FunctionSelectMice('RES','<?php echo $this->map['id'];?>');" class="w3-button w3-border w3-white w3-hover-white w3-text-pink" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;" />
                            </p>-->
				...
				?>
                    <div id="password_box" style="display:none;position:absolute;top:auto;left:auto;border:1px solid #000000;padding:10px;text-align:center;background:#FFFF99;">
                        <?php echo Portal::language('password');?>: <input  name="password" type="password" id="password" />
                        <input type="button" value="OK" onclick="SubmitForm('');" tabindex="-1" style="color:#000066;font-weight:bold;"/>
                        <a class="close" onclick="jQuery('#password_box').hide();$('password').value='';"><?php echo Portal::language('close');?></a>
                    </div>
                <?php }?>
            </div>
        </div>
        ...
        <table id="table_bound">
            <tr>
                <td class="info-left" id="hidden_expan" style="border: none!important;">    
                    <div class="info" style="background: none!important; border: none;">
                        <div style="float:left;">
                            <span style="font-size:12px;" ><?php echo Portal::language('create_date');?>: &nbsp;<?php echo $this->map['date'];?></span><br />
                            <?php 
				if((isset($this->map['code']) and $this->map['code']))
				{?><b><?php echo Portal::language('Bill_ID');?>: <?php echo $this->map['code'];?></b>
				<?php
				}
				?>
                            <input  name="status" type="text" id="status" value="<?php echo $this->map['status'];?>" readonly="readonly" style="border: none; font-weight: bold; background: none; width: 60px; " />
                        </div>
                        <div style="float:right;">
                            <input type="button" value="<?php echo Portal::language('table_select');?>" tabindex="-1" class="w3-button w3-light-grey w3-border" onclick="jQuery('#group_table_select').css('display','');"/>
				...
				{?>
                                <input type="button" name="preview" value="Tạo HĐ" tabindex="-1" class="w3-button w3-light-grey w3-border" onclick="location.href='?page=touch_bar_restaurant&cmd=invoice&id=<?php echo Url::get('id'); ?>&table_id=<?php echo Url::get('table_id'); ?>&bar_area_id=<?php echo Url::get('bar_area_id'); ?>&bar_id=<?php echo Url::get('bar_id'); ?>';"/>
                                 <?php }else{ ?>
                                <input type="button" name="preview" value="<?php echo Portal::language('view_order');?>" tabindex="-1" class="w3-button w3-light-grey w3-border" onclick="if(jQuery('#status').val()!=''){window.open('<?php echo Url::build('touch_bar_restaurant',array('cmd'=>'detail',md5('act')=>md5('print_bill'),md5('preview')=>1,'id'=>Url::get('id'),'bar_id','table_id','bar_area_id','package_id'=>Url::get('package_id')?Url::get('package_id'):''))?>');}else{jQuery('#act').val('submit_invoice');TouchBarRestaurantForm.submit();}"/>
                                
				<?php
				}
				?>                           
                            <?php }?>
                            <input type="button" name="preview" value=" <?php echo Portal::language('food_order');?> " onclick="sentToPrint('kitchen','<?php echo Url::get('id');?>');" tabindex="-1" class="expan" style="display:none;"/>
                            <input type="button" name="preview" value=" <?php echo Portal::language('drink_order');?> " onclick="sentToPrint('bar','<?php echo Url::get('id');?>');" tabindex="-1" class="expan" style="display:none;"/>
                            <input type="button" value="<?php echo Portal::language('print_banquet_event_order');?>" onClick="window.open('<?php echo Url::build('touch_bar_restaurant',array('cmd'=>'detail','act'=>'print_b_e_order','id'=>Url::get('id')));?>');" tabindex="-1" class="expan" style="display:none;"/>
                        </div>
                    </div>
                    <div class="w3-row">
                        <div class="w3-left">
                            <div id="foot_drink_bound" style="width:315px; border: none; box-shadow: 0px 0px 1px #CCC;">
                                <table width="100%" id="foot_drink">
                                    <tr style="background: #EEE; height: 35px;">
                                        <td width="75"><?php echo Portal::language('name_product');?></td>
                                        <td width="26" align="center">SL</td>
                                        <td width="65" align="right"><?php echo Portal::language('price');?></td>
                                        <td width="20" title="<?php echo Portal::language('detail');?>">CT</td>
                                        <td width="25" title="<?php echo Portal::language('note');?>">GC</td>
                                        <td width="25">&nbsp;</td>
                                    </tr>
                                </table>
                                <div id="show_detail"><?php 
				if((isset($this->map['items']) AND !empty($this->map['items'])))
				{?><?php echo $this->map['items'];?>
				<?php
				}
				?></div>
                            </div>
                            <input  name="items_id" id="items_id"/ type ="hidden" value="<?php echo String::html_normalize(URL::get('items_id'));?>"><script>jQuery("items_id").value = '<?php if(Url::get('items_id')){ echo Url::get('items_id');}?>';</script>
                            <table style="width:315px;">
                                <tr style="height: 30px;">
                                    <td><b><?php echo Portal::language('total_payment_traveller');?> (<?php echo HOTEL_CURRENCY;?>)</b></td>
                                    <td style="text-align: right;">
                                        <input name="total_payment_traveller" id="total_payment_traveller" type="text" class="input_number w3-input w3-border" style="text-align: right;" onkeyup="jQuery(this).val(number_format(to_numeric(jQuery(this).val())));GetTotalPayment();" value="<?php if(isset($this->map['total_payment_traveller'])){echo System::display_number($this->map['total_payment_traveller']);} ?>" />
                                    </td>
                                </tr>
                                <tr style="height: 30px;">
                                    <td><b><?php echo Portal::language('total_amount');?> (<?php echo HOTEL_CURRENCY;?>)</b></td>
                                    <td style="text-align: right;"><span id="amount_mini" style="font-weight: bold;"><?php echo $this->map['remain'];?></span></td>
                                </tr>
                                <tr style="height: 30px;">
                                    <td><b><?php echo Portal::language('total_remain_traveller');?> (<?php echo HOTEL_CURRENCY;?>)</b></td>
                        ...
                        <div class="w3-right">
                            <div id="menu_extra">
                                <ul id="menu_extra_ul" style="float:right;">
                                    <li class="menu_extra_li" id="li_quantity" lang="delete" onclick="getSelectQuantity('delete');return false;" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px;" title="<?php echo Portal::language('delete');?> / <?php echo Portal::language('reduce_number');?>"><img src="packages/hotel/skins/default/images/iosstyle/1.png" /></li>
                                    <li class="menu_extra_li" id="li_quantity" onclick="getSelectQuantity('quantity');return false;" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;" title="<?php echo Portal::language('add');?> <?php echo Portal::language('quantity');?>"><img src="packages/hotel/skins/default/images/iosstyle/quantity.png" /></li>
                                    <li class="menu_extra_li" id="li_percentage" onclick="getSelectQuantity('percentage');return false;" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;" title="<?php echo Portal::language('discount_percent');?>" title="<?php echo Portal::language('edit');?>"><img src="packages/hotel/skins/default/images/iosstyle/4.png" /></i></li>
                                    <li class="menu_extra_li" id="li_edit" onclick="items_checked();return false;" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;" title="<?php echo Portal::language('edit');?>"><img src="packages/hotel/skins/default/images/iosstyle/3.png" /></li>
                                    ...
                                    <li class="menu_extra_li_extra" id="li_extra" onclick="getSelectOutSide('extra');return false;" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;" title="<?php echo Portal::language('add_extra_product');?>"><img src="packages/hotel/skins/default/images/iosstyle/6.png" /></li>
                                    
                                    <li class="menu_extra_li_order1 <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>css_menu_estra_disable<?php }else{ } ?>" <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>style="cursor: default; background: rgb(214, 214, 214); overflow: hidden; margin: 0px; padding: 0px; line-height: 40px;"<?php }else{ ?> style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;"<?php } ?> id="li_deposit" onclick="<?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>return false;<?php }else{ ?>openWindowUrl('form.php?block_id=<?php echo BLOCK_PAYMENT;?>&cmd=deposit&id=<?php if(Url::get('id')){echo Url::get('id');}else{echo '';}?>&type=BAR',Array('payment','payment_for',80,210,950,500));<?php } ?>" title="<?php echo Portal::language('deposit');?>"><img src="packages/hotel/skins/default/images/iosstyle/7.png" /></li>
                                    <?php
                                      if($this->map['status'] && $this->map['status']!='BOOKED')
                                      {  
                                    ?>
                                    <li class="menu_extra_li_order1 <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>css_menu_estra_disable<?php }else{ } ?>" <?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>style="cursor: default; background: rgb(214, 214, 214); overflow: hidden; margin: 0px; padding: 0px; line-height: 40px;"<?php }else{ ?> style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;"<?php } ?> id="li_payment" onclick="<?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=''){ ?>return false;<?php }else{ ?><?php if($this->map['status']!='CHECKOUT'){ ?>SubmitForm('payment');<?php }else{ ?>jQuery('#act').val('payment');jQuery('#password_box').show();<?php } ?><?php } ?>" title="<?php echo Portal::language('payment');?>"><img src="packages/hotel/skins/default/images/iosstyle/8.png" /></li>
                                    <?php
                                      }  
                                    ?>
                                    ...
                                    <li class="menu_extra_li_order" style="display:none; overflow: hidden; margin: 0px; padding: 0px; line-height: 40px;"><input name="guest_info" type="button" id="guest_info" value="<?php echo Portal::language('for_sale');?>" class="summary" onclick="getResevationRoom();"/> </li>
                                    <li class="menu_extra_li_extra" id="li_summary" onclick="getSummary();" style="overflow: hidden; margin: 0px; padding: 0px; line-height: 40px; margin-top: 3px;" title="<?php echo Portal::language('info');?>"><img src="packages/hotel/skins/default/images/iosstyle/9.png" /></li>
                                </ul>              
                            </div>
                        </div>
                    </div>   
                ...
                <td class="info-right" id="expan_display" style="border: none;">
                  <div id="product_select_expansion">
                        <div class="select-menu-bound">
                            ...
                            <div class="title" style="background: none; border: none; width: 500px; vertical-align: top; margin: 0px; padding: 0px;">
                                <table id="search_div" style="border: none; width: auto; height: 35px; margin-left: 10px;">
                                    <tr>
                                        <td><span style="font-size:12px; text-transform: lowercase;"><?php echo Portal::language('search');?></span></td>
                                        <td style="border: 1px solid #EEE; border-right: none;">
                                            <input  name="input_product_name" id="input_product_name" style="width: 150px;" onkeypress="if(event.keyCode==13){searchProduct('product',jQuery('#bar_id_other').val());return false;}" onkeyup="if(event.keyCode==13){searchProduct('product',jQuery('#bar_id_other').val());return false;}else{checkSearchProduct(this,event.keyCode);searchProduct('product',jQuery('#bar_id_other').val());}" maxlength="400"/ type ="text" value="<?php echo String::html_normalize(URL::get('input_product_name'));?>">
                                        </td>
                                        ...
                                        <td>
                                            <input name="select_bar" type="button" value="<?php echo Portal::language('select_bar');?>" id="select_bar"  class="w3-button w3-light-grey w3-border w3-right" style="height: 35px; line-height: 35px; padding: 0px 8px; margin: 0px;"onclick="jQuery('#div_select_bar').css({'display':'block','top':'250px','left':'600px'});"/>
                                        </td>
                                        <td>
                                            <input name="bar_name_this" type="button" value="<?php echo $this->map['bar_name'];?>" class="w3-button w3-light-grey w3-border w3-right" style="height: 35px; line-height: 35px; padding: 0px 8px; margin: 0px; max-width: 145px; overflow: hidden;" onclick="jQuery('#restaurant_other').css('display','none');jQuery('#bar_id_other').val('<?php echo Session::get('bar_id');?>');searchProduct('this_bar',jQuery('#bar_id_other').val());"/>
                                        </td>
                                        <td>
                                            <input name="product_select_expansion" type="button" value="<?php echo Portal::language('expan');?>" id="expan"  class="w3-button w3-light-grey w3-border w3-right" onclick="search_product();" style="height: 35px; line-height: 35px; padding: 0px 8px; margin: 0px; display:none;"/>
                                        </td>
                                        <td>
                                            <input name="back" type="button" value="<?php echo Portal::language('Back');?>" id="back"  class="w3-button w3-light-grey w3-border w3-right" style="height: 35px; line-height: 35px; padding: 0px 8px; margin: 0px; display: none;" />
                                        </td>
                                    </tr>
                                </table>
                            </div>
                            <div>
                                <div id="div_food_category"><?php echo $this->map['food_categories'];?></div>
                                <div class="body" style="width: 510px;"><?php echo $this->map['products'];?></div>
                            </div>
                        </div>
                        ...
                        <div id="div_discount_percent" style="width:220px !important; display:none;">
                            <table width="200">
                                <tr>
                                    <td width="120"><?php echo Portal::language('category_discount');?></td>
                                    <td width="80"><?php echo Portal::language('percent');?></td>
                                </tr>
                            </table>
                        </div>
                    </div> 
          ...
          <div id="info_summary" style="border: none; background: none;"> 
            <?php echo $this->map['categories'];?>
          </div> 
          <div id="div_bound_summary">
            <div id="bound_summary" class="dragclass" style="display:block; position: absolute; top:100px; left:500px;">
            <div class="info"><span class="title_bound"><?php echo Portal::language('summary');?></span><img width="30" height="25" id="btnClose" src="packages/core/skins/default/images/buttons/delete.gif" onclick="HideDialog('bound_summary');" style="float:right;margin:5px;"/></div>
            <table cellpadding="0" width="100%">
            <tr>
            <td>
            <fieldset style="text-align:left; margin-bottom:10px;margin-top:10px;">
            <legend class="title"><?php echo Portal::language('guest_retail');?></legend>
                <table cellpadding="3" width="55%">
                    <tr>
                        <td nowrap><?php echo Portal::language('guest_name');?>:</td>
                        <td><input name="receiver_name" type="text" id="receiver_name" value="<?php echo $this->map['receiver_name'];?>" style="width:150px;"  /></td>
                      </tr>
                    <tr>
                        <td><?php echo Portal::language('company');?>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td nowrap="nowrap"><input name="customer_name" type="text" id="customer_name" value="<?php echo $this->map['customer_name'];?>" class="input-text-max" style="float:left;" onfocus="customerAutocomplete();" autocomplete="off" />
                          <input name="customer_id" type="text" id="customer_id" value="<?php if($this->map['customer_id']){echo $this->map['customer_id'];}else{ echo '';}?>" class="hidden"  />
                    ...
                    <tr>
                        <td nowrap><?php echo Portal::language('receiver_phone');?>:</td>
                        <td><input name="receiver_phone" type="text" id="receiver_phone" value="<?php echo $this->map['receiver_phone'];?>" style="width:150px;"  class="input_number"/></td>
                    </tr>
               </table>
            </fieldset>
            </td>
            ...
            <legend class="title"><img src="packages/core/skins/default/images/customer_icon.jpg" width="15" height="20" align="top"/>&nbsp;<?php echo Portal::language('guest_room');?></legend>
                <table cellpadding="3" width="55%">
                    <tr style="<?php if(isset($this->map['mice_reservation_id']) AND $this->map['mice_reservation_id']!=0 AND $this->map['mice_reservation_id']!=''){ ?> display: none; <?php } ?>">
                        <?php if($this->map['reservation_room_status']=='CHECKOUT'){ ?>
                            <td colspan="4">
                                <?php echo Portal::language('guest_room');?>: <?php echo $this->map['reservation_name'];?> <br /> <?php echo Portal::language('room');?>: <?php echo $this->map['room_name'];?>
                                <input name="pay_with_room" type="checkbox" class="checkbox" id="pay_with_room" <?php if($this->map['pay_with_room']==1){ echo 'checked="checked"';}?> style="display:none !important;" />
                                <input name="reservation_traveller_id" id="reservation_traveller_id" type="text" value="<?php echo $this->map['reservation_traveller_id'];?>" style="display: none;" />
                                <input name="reservation_room_id" id="reservation_room_id" type="text" value="<?php echo $this->map['reservation_room_id'];?>" style="display: none;" />
                            </td>
                        <?php }else{ ?>
                <td nowrap>&nbsp;</td>
                <th align="left"><?php echo Portal::language('select_guest_room');?> <br /><select  name="reservation_traveller_id" id="reservation_traveller_id" onchange="update_room(this);if(this.value!=0){jQuery('#pay_with_room').attr('checked',true); jQuery('#tr_total_payment_room').css('display',''); jQuery('#total_payment_room').val(jQuery('#amount_mini').html()); jQuery('#img_pay_with_room').css('display','block'); }else{ jQuery('#pay_with_room').attr('checked',false); jQuery('#img_pay_with_room').css('display','none'); jQuery('#tr_total_payment_room').css('display','none'); jQuery('#total_payment_room').val(0); }"  style="width:170px;height:35px;"><?php
					if(isset($this->map['reservation_traveller_id_list']))
					{
						foreach($this->map['reservation_traveller_id_list'] as $key=>$value)
						{
                    ...
                    if(URL::get('reservation_traveller_id',isset($this->map['reservation_traveller_id'])?$this->map['reservation_traveller_id']:''))
                    echo "<script>$('reservation_traveller_id').value = \"".addslashes(URL::get('reservation_traveller_id',isset($this->map['reservation_traveller_id'])?$this->map['reservation_traveller_id']:''))."\";</script>";
                    ?>
	</select></th>
                <td nowrap width="50%"><?php echo Portal::language('room_name');?> <br /><select  name="reservation_room_id" id="reservation_room_id"  onchange="update_traveller(this);if(this.value!=0){ jQuery('#pay_with_room').attr('checked',true); jQuery('#img_pay_with_room').css('display','block'); jQuery('#total_payment_room').val(jQuery('#amount_mini').html()); jQuery('#tr_total_payment_room').css('display','');}else{ jQuery('#pay_with_room').attr('checked',false); jQuery('#img_pay_with_room').css('display','none');  jQuery('#total_payment_room').val(0); jQuery('#tr_total_payment_room').css('display','none'); jQuery('#total_payment_room').val(0);}" style="width:100px;height:35px;"><?php
                ...
                <input name="pay_with_room" type="checkbox" class="checkbox" id="pay_with_room" <?php if($this->map['pay_with_room']==1){ echo 'checked="checked"';}?> style="display:none !important;" />
                <label for="pay_with_room"><span class="checkbox-span" style="border: none;" title="<?php echo Portal::language('pay_with_room');?>"><img id="img_pay_with_room" style="width: 30px; height:30px; <?php if($this->map['pay_with_room']==1){ echo "display:block;"; } else { echo "display:none;"; } ?>" src="packages/hotel/skins/default/images/iosstyle/success.png" /></span></label>
                        </td>
                        <?php } ?>
                      </tr>
                      <tr>
                        <td colspan="4">
                            ...
                            <span style="float: left; line-height: 25px;"> <?php echo Portal::language('member_code');?>: </span>
                      ...
                      <tr id="tr_total_payment_room" style="<?php if($this->map['pay_with_room']!=1){ echo 'display: none;';}?>">
                        <td colspan="4">
                            <span style="float: left; line-height: 25px;"> <?php echo Portal::language('total_payment_room');?>: </span>
            ...
            <fieldset>
            <legend class="title"><?php echo Portal::language('Order_info');?></legend>
            <table>
                    <tr>
                            <td nowrap="nowrap" width="100"><?php echo Portal::language('arrival_time');?><br />
                            <input name="arrival_date" type="text" id="arrival_date"  class="input-text" value="<?php echo $this->map['arrival_date'];?>" onclick="jQuery('#ui-datepicker-div').css('z-index',103);" /></td>
                         <td nowrap="nowrap"  width="90"><?php echo Portal::language('arrival_time_in');?><br />
                             <input  name="arrival_time_in_hour" id="arrival_time_in_hour" class="input_number" style="width:30px;" maxlength="2" / type ="text" value="<?php echo String::html_normalize(URL::get('arrival_time_in_hour'));?>">h
                             <input  name="arrival_time_in_munite" id="arrival_time_in_munite" class="input_number" style="width:30px;" maxlength="2"  / type ="text" value="<?php echo String::html_normalize(URL::get('arrival_time_in_munite'));?>"></td>
                         <td nowrap="nowrap"><?php echo Portal::language('num_people');?><br />
                         <input name="num_people" type="text" id="num_people" value="<?php echo $this->map['num_people'];?>" class="input-text"></td>
                         <td nowrap="nowrap"><?php echo Portal::language('order_person');?><br /><input name="order_person" type="text" value="<?php echo $this->map['order_person'];?>" id="order_person" class="input-text" /></td>
                         <td><?php echo Portal::language('banquet_order_type');?><br />
                             <input name="banquet_order_type" type="text" id="banquet_order_type" value="<?php echo $this->map['banquet_order_type'];?>" class="input-text" />
                         </td>
                    </tr>
                    <tr>
                    	<td nowrap="nowrap" width="100"><?php echo Portal::language('departure_time');?><br />
                            <input name="departure_date" type="text" id="departure_date"  class="input-text" value="<?php echo $this->map['departure_date'];?>" onclick="jQuery('#ui-datepicker-div').css('z-index',104);" /></td>
                    	<td nowrap="nowrap"  width="90"><?php echo Portal::language('arrival_time_out');?><br />
                             <input  name="arrival_time_out_hour" id="arrival_time_out_hour" class="input_number" style="width:30px;" maxlength="2"  / type ="text" value="<?php echo String::html_normalize(URL::get('arrival_time_out_hour'));?>">h
                            <input  name="arrival_time_out_munite" id="arrival_time_out_munite" class="input_number" style="width:30px;" maxlength="2"  / type ="text" value="<?php echo String::html_normalize(URL::get('arrival_time_out_munite'));?>">
                         </td>
                         <td nowrap="nowrap" colspan="3"><?php echo Portal::language('note');?><br /><input name="note" type="text" id="note" value="<?php echo $this->map['note'];?>" class="input-text-max" style="width:235px;"/></td>
                    </tr>                
            </table>
            </fieldset>
            <fieldset>
            <legend class="title"><?php echo Portal::language('billing_info');?></legend>
            <table class="general-info" align="center" border="1" cellpadding="3" cellspacing="3" width="100%" >
               <tr>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('amount');?>:</td>
                    <td><input name="total_amount" type="text" id="total_amount" value="<?php echo $this->map['total_amount'];?>" readonly="readonly"  maxlength="200" class="general">&nbsp; &nbsp;<label><?php echo HOTEL_CURRENCY;?></label></td>
                    <td width="1%" nowrap="nowrap">Full charge:<input name="full_charge" type="checkbox" id="full_charge" value="1" <?php if($this->map['full_charge']==1){echo 'checked="checked"';}?> disabled="disabled"/><input name="input_full_charge" type="text" id="input_full_charge" value="<?php echo $this->map['full_charge'];?>" style="display:none;" /> </td>
                    <td>Full rate: <input name="full_rate" type="checkbox" id="full_rate" value="1" <?php if($this->map['full_rate']==1){echo 'checked="checked"';}?> disabled="disabled"/><input name="input_full_rate" type="text" id="input_full_rate" value="<?php echo $this->map['full_rate'];?>" style="display:none;"/></td>
               </tr>
               <tr>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('service_charge');?>/<?php echo Portal::language('tax_rate');?>:</td>
                    <td><input name="service_charge" type="text" id="service_charge" value="<?php echo $this->map['service_charge'];?>" class="input_number"  maxlength="3" style="width:30px !important;text-align:right;margin-right:15px;" onkeyup="if(to_numeric(this.value)<=100){GetTotalPayment();}else{this.value=((this.value).substr(0,((this.value).length-1)));}" /><input name="tax_rate" type="text" id="tax_rate" value="<?php echo $this->map['tax_rate'];?>" class="input_number"  maxlength="3" style="width:30px !important;text-align:right;" onkeyup="this.value=number_format(this.value);GetTotalPayment();" onblur="GetTotalPayment();" /></td>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('discount');?>(%/VND):<input name="discount_after_tax" type="checkbox" id="discount_after_tax" title="<?php echo Portal::language('discount_after_tax');?>" value="1" <?php if($this->map['discount_after_tax']==1){echo 'checked="checked"';}?> disabled="disabled"/><input name="input_discount_after_tax" type="text" id="input_discount_after_tax" value="<?php echo $this->map['discount_after_tax'];?>" style="display:none;"/></td>
                    <td>
                        <input name="discount_percent" type="text" id="discount_percent" value="<?php echo $this->map['discount_percent'];?>" class="input_number"  maxlength="200" style="width:30px !important;text-align:right;margin-right:5px;" onkeyup="if(to_numeric(this.value)<=100){GetTotalPayment();}else{this.value=((this.value).substr(0,((this.value).length-1)));}CheckdDiscountPercent();" />
                        <input name="discount" type="text" id="discount" value="<?php echo $this->map['discount'];?>" class="input_number"  maxlength="200" style="width:125px !important;text-align:right;" onkeyup="this.value=number_format(this.value);GetTotalPayment(); CheckDiscount();if(to_numeric(this.value)>0 && to_numeric(jQuery('#total_amount').val())<0){this.value='';GetTotalPayment();}" onblur="GetTotalPayment();" placeholder="<?php echo Portal::language('discount_cash');?>" /></td>
                    </td>
                </tr>
                 <tr>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('total_amount');?>:</td>
                    <td><input name="total_payment" type="text" id="total_payment" value="<?php echo $this->map['total_payment'];?>" readonly="readonly" class="general"  maxlength="200">&nbsp; &nbsp;<label><?php echo HOTEL_CURRENCY;?></label></td>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('reason_discount');?>:</td>
                    <td><textarea  name="reason_discount" id="reason_discount" style="width: 224px; height: 50px; overflow: hidden;"><?php echo String::html_normalize(URL::get('reason_discount',''.$this->map['reason_discount']));?></textarea></td>
                </tr>
                 <tr style="display:none;">
                    <td><?php echo Portal::language('deposit_date');?>:</td>
                    <td><input name="deposit_date" type="text" id="deposit_date" value="<?php echo $this->map['deposit_date'];?>" class="general"  maxlength="200"/>&nbsp; &nbsp;</td>
                </tr>
                 <tr>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('remain');?>:</td>
                    <td><input name="remain" type="text" id="remain" value="<?php echo $this->map['remain'];?>" class="general" readonly="readonly"  maxlength="200" style="font-weight:bold;"/>&nbsp; &nbsp;<label><?php echo HOTEL_CURRENCY;?></label></td>
                    <td width="1%" nowrap="nowrap"><?php echo Portal::language('deposit');?>:</td>
                    <td><input name="deposit" type="text" id="deposit" value="<?php echo $this->map['deposit'];?>" onkeyup="this.value=number_format(this.value); GetTotalPayment();" class="input_number"  maxlength="200" style="width:80px !important;text-align:right; border:none" readonly="readonly" />&nbsp; &nbsp;<label><?php echo HOTEL_CURRENCY;?></label></td>
                </tr>
            </table>
            </fieldset>
                <div id="button_total">
                    <input name="button_ok" type="button" id="button_ok" value="<?php echo Portal::language('ok');?>" onclick="HideDialog('bound_summary');normal_submit()" style="height:35px; width:80px;float:right;"/>
                    <input name="button_reset" type="button" id="button_reset" value="<?php echo Portal::language('reset');?>" onclick="jQuery('#receiver_name').val('');jQuery('#discount_percent').val('');jQuery('#deposit').val(0);jQuery('#deposit_date').val('');HideDialog('bound_summary');" style="height:35px; width:80px;float:right;" />              
                </div>
            </div>
          </div>
        <div id="div_bound_note" style="display:none;"><div id="bound_note"><div class="info"><span class="title_bound"><?php echo Portal::language('note_product');?></span><img width="20" height="20" id="btnClose" src="packages/core/skins/default/images/buttons/delete.gif" onclick="submitNote(jQuery('#note_product').val(),jQuery('#id_note').val());HideDialog('div_bound_note');" style="float:right;"/></div>
        <input  name="id_note" id="id_note" value=""  / type ="hidden" value="<?php echo String::html_normalize(URL::get('id_note'));?>">
        <textarea id="note_product" cols="82" rows="3"></textarea>
        </div>
        </div>
        <div id="dialog" class="web-dialog"><div class="info"><span style="text-align:center; font-size:14px; color:#6D84B4;line-height:35px;" id="title_select_quantity"><b><?php echo Portal::language('edit_quantity_for_menu');?></b></span>
        <img width="30" height="25" id="btnClose" src="packages/core/skins/default/images/buttons/delete.gif" onclick="HideDialog('dialog');" style="float:right;"/></div>
            <table style="margin-left:10px;">
                <tr>
                    <td><?php echo Portal::language('name');?>:&nbsp;<input  name="name" type="text" border="0" id="name" readonly="readonly" style="width:300px;background:#fff;margin-left:10px;"/></td>
                    <td></td>
                </tr>
                <tr id="tr_note_outside" style="display:none;">
                    <td><?php echo Portal::language('unit');?>:&nbsp;<input  name="unit_abc" id="unit_abc" value="" style="width:285px;" / type ="text" value="<?php echo String::html_normalize(URL::get('unit_abc'));?>">
                    <input  name="unit_id" id="unit_id" value="" size="10" class="text-right" style="display:none;"/> </td>
                   <td></td>
                </tr>
                <tr><td><?php echo Portal::language('note');?>:&nbsp;<input  name="note-dialog" type="text" border="0" id="note-dialog" style="width:285px;background:#fff;" value="" /></td>
                    <td></td>
                </tr>
            </table>
            <input  name="pass" id="pass" value="1" style="display:none;"/ type ="text" value="<?php echo String::html_normalize(URL::get('pass'));?>">
            <input  name="prd_key" id="prd_key" value=""  style="display:none;"/ type ="text" value="<?php echo String::html_normalize(URL::get('prd_key'));?>">
                <table id="select_item" align="center" style="margin-left:10px;">
                    <tr>
                        <td valign="top">
                            <table id="caculator" cellpadding="2" cellspacing="0">
                                <tr><td><input  name="name_1" value="1" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_1'));?>"></td>
                                    <td><input  name="name_2" value="2" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_2'));?>"></td>
                                    <td><input  name="name_3" value="3" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_3'));?>"></td>
                                </tr>
                                <tr><td><input  name="name_4" type="button" value="4" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/></td>
                                    <td><input  name="name_5" value="5" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_5'));?>"></td>
                                    <td><input  name="name_6" value="6" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_6'));?>"></td>
                                </tr>
                                <tr><td><input  name="name_7" value="7" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_7'));?>"></td>
                                    <td><input  name="name_8" value="8" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_8'));?>"></td>
                                    <td><input  name="name_9" value="9" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_9'));?>"></td>
                                </tr>
                                <tr>
                                    <td><input  name="." value="." class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('.'));?>"></td>
                                    <td><input  name="name_0" value="0" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('name_0'));?>"></td>
                                    <td><input  name="backspace" value="<-" class="button" onclick="GetNumber(this.value,jQuery('#prd_key').val(),jQuery('#pass').val());"/ type ="button" value="<?php echo String::html_normalize(URL::get('backspace'));?>"></td>
                                </tr>
                            </table>
                        </td>    
                        <td valign="top">   
                            <table style="margin-top:20px;">
                                ...
                                <tr class="can_admin" style="display:none;">
                                    <td><input  name="radio_number" type="radio" id="radio_price" value="radio_price" checked="checked" class="radio_number" lang=""/><label for="radio_price"><?php echo Portal::language('price');?></label>:</td>
                                ...
                                <tr>
                                    <td><input  name="radio_number" type="radio" id="radio_quantity" value="radio_quantity" checked="checked" class="radio_number" lang=""/><label for="radio_quantity"><?php echo Portal::language('quantity');?></label>:</td>
                                ...
                                <tr>
                                    <td><label for="input_radio_complete_quantity"><?php echo Portal::language('complete');?></label>:</td>
                                ...
                                <tr id="tr_promotion">
                                    <td><input  name="radio_number" type="radio" id="radio_promotion" value="radio_promotion" class="radio_number" lang="quantity"/><label for="radio_promotion"><?php echo Portal::language('promotion');?></label>:</td>
                                    <td> <input name="promotion_quantity" type="text" id="input_radio_promotion" onclick="$('radio_promotion').checked=true;" class="text-right input_number" style="width:100px;" onchange="if(to_numeric(jQuery('#input_radio_quantity').val()) - to_numeric(jQuery('#input_radio_return').val()) < to_numeric(jQuery('#input_radio_promotion').val())){ alert('<?php echo Portal::language('so_luong_khuyen_mai_lon_hon_so_luong_thuc_te');?>');jQuery('#input_radio_promotion').val(0);}" /></td>
                                </tr>
                                <tr id="tr_return">
                                    <td><input  name="radio_number" type="radio" id="radio_return" value="radio_return" class="radio_number"  lang="quantity"/><label for="radio_return"><?php echo Portal::language('return');?></label></td>
                                ...
                                <tr id="tr_cancel" style="display:none;">
                                    <td><input  name="radio_number" type="radio" id="radio_cancel" value="radio_cancel" class="radio_number" lang="quantity"/><label for="radio_cancel"><?php echo Portal::language('cancel');?></label>:</td>
                                ...
                                <tr id="tr_percentage">
                                    <td><input  name="radio_number" type="radio" id="radio_percentage" value="radio_percentage" class="radio_number" lang="percentage"/><label for="radio_percentage"><?php echo Portal::language('discount');?>(%)</label>:</td>
                                    ...
                                    <input  name="discount_category" id="discount_category" value="0"  style="width:25px; display:none;" class="text-right" title="Discount by category" / type ="text" value="<?php echo String::html_normalize(URL::get('discount_category'));?>"></td>
                                </tr>
        ...
        <div id="set_menu_content" class="web-dialog" style="width: 550px; padding: 10px;"><div class="info"><span style="text-align:center; font-size:14px; color:#6D84B4;line-height:35px;" id="title_select_quantity"><b><?php echo Portal::language('edit_quantity_for_set_menu');?></b></span>
            ...
            <table border="1" cellpadding="5">
                <thead>
                    ...
                    <th style="width: 20px;"><?php echo Portal::language('SL');?></th>
                </thead>
        ...
        <div id="set_menu_child" class="web-dialog" style="width: 550px; padding: 10px;"><div class="info"><span style="text-align:center; font-size:14px; color:#6D84B4;line-height:35px;" id="title_select_set_menu"><b><?php echo Portal::language('edit_quantity_for_set_menu');?></b></span>
        ...
        <div id="table_map" style="display:none;">
            <div class="info"><span class="title_bound"><?php echo Portal::language('select_table');?></span><img width="30" height="25" id="btnClose" src="packages/core/skins/default/images/buttons/delete.gif" onclick="HideDialog('table_map');" style="float:right;"/></div>
            <table border="0" cellpadding="2" cellspacing="0" width="100%" bordercolor="#CCCCCC" rules="rows" frame="hsides" id="table_table_map">
                ...
                <tr>
                    <td width="80px" nowrap valign="top"><b><?php echo $this->map['floors']['current']['name'];?></b></td>
                    <td width="99%"> 
                        <input  name="table_change" id="table_change" / type ="hidden" value="<?php echo String::html_normalize(URL::get('table_change'));?>">  
                        <?php if(isset($this->map['floors']['current']['bar_tables']) and is_array($this->map['floors']['current']['bar_tables'])){ foreach($this->map['floors']['current']['bar_tables'] as $key4=>&$item4){if($key4!='current'){$this->map['floors']['current']['bar_tables']['current'] = &$item4;?>
                        <div class="room-bound" style="width:58px; height:60px;">
                        <input name="table_change_num_people" type="hidden" id="table_change_num_people" value="<?php echo $this->map['floors']['current']['bar_tables']['current']['num_people'];?>" />  
                         <a href="#" style="width:50px;height:50px;" <?php if($this->map['floors']['current']['bar_tables']['current']['status']=='AVAILABLE' || ($this->map['floors']['current']['bar_tables']['current']['status']=='OCCUPIED' && $this->map['status']=='CHECKIN') || ($this->map['floors']['current']['bar_tables']['current']['status']=='BOOKED' && ($this->map['status']=='RESERVATION' || $this->map['status']=='BOOKED'))){ echo 'style="width:50px;height:50px;"'; echo 'onclick="jQuery(\'#table_id_\'+jQuery(\'#table_change\').val()).val(this.id);jQuery(\'#table_name_\'+jQuery(\'#table_change\').val()).val(this.lang);jQuery(\'#num_people_\'+jQuery(\'#table_change\').val()).val(jQuery(\'#table_change_num_people\').val());HideDialog(\'table_map\');"';}else{ echo 'style="width:50px; height:50px;cursor:default;"';}?> class="room <?php echo $this->map['floors']['current']['bar_tables']['current']['class'];?>" lang="<?php echo $this->map['floors']['current']['bar_tables']['current']['name'];?>" id="<?php echo $this->map['floors']['current']['bar_tables']['current']['id'];?>"  >
                            <span style="font-size:9px;font-weight:bold;color:#039" ><?php echo $this->map['floors']['current']['bar_tables']['current']['name'];?></span><br />
                        </a>
                        ...
                        <a target="_blank" title="<?php echo Portal::language('code');?>: <?php echo $this->map['floors']['current']['bar_tables']['current']['status_tables_others']['current']['id'];?>" href="#"></a>
                        <?php }}unset($this->map['floors']['current']['bar_tables']['current']['status_tables_others']['current']);} ?>
                        </div>
                        </div>
                    <?php }}unset($this->map['floors']['current']['bar_tables']['current']);} ?>
                    </td>
                </tr>        
        ...
        <div id="div_select_bar" style="display:none;">
            <div class="info"><span class="title_bound"><?php echo Portal::language('select_bar');?></span><img width="30" height="25" id="btnClose" src="packages/core/skins/default/images/buttons/delete.gif" onclick="HideDialog('div_select_bar');" style="float:right;"/></div>
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="350" bordercolor="#CCCCCC" id="bar_select">
                <?php $i=0;?>
                <?php if(isset($this->map['bars']) and is_array($this->map['bars'])){ foreach($this->map['bars'] as $key6=>&$item6){if($key6!='current'){$this->map['bars']['current'] = &$item6;?>
                <?php if($i%2==0){
                    echo '<tr>';
                    echo '<td><input name="'.$this->map['bars']['current']['name'].'" type="button" id="'.$this->map['bars']['current']['name'].'" value="'.$this->map['bars']['current']['name'].'" lang="'.$this->map['bars']['current']['bar_id_from'].'" onclick="jQuery(\'#bar_id_other\').val(this.lang);jQuery(\'#div_select_bar\').css(\'display\',\'none\');jQuery(\'#bar_name\').val(this.name);searchProduct(\'bar\',jQuery(\'#bar_id_other\').val());" style="width:125px; height:30px;" /></td>';    
                }else{
                    echo '<td><input name="'.$this->map['bars']['current']['name'].'" type="button" id="'.$this->map['bars']['current']['name'].'" value="'.$this->map['bars']['current']['name'].'" lang="'.$this->map['bars']['current']['bar_id_from'].'" onclick="jQuery(\'#bar_id_other\').val(this.lang);jQuery(\'#div_select_bar\').css(\'display\',\'none\');jQuery(\'#bar_name\').val(this.name);searchProduct(\'bar\',jQuery(\'#bar_id_other\').val());"  style="width:125px; height:30px;"/></td>';        
                    echo '</tr>';
                } $i++;?>
        ...
        <div id="group_table_select" style="display: none; width: 100%; height: 100%; position: fixed; background: rgba(0,0,0,0.9); top: 0px; left: 0px;">
            <div style="width: 960px; height: 550px; z-index: 1; background: #eceff1; margin: 50px auto; border: 1px solid #555555; box-shadow: 0px 0px 3px #000000; border-bottom: 3px solid #34495e; border-top: 3px solid #34495e;">
                <table cellpadding="5" style="width: 100%; border-bottom: 1px solid #EEEEEE;">
                    <tr>
                        <td colspan="2">
                            <h3><?php echo Portal::language('order_info');?></h3>
                        </td>
                    ...
                    <tr>
                        <td><h3><?php echo Portal::language('table_action');?></h3></td>
                        <td colspan="2"><h3><?php echo Portal::language('select_table_for_group');?></h3></td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td colspan="2">
                            <div style="width: 550px; height: 330px; overflow: auto; border: 1px solid #DDDDDD; box-shadow: inset 0px 0px 3px #000000;">
                                <table style="width: 100%; margin: 10px auto;">
                                    <?php if(isset($this->map['floor_full']) and is_array($this->map['floor_full'])){ foreach($this->map['floor_full'] as $key7=>&$item7){if($key7!='current'){$this->map['floor_full']['current'] = &$item7;?>        
                                    <tr style="border-bottom: 1px solid #DDDDDD;">
                                        <td style="width: 80px;" valign="top"><b><?php echo $this->map['floor_full']['current']['name'];?></b></td>
                                        <td> 
                                            <?php if(isset($this->map['floor_full']['current']['bar_tables']) and is_array($this->map['floor_full']['current']['bar_tables'])){ foreach($this->map['floor_full']['current']['bar_tables'] as $key8=>&$item8){if($key8!='current'){$this->map['floor_full']['current']['bar_tables']['current'] = &$item8;?>
                                            <?php //if(($this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']=='' OR ($this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']!='' AND $this->map['floor_full']['current']['bar_tables']['current']['status']=='BOOKED') OR (Url::get('cmd')=='edit' AND $this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']==Url::get('id')))){ ?>
                                            <div <?php if( (Url::get('cmd')=='edit' AND $this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']==Url::get('id')) OR ($this->map['floor_full']['current']['bar_tables']['current']['id']==Url::get('table_id') AND Url::get('cmd')=='add') ){ ?> class="trueselect" <?php }else{ ?> class="falseselect" <?php } ?> onclick="selectgrouptable('<?php echo $this->map['floor_full']['current']['bar_tables']['current']['id'];?>');" style="width: 60px; height: 60px; padding: 5px; cursor: pointer; text-align: center; margin: 5px 5px 5px 5px; float: left; border: 1px solid #EEEEEE;box-shadow: 0 2px 2px 0 rgba(0, 0, 0, .05), 0 1px 4px 0 rgba(0, 0, 0, .08), 0 3px 1px -2px rgba(0, 0, 0, .2);">
                                                <input type="checkbox" name="selectgrouptable[<?php echo $this->map['floor_full']['current']['bar_tables']['current']['id'];?>][id]" id="selectgroup_<?php echo $this->map['floor_full']['current']['bar_tables']['current']['id'];?>" title="<?php echo $this->map['floor_full']['current']['bar_tables']['current']['name'];?>" lang="<?php echo $this->map['floor_full']['current']['bar_tables']['current']['id'];?>" value="<?php if(Url::get('cmd')=='edit' AND $this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']==Url::get('id')){ ?><?php echo $this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_table_id'];?><?php } ?>" class="checkitemsselecttable" <?php if( (Url::get('cmd')=='edit' AND $this->map['floor_full']['current']['bar_tables']['current']['bar_reservation_id']==Url::get('id')) OR ($this->map['floor_full']['current']['bar_tables']['current']['id']==Url::get('table_id') AND Url::get('cmd')=='add') ){ ?> checked="checked" <?php } ?> style="display: none;" />
                                                <span style="font-size: 11px;"><?php echo $this->map['floor_full']['current']['bar_tables']['current']['name'];?></span>
                                            </div>
                                            <?php //} ?>
                                            <?php }}unset($this->map['floor_full']['current']['bar_tables']['current']);} ?>
                                        </td>
                                    </tr>        
                                    <?php }}unset($this->map['floor_full']['current']);} ?>    
                                </table>
                            </div>
                        </td>
                    </tr>
...
<script> 
    ...
    ?>
    var amount_package = to_numeric(<?php echo $this->map['amount_package'];?>);
    var count_click = 0;
    ...
    var count = 0;
    units=<?php echo $this->map['units'];?>;
    var check_selected = 0;
    bar_id = <?php echo (Session::get('bar_id')?Session::get('bar_id'):'0');?>;
    paging(25);
    SetCss();
    GetStatus();
    product_list = <?php echo $this->map['product_list_js'];?>;
    //console.log(product_list);
    table_list = <?php echo $this->map['table_list_js'];?>;
    var flag2 = 0;
    var flag = 0;
    var sf=0;
    ...
    jQuery(document).ready(function(){
        ...
        discount_after_tax = <?php echo $this->map['discount_after_tax'];?>;
    ...
    function GetStatus(){
        var status = '<?php echo $this->map['status'];?>';    
        if(status == 'CHECKIN'){
            ...
            jQuery('#booked').val('<?php echo Portal::language('PRINT_&_CHECKOUT');?>');
        }else if(status == 'BOOKED' || status == 'RESERVATION'){
            jQuery('#dynamic_save_button_bound').html('<input name="cancel" type="button" value="<?php echo Portal::language('table_cancel');?>" onclick="SubmitForm(\'cancel\');" class="w3-button w3-border w3-dark-grey w3-hover-dark-grey" style="border: 3px solid #FFF!important; box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); text-transform: uppercase;" id="cancel" >');
...
//        {
    ...
    function SelectedItems(key,pass,type)
    {
       ...
       /** END **/
        ...
        var categories_discount = <?php echo $this->map['categories_discount_js'];?>;
        var discount_pct = 0;
        price_id = key;
            ...
            if((product_array[j]['id'] == price_id) || (product_array[j]['product_id'] == price_id && ( price_id=='FOUTSIDE' || price_id=='DOUTSIDE' || price_id=='SOUTSIDE')))
            {
                ...
                if((pass == 1 || pass == 2 || (product_array[j]['product_id'] == 'DOUTSIDE' || product_array[j]['product_id'] == 'FOUTSIDE' || product_array[j]['product_id'] == 'SOUTSIDE')) && key!='DOUTSIDE' && key!='FOUTSIDE' && key!='SOUTSIDE')
                {
                    //alert('tren');
                        ...
                        if(pass==1)
                        {
                            jQuery('#title_select_quantity').html('<?php echo Portal::language('edit_quantity_for_menu');?>');
                        }
                        else
                        {
                            jQuery('#title_select_quantity').html('<?php echo Portal::language('add_quantity_for_menu');?>');
                        }
                ...
                else
                {
                    ...
                    if(existsCheck){
                        ...
                        if((quantity_cancel)<quantity){
                            ...
                            alert('<?php echo Portal::language('this_product_has_canceled');?>');    
                            return false;
                        }
                    }
                    else
    ...
    {  
        ...
        {
            alert('<?php echo Portal::language('invalid_name_or_price_or_quantity_product');?>');
            return false;    
        }
        else
        {
...
//                    {
    ...
    function drawTableList(arr){ 
        ...
        items += '<td width=""><input name="split_table" type="button" value="<?php echo Portal::language('split_table_button');?>" id="split_table" style="height:30px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onClick="window.open(\'?page=split_table&from_code=<?php echo Url::get('id');?>&type=split\')"<?php } ?>></td>';
        items += '<td width=""><input name="tranfer_dish" type="button" value="<?php echo Portal::language('transfer_dish');?>" id="tranfer_dish" style="height:30px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onclick="openWindowUrl(\'form.php?block_id=915&cmd=deposit&from_code=<?php if(Url::get('id')){echo Url::get('id');}else{echo '';}?>&type=split\',Array(\'split\',\'transfer_for\',80,210,950,500));"<?php } ?>></td>';
        //items += '<td width=""><input name="coupling_table" type="button" value="<?php echo Portal::language('coupling_table');?>" id="coupling_table" style="height:40px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onClick="window.open(\'?page=split_table&from_code=<?php echo Url::get('id');?>\')"<?php } ?>></td>';
        items += '<td width=""><a href="<?php if(Url::get('cmd')=='edit' and Url::get('id')){?>?page=split_table&id=<?php echo  Url::get('id');?>&table_id=<?php echo Url::get('table_id'); } ?>"><input name="coupling_table" type="button" value="<?php echo Portal::language('coupling_table');?>" id="coupling_table" style="height:30px; width:73px;" /></a></td>'
        //items += '<td width="1%"><input name="select_table" type="button" id="select_table" value="<?php echo Portal::language('change_table');?>" style="height:30px; width:73px;" onclick="jQuery(\'#table_map\').css(\'display\',\'block\');jQuery(\'#table_change\').val(\''+arr['id']+'\');" /></td>';

...
<table cellpadding="0" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
	<tr height="40">
		<td width="60%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><i class="fa fa-file-text w3-text-orange" style="font-size: 26px;"></i> <?php echo Portal::language('manage_banquet_type');?></td>
		...
		<?php if(User::can_delete(false,ANY_CATEGORY)){?><a href="javascript:void(0);" onclick="if(ConfirmDelete()) mi_delete_selected_row('party_type');" class="w3-btn w3-red" style="text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('Delete');?></a></td><?php }?>
	</tr>
</table>
			...
			<div style="background-color:#EFEFEF;">
				<span id="party_type_all_elems">
					<span style="white-space:nowrap; width:auto; text-transform: uppercase; text-align: center;">
						...
						<span class="multi-input-header" style="width:30px; height: 24px; padding-top: 4px;"><?php echo Portal::language('order_number');?></span>
						<span class="multi-input-header" style="width:30px; height: 24px; padding-top: 4px;"><?php echo Portal::language('code');?></span>
						<span class="multi-input-header" style="width:150px; height: 24px; padding-top: 4px;"><?php echo Portal::language('banquet_type_name');?></span>
                        <span class="multi-input-header" style="width:350px; height: 24px; padding-top: 4px;"><?php echo Portal::language('note');?></span>
						<span class="multi-input-header" style="width:70px;text-align:center; display: none;"><?php echo Portal::language('config_banqet');?>?</span>
						<span class="multi-input-header" style="width:70px; height: 24px; padding-top: 4px;text-align:center;"><?php echo Portal::language('Delete');?>?</span>
						<br clear="all"/>
					</span>
				</span>
			</div>
			<div><a href="javascript:void(0);" onclick="mi_add_new_row('party_type');$('name_'+input_count).focus();" class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-top: 5px;"><?php echo Portal::language('Add');?></a></div>
...
function Confirm(index){
	...
	return confirm('<?php echo Portal::language('Are_you_sure_delete_party_type_name');?> '+ party_type_name+'?');
}
var DeleteMessage = '<?php echo Portal::language('Delete_party_type_which_is_used_cause_error_for_report_and_statistic');?>\n';
DeleteMessage += '<?php echo Portal::language('You_should_delete_when_you_sure_party_type_that_you_choose_never_in_used');?>';

...
<table cellpadding="0" cellspacing="0" width="100%" border="0">
	<tr height="40">
		<td width="70%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><?php echo Portal::language('sectors_project');?></td>
		<td width="30%"><input type="submit" value="<?php echo Portal::language('Save_and_close');?>" class="w3-btn w3-orange w3-text-white" style="text-transform: uppercase; margin-right: 5px;"/>
		<a href="javascript:void(0);" onclick="if(ConfirmDelete()) mi_delete_selected_row('sectors');" class="w3-btn w3-red" style="text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('Delete');?></a></td>
	</tr>
</table>
			...
			<div>
				<span id="sectors_all_elems">
					<span>
						...
						<span class="multi-input-header" style="width:155px;"><?php echo Portal::language('sectors_name');?></span>
						<span class="multi-input-header" style="width:70px; text-align: center;"><?php echo Portal::language('Delete');?></span>
					</span>
                    <br clear="all">
				</span>
			</div>
			<div><a href="javascript:void(0);" onclick="mi_add_new_row('sectors');$('name_'+input_count).focus();" class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-top: 5px;"><?php echo Portal::language('Add');?></a></div>
...
function Confirm(index)
{
    ...
    return confirm('<?php echo Portal::language('Are_you_sure_delete_sectors');?>'+ sectors_name+'?');
}

function ConfirmDelete()
{
    return confirm('<?php echo Portal::language('Are_you_sure_delete_sectors_selected');?>');
}
</script>
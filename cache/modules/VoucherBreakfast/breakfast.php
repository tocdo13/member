    ...
    <form name="searchForm" method="POST" onsubmit="return checkSubmit();" style="width: 100%;">
        <div style="margin: 50px 50px; width:100%;">
            <div style="float: left; margin-right: 10px;">
                <label for="re_id"><?php echo Portal::language('recode');?> : </label>
                <input  name="re_id" id="re_id" / type ="number" value="<?php echo String::html_normalize(URL::get('re_id'));?>">        
            </div>
            <div  style="float: left; margin-right: 10px;">
                <label for="re_id"><?php echo Portal::language('room_status');?> : </label>
                <select  name="status" id="status"  ><?php
                    ...
                    <option value="all"  <?php 
						if('all' == URL::get('status',$this->map['status']))
						{
							echo ' selected';
						}?>><?php echo Portal::language('all');?></option>
                    <option value="new_traveller"  <?php 
						if('new_traveller' == URL::get('status',$this->map['status']))
						{
							echo ' selected';
						}?>>Khách đến trong ngày</option>
                    <option value="old_traveller"  <?php 
						if('old_traveller' == URL::get('status',$this->map['status']))
						{
							echo ' selected';
						}?>>Khách ở lưu</option>    
                </select>     
            ...
            <div  style="float: left; margin-right: 10px;">
                <label for="re_id"><?php echo Portal::language('in_date');?> : </label>
                <input name="in_date" type="text" id="in_date" readonly=""  value="<?php if(!isset($this->map['in_date'])) echo date("d/m/Y"); else echo $this->map['in_date']; ?>"/>        
            </div>
            <div>
                <input type="submit" name="search" value="<?php echo Portal::language('search');?>" />
            </div>     
        </div>
                                ...
                                <div>
                                    ...
                                    <div  style="margin-top:5px;" class="center">
                                        ...
                                        <?php if(!isset($this->map['is_group'])){
                                         ?>
                                         <span style="font-weight: bold;"><?php echo $this->map['room_name'];?></span>
                                         <?php   
                                        } 
                                        else{
                                        ?>
                                          <span style="font-weight: bold;"><?php echo $this->map['list_customer']['current']['room_name'];?></span>  
                                        <?php    
                                        }
                                        ?>                               
                                        </div>
...
?>
var miny= <?php echo isset($this->map['time_in'])?date('Y',$this->map['time_in']):"''";?>;
var minm= <?php echo isset($this->map['time_in'])?(intval(date('m',$this->map['time_in']))-1):"''";?>;
var mind = <?php echo isset($this->map['time_in'])?date('d',$this->map['time_in']):"''";?>;
var maxy = <?php echo isset($this->map['time_out'])?date('Y',$this->map['time_out']):"''";?>;
var maxm= <?php echo isset($this->map['time_out'])?(intval(date('m',$this->map['time_out']))-1):"''";?>;
var maxd = <?php echo isset($this->map['time_out'])?(date('d',$this->map['time_out'])):"''";?>;

<?php System::set_page_title(HOTEL_NAME.' - '.Portal::language('delete_title'));?><table cellspacing="0" width="100%">
	...
	<tr bgcolor="#EEEEEE" valign="top">
		<td width="100%">
					...
					<form name="DeleteSelectedEquipmentInvoiceForm" method="post">
						...
						<tr valign="middle">
							...
							<th nowrap align="left">
								...
								<?php echo Portal::language('prepaid');?>
							</th><th nowrap align="right">
								<?php echo Portal::language('remain');?>
							</th><th nowrap align="left">
								<?php echo Portal::language('reservation_room_id');?>
							</th><th nowrap align="left">
								<?php echo Portal::language('room_id');?>
							</th>
						</tr>
						<?php if(isset($this->map['items']) and is_array($this->map['items'])){ foreach($this->map['items'] as $key1=>&$item1){if($key1!='current'){$this->map['items']['current'] = &$item1;?>
						<tr bgcolor="white" valign="middle" <?php Draw::hover('#EFEEEE');?>onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=<?php echo $this->map['items']['current']['id'];?>';}else{just_click=false;}" style="cursor:hand;">
							<td><input name="selected_ids[]" type="checkbox" value="<?php echo $this->map['items']['current']['id'];?>" onclick="just_click=true;" checked></td>
							<td nowrap align="left">
									<?php echo $this->map['items']['current']['time'];?>
								</td><td nowrap align="right">
									<?php echo $this->map['items']['current']['total'];?>
								</td><td nowrap align="right">
									<?php echo $this->map['items']['current']['prepaid'];?>
								</td><td nowrap align="right">
									<?php echo $this->map['items']['current']['remain'];?>
								</td><td nowrap align="left">
									<?php echo $this->map['items']['current']['reservation_room_id'];?>
								</td><td nowrap align="left">
									<?php echo $this->map['items']['current']['minibar_id'];?>
								</td>
						</tr>
			...
			<table><tr>
				<td><input type="submit" value="<?php echo Portal::language('delete_selected');?>"/></td>
				<td><input type="button" value="<?php echo Portal::language('list');?>" onclick="window.location='<?php echo Url::build_current(); ?>'"/></td>
			</tr></table>
			</p>
		</td>
	</tr>

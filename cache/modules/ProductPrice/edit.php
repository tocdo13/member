	...
	<span id="mi_product_group_sample">
		<div id="input_group_#xxxx#" style="text-align:left;display:block;">
            ...
            <span class="multi-input">
                ...
                <select   name="mi_product_group[#xxxx#][units_id]" id="units_id_#xxxx#" style="display:none;width:74px;" class="unit_product_value" onchange="$('unit_id_#xxxx#').value = this.value;"><?php echo $this->map['units'];?></select>
            </span>
            ...
            <span class="multi-input">
                ...
                <select   name="mi_product_group[#xxxx#][categorys_id]" id="categorys_id_#xxxx#" style="display:none;width:144px;" class="category_product_value" onchange="$('category_id_#xxxx#').value = this.value;"><?php echo $this->map['categorys'];?></select>
            </span>
            ...
            <span class="multi-input">
                ...
                <select   name="mi_product_group[#xxxx#][types_id]" id="types_id_#xxxx#"  style="display:none;width:74px;" class="product_type_value" onchange="$('type_#xxxx#').value = this.value;"><?php echo $this->map['types'];?></select>
            </span>
        ...
        <table cellpadding="15" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
    		<tr>
                ...
                <td width="30%" align="right" nowrap="nowrap" style="padding-right: 30px;">
                    ...
                    <input name="save" type="submit" value="<?php echo Portal::language('Save_and_close');?>" class="w3-btn w3-orange w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"/>
                    <?php }?>
    				
                    <?php if(User::can_delete(false,ANY_CATEGORY)){?>
                    <a href="<?php echo Url::build_current(array('type'));?>"  class="w3-btn w3-green" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"><?php echo Portal::language('back');?></a>
                    <?php }?>
                </td>
            </tr>
        </table>
    	...
    	<div class="content">
            ...
            <fieldset>
    			...
    			<table border="0" cellspacing="0" cellpadding="2">
    				<tr>
    					...
    					<td>
                            ...
                            <select  name="portal_id" id="portal_id" onchange="$('action').value='search_portal';EditProductPriceForm.submit();" style="height: 24px;"><?php
                    ...
                    ?>
                            ...
                            <select   name="department_code" id="department_code" onchange="select_department(value);" style="height: 24px;"><?php echo $this->map['department_list'];?></select>
				...
				{?>
                                ...
                                <a class="w3-btn w3-cyan w3-text-white" target="_blank" style="text-transform: uppercase; margin-top: 5px;" href="<?php echo Url::build('product',array('cmd'=>'add','warehouse_id'));?>">[ <?php echo Portal::language('Add_product');?> ]</a>
            ...
            <strong><?php echo Portal::language('unit_id');?></strong><select   style="width:83px; height: 24px;"  onchange="change_unit(this.value);"><option value=""></option>
            					<?php echo $this->map['unit_id_options'];?>
            </select>
            <strong><?php echo Portal::language('category_id');?></strong>: <select  name="categories_id" id="categories_id"  style="width:150px; height: 24px;" onchange="change_type_cate(this.value);"><?php
					if(isset($this->map['categories_id_list']))
					{
						foreach($this->map['categories_id_list'] as $key=>$value)
						{
							echo '<option value="'.$key.'"';
							echo '>'.$value.'</option>';
                    ...
                    if(URL::get('categories_id',isset($this->map['categories_id'])?$this->map['categories_id']:''))
                    echo "<script>$('categories_id').value = \"".addslashes(URL::get('categories_id',isset($this->map['categories_id'])?$this->map['categories_id']:''))."\";</script>";
                    ?>
	</select>
            <strong><?php echo Portal::language('type');?>: </strong><select  style="width:150px; height: 24px;" onchange="change_type(this.value);">
				<option value=""></option>
    		...
    		<fieldset>
    			<legend class="title"><?php echo Portal::language('products');?></legend>
    				<span id="mi_product_group_all_elems" style="text-align:left;">
                        ...
                        <span class="multi-input-header" style="width:140px;float:left;"><?php echo Portal::language('name');?></span>
                        <span class="multi-input-header" style="width:74px;"><?php echo Portal::language('unit');?></span>
                        <span class="multi-input-header" style="width:144px;"><?php echo Portal::language('category');?></span>
                        <span class="multi-input-header" style="width:74px;"><?php echo Portal::language('type');?></span>
                        <!---<span class="multi-input-header" style="width:100px;"><?php echo Portal::language('product_pipeline');?></span>--->
                        <span class="multi-input-header" style="width:100px;float:left;text-align:right;"><?php echo Portal::language('price');?></span>
                        <span class="multi-input-header" style="width:100px;float:left;text-align:right;"><?php echo Portal::language('use_time');?></span>
                        ...
                        <span class="multi-input-header" style="width:30px;float:left;"><?php echo Portal::language('order_number');?></span>
                        <br clear="all" />
    				</span>
                    <?php if(Url::get('cmd')=='add'){?>
    				<input class="w3-btn w3-cyan w3-text-white" type="button" value="<?php echo Portal::language('add_product');?>" onclick="mi_add_new_row('mi_product_group');myAutocomplete(input_count);checkusetime(input_count);show_datepicker(input_count);jQuery('#product_id_'+input_count).ForceCodeOnly();jQuery('#order_number_'+input_count).ForceNumericOnly();" style="text-transform: uppercase; margin-top: 5px;"/>
                    <?php }?>
            </fieldset>	
            <br />
    	</div>
...
<script type="text/javascript">
		...
		if($('product_id_'+id))
        {
            ...
            else
            {
				if(value)
                {
					...
					//$('name_'+id).value = '<?php echo Portal::language('products_does_not_exist');?>';
                    //$('unit_'+id).className = 'notice';
					//$('unit_'+id).value = '<?php echo Portal::language('none');?>';
				}
                else
                {
                    jQuery('#name_'+id).attr('readonly',true);
    ...
    function checkSubmit(){
        ...
        if(check == false)
        {
            alert('<?php echo Portal::language('ban_phai_chon_danh_muc_hang_hoa');?>');
            return false;
        }**/
        var i = 0;
        var arr = [];

...
var product_department = <?php echo String::array2js($this->map['product_department']);?>;
		...
		<div id="input_group_#xxxx#">
			...
			<span class="multi-input">
                <select  name="mi_group[#xxxx#][department]" style="width:100px;"  id="department_#xxxx#">
					<option value=""></option><?php echo $this->map['department_options'];?><option value="ALL"><?php echo Portal::language('all');?></option>
				</select>
            </span>
...
<form name="EditDepartmentProductForm" id="EditDepartmentProductForm" method="post" >
    <table cellpadding="0" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
    	<tr height="40">
    		...
    		<?php if(User::can_add(false,ANY_CATEGORY)){?><td width="30%" style="text-align: right;"><input type="submit" name="btnSave" id="btnSave" value="<?php echo Portal::language('Save');?>" onclick="return Check_submit();" class="w3-btn w3-orange w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"/><?php }?>
    		<a class="w3-green w3-btn" onclick="history.go(-1)" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"><?php echo Portal::language('back');?></a></td>
        </tr>
    ...
    <fieldset>
		...
		<table border="0" cellspacing="0" cellpadding="5">
			<tr>
				<td><strong><?php echo Portal::language('department');?></strong>: 
                    <select  name="department_id" id="department_id" style="width:100px; height: 26px;">
                        <option value=""></option></option><?php echo $this->map['department_options'];?><option value="ALL"><?php echo Portal::language('all');?>
                    </select>&nbsp;&nbsp;
                    <script type="text/javascript">
		...
		<table cellspacing="0">
        	...
        	<tr>
				...
				?>
                    			...
                    			<div><a href="javascript:void(0);" onclick="mi_add_new_row('mi_group');AddInput(input_count);ProductAutocomplete(input_count);ProductAutocomplete2(input_count);jQuery('#add_charge_'+input_count).ForceNumericOnly();" class="w3-btn w3-cyan w3-text-white" style="text-transform: uppercase; text-decoration: none; margin-top: 5px;"><?php echo Portal::language('Add');?></a></div>
                            </td>
                       </tr>
                   </table>
                </td>
...
<script>
    ...
    function Confirm(index){
    	...
    	//return confirm('<?php echo Portal::language('Are_you_sure_delete_mi_group_name');?> '+ mi_group_name+'?');
        return confirm('<?php echo Portal::language('Are_you_sure_delete_mi_group_name');?> ?');
    }
    function Check_submit()
    {
        ...
        if(check==false)
        {
            if(ale==1)
            {
                alert('<?php echo Portal::language('product_does_not_exist');?>');                
            }else if(ale==2)
            {
                alert('<?php echo Portal::language('ban_chua_chon_bo_phan');?>');
            }else if(ale==3)
            {
                alert('<?php echo Portal::language('ban_co_san_pham_trung_nhau');?>');
            }
        }
        return check;
    }

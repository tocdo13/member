    ...
    <div class="w3-container">
        <div class="w3-row" style="min-width: 1200px!important; max-width: 1420px; margin: 0px auto;">
            <div class="w3-row">
                ...
                <div class="w3-button w3-margin w3-right w3-text-grey" onclick="location.href='?page=golf_pitch_price_manager'" style="font-weight: bold; text-transform: uppercase; background: none!important;">
...
<div id="InventoryContent" style="width: 100%; height: auto;">
    <div class="w3-container" style="margin-bottom: 50px;">
        <div class="w3-row" style="min-width: 1200px!important; max-width: 1420px; margin: 10px auto;">
            <p><b class="w3-text-green"><?php echo Portal::language('price');?></b><br /><span><i>(Không áp dụng với ngày quá khứ)</i></span></p>
            <table id="BulkDateRange" style="" cellspacing="5" cellpadding="5">
                
            </table>
            <div onclick="AddDateRange();" class="w3-button w3-white w3-hover-white w3-margin w3-border w3-text-green" style="font-weight: normal;">
                <?php echo Portal::language('add_date_range');?>
            </div>
            <hr />
            <p><b class="w3-text-green"><?php echo Portal::language('golf_hole');?> / <?php echo Portal::language('group_traveller');?></b> </p>
            <?php if(isset($this->map['golf_hole']) and is_array($this->map['golf_hole'])){ foreach($this->map['golf_hole'] as $key1=>&$item1){if($key1!='current'){$this->map['golf_hole']['current'] = &$item1;?>
            <table cellspacing="5" cellpadding="5" class="w3-left" style="background: rgba(255,255,255,0.7); box-shadow: 0px 1px 3px 1px rgba(60,64,67,.15); border: 5px solid #FFF; border-radius: 5px; margin: 15px;">
                    <tr>
                        <td colspan="2">
                            <input class="w3-check golf_hole" type="checkbox" name="golf_hole[<?php echo $this->map['golf_hole']['current']['id'];?>][id]" id="golf_hole_id_<?php echo $this->map['golf_hole']['current']['id'];?>" lang="<?php echo $this->map['golf_hole']['current']['id'];?>" />
                            <input type="hidden" name="golf_hole[<?php echo $this->map['golf_hole']['current']['id'];?>][name]" value="<?php echo $this->map['golf_hole']['current']['name'];?>" />
                            <label for="golf_hole_id_<?php echo $this->map['golf_hole']['current']['id'];?>" style="font-weight: bold;" class="w3-text-green"><?php echo Portal::language('golf_hole');?>: <?php echo $this->map['golf_hole']['current']['code'];?> - <?php echo $this->map['golf_hole']['current']['name'];?></label>
                        </td>
                    </tr>
                    <?php if(isset($this->map['group_traveller']) and is_array($this->map['group_traveller'])){ foreach($this->map['group_traveller'] as $key2=>&$item2){if($key2!='current'){$this->map['group_traveller']['current'] = &$item2;?>
                    <tr>
                        <td style="width: 80px;"></td>
                        <td>
                            <input class="w3-check hole_item hole_item_<?php echo $this->map['golf_hole']['current']['id'];?>" lang="<?php echo $this->map['golf_hole']['current']['id'];?>" type="checkbox" name="golf_hole[<?php echo $this->map['golf_hole']['current']['id'];?>][group_traveller][<?php echo $this->map['group_traveller']['current']['id'];?>][id]" id="golf_hole_<?php echo $this->map['golf_hole']['current']['id'];?>_group_traveller_<?php echo $this->map['group_traveller']['current']['id'];?>" />
                            <input type="hidden" name="golf_hole[<?php echo $this->map['golf_hole']['current']['id'];?>][group_traveller][<?php echo $this->map['group_traveller']['current']['id'];?>][name]" value="<?php echo $this->map['group_traveller']['current']['name'];?>" />
                            <label for="golf_hole_<?php echo $this->map['golf_hole']['current']['id'];?>_group_traveller_<?php echo $this->map['group_traveller']['current']['id'];?>" style="font-weight: bold;"><?php echo $this->map['group_traveller']['current']['name'];?></label>
                        </td>
                    </tr>
...
<table id="BulkDateRangeTemplate" style="display: none;">
    <tr id="DateRangeGroup_X######X">
        ...
        <td><i class="fa fa-remove fa-fw w3-text-green" onclick="DeleteDateRange(X######X);"></i></td>
    </tr>
</table>

...
<form name="ListRoomLevelForm" method="post">
	<table cellpadding="0" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
		<tr height="40">
        	<td width="80%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><?php echo $this->map['title'];?></td>
			<td width="20%" style="text-align: right; padding-right: 30px;"><a style="display:none;" href="<?php echo URL::build_current(array('update_to_lock'=>'1'));?>"  class="w3-btn w3-lime" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;"><?php echo Portal::language('Export_to_Lock');?></a>
            ...
            <?php if(User::can_delete(false,ANY_CATEGORY)){?><a href="javascript:void(0)" onclick="if(!confirm('<?php echo Portal::language('are_you_sure');?>')){return false};ListRoomLevelForm.cmd.value='delete';ListRoomLevelForm.submit();"  class="w3-btn w3-red" style="text-transform: uppercase; text-decoration: none;"><?php echo Portal::language('Delete');?></a></td><?php }?>
        </tr>
    </table><br />
	<div class="content">
		<table width="100%" border="1" cellspacing="0" cellpadding="5" bordercolor="#CCCCCC">
			<tr class="w3-light-gray" style="height: 30px; text-transform: uppercase;">
			  ...
			  <th width="15%" align="left"><?php echo Portal::language('brief_name');?></th>
			  <th width="10%" align="center"><?php echo Portal::language('num_people');?></th>
			  <th width="10%" align="right"><?php echo Portal::language('price');?></th>
                          <th width="10%" align="center"><?php echo Portal::language('order_by');?></th>
			  <th width="10%" align="center"><?php echo Portal::language('display_color');?></th>
			  <th width="1%">&nbsp;</th>
		      <th width="1%">&nbsp;</th>
		  </tr>
		  	<?php $temp = '';?>
				<?php if(isset($this->map['items']) and is_array($this->map['items'])){ foreach($this->map['items'] as $key1=>&$item1){if($key1!='current'){$this->map['items']['current'] = &$item1;?>
				<?php if($temp!=$this->map['items']['current']['portal_name']){$temp = $this->map['items']['current']['portal_name'];?>
				<tr>
				  <td colspan="9" class="category-group"><?php echo $this->map['items']['current']['portal_name'];?></td>
			  </tr>
			  <?php }?>
			<tr>
		      ...
		      <td align="center"><?php echo $this->map['items']['current']['num_people'];?></td>
                      <td align="right"><?php echo $this->map['items']['current']['price'];?></td>
                      <td align="center"><?php echo $this->map['items']['current']['position'];?></td>
		      <td align="center" bgcolor="<?php echo $this->map['items']['current']['color'];?>"><?php echo $this->map['items']['current']['color'];?></td>

	...
	<span id="mi_product_group_sample">
		<div id="input_group_#xxxx#">
        	<span>
                ...
                <span class="multi-input"><select  name="mi_product_group[#xxxx#][service_id]" style="width:185px; height: 24px;" id="service_id_#xxxx#" onblur="updateServiceInfo(this,'#xxxx#');" onchange="updateServiceInfo(this,'#xxxx#');"><?php echo $this->map['service_options'];?></select></span>
...
<input  name="deleted_ids" type="hidden"  id="deleted_ids" value="<?php echo URL::get('deleted_ids');?>">
	<table cellpadding="0" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
		<tr height="40">
        	<td width="70%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;"><?php echo Portal::language('add_package');?></td>
			<?php if(User::can_add(false,ANY_CATEGORY)){?><td width="30%"><input  name="save"  type="submit" style="text-transform: uppercase; text-decoration: none; margin-right: 5px;" class="w3-btn w3-orange w3-text-white" <?php echo (isset($this->map['status']) and $this->map['status']=='CHECKOUT' and !User::is_admin())?'disabled="disabled" title="'.Portal::language('room_checked_out_can_not_edit').'"  value="'.Portal::language('room_checked_out_can_not_edit').'"':' value="'.Portal::language('Save').'"';?> onclick="return check_input_product();"/><?php }?>
	...
	<div class="content">
		...
		<fieldset>
        <legend class="legend-title"><?php echo Portal::language('package_information');?></legend>
			<table border="0" cellspacing="0" cellpadding="2">
				<tr valign="middle">
					<td class="label"><?php echo Portal::language('package_code');?></td>
				    <td><input  name="code" id="code" style="width:150px; height: 24px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('code'));?>"></td>
		         	<td class="label"><?php echo Portal::language('package_name');?></td>
				    <td><input  name="name" id="name" style="width:150px; height: 24px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('name'));?>"></td>
				</tr>
                <tr valign="top">
                ...
                <tr valign="middle">
					<td class="label"><?php echo Portal::language('from date');?></td>
				    <td class="label"><input  name="start_date" id="start_date" style="width:150px; height: 24px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('start_date'));?>"></td>
                    <td><?php echo Portal::language('to_date');?></td> 
                    <td><input  name="end_date" id="end_date" style="width:150px; height: 24px;"/ type ="text" value="<?php echo String::html_normalize(URL::get('end_date'));?>"></td>
				</tr>-
                
                <tr valign="top">
                  <td class="label"><?php echo Portal::language('content');?></td>
				    <td colspan="3"><textarea  name="content" id="content" rows="5" style="width: 400px;"><?php echo String::html_normalize(URL::get('content',''));?></textarea></td>
      ...
      <fieldset>
        <legend class="legend-title"><?php echo Portal::language('detail');?></legend>
            <span id="mi_product_group_all_elems" style="text-align:left;">
                <span>
                    <span class="multi-input-header" style="width:185px; height: 24px;"><?php echo Portal::language('name');?></span>
                    <span class="multi-input-header" style="width:63px; height: 24px;"><?php echo Portal::language('number');?></span>
                    <span class="multi-input-header" style="width:104px; height: 24px;"><?php echo Portal::language('unit');?></span>
                    <span class="multi-input-header price" style="width:104px; height: 24px;"><?php echo Portal::language('price');?></span>
                    <span class="multi-input-header price" style="width:104px; height: 24px;"><?php echo Portal::language('department');?></span>
                    <span class="multi-input-header price" style="width:104px; height: 24px;"><?php echo Portal::language('amount');?></span>
                    <!--<span class="multi-input-header" style="width:80px;"><?php echo Portal::language('create_date');?></span>-->
					<span class="multi-input-header" style="width:154px;height: 24px;"><?php echo Portal::language('note');?></span>
                    
                    <!--<span class="multi-input-header" style="width:70px;"><?php echo Portal::language('used');?></span>-->
                </span><br clear="all">
            </span>
            <input class="w3-btn w3-cyan w3-text-white" type="button" style="float: left; text-transform: uppercase; margin-top: 5px;" value="<?php echo Portal::language('add_service');?>" id="add_product" onclick="mi_add_new_row('mi_product_group');jQuery('#service_id_'+input_count).jec();jQuery('#quantity_'+input_count).ForceNumericOnly();jQuery('#price_'+input_count).ForceNumericOnly();jQuery('#price_'+input_count).FormatNumber();jQuery('#in_date_'+input_count).datepicker();"/>
			<div style="width: 100%; margin: 0px auto;">
            
			<div style="float:left;padding-left:390px;"><?php echo Portal::language('total');?>: <input  name="total_amount" type="text" id="total_amount" readonly="readonly" class="readonly" style="width:104px;font-weight:bold;text-align:right;height: 24px;"/></div>
            </div>
    </fieldset>	
    </div>  
...
<?php
   ...
   {
        ?>
        $('code').value ='<?php echo $this->map['code'];?>';
        $('name').value = '<?php echo $this->map['name'];?>';
        $('content').value = '<?php echo $this->map['content'];?>';
        
        <?php 
   }
?>
$('start_date').value = '<?php echo $this->map['start_date'];?>';
$('end_date').value = '<?php echo $this->map['end_date'];?>';

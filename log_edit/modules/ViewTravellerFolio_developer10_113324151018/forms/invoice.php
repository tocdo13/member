...
class ViewTravellerFolioForm extends Form{
    ...
    {                
        ...
        $total_payment = DB::fetch('select sum(amount) as amount,SUM(amount*payment.exchange_rate) as total_vnd from payment where folio_id ='.$folio_id." AND payment_type_id!='REFUND'");
        $this->map['total_payment1'] =  $total_payment['amount'];
        
        $total_payment_refund = DB::fetch('select sum(amount) as amount,SUM(amount*payment.exchange_rate) as total_vnd from payment where folio_id ='.$folio_id." AND payment_type_id='REFUND'");
        
        $this->map['total_refund_vnd'] =  $total_payment_refund['total_vnd'];
        
        $this->map['total_payment_vnd'] =  $total_payment['total_vnd'] - $total_payment_refund['total_vnd'];

                    ...
                    <!--IF:cond_bill(isset([[=code=]]) and [[=code=]])--><b>[[.Bill_ID.]]: [[|code|]]</b><!--/IF:cond_bill-->
                      ...
                      <input  name="status" type="text" id="status" value="[[|status|]]" readonly="readonly" style="border:none; font-weight:bold;background:#F7F6F6; width:80px; " />
                		...
                		<div style="float:right;">
                        	<input type="button" name="preview" value=" [[.view_order.]]" tabindex="-1" class="expan" onclick="if(jQuery('#status').val()!=''){window.open('<?php echo Url::build('touch_bar_restaurant',array('cmd'=>'detail',md5('act')=>md5('print'),md5('preview')=>(([[=status=]]=='CHECKOUT')?0:1),'id'=>Url::get('id'),'bar_id'))?>');}else{jQuery('#act').val('submit_invoice');TouchBarRestaurantForm.submit();}">                           
...
<script>
	...
	function drawTableList(arr){ 
		...
		items += '<td width=""><input name="split_table" type="button" value="[[.split_table_button.]]" id="split_table" style="height:40px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onClick="window.open(\'?page=split_table&from_code=<?php echo Url::get('id');?>&type=split\')"<?php } ?>></td>';

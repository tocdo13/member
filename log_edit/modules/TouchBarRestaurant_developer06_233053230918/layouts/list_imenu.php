...
<div style="padding:0px;">
	<div style="text-align:center;font-size:20px;text-transform:uppercase;" class="title">[[|bar_name|]] - IMENU</span></div>
    <div><?php echo Form::$current->error_messages();?></div>
    <div>
    <div style="float:left; line-height:25px; padding-left:10px;width:400px;display:none;" id="restaurant_other">[[.restaurant_menu.]]: <input name="bar_name" type="text" id="bar_name" style="width:200px; border:none; font-size:14px; font-weight:bold;" readonly="readonly" />
	...
	<div style="float:right; width:60%;margin-right:0px;text-align:right;white-space:nowrap;">
        ...
        <input name="back" type="button" value="[[.back.]]" class="button-medium-back" onclick="window.location='<?php echo Url::build('table_map');?>'" />
            ...
            <td class="info-right" id="expan_display">
                <div class="info">
                		<div style="float:right; <?php if([[=status=]]==''){echo 'display:none;';}?>">
                        	<input type="button" name="preview" value=" [[.view_order.]]" tabindex="-1" class="expan" onclick="window.open('<?php echo Url::build('touch_bar_restaurant',array('cmd'=>'detail',md5('act')=>md5('print'),md5('preview')=>1,'id'=>Url::get('id'),'bar_id'))?>');">                           
                            <input type="button" name="preview" value=" [[.food_order.]] " onclick="sentToPrint('kitchen','<?php echo Url::get('id');?>');" tabindex="-1" class="expan">
                            <input type="button" name="preview" value=" [[.drink_order.]] " onclick="sentToPrint('bar','<?php echo Url::get('id');?>');" tabindex="-1" class="expan">
              ...
              <div id="menu_extra">
					<ul id="menu_extra_ul" style="float:left;">
                    	<li class="menu_extra_li" id="li_quantity" onclick="getSelectQuantity('delete');"><img src="packages/hotel/skins/default/images/iosstyle/delete_button.png" /></li>
                        ...
                        <li class="menu_extra_li_order" id="li_deposit" onclick="openWindowUrl('form.php?block_id=428&cmd=deposit&id=<?php if(Url::get('id')){echo Url::get('id');}else{echo '';}?>&type=BAR',Array('payment','payment_for',80,210,950,500));"><img src="packages/hotel/skins/default/images/iosstyle/deposit_text.png" /></li>
                        ...
                        <li class="menu_extra_li_order" id="li_vat" style="background:#ffffc0;" onclick="window.location='?page=vat_bill&department=RESTAURANT&cmd=entry_restaurant&b_r_id=<?php echo Url::get('id');?>'">
    ...
    <div id="div_select_bar" style="display:none;">
        ...
        <table border="0" cellpadding="2" cellspacing="0" width="350" bordercolor="#CCCCCC" id="bar_select">
            ...
            <?php if($i%2==0){
				echo '<tr>';
				echo '<td><input name="'.[[=bars.name=]].'" type="button" id="'.[[=bars.name=]].'" value="'.[[=bars.name=]].'" lang="'.[[=bars.id=]].'" onclick="jQuery(\'#bar_id_other\').val(this.lang);jQuery(\'#div_select_bar\').css(\'display\',\'none\');jQuery(\'#bar_name\').val(this.name);searchProduct(\'bar\',jQuery(\'#bar_id_other\').val());" style="width:125px; height:30px;" /></td>';	
			}else{
				echo '<td><input name="'.[[=bars.name=]].'" type="button" id="'.[[=bars.name=]].'" value="'.[[=bars.name=]].'" lang="'.[[=bars.id=]].'" onclick="jQuery(\'#bar_id_other\').val(this.lang);jQuery(\'#div_select_bar\').css(\'display\',\'none\');jQuery(\'#bar_name\').val(this.name);searchProduct(\'bar\',jQuery(\'#bar_id_other\').val());"  style="width:125px; height:30px;"/></td>';		
				echo '</tr>';
			} $i++;?>
...
<script>
	...
	jQuery(".category-list-item,.category-list-item-parent").each(function(){
		jQuery(this).click(function(){		
			...
			jQuery.ajax({
				...
				type:"POST",
				data:{category_id:jQuery(this).attr('lang'),bar_id_other:bar_id,cmd:'draw_products',type:'CATEGORY'},
	...
	jQuery('#act').val(0);
	 openWindowUrl('form.php?block_id=428&cmd=payment&id=<?php echo Url::get('id');?>&type=BAR&total_amount='+to_numeric(jQuery('#total_payment').val())+'&portal_id=<?php echo PORTAL_ID;?>',Array('payment','payment_for','80','210','950','500'));
	<?php
		}?>
	function searchProduct(act,bar_id){
		if(jQuery('#input_product_name').val()=='' && act=='product'){
			...
			jQuery.ajax({
				...
				type:"POST",			
				data:{product_name:jQuery("#input_product_name").val(),bar_id_other:bar_id,bar_id:<?php echo Url::get('bar_id');?>,cmd:'draw_products',type:'PRODUCT'},
				success:function(html){
				...
				if((pass == 1 || pass == 2 || (product_array[j]['product_id'] == 'DOUTSIDE' || product_array[j]['product_id'] == 'FOUTSIDE')) && key!='DOUTSIDE' && key!='FOUTSIDE' && key!='SOUTSIDE'){
					...
					if(existsCheck){
						...
						if((quantity_cancel)<quantity){
							...
							//var percentage = jQuery("#percentage_"+key).val();
							jQuery("#discount_category_"+key).val(discount_pct);
							update_total_amount(key);
							//price = price *to_numeric(jQuery("#quantity_"+key).val())*((100-discount_pct)/100);
							//price = price*((100-percentage)/100);
							//jQuery("#amount_"+key).val(number_format(price));
							//jQuery("#total_amount").val(number_format(to_numeric(jQuery("#total_amount").val()) + price - oldAmount));	
							//jQuery("#amount_mini").val(jQuery("#total_amount").val());	

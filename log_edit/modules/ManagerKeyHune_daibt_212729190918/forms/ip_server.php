<?php
class IpServerForm extends Form
{
	function IpServerForm()
	{
		Form::Form('IpServerForm');
    ...
    function on_submit()
	{
       ...
       if(isset($_REQUEST['mi_group']))
	   {
        ...
        echo '<script>
            window.setTimeout("location=\''.URL::build_current(array('cmd'=>'ip_server')).'\'",2000);
            </script>';
        exit();
	}
	function draw()
	{
        ...
        $result = DB::fetch_all("SELECT  * FROM  manage_ipsever ORDER BY reception");
        $i = 1;
       ...
       // exit();
        $this->parse_layout('ip_server',$this->map);
	}
}
?>
...
class RoomStatus extends Form{
    ...
    function on_submit()
	{
        foreach($_REQUEST['room_repair'] as $k => $v)
        {
            ...
            $result = $this->setStatus($v['room_id'],$v['start_time'],$v['start_date'],$v['end_time'],$v['end_date'],$v['note'],$status);
            if(!empty($result))
            {               
                echo $result;
            }
    ...
    function setStatus($room_id,$s_time,$start_date,$e_time,$end_date, $note, $status)
	{
        $today = Date_Time::to_time(date('d/m/Y'));
        $s_t = split(':',$s_time);
        $e_t = split(':',$e_time);
        $start_time = Date_Time::to_time($start_date) + $s_t[0]*60*60 + $s_t[1]*60;
        if($start_time < $today)
            $start_time = $today;
        $end_time = Date_Time::to_time($end_date) + $e_t[0]*60*60 + $e_t[1]*60;
        $time = $start_time;
        ...
        //System::debug(Date_Time::to_time($start_date));
        $mid_date_end=$end_time;
        $bot_status;
        $bot_date_start;
        $bot_date_end;     
        $checkTop = DB::fetch("select HOUSE_STATUS, start_date as date_start, end_date as date_end
                        from room_status
                        where in_date='".Date_Time::to_orc_date(date('d/m/Y',$start_time - 3600*24))."' 
                        AND room_id='".$room_id."'");
        $checkBot = DB::fetch("select HOUSE_STATUS, start_date as date_start, end_date as date_end
                        from room_status
        ...
        if(isset($top_status) and $top_status)
        {
            ...
            while($time<$start_time)
            {
                ...
                DB::update('room_status',
                    array(
                        'start_date'=>$top_date_start,
                        'end_date'=>$top_date_end,
                        'note'=>$note
                    ),
                    $cond
                );
        ...
        if(isset($bot_status) and $bot_status)
        {
            ...
            while($time<=$bot_date_end)
            {
                ...
                DB::update('room_status',
                    array(
                        'start_date'=>$bot_date_start,
                        'end_date'=>$bot_date_end,
                        'note'=>$note
                    ),
                    $cond
                );
            ...
            */
            ...
            if(DB::exists($sql))
            {
                ...
                DB::update('room_status',
                    array(
                        'house_status'=>$status,
                        'start_date'=>$mid_date_start,
                        'end_date'=>$mid_date_end,
                        'note'=>$note
                    ),
                    $cond
                );
                ...
                .Portal::language('start_date').':'.date('H:i:s d/m/Y',$start_time).'<br> '
                .Portal::language('end_date').':'.date('H:i:s d/m/Y',$end_time).'<br>  ';
				System::log('edit',$title,$description,$room_id,'',User::id(),$room_id);
            }else
            {

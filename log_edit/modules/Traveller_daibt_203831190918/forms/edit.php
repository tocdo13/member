...
class EditTravellerForm extends Form
{
	...
	function on_submit()
	{
	   require_once 'packages/hotel/includes/member.php';
        ...
        $confix_passport = DB::fetch_all("SELECT id FROM traveller WHERE passport='".Url::get('passport')."' AND passport !='?' AND id!=".Url::iget('id')."");
       
        if(sizeof($confix_passport)>0){
            $this->add('passport',new TextType(true,'Trùng số CMND!',255,255));
        }
		if($this->check())
		{
		  if(Url::get('create_member_code')){
			...
			DB::update_id('traveller', 
				array(
					...
					'passport',
                    'supply_passport_day'=>Date_Time::to_orc_date(URL::get('supply_passport_day')),
                    'place_issuance_passport'=>URL::get('place_issuance_passport'),
                    'visa', 'address', 'email', 'phone', 'fax', 'note','province_id',
	...
	function draw()
	{
		...
		DB::query('
			select 
                ...
                to_char(traveller.supply_passport_day,\'DD/MM/YYYY\')as supply_passport_day,
		...
		//$is_vn = array(''=>'','0'=>'0-'.Portal::language('Alien'),'1'=>'1-'.Portal::language('Overseas_Vietnamese'),'2'=>'2-'.Portal::language('Viet_nam'),'3'=>'3-'.Portal::language('Viet_nam_in_foreign'));
        $is_vn = array('2'=>'2-'.Portal::language('Viet_nam'),'3'=>'3-'.Portal::language('Viet_nam_in_foreign'));
        $this->map += array(

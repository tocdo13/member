...
class BookingConfirmForm extends Form
{
	...
	{	   
        ...
        $cond = " reservation.id = ".$revertion_id." ";
        if($_REQUEST['portal'])
        {
            $portal_id = "#".$_REQUEST['portal'];
        ...
        $reservation_traveller = DB::fetch_all("
                                                SELECT
                                                    ...
                                                    ,reservation_traveller.flight_code as pick_up_code
                                                    ,reservation_traveller.flight_arrival_time as pick_up_time
                                                    ,reservation_traveller.car_note_arrival as pick_up_note
                                                    ,reservation_traveller.flight_code_departure as see_off_code
                                                    ,reservation_traveller.flight_departure_time as see_off_time
                                                    ,reservation_traveller.car_note_departure as see_off_note
                                                ...
                                                WHERE
                                                    ".$cond."
                                                ");
        $this->map['list_traveller_phone'] = array();
        $this->map['traveller_phone'] = '';
        $this->map['list_traveller_address'] = array();
        $this->map['traveller_address'] = '';
        $this->map['list_traveller_email'] = array();
        $this->map['traveller_email'] = '';
        $this->map['guest_name'] = '';
        foreach($reservation_traveller as $id=>$content)
        {
            if($this->map['guest_name']=='')
                $this->map['guest_name'] = $content['traveller_name'];
            else
                $this->map['guest_name'] .= " , ".$content['traveller_name'];
            
            $this->map['list_traveller_phone'] += array($content['id']=>$content['traveller_phone']);
            $this->map['traveller_phone'] = $content['traveller_phone'];
            $this->map['list_traveller_address'] = array($content['id']=>$content['traveller_address']);
            $this->map['traveller_address'] = $content['traveller_address'];
            $this->map['list_traveller_email'] = array($content['id']=>$content['traveller_email']);
            $this->map['traveller_email'] = $content['traveller_email'];
            
            $reservation_traveller[$id]['pick_up_time'] = date('H:i d/m/Y',$content['pick_up_time']);
            $reservation_traveller[$id]['see_off_time'] = date('H:i d/m/Y',$content['see_off_time']);
        }
        //System::debug($reservation_traveller);
        $this->map['reservation_traveller'] = $reservation_traveller;
        /** lay thong tin chung ve dat phong **/
        $this->map['reservation'] = DB::fetch("
                                            SELECT
                                                reservation.id
        ...
        if(Url::get('cmd') == 'booking_confirm')
        {
            ...
            {
                $this->parse_layout('book_confirm', $this->map+$reservation_traveller);
            }
        }
    }
}

...
$this->link_js('packages/core/includes/js/jquery/jquery.battatech.excelexport.js');
	...
	function draw()
	{
		...
		foreach($product_items as $key=>$value)
		{
            ...
            {
				$product_items[$key]['product__price'] = System::display_number(round(($value['price']/(1+$value['tax_rate']/100))/(1+$value['bar_fee_rate']/100)));
                ...
                //+ (($ttl*$discount_category/100)*$discount_rate/100);
                //echo $discnt.'after';
        ...
        foreach($product_items as $key=>$value){
            if(!empty($value['set_menu_id'])){
                if($value['set_menu_id']!=$bar_set_menu_id_temp){
                    ...
                    foreach($product_items as $k=>$v)
                    {
                        if(mb_strtoupper($v['name'])==mb_strtoupper($value['set_menu_name']))
                        {
                    ...
                    $arr_set_menu[$bar_set_menu_id_temp]['total'] += System::calculate_number($value['product__total']); 
                    //$total+=$value['product__total'];
                    //$product_items[$key]['set_menu'] = $arr_set_menu;
                    //$arr_set_menu = array();
                }
		...
		$row['bar_fee'] = System::display_number(round($row['sum_total']+$row['discount']+$row['order_discount'],0)-round($row['tax'])-round($total_price,0));
          ...
          else
          {
              ...
              elseif($_REQUEST['paper']=='A4')
              {
    			...
    			$this->parse_layout($layout,$row+$hotel+array(
    				...
    				'total_discount'=>System::display_number($total_discount),
                    'discount_percent_dn'=>$row['discount'], /** daund them giam gia so tien */                     
    				'amount'=>System::display_number(round($total_price + $total_discount,0)),
    				'payment_kind'=>$payment_kind,
                    'bar_name'=>$bar_items['bar_name'],
                    'table_checkout'=>date('H:i d/m/Y',$bar_items['departure_time']),
    				'payment_type_name'=>$payment_name,
        ...
        else if(Url::check(array('act'=>'print_kitchen')))
		{
			...
			$this->parse_layout('print_kitchen',$row+$hotel+array(
                ...
                'discount_percent_dn'=>$row['discount'], /** daund them giam gia so tien */                
				'amount'=>System::display_number(round($total_price,0)),
				'payment_kind'=>$payment_kind,
                'bar_name'=>$bar_items['bar_name'],

...
class ChangeRoomForm extends Form
{
    ...
    **/
    function create_new_reservation_room($reservation_room,$from_room_name,$to_room_name,$to_room_id,$use_old_price,$note,$old_reservation_traveller)
    {
        ...
        {
            $old_reservation_traveller_id_list = '';
            foreach($old_reservation_traveller as $key=>$value)
            {
                $old_reservation_traveller_id_list .= $old_reservation_traveller_id_list==''?$old_reservation_traveller[$key]['id']:','.$old_reservation_traveller[$key]['id'];
                ...
                $new_reservation_traveller = DB::insert('reservation_traveller',$old_reservation_traveller[$key]);
            }
        }
        ...
        /** Daund: Update reservation_room mới và description mới*/
        /** manh them: chuyen cac hoa don phong chua thanh toan het sang chang moi **/
        if(isset($new_reservation_traveller)){
            //echo 1;
            $folio_room_old = DB::fetch_all('select folio.id,folio.total,folio.reservation_room_id from folio where folio.reservation_traveller_id in ('.$old_reservation_traveller_id_list.') and folio.reservation_room_id is not null and folio.total!=0 and folio.customer_id is null');
            //System::debug($folio_room_old);
            foreach($folio_room_old as $keyFolioOld=>$valueFolioOld){
                $payment_folio_old = DB::fetch_all('select * from payment where folio_id='.$valueFolioOld['id']);
                $total_payment = 0;
                //System::debug($payment_folio_old);
                foreach($payment_folio_old as $keyPay=>$valuePay){
                    if($valuePay['payment_type_id']=='REFUND')
                        $total_payment -= $valuePay['amount']*$valuePay['exchange_rate'];
                    else
                        $total_payment += $valuePay['amount']*$valuePay['exchange_rate'];
                }
                if($total_payment<$valueFolioOld['total']){
                    if($valueFolioOld['reservation_room_id']!=$old_reservation_room)
                        DB::update('folio',array('reservation_traveller_id'=>$new_reservation_traveller),'id='.$valueFolioOld['id']);
                    else
                        DB::update('folio',array('reservation_traveller_id'=>$new_reservation_traveller,'reservation_room_id'=>$new_reservation_room),'id='.$valueFolioOld['id']);
                    
                    DB::update('traveller_folio',array('reservation_traveller_id'=>$new_reservation_traveller),'folio_id='.$valueFolioOld['id']);
                    DB::update('payment',array('bill_id'=>$new_reservation_room,'reservation_room_id'=>$new_reservation_room,'reservation_traveller_id'=>$new_reservation_traveller),'folio_id='.$valueFolioOld['id']);
                }
                if(DB::exists('select id from traveller_folio where reservation_room_id='.$new_reservation_room.' and folio_id='.$valueFolioOld['id'])){
                    if($valueFolioOld['reservation_room_id']!=$old_reservation_room)
                        DB::update('folio',array('reservation_traveller_id'=>$new_reservation_traveller),'id='.$valueFolioOld['id']);
                    else
                        DB::update('folio',array('reservation_traveller_id'=>$new_reservation_traveller,'reservation_room_id'=>$new_reservation_room),'id='.$valueFolioOld['id']);

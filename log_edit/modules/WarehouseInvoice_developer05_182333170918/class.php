	...
	function WarehouseInvoice($row)
	{
		...
		switch (Url::get('cmd'))
        {
            ...
            case 'edit':
                ...
                {
                    $purchases = ' AND purchases_id='.Url::get('purchases_invoice_id').'';
                }
				if(User::can_edit(false,ANY_CATEGORY) and Url::get('id') and WarehouseInvoice::$item = DB::select('wh_invoice','id ='.Url::iget('id').$purchases))
                {
                    if(Url::get('choose_warehouse')==1)
                    {
                        if(User::can_view(false,ANY_CATEGORY))
			...
			case 'delete':
				if(User::can_delete(false,ANY_CATEGORY))
                {
                    ...
                    if(Url::get('id') and DB::exists('SELECT id, warehouse_id FROM wh_invoice WHERE id = '.Url::iget('id').''))
                    {
                        ...
                        $description.= 'Delete warehouse invoice '.$code.'';
					...
					if(Url::get('item_check_box'))
                    {
						...
						for($i=0;$i < sizeof($arr); $i++)
                        {
                            ...
                            $invoice_id = $arr[$i];
                            $code = DB::fetch('select bill_number from wh_invoice where id='.$arr[$i],'bill_number');
                            $title = 'Delete warehouse invoice '.$code.'';
                            $description.= 'Delete warehouse invoice '.$code.'';
                            System::log($action,$title,$description,$invoice_id);

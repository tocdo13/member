...
//----------------------------------------Tien phong`----------------------------------------------------
		...
		$bar_charge = 0;
        $karaoke_charge = 0;
        $ve_charge = 0; //tong tien` su dung dich vu bar
        $ticket_charge = 0; //tong tien` su dung dich vu ve
        $mass_charge = 0;//tong tien` su dung dich vu spa
		$sql = '
			SELECT
		...
		while($d>=$from and $d<=$to)
		{
			...
			$sql=('
				select
					...
					,extra_service_invoice_detail.quantity*extra_service_invoice_detail.price as amount_net
                    ,extra_service_invoice.net_price as net,
					...
					extra_service_invoice.total_before_tax as total_before_tax,
				...
				where
                    ...
                    (
                        (
                        extra_service_invoice_detail.in_date = \''.Date_Time::to_orc_date(date('d/m/Y',$d)).'\' 
                        and extra_service_invoice_detail.in_date > reservation_room.arrival_time 
                        and extra_service_invoice_detail.in_date< reservation_room.departure_time
                        )
                        or
                        (
                          extra_service_invoice_detail.in_date <= reservation_room.arrival_time
                          and \''.Date_Time::to_orc_date(date('d/m/Y',$d)).'\' <= reservation_room.arrival_time 
                        )
                        or
                        (
                          extra_service_invoice_detail.in_date >= reservation_room.departure_time 
                          and \''.Date_Time::to_orc_date(date('d/m/Y',$d)).'\' >= reservation_room.departure_time  
                        )
                    )
			');//					AND extra_service_invoice.payment_type <> \'ROOM\'
			//AND extra_service_invoice_detail.in_date = \''.Date_Time::to_orc_date(date('d/m/Y',$d)).'\'
           $row['extra_services'] = DB::fetch_all($sql);
           
            //System::debug($row['extra_services']);
            //echo Url::get('extra_service_invoice');
					...
					//if($s_value['payment_type'] != 'ROOM'){
					   if($s_value['net']==0){
                        ...
                        $row['extra_services'][$s_key]['total'] = System::display_number($s_value['amount']);
                            ...
                            $total += $s_value['amount_net'];
                            $row['extra_services'][$s_key]['total'] = System::display_number($s_value['amount_net']);
                        }
            ...
            if(URL::get('ticket_invoice'))
            {
                ...
                $sql = '
					select 
                        ...
                        ticket_reservation.amount_pay_with_room as total_before_tax,
			...
			if(URL::get('bar_invoice'))
            {
				...
				$sql = '
					select
						...
						CONCAT(bar.name,CONCAT(\' #\',bar_reservation.code)) AS bar_name,
                ...
                if($bar_resrs = DB::fetch_all($sql))
				{
					foreach($bar_resrs as $bk=>$reser)
					{
					    $bar_resrs[$bk]['total'] = $reser['amount_pay_with_room'];   					   
					    $total_bar_bft = $reser['amount_pay_with_room']/(1 + $reser['bar_fee_rate']/100)/(1 + $reser['tax_rate']/100);   
						$bar_resrs[$bk]['total_before_tax'] = $total_bar_bft;
                        $bar_resrs[$bk]['bar_reservation_id'] = $reser['id'];
						$bar_resrs[$bk]['service_charge'] = ($total_bar_bft*$reser['bar_fee_rate']/100);
						$bar_resrs[$bk]['bar_chrg_tax'] = ($total_bar_bft+$bar_resrs[$bk]['service_charge'])*$reser['tax_rate']/100;
						$bar_resrs[$bk]['bar_chrg'] = System::display_number($reser['amount_pay_with_room']);
						$bar_chrg = 0;
						{
							$bar_charge += $reser['amount_pay_with_room'];
						}
						$ni+=2;
					}
            ...
            if(URL::get('karaoke_invoice'))
            {
				$sql = '
					select
						...
						karaoke_reservation.time_out, karaoke_reservation.prepaid,
					    karaoke_reservation.tax_rate,karaoke_reservation.karaoke_fee_rate,
						CONCAT(karaoke.name,CONCAT(\' #\',karaoke_reservation.code)) AS karaoke_name,
                ...
                if($karaoke_resrs = DB::fetch_all($sql))
				{
					...
					{
					    $karaoke_resrs[$bk]['total'] = $reser['amount_pay_with_room'];   					   
					    $total_karaoke_bft = $reser['amount_pay_with_room']/(1 + $reser['karaoke_fee_rate']/100)/(1 + $reser['tax_rate']/100);   
						$karaoke_resrs[$bk]['total_before_tax'] = $total_karaoke_bft;
                        $karaoke_resrs[$bk]['karaoke_reservation_id'] = $reser['id'];
						$karaoke_resrs[$bk]['service_charge'] = ($total_karaoke_bft*$reser['karaoke_fee_rate']/100);
						$karaoke_resrs[$bk]['karaoke_chrg_tax'] = ($total_karaoke_bft+$karaoke_resrs[$bk]['service_charge'])*$reser['tax_rate']/100;
						$karaoke_resrs[$bk]['karaoke_chrg'] = System::display_number($reser['amount_pay_with_room']);
						$karaoke_chrg = 0;
						{
							$karaoke_charge += $reser['amount_pay_with_room'];
						}
						$ni+=2;
					}
					$day[$n]['karaoke_resers'] = $karaoke_resrs;
					$day[$n]['karaoke_name'] = $reser['karaoke_name'];
					$day[$n]['karaoke_charge'] = 1;
				}
				else
				{
					$day[$n]['karaoke_charge'] = 0;
				}
				//$row['discount_total']=System::display_number($row['discount_total']);
			}
            
            if(URL::get('vend_invoice'))
            {
				$sql = '
					select
						ve_reservation.id,
					--	ve_reservation.time_out, 
                       -- ve_reservation.prepaid,
					...
					where
						reservation_room_id=\''.Url::iget('id').'\'
						and (ve_reservation.ARRIVAL_TIME>='.$d.' and ve_reservation.ARRIVAL_TIME<='.($d+24*3600).')
						and (ve_reservation.status=\'CHECKOUT\' or ve_reservation.status=\'CHECKIN\')
				';
                if($ve_resrs = DB::fetch_all($sql))
				{
					foreach($ve_resrs as $bk=>$reser)
					{
					    $ve_resrs[$bk]['total'] = $reser['amount_pay_with_room'];   					   
					    $total_ve_bft = $reser['amount_pay_with_room']/(1 + $reser['bar_fee_rate']/100)/(1 + $reser['tax_rate']/100);   
						$ve_resrs[$bk]['total_before_tax'] = $total_ve_bft;
                        $ve_resrs[$bk]['ve_reservation_id'] = $reser['id'];
						$ve_resrs[$bk]['service_charge'] = ($total_ve_bft*$reser['bar_fee_rate']/100);
						$ve_resrs[$bk]['ve_chrg_tax'] = ($total_ve_bft+$ve_resrs[$bk]['service_charge'])*$reser['tax_rate']/100;
						$ve_resrs[$bk]['ve_chrg'] = System::display_number($reser['amount_pay_with_room']);
						$ve_chrg = 0;
						{
							$ve_charge += $reser['amount_pay_with_room'];
						}
						$ni+=2;
					}
                    
					$day[$n]['ve_resers'] = $ve_resrs;
					$day[$n]['ve_name'] = Portal::language('vending');
					$day[$n]['ve_charge'] = 1;
				}
				else
				{
            ...
            if(URL::get('massage_invoice'))
            {
    			$sql_massage='
    				SELECT
                        ...
                        (massage_reservation_room.amount_pay_with_room/(massage_reservation_room.tax + 100)*100)/(massage_reservation_room.service_rate+100)*100 as total_amount
                        
    				FROM
    					massage_reservation_room
    				WHERE
    					massage_reservation_room.hotel_reservation_room_id='.$id.'
                        and (massage_reservation_room.time>='.$d.' and massage_reservation_room.time<='.($d+24*3600).')
    				
    			';
                if($massge = DB::fetch_all($sql_massage))
    			{
    				foreach($massge as $msg=>$ms)
    				{
    					
    					$massge[$msg]['massage_reservation_id'] = $ms['id'];
    					$massge[$msg]['mass_service_rate'] = ($ms['total_amount']*$ms['service_rate']/100);
    					$massge[$msg]['mass_tax'] = ($ms['total_amount']+$massge[$msg]['mass_service_rate'])*$ms['tax']/100;
    					
    					$massge[$msg]['mass_chrg'] = System::display_number($ms['total_amount']+$massge[$msg]['mass_service_rate'] +$massge[$msg]['mass_tax']);
    					$massge[$msg]['massage_name'] = $ms['id'];
                        $mass_chrg = 0;
    					{
    						$mass_charge += ($ms['total_amount']+$massge[$msg]['mass_service_rate'] +$massge[$msg]['mass_tax']);
    					}
    					$ni+=2;
    				}
    				$day[$n]['massage'] = $massge;
    				$day[$n]['mass_charge'] = 1;
    			}
    			else
    			{
...
//----------------------------------------/Other services------------------------------------------------
	...
	function print_page($items, $ipp, &$report, $last_page, $num_page, $room_result, $row, $curr, $checkout_id, $tax_total, $total, $total_by_text,$service_charge_total,$total_before_tax)
	{
		...
		}else
        {
            ...
            if(Url::get('karaoke'))
            {
				$description .= ($description?' + ':'').Portal::language('karaoke_invoice').'';
			}
            
            if(Url::get('vend_invoice'))
            {
				$description .= ($description?' + ':'').Portal::language('vend_invoice').'';
			}

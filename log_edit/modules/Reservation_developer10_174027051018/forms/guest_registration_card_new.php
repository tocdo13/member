...
class GuestRegistrationCardFormNew extends Form
{
	...
	function draw()
	{
        ...
        $price=DB::fetch('select avg(change_price) as id 
                            from room_status inner join  reservation_room on reservation_room.id=room_status.reservation_room_id
                        where  reservation_room.id='.Url::get('id').'
                         and ((reservation_room.arrival_time = reservation_room.departure_time and room_status.in_date=reservation_room.arrival_time)
                          or
                          (reservation_room.arrival_time < reservation_room.departure_time and room_status.in_date < reservation_room.departure_time))
                          
                        ','id');
                        $this->map['price']=$price;

...
class ReportForm extends Form
{
    ...
    function on_submit()
	{
        ...
        if(isset($_REQUEST['get_delete']) && $_REQUEST['get_delete']!='')
        {
            if(!empty($_REQUEST['check_list']))
            {
                ...
                DB::delete('manage_key_hune',$conditions);
            }
        }
	...
	function draw()
	{
	   //xoa di tat ca nhung the rac=> nhung the khong co lien ket toi reservation_room
        ...
        $sql ="SELECT reservation_room.id,reservation_room.room_id,room.name,
                ...
                INNER JOIN manage_key_hune ON reservation_room.id=manage_key_hune.reservation_room_id
                WHERE $cond AND room.portal_id='".PORTAL_ID."'
                ORDER BY manage_key_hune.create_time desc";
    ...
    function list_key_r_r($r_r_id,$start=0,$end=0)
    {
        ...
        if($start!=0 && $end!=0)
        {
            $cond .=' AND manage_key_hune.create_time >='.$start.' AND manage_key_hune.create_time <='.($end + 86400);
        }
        $sql = 'SELECT manage_key_hune.id,
                    manage_key_hune.begin_time,
                    manage_key_hune.end_time,
                    manage_key_hune.create_time,
                    manage_key_hune.create_user,
                    manage_key_hune.checkout_user,
                    manage_key_hune.checkout_time
                FROM manage_key_hune where manage_key_hune.reservation_room_id='.$r_r_id.$cond .'
                ORDER BY manage_key_hune.create_time desc';
    ...
    function remove_key_exception()
    {
        $sql ="SELECT manage_key_hune.id
                ...
                LEFT JOIN reservation_room ON manage_key_hune.reservation_room_id=reservation_room.id WHERE reservation_room.id is null";
        ...
        if(trim($conditions)!='')
            DB::query("DELETE FROM manage_key_hune WHERE ".$conditions);
    }
    function remove_key_expiry_date()
    {
        ...
        $sql ="SELECT manage_key_hune.id
            FROM manage_key_hune 
            INNER JOIN reservation_room ON manage_key_hune.reservation_room_id=reservation_room.id AND (reservation_room.status='CHECKOUT' OR reservation_room.status='CANCEL')
            WHERE manage_key_hune.create_time<=$past";
        $ids = DB::fetch_all($sql);
        $conditions ='';
        foreach($ids as $key=>$value)
        {

...
<form name="EditMinibarProductForm" method="post" >
	<table cellpadding="15" cellspacing="0" width="50%" border="0" bordercolor="#CCCCCC" class="table-bound" style="margin-left: 20px;">
		<tr>        	
			<td width="55%" class="" style="text-transform: uppercase; font-size: 18px; padding-left: 15px;">[[.minibars_norm.]] (<?php if(Url::get('cmd')=='add'){echo Portal::language('asc_amenities');}else{echo Portal::language('desc_amenities');} ?>)</td>			
			<td width="45%"><input type="button" value="[[.apply.]]" onclick="checkClick();" class="w3-btn w3-orange w3-text-white" name="confirm_edit" style="text-transform: uppercase; margin-right: 5px;" />
                <input type="button" name="list" class="w3-btn w3-green" onclick="window.location='<?php echo Url::build_current();?>'" value="[[.back.]]" style="text-transform: uppercase; margin-right: 5px;"/>                
            </td>
		</tr>
    </table>      
	...
	<div style="margin-left: 20px;">
		<span id="mi_room_amenities_all_elems">
			<span style="white-space:nowrap;">
				<span class="multi-input-header" style="width:100px;height: 25px;">[[.product_id.]]</span>
				<span class="multi-input-header" style="width:200px;height: 25px;">[[.product_name.]]</span>
                	
                    <span class="multi-input-header" style="width:150px;height: 25px;">[[.norm_quantity.]] <?php if(Url::get('cmd')=='add'){echo 'Tăng';}else{echo 'Giảm';} ?></span>
				...
				<span class="multi-input-header" style="width:50px;height: 25px;">[[.position.]]</span>
		...
		<input type="button" value="[[.add_product.]]" onclick="mi_add_new_row('mi_room_amenities');my_autocomplete();jQuery('#position_'+input_count).ForceNumericOnly();jQuery('#norm_quantity_'+input_count).ForceNumericOnly();" style="text-transform: uppercase; margin-top: 5px;" class="w3-btn w3-cyan w3-text-white"/>
        <?php }?>        
	</div>
	</form>
</div>

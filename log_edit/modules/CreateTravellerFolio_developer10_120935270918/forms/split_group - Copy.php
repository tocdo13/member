		...
		if(Url::get('customer_id') && Url::get('action')==1 && Url::get('id')){
            ...
            if(!empty($split_invoices))
            {
				...
				if(Url::get('folio_id'))
                {
                    ...
                    $title = "Edit Group Folio id: #".Url::get('folio_id')."";
                ...
                else
                {
                    ...
                    $title = "ADD Group Folio id: #".$folio_id."";
...
/*--------------------------send mail--------------------------*/            
            ...
            {// Save và th?c hi?n thanh toán
                    ...
                    $tt = 'form.php?block_id=428&cmd=payment&id='.Url::get('id').'&member_code='.$get_member_code.'&customer_id='.$customer_id.'&type=RESERVATION&act=group_folio&total_amount='.System::calculate_number(Url::get('total_payment')).'&folio_id='.$folio_id.'&portal_id='.PORTAL_ID.'';
			}
            else
            {
	...
	{	
		...
		foreach($others as $k => $val)
        {
            ...
            {
				$folio_other[$val['type'].'_'.$val['invoice_id']]['href'] .= '<br><a target="_blank" href="'.'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root.'?page=view_traveller_folio&cmd=group_invoice&folio_id='.$val['folio_id'].'&customer_id='.$val['customer_id'].''.'">_ Group Folio: '.$val['folio_id'].' - Recode:'.$val['reservation_id'].'</a>';
			}
		}	
      ...
      //end 
		foreach($reservation_rooms as $k => $rr)
        {
			foreach($rr['items'] as $key =>$item)
            {
                ...
                /** THL **/
                $reservation_rooms[$k]['items'][$key]['amount'] = (number_format(System::calculate_number($item['amount'])*(1+$item['service_rate']/100)*(1+$item['tax_rate']/100),2));
	...
	function get_folio_other($reservation_id,$customer_id)
    {
		return $folio = DB::fetch_all('
            ...
            GROUP BY
                folio.id
                ,customer.name
                ,CONCAT(folio.customer_id,CONCAT(\' \',folio.num))
                ,folio.num
    ...
    function log_delete_folio($data,$folio)
    {
        ...
        {
            /** xóa toàn bộ folio **/
            System::log('DELETE','DELETE GROUP FOLIO ID #'.$folio,"User id delete: ".User::id()."<br/> Time: ".date('H:i d/m/Y'),$folio);
        }
        else
        {

<?php
class AddTicketInvoiceForm extends Form
{
    //Bien nay luu lai ma~ ve dc in, de truyen sang layout => ko phai chon lai ve nua 
    static $ticket_id;
	function AddTicketInvoiceForm()
	{
		Form::Form('AddTicketInvoiceForm');
	...
	function on_submit()
	{
       if(Url::get('save'))
       {
            ...
            if( isset($_REQUEST['mi_ticket']) )
            {
                    ...
                    if($record['quantity'] != 0)
                    {
                        ...
                        if(extension_loaded ('printer'))
                        {
                            ...
                            if($printer_name)
                            {
                                ...
                                {
                                    $last_code++;
                                    $code_print = date('dmy').'-'.$this->get_ticket_code($last_code);
                                    Printer::write_ticket( 1840,HOTEL_NAME,$code_print,$ticket_name,System::display_number($record['price']) ); 
                        ...
                        {
                            $this->error('could_not_connect_printer','could_not_connect_printer');
                            return;
                        } 
                        */
                        $id = DB::insert('ticket_invoice',$record);
                        ...
                        */
                        ...
                        //echo "<script>window.open('".Url::build_current(array("cmd"=>"print","invoice_id"=>$id,"ticket_id"=>$record['ticket_id'],"quantity"=>$record['quantity'],"ticket_name"=>$ticket_name,"ticket_group_name"=>$ticket_group_name,"ticket_prefix"=>$ticket_prefix,"ticket_price"=>$ticket_price,"last_code"=>$last_code,))."','_blank');</script>";
                        echo "<script>window.open('".Url::build_current(array("cmd"=>"print","invoice_id"=>$id,"form"=>$ticket_form,"denoted"=>$ticket_denoted,"ticket_id"=>$record['ticket_id'],"quantity"=>$record['quantity'],"ticket_name"=>$ticket_name,"ticket_name_2"=>$ticket_name_2,"ticket_group_name"=>$ticket_group_name,"ticket_prefix"=>$ticket_prefix,"ticket_price"=>$ticket_price,"last_code"=>$last_code,))."','_blank');</script>";
                        /*
                        for($i=1; $i<=$record['quantity']; $i++)
                        {
                            $last_code++;
                ...
                //onclick="window.open('?page=ticket_invoice&portal=default','_blank');"
                //echo "<script>window.open('".Url::build_current()."','_blank');</script>";
            }
       }
	}
	function draw()
	{
        ...
        //System::debug($area);
        //Nếu không có quầy thì Tạo request dể chọn quầy sẵn
        ...
        if(Url::get('ticket_area_id'))
        {
            ...
            $_REQUEST['ticket_id'] = AddTicketInvoiceForm::$ticket_id;
            //System::debug($ticket);
        }
        /**
         * Danh sách bán vé
    ...
    //function get_last_code($id)
//	{
//        if($lastest_item = DB::fetch('SELECT id,last_code, to_char(date_used,\'YYYY\') as year FROM ticket_invoice where id != '.$id.' and portal_id = \''.PORTAL_ID.'\' ORDER BY id DESC'))
//        {

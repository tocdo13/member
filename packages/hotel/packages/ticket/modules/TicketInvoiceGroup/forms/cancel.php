    ...
    function on_submit()
    {
        if(isset($_REQUEST['cancel']) and Url::get('invoice_id'))
        {
            ...
            foreach($_REQUEST['cancel'] as $key => $value)
            {
                ...
                $invoice_data['num_cancel'] = $invoice_detail['num_cancel'] + 1;
                ...
                if(!DB::fetch('select * from ticket_cancelation where ticket_invoice_id = '.Url::get('invoice_id').' and ticket_serie ='.$value))
                {
                    ...
                    $log_mi_ticket .= Portal::language('cancel_ticket'). ': '. $invoice_detail['ticket_name']. ' Serie: '. $data['ticket_serie']. ' '. Portal::language('cancel_reason'). ': '. $data['note'].'<br>';
                }
            ...
            $title = 'Cancel ticket, Code: '.$invoice_detail['ticket_reservation_id'];
    		$description = ''
    		.Portal::language('time').':'.date('d/m/Y H:i:s').'<br>  ' 
    		.'<hr>'.$log_mi_ticket.'';
    		System::log('cancel ticket',$title,$description,$invoice_detail['ticket_reservation_id']);    
        } 
    }
    function draw()
    {

...
function GetTotalPayment($input)
{
    if(!$input){
        $input = false;
    }
   ...
   jQuery("#amount_remain").html(number_format(to_numeric(jQuery("#remain").val())-total_payment));
   var total_payment_traveller_js = to_numeric(jQuery("#total_payment_traveller").val());
   if(total_payment_traveller_js==0)
   {
        jQuery("#total_remain_traveller").html(0);
   }
   else
   {
        jQuery("#total_remain_traveller").html(number_format(total_payment_traveller_js-to_numeric(jQuery("#amount_mini").html())))
   }
   
   /** payment **/
   $payment_cash = to_numeric(jQuery("#payment_cash").val());
   $payment_credit_card = to_numeric(jQuery("#payment_credit_card").val());
   $payment_refund = to_numeric(jQuery("#payment_refund").val());
   $payment_foc = to_numeric(jQuery("#payment_foc").val());
   $payment_debit = to_numeric(jQuery("#payment_debit").val());
   
   /** auto set payment cash **/
   if( ($payment_credit_card - $payment_refund + $payment_foc + $payment_debit) <= to_numeric(jQuery("#amount_mini").html()) ){
        jQuery("#payment_cash").val(number_format( to_numeric(jQuery("#amount_mini").html()) - ($payment_credit_card - $payment_refund + $payment_foc + $payment_debit) ));
        $payment_cash = to_numeric(jQuery("#payment_cash").val());
   }
   /** end auto **/
   
   $payment = $payment_cash + $payment_credit_card - $payment_refund + $payment_foc + $payment_debit;
   
   $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
   if($payment_remain<0){
        alert('Số tiền thanh toán không được lớn hơn tổng số tiền của hóa đơn!');
        if($input){
            $input_id = $input.id;
            if($input_id=='payment_cash'){
                $payment = $payment_credit_card - $payment_refund + $payment_foc + $payment_debit;
                $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
                jQuery("#payment_cash").val(number_format($payment_remain));
                
            }else if($input_id=='payment_credit_card'){
                $payment = $payment_cash - $payment_refund + $payment_foc + $payment_debit;
                $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
                jQuery("#payment_credit_card").val(number_format($payment_remain));
            }else if($input_id=='payment_refund'){
                $payment = $payment_cash + $payment_credit_card + $payment_foc + $payment_debit;
                $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
                jQuery("#payment_refund").val(number_format($payment_remain));
            }else if($input_id=='payment_foc'){
                $payment = $payment_cash + $payment_credit_card - $payment_refund + $payment_debit;
                $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
                jQuery("#payment_foc").val(number_format($payment_remain));
            }else if($input_id=='payment_debit'){
                $payment = $payment_cash + $payment_credit_card - $payment_refund + $payment_foc;
                $payment_remain = to_numeric(jQuery("#amount_mini").html()) - $payment;
                jQuery("#payment_debit").val(number_format($payment_remain));
            }
            jQuery("#payment_remain").val(0);
        }
   }else{
        jQuery("#payment_remain").val(number_format($payment_remain));
   }
   /** end payment **/
...
function get_keyboard(obj,key)
{
	...
	textkeyboard += '<li class="symbol" ><span class="off">;</span><span class="on">:</span></li>';
	textkeyboard += '<li class="symbol" ><span class="off">\'</span><span class="on">"</span></li>';
	textkeyboard += '<li class="return lastitem" onclick="HideDialog(\'dialog_keyboard\');if(jQuery(\'#name_keyboard\').val()==\'input_product_name\'){searchProduct(\'product\',jQuery(\'#bar_id_other\').val());}else if(jQuery(\'#name_keyboard\').val()==\'note_product\'){submitNote(jQuery(\'#text_keyboard\').val(),jQuery(\'#id_note\').val());}">OK</li>';
	...
	textkeyboard += '<li class="accept" onclick="HideDialog(\'dialog_keyboard\');if(jQuery(\'#name_keyboard\').val()==\'input_product_name\'){searchProduct(\'product\',jQuery(\'#bar_id_other\').val());}else if(jQuery(\'#name_keyboard\').val()==\'note_product\'){submitNote(jQuery(\'#text_keyboard\').val(),jQuery(\'#id_note\').val());}">ACCEPT</li>';
	textkeyboard += '</ul></div>';	

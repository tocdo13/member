...
class TicketRevenueReportForm extends Form
{
	...
	function draw()
	{
        ...
        $sql='select ve_reservation.id as ve_reservation_id,
                    ve_reservation_product.id,
                    --sum((ve_reservation_product.price*(ve_reservation_product.quantity))*(100-NVL(ve_reservation_product.discount_rate,0))/100) as price,
                    --sum((ve_reservation_product.price*(ve_reservation_product.quantity))*NVL(ve_reservation_product.discount_rate,0)/100) as product_discount
		...
		foreach($items as $key=>$value)
		{
            ...
            {
                if($value['id']==$value2['ve_reservation_id'])
                {
                    if($value['id']==$value2['ve_reservation_id'])
                    {
                        //$items[$key]['total_price'] = $value2['price'];
                        //$total_price = $items[$key]['total_price'];
                        $items[$key]['product_discount'] += $value2['price']*($value2['quantity'] -$value2['quantity_discount'])*($value2['promotion']/100) ;
                    }
                }
            }
            if($value['full_rate']==0 and $value['full_charge']==0)//gia chua thue phi lay luon
            {
                $total_before_discount = $value['total'];
            }
            if($value['full_charge']==1)//da co phi bo phi di
            {
                $total_before_discount = $value['total']/(1+$value['bar_fee_rate']/100);
            }   
            if($value['full_rate']==1)//da co thue phi bo thue phi
            {
                $total_before_discount = $value['total']/(1+$value['tax_rate']/100)/(1+$value['bar_fee_rate']/100);
            } 
            
            //System::debug($total_price);
            //tong truoc giam gia

...
class WarehouseReportOptionsForm extends Form
{
	...
	function draw()
	{
        ...
        **/
		...
		$this->map['supplier_id_list'] = String::get_list(DB::select_all('supplier','','code'));
    ...
    {
		if(Url::get('code') and $row = DB::select('product','id = \''.Url::get('code').'\''))
        {
			...
			if(isset($old_items[$row['id']]))
            {
				if(isset($old_items[$row['id']]['import_number']))
					$this->map['start_remain'] += $old_items[$row['id']]['import_number'];
				if(isset($old_items[$row['id']]['export_number']))
					$this->map['start_remain'] -= $old_items[$row['id']]['export_number'];			
			}
    ...
    static function get_store_remain_products()
    {
		...
		$sql = '
			SELECT
				...
				unit.name_'.Portal::language().' as unit,
                wh_start_term_remain.quantity as start_term_quantity,
				wh_start_term_remain.quantity as remain_number,
				0 as import_number,
                0 as export_number,
	...
	function get_warehouse_export()
    {
        ...
        {
			$items[$key]['i'] = ++$i;
			$items[$key]['amount'] = System::display_number_report($value['total_payment_price']);
			$this->map['total_quantity'] += round ($value['quantity'], 2);
			$this->map['total_amount'] += round($value['total_payment_price'], 2);
			$items[$key]['quantity'] = number_format($value["quantity"],3,".",",");//System::display_number_report($value['quantity']);
		...
		$this->map['total_amount'] = System::display_number_report($this->map['total_amount']);
		$this->map['total_quantity'] = System::display_number_report($this->map['total_quantity']);
		return $items;
	}	
	function get_warehouse_import()
    {
		...
		$sql = '
			SELECT
				...
				unit.name_'.Portal::language().' as unit_name,wh_invoice_detail.num as quantity,
        ...
        {
			$items[$key]['i'] = ++$i;
			$items[$key]['amount'] = System::display_number_report($value['payment_price']);
			$this->map['total_quantity'] += round ($value['quantity'], 2);
        ...
        {
			$back_items[$key]['i'] = ++$i;
			$back_items[$key]['amount'] = System::display_number_report($value['payment_price']);
			$this->map['total_quantity'] -= round ($value['quantity'], 2);
			$this->map['total_amount'] -= round($value['payment_price'], 2);
			$back_items[$key]['quantity'] = number_format($value["quantity"],3,".",",");//System::display_number_report($value['quantity']);
			$back_items[$key]['price'] = System::display_number_report($value['price']);

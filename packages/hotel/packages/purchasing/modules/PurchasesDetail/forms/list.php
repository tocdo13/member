<?php
class ListPurchasesDetailForm extends Form
{
	function ListPurchasesDetailForm()
	{
		Form::Form('ListPurchasesDetailForm');
	...
	function draw()
	{
       ...
       $cond .= 'AND (purchases_proposed.status=\'CONFIRM\' OR purchases_proposed.status=\'ADJUSTED\')';
       //echo $cond;
	   $items = DB::fetch_all('SELECT * from purchases_proposed where '.$cond.' order by confirm_time');
       $i=1;
       //System::debug($items);
       ...
       $this->map['status_list'] = array(''=>'--ALL--','CONFIRM'=>Portal::language('confirm_1'),'ADJUSTED'=>Portal::language('adjusted'));
       //System::debug($items);

...
class ListBarNoteForm extends Form
{
    ...
    function draw()
    {
        ...
        $cond = ' portal_id = \''.PORTAL_ID.'\' and bar_note.create_time <= '.$time_to.' and bar_note.create_time >= '.$time_from.' ';
        $item_per_page = 50;
		$sql = 'SELECT count(*) AS acount FROM bar_note where '.$cond.' ';
        ...
        $sql = '
			...
			(
				SELECT
					bar_note.*,
					ROW_NUMBER() OVER (ORDER BY bar_note.id) as rownumber
				FROM
					bar_note
                where 
                    '.$cond.'

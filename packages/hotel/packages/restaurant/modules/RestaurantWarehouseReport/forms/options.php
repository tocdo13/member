<?php
class RestaurantWarehouseReportOptionsForm extends Form
{
	function RestaurantWarehouseReportOptionsForm()
	{
		Form::Form('RestaurantWarehouseReportOptionsForm');
	...
	function get_store_card(){
		if(Url::get('code') and $row = DB::select('wh_product','id = \''.Url::get('code').'\'')){
			...
			$old_cond = 'res_wh_invoice_detail.product_id = \''.$row['id'].'\'
					'.(Url::get('date_from')?' AND res_wh_invoice.create_date <\''.Date_Time::to_orc_date(Url::get('date_from')).'\'':'').'
			';		
			$sql = '
				SELECT
					res_wh_invoice_detail.id,res_wh_invoice_detail.product_id,res_wh_invoice_detail.num,res_wh_invoice.type
				FROM
					res_wh_invoice_detail
					INNER JOIN res_wh_invoice ON res_wh_invoice.id= res_wh_invoice_detail.invoice_id
				WHERE
					'.$old_cond.'
				ORDER BY
					res_wh_invoice_detail.product_id	
			';
			...
			$cond = '
				'.(Url::get('date_from')?' AND res_wh_invoice.create_date >=\''.Date_Time::to_orc_date(Url::get('date_from')).'\'':'').'
				'.(Url::get('date_to')?' AND res_wh_invoice.create_date <=\''.Date_Time::to_orc_date(Url::get('date_to')).'\'':'').'
				
			';
			$sql = '
				SELECT
					res_wh_invoice.*,
					DECODE(res_wh_invoice.type,\'IMPORT\',res_wh_invoice.bill_number,\'\') AS import_invoice_code,
					DECODE(res_wh_invoice.type,\'EXPORT\',res_wh_invoice.bill_number,\'\') AS export_invoice_code,
					DECODE(res_wh_invoice.type,\'IMPORT\',res_wh_invoice_detail.num,0) AS import_number,
					DECODE(res_wh_invoice.type,\'EXPORT\',res_wh_invoice_detail.num,0) AS export_number
				FROM
					res_wh_invoice
					INNER JOIN res_wh_invoice_detail ON res_wh_invoice_detail.invoice_id = res_wh_invoice.id
				WHERE
					res_wh_invoice_detail.product_id = \''.$row['id'].'\' '.$cond.'
				ORDER BY
					res_wh_invoice.create_date,res_wh_invoice.id
			';
			$items = DB::fetch_all($sql);
	...
	function get_store_remain_products(){
		...
		$sql = '
			SELECT
				res_wh_invoice_detail.id,res_wh_invoice_detail.product_id,wh_product.name_'.Portal::language().' as name,unit.name_'.Portal::language().' as unit,res_wh_invoice_detail.num,res_wh_invoice.type,wh_product.start_term_quantity
			FROM
				res_wh_invoice_detail
				INNER JOIN wh_product ON wh_product.id = res_wh_invoice_detail.product_id
				INNER JOIN unit ON unit.id = wh_product.unit_id
				INNER JOIN res_wh_invoice ON res_wh_invoice.id= res_wh_invoice_detail.invoice_id
				LEFT OUTER JOIN warehouse ON warehouse.id = res_wh_invoice.warehouse_id
			WHERE
				'.$cond.'
			ORDER BY
				res_wh_invoice_detail.product_id	
	...
	function get_warehouse_export(){
		$old_cond = ' res_wh_invoice.type = \'EXPORT\' and res_wh_invoice.warehouse_id = '.Url::get('warehouse_id').'
				'.(Url::get('date_from')?' AND res_wh_invoice.create_date <\''.Date_Time::to_orc_date(Url::get('date_from')).'\'':'').'
		';		
		$sql = '
			SELECT
		...
		';
		$products = DB::fetch_all($sql);
		$cond = ' res_wh_invoice.type = \'EXPORT\' and res_wh_invoice.warehouse_id = '.Url::get('warehouse_id').'
			'.(Url::get('date_from')?' AND res_wh_invoice.create_date >=\''.Date_Time::to_orc_date(Url::get('date_from')).'\'':'').'
			'.(Url::get('date_to')?' AND res_wh_invoice.create_date <=\''.Date_Time::to_orc_date(Url::get('date_to')).'\'':'').'

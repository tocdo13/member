<?php 
class BarReservationNew extends Module{
	function BarReservationNew($row){
		Module::Module($row);
		require_once 'db.php';	
		if(User::can_view(false,ANY_CATEGORY)){
			if(Url::check(array('act'=>'cancel','id')) and User::can_delete(false,ANY_CATEGORY)){
				...
				Url::redirect('bar_reservation',array('cmd','id'));
			}elseif(URL::check(array('cmd'=>'recover','id')) and $row = DB::exists_id('bar_reservation',Url::iget('id')) and User::can_delete(false,ANY_CATEGORY)){
				DB::update_id('bar_reservation', array('status'=>'RESERVATION','cancel_time'=>0),Url::iget('id'));
				...
				((
					Url::check('id') and $reservation=DB::exists('select id from bar_reservation where id='.Url::iget('id').'') and
			...
			)
			{
				...
				case 'delete':
					require_once 'forms/delete.php';
					$this->add_form(new DeleteBarReservationNewForm());break;
				case 'edit':
					require_once 'forms/edit.php';
					$this->add_form(new EditBarReservationNewForm());break;
				case 'check_in':
				...
				case 'add':
					...
					$this->add_form(new AddBarReservationNewForm());break;
				case 'list_debt':
					require_once 'forms/list.php';
					$this->add_form(new ListBarReservationNewForm());break;
				case 'list_cancel':
				...
				default: 
					if(URL::check('id') and DB::exists_id('bar_reservation',Url::iget('id')))
					{
						require_once 'forms/detail.php';
						$this->add_form(new BarReservationNewForm());
					}
					else
					{
						require_once 'forms/list.php';
	...
	function cancel_reservation($id){
		$log_description = '<div>';
		$row = DB::select('bar_reservation','id = '.$id.'');
		...
		$log_description .= $row['departure_time']?'<div>Departure: '.date('H:i\' d/m/Y',$row['departure_time']).'</div>':'';
		$log_description .= '</div>';
		System::log('cancel','Cancel bar reservation with id: '.$id.'',$log_description,$id);
		DB::update_id('bar_reservation',array('status'=>'CANCEL','cancel_time'=>time()),$id);
	}
	function check_out()
	{

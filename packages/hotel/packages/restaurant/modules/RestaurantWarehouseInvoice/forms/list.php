<?php
class ListRestaurantWarehouseInvoiceForm extends Form
{
	function ListRestaurantWarehouseInvoiceForm()
	{
		Form::Form('ListRestaurantWarehouseInvoiceForm');
	...
	function draw()
	{
		...
		$item_per_page = 50;
		$cond = '1=1
			'.(Url::get('type')?' AND res_wh_invoice.type = \''.Url::sget('type').'\'':'').'
			'.(Url::get('note')?' AND UPPER(res_wh_invoice.note) LIKE \'%'.strtoupper(Url::sget('note')).'%\'':'').'
			'.(Url::get('receiver_name')?' AND res_wh_invoice.receiver_name LIKE \'%'.Url::sget('receiver_name').'%\'':'').'
			'.(Url::get('create_date')?' AND res_wh_invoice.create_date = \''.Date_Time::to_orc_date(Url::sget('create_date')).'\'':'').'
			'.(Url::get('warehouse_id')?' AND res_wh_invoice.warehouse_id = '.Url::iget('warehouse_id').'':'').'
			'.(Url::get('supplier_id')?' AND res_wh_invoice.supplier_id = '.Url::iget('supplier_id').'':'').'
			';
		...
		$sql = '
			...
			(
				...
				WHERE
					'.$cond.'
				ORDER BY
					'.(Url::get('order_by')?Url::get('order_by'):'res_wh_invoice.id').' '.(Url::get('dir')?Url::get('dir'):'ASC').' 
			)
			WHERE

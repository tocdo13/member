...
//			}
			...
			foreach($products as $key=>$value)
			{
				...
				else
				{
					if(($value['quantity'] - $value['printed'])>0 ){
                        ...
                        $product_tmp[$key]['chair_number'] = $value['chair_number'];
                        //system::debug(substr($key,2,strlen($key)-2));exit();
						DB::query('Update bar_reservation_product set bar_reservation_product.printed = '.($value['quantity']).',bar_reservation_product.note=\'\' where id='.$key.'');	
					}
				}
			}
			foreach($product_remain as $k=>$v)
			{
				if($v['remain'] - $v['printed_cancel']>0)
				{
					...
					DB::query('Update bar_reservation_product set bar_reservation_product.printed_cancel = '.$v['remain'].',bar_reservation_product.printed=bar_reservation_product.printed-'.$product_tmp[$k]['remain'].' where id='.$k.'');
				}
			}
			...
			if($products and (($row['print_kitchen_name'] and $act=='kitchen') or ($row['print_bar_name'] and $act=='bar')))
			{
                ...
                $printer->write_r($products,$row['id'],$table_name,$row['bar_name'],$row['bar_area_name']);

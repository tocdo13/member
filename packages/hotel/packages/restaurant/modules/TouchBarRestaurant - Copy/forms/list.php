			...
			if($this->check())
            {
                ...
                else
                {
                    ...
                    if(Url::get('id'))
                    {
                        ...
                        //System::debug($amount_part_payment); exit();
                        ...
                        if($rr_id!=$old_rr_id['reservation_room_id'])
                        {
						      ...
						      $bar_set_menu_id = $arr_temp[0];
                               $sql = "SELECT 
                                ...
                                {                                     
                                    ...
                                    $arr_product[$prd_set_k]['quantity'] = $set_menu_quantity_list[$prd_set_val['price_id']][$prd_set_val['id']]['quantity'];
                    ...
                    */
                    //System::debug($bar_reservation_products);
                    //System::debug($bar_set_menu); exit();
					...
					$array = array();
					if(Url::get('cancel') && Url::get('act')=='cancel')
                    {
						$status = 'CANCEL';	
					}
		      ...
		      DB::update('reservation',array('last_time'=>time()),'id='.$reservation['reservation_id']);
					...
					$array += array(	
                            ...
                            /** Daund: thêm nút tích giảm giá trước và sau thuế phí */
                            'discount_after_tax' => isset($_REQUEST['discount_after_tax'])?Url::get('discount_after_tax'):0,	
      	             );	
                    ...
                    if($row = $this->check_edit())
                    {
						...
						if((Url::get('act')!='') || (Url::get('acction')==1) || Url::get('cancel'))
                        {
							...
							foreach($bar_reservation_products as $id =>$value)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(isset($old_product_ids[$id]['id']) && !empty($prd_id))
                                    {	
                               ...
                               foreach($bar_set_menu as $id =>$value)
                                {
                                    ...
                                    {
                                        if(isset($old_set_menu_product_ids[$id]['id']) && !empty($prd_id))
                                        {	
                        ...
                        {// Xuất sản phẩm
                            ...
                            $warehouse = DB::fetch('select warehouse_id, warehouse_id_2 from portal_department where portal_id = \''.PORTAL_ID.'\' and department_code = \''.$bar['department_id'].'\' '); 
                        ...
                        /** START: Daund them khi checkout co event huy mon van in order */
                        if(Url::get('act')=='checkout')
                        {
                            TouchBarRestaurant::print_to_kitchen('kitchen');
				            TouchBarRestaurant::print_to_kitchen('bar');
                    ...
                    else
                    {
                        ...
                        { // Check conflix
                            ...
                            {
								$this->error('table_id',Portal::language('table_code').' '.Url::get('table_id').' '.Portal::language('conflict_of_time_to_reservation').' <a target="blank" href="?page=touch_bar_restaurant&cmd=edit&id='.$conflix[Url::get('table_id')].'#'.$conflix[Url::get('table_id')].'">#'.$conflix[Url::get('table_id')].'</a>',false);
								return;
							}
                            else
                            {
                ...
                if(Url::get('act') =='checkin' || (Url::get('status') =='CHECKIN' && Url::get('act')!='payment')){

<?php
    class GolfPriceManagerEditForm extends Form
{
	function GolfPriceManagerEditForm()
    {
    ...
    function on_submit(){
                                ...
                                if(isset($valueHole['group_traveller']))
                                {
                                    ...
                                    foreach($valueHole['group_traveller'] as $keyGTraveller=>$valueGTraveller)
                                    {
                                        ...
                                        if(isset($valueGTraveller['id']))
                                        {
                                            ...
                                            if(DB::exists('select golf_price.id from golf_price where golf_hole_id='.$golf_hole_id.' and group_traveller_id='.$group_traveller_id.' and in_date=\''.Date_Time::to_orc_date($in_date).'\' and start_time<='.$end_time.' and end_time>='.$start_time.' and portal_id=\''.PORTAL_ID.'\''))
                                            {
                                                $golf_price_old = DB::fetch_all('
                                                                                select 
                                                                                    golf_price.* 
                                                                                from 
                                                                                    golf_price 
                                                                                ...
                                                                                ');
                                                foreach($golf_price_old as $keyOld=>$valueOld){
                                                    if($valueOld['start_time']<$start_time){
                                                        if($valueOld['end_time']<=$end_time){
                                                            DB::update('golf_price',array('end_time'=>$start_time-60),'id='.$valueOld['id']);
                                                        }else{
                                                            DB::update('golf_price',array('end_time'=>$start_time-60),'id='.$valueOld['id']);
                                                            DB::insert('golf_price',array(
                                                                                'golf_hole_id'=>$golf_hole_id,
                                                    ...
                                                    elseif($valueOld['start_time']>=$start_time){
                                                        if($valueOld['end_time']<=$end_time){
                                                            DB::delete('golf_price','id='.$valueOld['id']);
                                                        }else{
                                                            DB::update('golf_price',array(
                                                                                'golf_hole_id'=>$golf_hole_id,
                                                                                'group_traveller_id'=>$group_traveller_id,
                                                                                'in_date'=>Date_Time::to_orc_date($in_date),
                                                                                'start_time'=>$end_time+60,
        ...
        Url::redirect('golf_price_manager');
    }
    function draw(){
        $this->map = array();

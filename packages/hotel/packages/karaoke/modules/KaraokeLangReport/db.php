...
{
    //ham tra ve doanh thu cua mot karaoke_reservation_id
    ...
    {
        //lay ra cac san pham trong mot karaoke reservation
    ...
    static function get_restaurant_revenue($cond,$for_portal = false,$portal_id = false)
    {
        ...
        $sql = 'select 
                ...
                from
                    karaoke_reservation br
                    inner join karaoke on br.karaoke_id = karaoke.id
                where 
                    br.status = \'CHECKOUT\'
                    and '.$cond.'
                ';
        ...
        if(!$for_portal)
        {
            foreach($items as $key => $value)
            {
                ...
                if(isset($return_items[$value['karaoke_name']]))
                {
                    //tinh tong cho tung karaoke
                    $return_items[$value['karaoke_name']]['real_price'] += $revenue['is_processed']['real_price'] + $revenue['product']['real_price'] + $revenue['service']['real_price'];
                    $return_items[$value['karaoke_name']]['original_price'] += $revenue['is_processed']['original_price'] + $revenue['product']['original_price'] + $revenue['service']['original_price'];
                    $return_items[$value['karaoke_name']]['is_processed']['original_price'] += $revenue['is_processed']['original_price'];
                    $return_items[$value['karaoke_name']]['is_processed']['real_price'] += $revenue['is_processed']['real_price'];
                    $return_items[$value['karaoke_name']]['product']['original_price'] += $revenue['product']['original_price'];
                    $return_items[$value['karaoke_name']]['product']['real_price'] += $revenue['product']['real_price'];
                    $return_items[$value['karaoke_name']]['service']['original_price'] += $revenue['service']['original_price'];
                    $return_items[$value['karaoke_name']]['service']['real_price'] += $revenue['service']['real_price'];
                }
                else
                {
                    $return_items[$value['karaoke_name']]['real_price'] = $revenue['is_processed']['real_price'] + $revenue['product']['real_price'] + $revenue['service']['real_price'];
    ...
    static function get_reservation_product($id)
	{
		 $sql = '
			select 
                ...
                karaoke_reservation_product.is_processed,
                karaoke_reservation_product.original_price,			
				product.category_id,
			...
			where 1>0 and karaoke_reservation_product.karaoke_reservation_id=\''.$id.'\'
			order by
				karaoke_reservation_product.id
		';
        //System::debug($sql);

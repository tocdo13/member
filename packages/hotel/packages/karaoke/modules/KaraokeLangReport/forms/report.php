...
class RestaurantLangReportForm extends Form
{
    ...
    function draw()
    {
        ...
        $karaoke_revenue = RestaurantLangReportDB::get_restaurant_revenue($cond,$for_portal = false, $portal_id);
        //lay so lieu ki truowc
        $decrese_karaoke_revenue = RestaurantLangReportDB::get_restaurant_revenue($decrese_cond,$for_portal = false, $portal_id);
        ...
        //System::debug($karaoke_revenue);
        //khop noi cac so lieu
        foreach($karaoke_revenue as $key => $value)
        {
            if(isset($decrese_karaoke_revenue[$key]))
            {
                $karaoke_revenue[$key]['decrese_real_price'] = $decrese_karaoke_revenue[$key]['real_price'];
                $karaoke_revenue[$key]['decrese_original_price'] = $decrese_karaoke_revenue[$key]['original_price'];
                $karaoke_revenue[$key]['is_processed']['decrese_real_price'] = $decrese_karaoke_revenue[$key]['is_processed']['real_price'];
                $karaoke_revenue[$key]['product']['decrese_real_price'] = $decrese_karaoke_revenue[$key]['product']['real_price'];
                $karaoke_revenue[$key]['service']['decrese_real_price'] = $decrese_karaoke_revenue[$key]['service']['real_price'];
            }
            else
            {
                $karaoke_revenue[$key]['decrese_real_price'] = 0;
                $karaoke_revenue[$key]['decrese_original_price'] = 0;
                $karaoke_revenue[$key]['is_processed']['decrese_real_price'] = 0;
                $karaoke_revenue[$key]['product']['decrese_real_price'] = 0;
                $karaoke_revenue[$key]['service']['decrese_real_price'] = 0;
            }
        }
        ...
        //System::debug($resort_karaoke_revenue);
        // parse array
        ...
        $this->parse_layout('report',array(
                                            'karaoke_revenue'=>$karaoke_revenue,
                                            'summary'=>$summanry,

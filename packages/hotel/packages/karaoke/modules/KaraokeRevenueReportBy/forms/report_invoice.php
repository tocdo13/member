...
***/
class KaraokeRevenueReportByInvoiceForm extends Form
{
	function KaraokeRevenueReportByInvoiceForm()
	{
		Form::Form('KaraokeRevenueReportByInvoiceForm');
	...
	function draw()
	{
        ...
        if(Url::get('do_search'))
        {
            if($this->check())
            {
                ...
                {
                    //check date
                    if(!isset($items[$value['karaoke_reservation_id']]))
                    {
                        $items[$value['karaoke_reservation_id']] = array( 'karaoke_reservation_id'=>$value['karaoke_reservation_id'],
                                                        'code'=>$value['code'],
                                                        'agent_name'=>$value['agent_name'],
                                                        'day'=>$value['day'],
                                                        'karaoke_id'=>$value['karaoke_id'],
                                                        'stt'=>$stt++,
                    ...
                    foreach($categories_invi as $k=>$v)
                    {
                        ...
                        {
                            $items[$value['karaoke_reservation_id']]['categories_invi'][$k]['total_amount'] += $value['total_amount'];
                            $items[$value['karaoke_reservation_id']]['total_p'] += $value['total_amount'];
                        }
                    }
                    foreach($categories_hidd as $k=>$v)
                    {
                        if(IDStructure::is_child($value['structure_id'],$v['structure_id']) or $value['structure_id'] == $v['structure_id'])
                        {
                            $items[$value['karaoke_reservation_id']]['total_amount_nonfood_other'] += $value['total_amount'];
                            $items[$value['karaoke_reservation_id']]['total_p'] += $value['total_amount'];
                        }
                    }
                }
                ...
                {
                    if(!isset($arr_karaoke_resers[$value['karaoke_reservation_id']]))
                    {
                        $arr_karaoke_resers[$value['karaoke_reservation_id']] = array('deposit'=>0,
                                                                        'cash'=>0,
                                                                        'credit'=>0,
                                                                        'with_room'=>0,
                                                                        'free'=>0,
                    ...
                    $arr_karaoke_resers[$value['karaoke_reservation_id']]['karaoke_reservation_id'] = $value['karaoke_reservation_id'];
                    $arr_karaoke_resers[$value['karaoke_reservation_id']]['pay_with_room'] = $value['pay_with_room'];
                    $arr_karaoke_resers[$value['karaoke_reservation_id']]['total'] = $value['total'];
                    if($value['type_dps'])
                        $arr_karaoke_resers[$value['karaoke_reservation_id']]['deposit'] += $value['amount'];
                    else
                    {
                        switch($value['payment_type_id'])
                        {
                            case 'CASH' : $arr_karaoke_resers[$value['karaoke_reservation_id']]['cash'] += $value['amount']; break;
                            case 'CREDIT_CARD' : $arr_karaoke_resers[$value['karaoke_reservation_id']]['credit'] += $value['amount']; break;
                            case 'DEBIT' : $arr_karaoke_resers[$value['karaoke_reservation_id']]['debit'] += $value['amount']; break;
                            case 'FOC' : $arr_karaoke_resers[$value['karaoke_reservation_id']]['free'] += $value['amount']; break;
                            case 'BANK' : $arr_karaoke_resers[$value['karaoke_reservation_id']]['credit'] += $value['amount']; break;
                            default : $arr_karaoke_resers[$value['karaoke_reservation_id']]['cash'] += $value['amount']; break;
                        }
                    }
                }
                ...
                foreach($arr_karaoke_resers as $key => $value)
                {
                    ...
                    $items[$value['karaoke_reservation_id']]['deposit'] += $arr_karaoke_resers[$key]['deposit'];
                    $items[$value['karaoke_reservation_id']]['cash'] += $arr_karaoke_resers[$key]['cash'];
                    $items[$value['karaoke_reservation_id']]['credit'] += $arr_karaoke_resers[$key]['credit'];
                    $items[$value['karaoke_reservation_id']]['with_room'] += $arr_karaoke_resers[$key]['with_room'];
                    $items[$value['karaoke_reservation_id']]['free'] += $arr_karaoke_resers[$key]['free'];
                    $items[$value['karaoke_reservation_id']]['debit'] += $arr_karaoke_resers[$key]['debit'];
                    $items[$value['karaoke_reservation_id']]['total'] += $arr_karaoke_resers[$key]['total'];
                }
                //System::debug($items);exit();
	...
	function print_page($items, &$report, $page_no,$total_page)
	{
		...
		$this->parse_layout('report_invoice',array(
				'items'=>$items,
                'categories_invi'=>$report->categories_invi,
                'categories_hidd'=>$report->categories_hidd,

<?php
class CheckInKaraokeForm extends Form
{
	function CheckInKaraokeForm()
	{
		Form::Form('CheckInKaraokeForm');
		$this->add('table.table_id',new IDType(true,'miss_table','karaoke_table'));
		if($br = DB::fetch('select status from karaoke_reservation where id=\''.Url::iget('id').'\' and status in (\'CHECKIN\',\'CHECKOUT\')'))
		{
	...
	function on_submit()
	{
		if($this->check())
		{
			...
			}
			{
				...
				$karaoke_reser = DB::select('karaoke_reservation',Url::get('id'));
				$data = array();
				$check_out = false;
				if(Url::get('check_out'))
				{
					...
					if($row = DB::select('karaoke_reservation','id = '.Url::iget('id').'')){
				...
				DB::update('karaoke_reservation', $data+array(
					'note',
				...
				{
					DB::delete('pay_by_currency','bill_id=\''.Url::get('id').'\' and type=\'karaoke\'');
				}
				//xoa cac ban ghi hien co trong bang karaoke_reservation_table va karaoke_reservation_product
				//them vao bang karaoke_reservation_table
				$table_old_items = array();
				if(URL::get('table_deleted_ids'))
				{
					if(strpos(URL::get('table_deleted_ids'),','))
					{
						$table_old_items = DB::select_all('karaoke_reservation_table','id in (\''.str_replace(',','\',\'',URL::get('table_deleted_ids')).'\')');
					}
					else
					{
						$table_old_item = DB::select('karaoke_reservation_table','id=\''.URL::get('table_deleted_ids').'\'');
				...
				if(isset($_REQUEST['mi_table']))
				{
					...
					{
						$list_tables .= 'Table: '.DB::fetch('select name from karaoke_table where id = '.$row_data['table_id'].'','name').'<br>';
						$item['karaoke_reservation_id'] = Url::get('id');
						$item['table_id'] = $row_data['table_id'];
						$item['num_people'] = $row_data['num_people'];
						$item['order_person'] = $row_data['order_person'];
						if($row_data['id'] and DB::select('karaoke_reservation_table','id='.$row_data['id']))
						{
							DB::update('karaoke_reservation_table',$item,'id='.$row_data['id']);
						}
						else
						{
							DB::insert('karaoke_reservation_table',$item);
						}
					}
				}
				//Xoa danh sach ban
				if($table_old_items)
				{
					...
					{
						DB::delete('karaoke_reservation_table','id='.$table_item['id']);	
					}
				}
				//--het xoa ban
				...
				if(URL::get('product_deleted_ids'))
				{
					...
					{
						$product_old_items = DB::select_all('karaoke_reservation_product','id in (\''.str_replace(',','\',\'',URL::get('product_deleted_ids')).'\')');
					}
					else
					{
						$product_old_item = DB::select('karaoke_reservation_product','id=\''.URL::get('product_deleted_ids').'\'');
				...
				if(isset($_REQUEST['mi_product']))
				{
					foreach($_REQUEST['mi_product'] as $row=>$row_data)
					{
						...
						$product['karaoke_reservation_id'] = Url::get('id');
						...
						if($row_data['id'] and DB::select('karaoke_reservation_product','id='.$row_data['id']))
						{
							DB::update('karaoke_reservation_product',$product,'id='.$row_data['id']);
						}
						else
						{
							unset($row_data['id']);	
							$id = DB::insert('karaoke_reservation_product',$product);
						}
					}
				}
				$description  = '';
				...
				if($product_old_items)
				{
					...
					{
						DB::delete('karaoke_reservation_product','id='.$product_item['id']);
						$description .= '<br>**************<br>Delete karaoke reservation product: '.$product_item['id'].', Quantity: '.$product_item['quantity'].', Price: '.$product_item['price'].'<br>**************<br>';
					}
				}
				//--- Het xoa sp ---
				$currency = DB::select_all('currency');
				$old_item = DB::select_all('pay_by_currency','bill_id='.Url::get('id').' and type=\'karaoke\'');
				if(Url::get('payment_result') == 'CASH' and isset($_REQUEST['currency_selecteds']))
				{
					...
					foreach($_REQUEST['currency_selecteds'] as $record)
					{
						...
						$karaoke_row = DB::select('pay_by_currency','bill_id='.$data['bill_id'].' and type=\'karaoke\' and currency_id=\''.$record.'\'');
						if($record and isset($old_item[$record]) and $karaoke_row)
						{
							DB::update('pay_by_currency',$data,'id='.$karaoke_row['id']);
				...
				$title = 'Edit karaoke reservation , Code: '.Url::get('id').', Status: ' .Url::get('status').'';
				if(isset($row))
				{
				$description .= ''
				...
				if($error==false){
					...
					'.($check_out?'window.location="'.Url::build('table_map').'";':'window.setTimeout("location=\''.Url::build('karaoke_reservation',array('cmd','id'=>Url::get('id'))).'\'",100);').'
					</script>';
					exit();
				}
			}
	...
	function draw()
	{
		...
		$row = DB::select('karaoke_reservation','id='.Url::iget('id').' and portal_id=\''.PORTAL_ID.'\'');
		...
		//$db_items = DB::select_all('karaoke_table');
		...
		$tables = DB::select_all('karaoke_table','portal_id=\''.PORTAL_ID.'\'','id');
		$i=0;
		...
		$db_items = DB::select_all('karaoke_table','portal_id=\''.PORTAL_ID.'\'','name');
		$table_id_options = '';
		if($table_items)
		{
		...
		//============================== product ===============================
		$product = KaraokeReservationNewDB::get_products(' and res_product.status = \'avaiable\' and portal_id=\''.PORTAL_ID.'\'');
		$i=0;
		...
		$category_items = KaraokeReservationNewDB::get_reservation_category();
		$total_before_tax = 0;
		...
		$karaoke_fee = $total_before_tax*$row['karaoke_fee_rate']/100;
		$row['karaoke_fee'] = System::display_number_report($karaoke_fee);
		
		$total_before = $total_before_tax+round($total_before_tax*$row['karaoke_fee_rate']/100,2);
		$row['total_before_tax'] = System::display_number_report($total_before_tax+$karaoke_fee);
		$tax = ($total_before_tax + $karaoke_fee)*$row['tax_rate']/100;
		...
		$karaoke = Hotel::get_new_karaoke($row['karaoke_id']);

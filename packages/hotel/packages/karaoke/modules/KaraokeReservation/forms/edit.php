<?php
class EditKaraokeReservationNewForm extends Form
{
	function EditKaraokeReservationNewForm()
	{
		Form::Form('EditKaraokeReservationNewForm');
		if(!Url::check('id') or (Url::get('id') and !DB::exists('select id from karaoke_reservation where id='.Url::iget('id').'')))
		{
			Url::redirect_current();
		}
		else
		{
			if($br = DB::fetch('select id,status from karaoke_reservation where id=\''.Url::get('id').'\' and (status=\'CHECKIN\' OR status=\'CHECKOUT\')'))
			{
			...
			else
			{
				...
				$this->add('karaoke_reservation_product.product_id',new TextType(true,'product_id_is_required',0,255));
				...
				$this->add('karaoke_reservation_table.table_id',new IDType(true,'miss_table','karaoke_table')); 
				//$this->add('room_id',new IDType(false,'invalid_room_id','room'));
				$this->add('karaoke_reservation_product.product_id',new IDType(true,'miss_product_id','res_product')); 
	...
	function on_submit()
	{
		if(URL::check(array('select_karaoke'=>0)))
		{
			...
			if($this->check())
			{
				...
				DB::update('karaoke_reservation',$array + array(
					...
					'prepaid'=>$deposit,
					'tax_rate', 
					'karaoke_fee_rate'=>Url::get('service_rate'),
					'total'=>$total,
				...
				DB::delete('karaoke_reservation_table','karaoke_reservation_id=\''.Url::get('id').'\'');
				DB::delete('karaoke_reservation_product','karaoke_reservation_id=\''.Url::get('id').'\'');
	...
	{	
		...
		$row['service_rate'] = $row['karaoke_fee_rate'];

...
class DetailKaraokeForm extends Form
{
	function DetailKaraokeForm()
	{
		...
		DB::query('select status from karaoke_reservation where id=\''.Url::get('id').'\' and status in (\'CHECKIN\',\'CHECKOUT\',\'RESERVATION\')');
		if(!DB::fetch())
		{
	...
	function draw()
	{
		...
		foreach($product_items as $key=>$value)
		{
			...
			if(Url::get('act')=='print_kitchen'){
				DB::update('karaoke_reservation_product',array('printed'=>1),'id = '.$key.'');
			}
		}
		...
		$row['karaoke_fee'] = (($total_price*$row['karaoke_fee_rate']/100));
		$row['total_before_tax'] = ($total_price+$total_price*$row['karaoke_fee_rate']/100);

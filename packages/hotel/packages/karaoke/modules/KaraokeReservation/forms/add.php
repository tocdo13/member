<?php
class AddKaraokeReservationNewForm extends Form
{
	function AddKaraokeReservationNewForm()
	{
		Form::Form('AddKaraokeReservationNewForm');
		...
		//$this->add('code',new UniqueType(true,'dupplicated_code','karaoke_reservation','code',false,PORTAL_ID));  
			...
			if(URL::check(array('select_karaoke'=>0)))
			{
				...
				{ 
					if(isset($_REQUEST['mi_table'])){
						...
						$id = DB::insert('karaoke_reservation', 
							$array + array(							
						...
						DB::update('karaoke_reservation',array('code'=>$id),'id='.$id);
						$list_tables = '';
						foreach($_REQUEST['mi_table'] as $row=>$row_data)
						{
							...
							$row_data['karaoke_reservation_id'] = $id;
							unset($row_data['code']);
							DB::insert('karaoke_reservation_table',$row_data);
						}
						$list_product = '';
						if(isset($_REQUEST['mi_product']))
						{
							...
							{
								$data['karaoke_reservation_id'] = $id;
								...
								DB::insert('karaoke_reservation_product',$data);
							}
						}
						$title = 'Add karaoke reservation , Code: '.Url::get('code').', Status: ' .$status.'';
						$description = ''
						...
						window.setTimeout("location=\''.((Url::get('check_in'))?Url::build('karaoke_reservation',array('cmd'=>'check_in','id'=>$id)):Url::build('table_map')).'\'",1000);
						</script>';
						exit();
					}else{
	...
	function draw()
	{
		...
		DB::query('select code as id from karaoke_reservation where karaoke_reservation.portal_id=\''.PORTAL_ID.'\' order by code desc');
		$res = DB::fetch();
		...
		if(Url::check('table_ids'))
		{
			...
			{
				$tbl=DB::select('karaoke_table',$table_ids[$c]);
		...
		if(Url::get('table_id') and $table = DB::select('karaoke_table','id='.Url::iget('table_id')) and !isset($_REQUEST['mi_table'])){
		...
		if(!isset($_REQUEST['tax_rate'])){
			$_REQUEST['tax_rate'] = KARAOKE_TAX_RATE;
		}
		if(!isset($_REQUEST['service_rate'])){
			$_REQUEST['service_rate'] = KARAOKE_SERVICE_CHARGE;
		}

...
function GetTotalPayment(){
		...
		if(full_rate){
			...
			//jQuery('#total_amount').val(number_format(Math.floor(totalAmount/param,2)));
            jQuery('#total_amount').val(number_format(totalAmount/param,2));	
		}else if(full_charge){
            ...
            jQuery('#total_payment').val(number_format(totalAmount + totalAmount*tax_rate*0.01,0));
			jQuery('#total_amount').val(number_format(totalAmount/param,0));			
		}else{
			...
			//jQuery('#total_payment').val(number_format(roundNumber(totalAmount + total_service + total_tax,0)));
            jQuery('#total_payment').val(number_format(totalAmount + total_service + total_tax,0));	
		}
       var total_sing = 0;
       for(var i in sing_room_js)
       {
            total_sing += to_numeric(jQuery("#total_sing_room_"+sing_room_js[i]['id']).val());
       }
	   jQuery("#remain").val(number_format(to_numeric(jQuery("#total_payment").val()) + to_numeric(total_sing) - deposit));
...
function getSelectQuantity(name){
	...
	if((can_action == 1 || name=='discount') && ((status=='CHECKOUT' && can_admin_karaoke) || status !='CHECKOUT')){
		var obj = name;
		var textkeyboard= '<div id="select_number" class="select_number" onclick="event.stopPropagation();"><span id="title_quantity" style="font-size:14px;font-weight:bold;"></span>';
...
function getSelectOutSide(name){
	...
	if((status=='CHECKOUT' && can_admin) || status !='CHECKOUT'){
		var obj = name;
		var textkeyboard= '<div id="select_outside" class="select_outside" onclick="event.stopPropagation();"><span id="title_quantity" style="font-size:14px;font-weight:bold;">SELECT EXTRA MENU</span>';
		...
		textkeyboard += 'QUANTITY: <input name="quantity_outside" type="text" id="quantity_outside" style="width:200px; height:25px; margin-bottom:10px; text-align:right;"><img src="packages/hotel/skins/default/images/iosstyle/delete-smaller.png" onclick="jQuery(\'#quantity_outside\').val(\'\');" style="margin-left:5px;"/>';
		textkeyboard += '<br>';
...
function get_keyboard(obj,key){
		...
		textkeyboard += '<li class="space lastitem">&nbsp;</li>';

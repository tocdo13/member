	...
	<table cellpadding="2" width="100%" border="1" bordercolor="#A6A6A6" id="table_bound">
		<tr>
                    ...
                    <div class="select-menu-bound">
                        ...
                        <table id="search_div" width="250px;">
                        	<tr>
                                ...
                                <td width="30%"><img src="packages/hotel/skins/default/images/iosstyle/tia.png" class="keyboard" onclick="get_keyboard('input_product_name','');"/><img src="packages/hotel/skins/default/images/iosstyle/delete-smaller.png" onclick="jQuery('#input_product_name').val('');searchProduct('product',jQuery('#karaoke_id_other').val());return false;" style="margin-left:5px;"/> 
                                </td>
                            </tr>
                        </table>
            ...
            <td class="info-right" id="expan_display">
                    ...
                    <!--IF:cond_bill(isset([[=code=]]) and [[=code=]])--><b>[[.Bill_ID.]]: [[|code|]]</b><!--/IF:cond_bill-->
                      ...
                      <input  name="status" type="text" id="status" value="[[|status|]]" readonly="readonly" style="border:none; font-weight:bold;background:#F7F6F6; width:80px; " />
                		...
                		<div style="float:right;">
                        	<input type="button" name="preview" value=" [[.view_order.]]" tabindex="-1" class="expan" onclick="if(jQuery('#status').val()!=''){window.open('<?php echo Url::build('karaoke_touch',array('cmd'=>'detail',md5('act')=>md5('print'),md5('preview')=>(([[=status=]]=='CHECKOUT')?0:1),'id'=>Url::get('id'),'karaoke_id'))?>');}else{jQuery('#act').val('submit_invoice');KaraokeTouchForm.submit();}">                           
              ...
              <div id="menu_extra">
					<ul id="menu_extra_ul" style="float:left;width:25%;">
                        ...
                        <li class="menu_extra_li_order" id="li_deposit" onclick="openWindowUrl('form.php?block_id=<?php echo BLOCK_PAYMENT;?>&cmd=deposit&id=<?php if(Url::get('id')){echo Url::get('id');}else{echo '';}?>&type=karaoke',Array('payment','payment_for',80,210,950,500));"><img src="packages/hotel/skins/default/images/iosstyle/7.png" /></li>
				...
				<div id="foot_drink_bound" style="width:74%; float:right;" class="selected-foot-and-drink-table">
      ...
      <div id="info_summary"> [[|categories|]]  </div> 
            	...
            	<table class="general-info" align="center" border="1" cellpadding="3" cellspacing="3" width="100%" >
                   ...
                   <tr>
                        ...
                        <td><input name="service_charge" type="text" id="service_charge" value="[[|service_charge|]]" class="input_number" readonly="readonly"  maxlength="3" style="width:30px !important;text-align:right;margin-right:15px;" onkeyup="if(to_numeric(this.value)<=100){GetTotalPayment();}else{this.value=((this.value).substr(0,((this.value).length-1)));}" /><input name="tax_rate" type="text" id="tax_rate" value="[[|tax_rate|]]" class="input_number"  maxlength="3" readonly="readonly" style="width:30px !important;text-align:right;" onkeyup="this.value=number_format(this.value);GetTotalPayment();" onblur="GetTotalPayment();" /></td>
...
<script>
	...
	var can_admin = '<?php echo User::can_admin(false,ANY_CATEGORY)?>';
	var count = 0;
	units=[[|units|]];
	...
	jQuery(document).ready(function(){
                jQuery('.jcarousel-next-vertical').click(function(){
	       
                jQuery('.jcarousel-prev-vertical').addClass('normal');
	          });
                jQuery('.jcarousel-prev-vertical').click(function(){
	       
                jQuery('.jcarousel-prev-vertical').addClass('ucche');
	        });
		...
		for(var j in product_array){
			if((product_array[j]['id'] == price_id) || (product_array[j]['product_id'] == price_id && ( price_id=='FOUTSIDE' || price_id=='DOUTSIDE' || price_id=='SOUTSIDE'))){
				...
				if(price_id=='FOUTSIDE'){
					var name = ' �? an ngo�i th?c don ';	
				}
				if(price_id=='DOUTSIDE'){
					var name = ' �? u?ng ngo�i th?c don ';	
				}
				if(price_id=='SOUTSIDE'){
					var name = ' D?ch v? ngo�i th?c don ';	
				}
				alert('B?n chua c?p nh?t gi� cho s?n ph?m '+name);	
				return false;
			}*/
		}
	}
	...
	function drawTableList(arr){ 
		...
		items += '<td width=""><input name="split_table" type="button" value="[[.split_table_button.]]" id="split_table" style="height:40px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onClick="window.open(\'?page=karaoke_split_table&from_code=<?php echo Url::get('id');?>&type=split\')"<?php } ?>></td>';
		items += '<td width=""><input name="coupling_table" type="button" value="[[.coupling_table.]]" id="coupling_table" style="height:40px; width:73px;" <?php if(Url::get('cmd')=='edit' and Url::get('id')){?>onClick="window.open(\'?page=karaoke_split_table&from_code=<?php echo Url::get('id');?>\')"<?php } ?>></td>';
		items += '<td width="1%"><input name="select_table" type="button" id="select_table" value="[[.change_table.]]" style="height:40px; width:73px;" onclick="jQuery(\'#table_map\').css(\'display\',\'block\');jQuery(\'#table_change\').val(\''+arr['id']+'\');" /></td>';
	...
	function drawItems(arr){
		...
		items += '<tr class="foot-drink-tr" id="item_detail_'+arr['id']+'" onclick="if((jQuery(\'#status\').val()==\'CHECKOUT\' && can_admin) || jQuery(\'#status\').val()!=\'CHECKOUT\'){if(jQuery(\'#selected_'+arr['id']+'\').attr(\'checked\')==true){jQuery(\'#selected_'+arr['id']+'\').attr(\'checked\',false);}else{jQuery(\'#selected_'+arr['id']+'\').attr(\'checked\',true); }CssSelected();}">';

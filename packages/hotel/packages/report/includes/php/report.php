...
class Report
{
	...
	function display_date_params()
	{
		...
		if(URL::get('day')||URL::get('from_day'))
		{
			if($end_day!=$day)
			{
				echo Portal::language('from_day').' '.$day.'/'.$month.'/'.$year.' '.Portal::language('to').' '.$end_day.'/'.$end_month.'/'.$end_year;
			}
			else
			{
				echo Portal::language('day').' '.$day.'/'.$month.'/'.$year;
			}
		}
		else
		if(URL::get('day')||URL::get('from_day'))
		...
		if(URL::get('month')||URL::get('from_month'))
		{
			if($end_month!=$month)
			{
				if($end_year!=$year)
				{
					echo Portal::language('from_month').' '.$month.'/'.$year.' '.Portal::language('to').' '.$end_month.'/'.$end_year;
				}
				else
				{
					switch(URL::get('month'))
					{
					case '1-3':echo ''.Portal::language('quarter').' I '.Portal::language('year').' '.$year;break;
					case '4-6':echo ''.Portal::language('quarter').' II '.Portal::language('year').' '.$year;break;
					case '7-9':echo ''.Portal::language('quarter').' III '.Portal::language('year').' '.$year;break;
					case '10-12':echo ''.Portal::language('quarter').' IV '.Portal::language('year').' '.$year;break;
					default:echo Portal::language('from_month').' '.$month.' '.Portal::language('to_month').' '.$end_month.' '.Portal::language('year').' '.$year;
					}
				}
			}
			else
			{
				echo Portal::language('month').' '.$month.' '.Portal::language('year').' '.$year;
			}
		}
		else
		if(URL::get('year')||URL::get('from_year'))
		{
			if($end_year!=$year)
			{
				echo Portal::language('from_year').' '.$year.' '.Portal::language('to').' '.$end_year;
			}
			else
			{
				echo Portal::language('year').' '.$year;
			}
		}

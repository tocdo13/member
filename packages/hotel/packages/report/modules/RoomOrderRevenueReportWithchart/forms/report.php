...
//				WHERE rt.folio_id = 93 AND rt.add_payment!=1
//			';
//            $at_items = DB::fetch_all($sql);
//            System::debug($at_items);
//        }
		if(URL::get('do_search'))
		{
			...
			$folios = $this->get_foios($cond2.$cond3.$cond_page);// Lấy ra các hóa đơn đã tạo
						...
						if($folio['time']>=($i+$shift_from) && ($folio['time']<=($i+$shift_to)) || $check==true){
                            ...
                            if($count >((URL::get('start_page')-1)*$this->line_per_page) && $count<=(URL::get('no_of_page')*$this->line_per_page)){
                               ...
                               /* foreach($reservations as $r => $reser){
                                ...
                                foreach($payments as $k => $pay)
                                {
									if($pay['folio_id']==$folio['id'])
                                    {
                                        ...
                                        if($pay['currency_id'] == 'USD')
                                            $items[$key]['exchange_rate'] = $pay['exchange_rate'];
                                            //------------------------
                                ...
                                //System::debug($items[$key]['debit']);
                                //System::debug($total_paid);
                                //System::debug($folio['total']);
                                //echo $items[$key]['debit'].'aa'.$total_debit.'aa'.$folio['total'].'aa'.$total_paid;        
...
//        System::debug($items)	;		
	...
	function print_page($items, &$report, $page_no,$total_page)
	{
        ...
        {
            $total_cash += (isset($value['CASH_VND'])?$value['CASH_VND']:0) + (isset($value['CASH_USD'])?$value['CASH_USD']:0)*$value['exchange_rate'];
            $total_bank += (isset($value['BANK_VND'])?$value['BANK_VND']:0) + (isset($value['BANK_USD'])?$value['BANK_USD']:0)*$value['exchange_rate'];
        }
        
        $this->group_function_params['total_cash'] = $total_cash;
        $this->group_function_params['total_bank'] = $total_bank;
        //-----------------------
		$this->parse_layout('header',
			array(
				'page_no'=>$page_no,
...
$to_code = 0;$cond_code ='';
$check=false;
if(Url::get('from_code')>0 && Url::get('to_code')>0 && Url::get('to_code')>Url::get('from_code')){
	$from_code = Url::get('from_code');
	$to_code = Url::get('to_code');	
	$cond_code = ' AND folio.id>='.Url::get('from_code').' AND folio.id<='.Url::get('to_code').'';
	$check = true;
}

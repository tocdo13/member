...
EOH;
				...
				//ham in ra file excel// luu y voi mang $arr_key de theo kieu=(1=>'stt',2=>'sophong'.....)
				
				

  }
  function print_excel($repo,$arr_key = array(),$tieude=array(),$name_file_excel='myexcel')
	{  
	//echo "nguyen hung @ tcv .vn .com";
	 $excel=new ExcelWriter("packages/core/includes/utils/hung/".$name_file_excel."xls");
						
	 if($excel==false)       
						
	 echo $excel->error;
						
	 $excel->writeLine($tieude); //in ra tieu de
						
	 foreach($repo as $khoa=>$giatri)
	 { 
	   $excel->writeRow();
	   for($jj = 1; $jj < count($arr_key); $jj++)
		 { 
		   $excel->writeCol($giatri[$arr_key[$jj]]);
		 }

...
class ReservationSummaryBySalesmanForm extends Form
	...
	function draw()
    {
		if(URL::get('do_search'))
        {
			...
			if(Url::get('user_id'))
            {
				$cond .= ' AND (customer.sale_code = \''.Url::sget('user_id').'\')';
            }
...
//System::debug($report);
            ...
            //System::debug($record);
            $this->parse_layout('header',array('to_date'=>$to_date,'from_date'=>$from_date));
            $this->parse_layout('report',array('items'=>$items,'summary'=>$summary)+$this->map);

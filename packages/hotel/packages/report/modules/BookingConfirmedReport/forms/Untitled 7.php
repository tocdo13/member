...
class EditTableMapForm extends Form
{
	...
	{  
        ...
        // L?y id nhà hàng/phòng trong nhà hàng. - m?i portal du?c phân nhánh nhi?u nhà hàng
        ...
        // l?y tên c?a nhà hàng theo bar_id.
		$sql_group='SELECT 
        ...
        //System::debug($groups);
        // gán tên nhà hàng cho bi?n $name_group;
        $name_group = '';
        foreach($groups as $k => $gr){
            $name_group = $gr['id'];
            break;
        }
        // m?ng $table_group l?y list danh sách tên nhà hàng theo bar_id
		$table_group[0] = '--Group--';
        ...
        // Ð?m s? bàn/phòng trong nhà hàng
        ...
        // L?y danh sách bàn trong nhà hàng. m?c d?nh tr?ng thái available-> chua có d?t bàn.
		$sql = '
				SELECT
					bar_table.id
					,bar_table.code
        ...
        */
		...
		$last_floor = false;			
		$i = 1;
        // l?y nh?ng phòng book ho?c checkout trong kho?ng th?i gian dang xem
		$status_tables_others = DB::fetch_all('
			SELECT
				bar_reservation.id,bar_table.id table_id,bar_reservation.status,bar_table.bar_area_id
		...
		//System::debug($status_tables_others);
        // l?y phòng d? li?u phòng dang book ho?c dang check in trong kho?ng th?i gian dang xem
        $sql = '
			SELECT
				bar_table.id,
			...
			foreach($status_tables_others as $k=>$v){
				foreach($table_out_list as $tbl_id=>$tbl)
				{
					if($tbl['table_id']==$key and $k==$tbl_id && $tbl_id!=$id_booked){
                        ...
                        $value['status_tables_others'][$k]['bar_area_id'] = $v['bar_area_id'];
                        $value['status_tables_others'][$k]['table_id'] = $v['table_id'];
					}
				}
			}
        ...
        if(USE_DISPLAY && USE_DISPLAY==1 && isset($_GET['bar_reservation_id'])){
                if(isset($_GET['type'])){
                    ...
                    if(isset($_GET['to_table'])){ // Phan nay su dung de chuyen mon
                        ...
                        WHERE bar_reservation_table.table_id = $to_table AND bar_reservation.status = 'CHECKIN' AND bar_reservation.time>=$start_date AND bar_reservation.time<$end_date";

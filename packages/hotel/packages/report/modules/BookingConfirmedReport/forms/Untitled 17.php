...
<script> 
    ...
    jQuery(document).ready(function(){
        ...
        SetCss();    
        category_action();    
        /** Daund:  thêm nút gi?m giá tru?c & sau thu? phí */
...
//        {
    ...
    function check_printed(id){
        ...
        if(check==false){
            alert("T?t c? s?n ph?m dã in, không du?c s?a!");
            jQuery("#input_radio_quantity").val(printed);
        }else{
            if(input_radio_quantity<printed){
               alert(printed+" S?n ph?m dã in, không du?c s?a!"); 
            ...
            if(key)
            {
                ...
                if(act=='edit')
                {
                    if(jQuery("#input_radio_return").val()!=0 && jQuery("#input_radio_return").val().trim()!="")
                    {
                        ...
                        {
                            alert("S? lu?ng món dã b? thay d?i so v?i ban d?u. B?n ph?i ghi l?i tru?c khi th?c hi?n thao tác tr? l?i món!");
                            return false;
                        }
                    }
    ...
    function normal_submit()
    {
        <?php if(Url::get('cmd')=='edit'){ ?>
            if(jQuery('#discount_percent').val().trim() != '' || jQuery('#discount').val().trim() != '')
            {
                ...
                {
                    alert('B?n chua nh?p lý do gi?m giá. Xin vui lòng th? l?i!');
                    getSummary();
            ...
            jQuery.ajax({
        				...
        				success:function(html)
                        {
                            ...
                            {
                                alert('RealTime:\n Luu ý, Hóa don nhà hàng này dã du?c tài kho?n '+html['user']+' ch?nh s?a tru?c dó, vào lúc :'+html['time']+' \n D? li?u hi?n t?i c?a b?n chua du?c c?p nh?p n?i dung ch?nh s?a dó \n \n Ð? ti?p t?c thao tác b?n vui lòng t?i l?i trang !')
                                return false;
                            }
                            else
                            {
            ...
            if (xmlhttp.readyState==4 && xmlhttp.status==200)
            {
                ...
                for(obj in otbjs){
                    if(otbjs[obj]['no_member']==0){
                        alert("Mã thành viên không t?n t?i!");
                        return;
                    }else{
...
function fun_view_info_member(){
    ...
    if(member_level_id == ''){
        alert("không có chuong trình gi?m giá cho thành viên này!");
    }else{
    if (window.XMLHttpRequest){
            xmlhttp=new XMLHttpRequest();
        }

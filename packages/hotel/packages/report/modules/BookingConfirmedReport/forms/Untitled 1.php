...
class TicketRevenueReportForm extends Form
{
	...
	function draw()
	{
		...
		$cond .= ' '
                ...
                .(URL::get('customer_name')?' and UPPER(ve_reservation.agent_name) LIKE \''.strtoupper(URL::get('customer_name')).'%\'':'')
		...
		$sql = '
			...
			(
				...
				FROM 
                   ...
                   -- left join customer on customer.id =  ve_reservation.customer_id
				WHERE 
                    '.$cond.'
                ORDER BY
    ...
    function get_payments($bill_id)
    {
		return $payments = DB::fetch_all('
						...
						,payment.currency_id,payment.type_dps,payment.credit_card_id,payment.user_id
				');	
	}
}
?>
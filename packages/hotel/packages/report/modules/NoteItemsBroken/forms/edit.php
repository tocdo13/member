        ...
        else if(Url::get('cmd')=='add' )//truong hop them moi
        {
            ...
            {  
                ...
                foreach($floor as $key=>$value){
                    if($floor_condition=='and 1=1'){
                        $floor=strtoupper(convert_utf8_to_latin('Tầng '.$value,'utf-8'));
                        //echo $floor;exit();
                       $floor_condition.=' and (UPPER(FN_CONVERT_TO_VN(room.floor)) like \'%'.$floor.'%\' '; 
                    }else{
                         $floor=strtoupper(convert_utf8_to_latin('Tầng '.$value,'utf-8'));
                       $floor_condition.=' or UPPER(FN_CONVERT_TO_VN(room.floor)) like \'%'.$floor.'%\' ';  
                    }
                }
                $floor_condition.=')';
            ...
            foreach($floors as $k=>$v){
                $floor = explode(' ',$v['id']);
                $floor_option.='<li><input type="checkbox" onclick="get_ids()" value='.$floor[1].' />'.$v['id'].'</li>';
            }
...
//        $items = DB::fetch_all($sql); 
		...
		from
            ...
            left outer join room on room_status.room_id = room.id
            left join extra_service_invoice on reservation_room.id = extra_service_invoice.reservation_room_id and extra_service_invoice.use_extra_bed=1
		where 

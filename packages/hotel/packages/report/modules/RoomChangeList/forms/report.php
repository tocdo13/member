 <?php
class ReportRoomChangeList extends Form
{
    function ReportRoomChangeList()
    {
        Form::Form('ReportRoomChangeList');
    ...
    function draw()
    {
        ...
        if(Url::get('time'))
        {
            $time = $this->calc_time(Url::get('time'));
            $this->map['time'] = Url::get('time');
        }
        else
        {
            $this->map['time'] = date('H').':'.date('i');            
            $time = $this->calc_time($this->map['time']);
        ...
        if( Date_Time::to_time($this->map['date']) == Date_Time::to_time(date('d/m/Y')) )
        {
            ...
            $cond .= ' 
                        ...
                        (
                            (
                                reservation_room.status = \'CHECKIN\' AND reservation_room.departure_time >= \''.$day_orc.'\'
                                AND reservation_traveller.status =\'CHECKIN\'
                            )
                            OR
                            (
                                reservation_room.status = \'CHECKOUT\' AND reservation_room.departure_time = reservation_room.arrival_time
                            )  
                        ) 
            ...
            */
            ...
            $cond .= ' 
                        ...
                        AND reservation_room.time_in > \''.Date_Time::to_time($this->map['date']) .'\'
                        AND 
                        (
        ...
        $cond .= ' 
                    ...
                    (
                        ...
                        AND
                        reservation_traveller.arrival_date <= \''.$day_orc.'\'
                    )
                    AND reservation_traveller.status is not null
                    AND traveller.first_name || \' \' || traveller.last_name != \' \'  
                    AND 
        ...
        */                
        ...
        $sql = 'select
					...
					reservation_room.note_change_room as note_room_change,
					reservation_room.note as resion_note,
                    ...
                    DECODE(traveller.gender,0,\''.Portal::language('female').'\',DECODE(traveller.gender,1,\''.Portal::language('male').'\',\' \')) as gender,
                    DECODE(
                ...
                from 
                    ...
                    inner join room_status on room_status.reservation_room_id = reservation_room.id AND room_status.in_date=\''.$day_orc.'\'
                where
                    '.$cond.' and reservation_room.note_change_room is not null                
                order by
                 length(room.name),
        ...
        //giap.ln tinh ca so dem chang truoc
        foreach($items as  $key=>$value)
        {
            if(strpos($value['change_room_from_rr'],',')==false)
            {
                //voi moi chang sau chung ta tim chang truoc cua doi phong
                $res = $value['change_room_from_rr'];
            }
            else
            {
                $res_id = explode(',',$value['change_room_from_rr']);
                $res = $res_id[count($res_id)-1];
                
            }
            $sql = 'SELECT id,time_in as arrival_time_before,
                        departure_time-arrival_time as night
                        FROM reservation_room
                        WHERE id='. $res;
            $before_room = DB::fetch($sql);
            $items[$key]['arrival_time_before'] = $before_room['arrival_time_before'];
            $items[$key]['night'] +=$before_room['night'];

            ...
            */
			...
			$sql = 'SELECT * FROM
					(	SELECT
						...
						WHERE 
							1>0 '.$cond.'  AND bar_reservation.bar_id in ('.$bar_ids.') AND bar_reservation_product.bar_set_menu_id IS NULL
						ORDER BY
							product_category.id
					)
			WHERE
            ...
            $summary['SERVICE'] = $summary['FOOD'];     
            $stt = 1;       
            //System::debug($summary);
            $da = DB::fetch('SELECT * FROM product_category WHERE product_category.code=\'DA'.'\'');   
            $du = DB::fetch('SELECT * FROM product_category WHERE product_category.code=\'DU'.'\'');
            $dvnh = DB::fetch('SELECT * FROM product_category WHERE product_category.code=\'DVNH'.'\'');       
			foreach($items as $key=>$value)
            {
                ...
                $items[$key]['price_old'] = $value['price'];                
                ...
                /** Son Le Thanh gom cac nhom theo DA DU DV*/
                if($value['structure_id'] >= $da['structure_id'] and $value['structure_id'] < IDStructure::next($da['structure_id']))
                {
                    $index = 'FOOD';
                }
                else if($value['structure_id'] >= $du['structure_id'] and $value['structure_id'] < IDStructure::next($du['structure_id']))
                {
                    $index = 'DRINK';
                }else if($value['structure_id'] >= $dvnh['structure_id'] and $value['structure_id'] < IDStructure::next($dvnh['structure_id']))
                {
                    $index = 'SERVICE';
                }
                ...
                $summary[$index]['type'] = $index;
                //$summary[$index]['child'][$key] = $value;
                $summary[$index]['child'] = array();
                /** Son Le Thanh gom cac nhom theo DA DU DV*/
			}
            /** Daund them gom theo san pham */
            foreach($items as $key => $value)
            {
                if($value['structure_id'] >= $da['structure_id'] and $value['structure_id'] < IDStructure::next($da['structure_id']))
                {
                ...
                $key_child = $items[$key]['product_id'].'_'.$items[$key]['price_old'];
                if(User::id()=='developer14')
                {
                    //System::debug($key_child);
                }
                if(!isset($summary[$index]['child'][$key_child]))
                {
                    $summary[$index]['child'][$key_child]['id'] = $key_child;
                    $summary[$index]['child'][$key_child]['product_name'] = $items[$key]['product_name'];
                    $summary[$index]['child'][$key_child]['quantity'] = $items[$key]['quantity'];
                    $summary[$index]['child'][$key_child]['price'] = $items[$key]['price'];
                    $summary[$index]['child'][$key_child]['charge'] = $items[$key]['charge'];
                    $summary[$index]['child'][$key_child]['total_after_charge'] = $items[$key]['total_after_charge'];
                    $summary[$index]['child'][$key_child]['tax'] = $items[$key]['tax'];
                    $summary[$index]['child'][$key_child]['total_after_tax'] = $items[$key]['total_after_tax'];
                }else
                {
                    $summary[$index]['child'][$key_child]['quantity'] += $items[$key]['quantity'];
                    $summary[$index]['child'][$key_child]['price'] += $items[$key]['price'];
            ...
            /** END Daund them gom theo san pham */                     
                     
            $this->map['product'] = $product;
            //System::debug($this->map['product']);

...
class api extends restful_api
{
    ...
    function draw()
    {
        if($this->method == 'GET')
        {
            if(Url::get('secretkey') and Url::get('secretkey') == '9a8fa234b2520e9bb4f59d8178545a63')
            {
                ...
                $to_day = $this->get_data(date('d/m/Y',$time_from_day),date('d/m/Y',$time_from_day),$portal_id); /** Thuc hien cua ngay xem bc **/
                $last_month_td = date('d',$time_from_day).'/'.date('m/Y',(Date_Time::to_time(date('01/m/Y',$time_from_day))-86400));
                if(date('d',$time_from_day)<=date('t',(Date_Time::to_time(date('01/m/Y',$time_from_day))-86400)))
                {
                    $last_month = $this->get_data($last_month_td,$last_month_td,$portal_id); /** Thuc hien cua ngay nay thang truoc**/
                }
                else
                {
                    $last_month = array('total_room' => 0,
                                        'repair_room' => 0,
                                        'room_available_for_sale' => 0,
                                        'total_room_occ' => 0,
                                        'foc_room' => 0,
                                        'hu_room' => 0,
                                        'adult' => 0,
                                        ...
                                        'spa' => 0,
                                        'shop' => 0,
                                        'hotel_revenue_total' => 0,
                                        'occupancy_rate' => 0,
                                        'average_room_rate' => 0,
                                        'rev_par' => 0,
                                        'spend_per_guest' => 0,
                                        'room_revenue_percent' => 0,
                                        'bar_revenue_percent' => 0,
                                        'total_telephone_percent' => 0,
                                        'laundry_percent' => 0,
                                        'minibar_percent' => 0,
                                        'transport_percent' => 0,
                                        'spa_percent' => 0,
                                        'service_other_percent' => 0,
                                        'shop_percent' => 0
                                        );
                }
                $last_year_td = date('d/m',$time_from_day).'/'.(date('Y',$time_from_day)-1);
                if(date('d',$time_from_day)<=date('t',(Date_Time::to_time(date('d/m',$time_from_day).'/'.(date('Y',$time_from_day)-1)))))
                {
                    $last_year = $this->get_data($last_year_td,$last_year_td,$portal_id); /** Thuc hien cua ngay nay nam truoc**/
                }
                else
                {
                    $last_year = array('total_room' => 0,
                                        'repair_room' => 0,
                                        'room_available_for_sale' => 0,
                                        'total_room_occ' => 0,
                                        'foc_room' => 0,

...
class ReportPickupSeeoff extends Form
{
    ...
    function draw()
    {
        ...
        {
            $cond .= ' and reservation_traveller.pickup = 1 and FROM_UNIXTIME(reservation_traveller.flight_arrival_time) = \''.$day_orc.'\'  ';
        }
        else
            if(Url::sget('status')=='SEEOFF')
            {
                $cond .= ' and reservation_traveller.see_off = 1 and FROM_UNIXTIME(reservation_traveller.flight_departure_time) = \''.$day_orc.'\' ';
            }
            else
            {
                $cond .= ' and (reservation_traveller.pickup = 1 or reservation_traveller.see_off = 1) and ( FROM_UNIXTIME(reservation_traveller.flight_arrival_time) = \''.$day_orc.'\' or FROM_UNIXTIME(reservation_traveller.flight_departure_time) = \''.$day_orc.'\' ) ';
            }

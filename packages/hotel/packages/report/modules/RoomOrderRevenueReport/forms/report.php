...
class RoomOrderRevenueReportForm extends Form
{
	...
	function draw()
	{
        ...
        if(URL::get('do_search'))
		{
           ...
           if(Url::get('search_invoice')){
            ...
            {
				$payments = $this->get_payments($cond2.$cond3);//$payments = $this->get_payments($time_from,$time_to);//$cond2
                //System::debug($payments);
                $traveller_folio = $this->get_item_folio(); 
                //System::debug($traveller_folio);
                $code_vats = $this->get_vat_invoice($cond2.$cond3);// Lay code vat de gan vao folio
				...
				for($i=$time_from; $i<=$time_to;$i=$i+86400)
                {
					foreach($folios as $key => $folio)
                    {
						...
						if($folio['time']>=($i+$shift_from) || ($folio['time']<=($i+$shift_to)) || $check==true )
                        {
                            ...
                            //giap.ln add truong hop dat coc = tong hoa don thi khong co thanh toan
                            if($items[$key]['total']==0)
                                $items[$key]['total'] = $items[$key]['deposit'];
                            //end
	...
	function get_foios($cond=' 1>0')
    {
		...
		$sql = '  SELECT
						...
						END as user_id
                        ,party.full_name as name_user
					FROM
						folio
						INNER JOIN reservation ON reservation.id = folio.reservation_id
                        INNER join payment ON folio.id = payment.folio_id
					...
					FROM
						...
						LEFT OUTER JOIN customer ON customer.id = reservation.customer_id
					WHERE 
						'.$cond.' AND reservation_room.status !=\'CANCEL\'  or
                    folio.id in (
                    select folio.id from folio where folio.reservation_room_id is NULL 
                     AND '.$cond.'
                    )
	...
	function get_item_folio($cond=' 1>0')
    {
		$sql = 'select 
				...
				FROM traveller_folio rt
                    inner join folio on folio.id=rt.folio_id
                    LEFT join payment ON folio.id = payment.folio_id 

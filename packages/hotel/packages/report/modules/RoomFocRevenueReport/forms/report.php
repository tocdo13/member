...
class RoomRevenueReportForm extends Form{
	...
	function draw()
    {
		...
		if(URL::get('do_search') )
        {
			...
			$cond = '1=1 and (reservation_room.foc is not NULL or reservation_room.foc_all !=0)'
			...
			;
			$reservation_cond = (URL::get('date_from')?' and reservation_room.departure_time>=\''.(Date_Time::to_orc_date($from_day)).'\' and reservation_room.arrival_time<=\''.(Date_Time::to_orc_date($to_day)).'\'':'');
			
			if(Url::get('revenue')=='ALL'){
				$cond.=' and (reservation_room.status!=\'CANCEL\')';
                $reservation_cond.=' and (reservation_room.status!=\'CANCEL\')';
            ...
            foreach($total_room as $key => $value)
            {
                if($value['foc']=='')
                {
                    $sql="
                        ...
                        where reservation_room.foc is not NULL or reservation_room.foc_all !=0
                        and reservation_room.id=".$key.(Url::get('date_from')?' and ROOM_STATUS.in_date>=\''.(Date_Time::to_orc_date($from_day)).'\' and ROOM_STATUS.in_date<=\''.(Date_Time::to_orc_date($to_day)).'\'':'');
                        $total_room[$key]["room_total"] = DB::fetch($sql,"total");
                        if($value['foc_all']==1) $total_room[$key]["room_total"] =0;
            ...
            $exception_liloei =" AND extra_service.code != 'LATE_CHECKOUT' AND extra_service.code!='LATE_CHECKIN' AND extra_service.code <>'EARLY_CHECKIN'";
            $liloei =" AND (extra_service.code='LATE_CHECKOUT' OR extra_service.code='LATE_CHECKIN' OR extra_service.code='EARLY_CHECKIN') ";
            $sql = 'select 
            ...
            $sql = str_replace($exception_liloei,$liloei,$sql);
            $extra_services_liloei = DB::fetch_all($sql);
            //end giap.ln
            /****massage****/
			$massage_cond  = '1=1';
			if(Url::get('revenue')!='ALL'){
...
//            system::debug($extra_services);
            ...
            foreach($report->items as $key => $value)
            {
				...
				where reservation_room.foc is not NULL or reservation_room.foc_all !=0 and reservation_room.id=".$key;

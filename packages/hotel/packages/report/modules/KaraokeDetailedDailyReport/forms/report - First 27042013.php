	...
	function draw()
	{
		if(Url::get('do_search'))
		{
            ...
            */
            ...
            $karaoke_reservations = DB::fetch_all('
						SELECT
						...
						FROM 
							karaoke_reservation
							inner join karaoke_reservation_product brp ON karaoke_reservation.id = brp.karaoke_reservation_id
						WHERE 
							1>0 '.$cond.'  AND karaoke_reservation.karaoke_id in ('.$karaoke_ids.')
						GROUP BY karaoke_reservation.id,karaoke_reservation.discount			
			');
			foreach($karaoke_reservations as $b => $reser){
				$karaoke_reservations[$b]['discount_exchange'] = $reser['discount']/$reser['discount_exchange'];
			}
			//System::Debug($karaoke_reservations);
			$sql = 'SELECT * FROM
					(	SELECT
							brp.id as id,
                            brp.karaoke_reservation_id,
            ...
            {
				if(isset($karaoke_reservations[$value['karaoke_reservation_id']])){
					$value['discount_exchange'] = $karaoke_reservations[$value['karaoke_reservation_id']]['discount_exchange'];
				}

...
class DailyRevenueExtraServiceReportForm extends Form
{
	...
	function draw()
	{
	   //System::debug($_REQUEST);
        ...
        {
            $cond_portal =Url::get('portal_id')?'AND reservation.portal_id = \''.Url::get('portal_id').'\'':''; 
        }
		//if(URL::get('do_search'))
		{
			...
			$cond = '(reservation_room.status=\'CHECKIN\' OR reservation_room.status=\'CHECKOUT\' OR reservation_room.status=\'BOOKED\')';// AND reservation_room.departure_time>=\''.$in_date.'\' AND reservation_room.departure_time<=\''.$to_date.'\'
			$report = new Report;
			$no_of_page = 500;
			$line_per_page = 24;
			...
			$sql = 'select 
        				...
        				from 
        					...
        					left join room_type on room_type.id = room.room_type_id
        				...
        				where 
        					'.$cond.$cond_portal.'
                            AND extra_service_invoice_detail.in_date>=\''.$in_date.'\'  AND extra_service_invoice_detail.in_date<=\''.$to_date.'\'
        				order by 
        					departure_time DESC';
            $report->items = array();
			...
			$sql = 	'	select 
						...
						where 
                            ...
                            --AND extra_service_invoice_detail.service_id!=45 AND extra_service_invoice_detail.service_id	!=46 AND extra_service_invoice_detail.service_id !=47
            ...
            //giap.ln chi hien thi nhung dich vu trong khoang thoi gian search 
            $sql_service = 'SELECT extra_service.id,
                                extra_service.code,
                                extra_service.name 
                            FROM extra_service 
                            where 1=1 
                            --AND extra_service.id!=45 AND extra_service.id!=46 
                            --ANd extra_service.id!=47 
                            --and extra_service.status=\'SHOW\'
                            AND extra_service.code != \'LATE_CHECKIN\'
                            AND exists(SELECT extra_service_invoice_detail.*
                                        FROM extra_service_invoice_detail 
                                        INNER JOIN extra_service_invoice ON extra_service_invoice.id=extra_service_invoice_detail.invoice_id
                                        INNER JOIN reservation_room ON reservation_room.id=extra_service_invoice.reservation_room_id
                                        INNER JOIN reservation ON reservation_room.reservation_id= reservation.id
                                        WHERE '.$cond.$cond_portal.' 
                                        AND extra_service_invoice_detail.service_id=extra_service.id
                                        AND extra_service_invoice_detail.in_date>=\''.$in_date.'\'  AND extra_service_invoice_detail.in_date<=\''.$to_date.'\')';
			$num_services = DB::fetch_all($sql_service);
            //end giap.ln 
            //$sql_service = 'SELECT id,name FROM extra_service where extra_service.id!=45 AND extra_service.id!=46 ANd extra_service.id!=47';
			//$num_services = DB::fetch_all($sql_service);				
			...
			foreach($report->items as $key=>$item)
			{
				...
				foreach($extra_services as $id=> $service)
                {
                    ...
                    {	
						$report->items[$key][$service['service_code']] = $service['total_amount'];	
	...
	function print_all_pages(&$report,$pay_by_currency,$extra_service,$num_services)
	{
		...
		if(sizeof($pages))
		{
			...
			foreach($num_services as $id => $value){
				$this->group_function_params[$value['id']] = 0;
			}
		...
		else
		{
			...
			$this->parse_layout('footer-new',array(
				'page_no'=>1,
				'total_page'=>1
			));
		}
	...
	function print_page($items, &$report, $page_no,$total_page,$pay_by_currency,$extra_service,$num_services)
	{
		...
		foreach($items as $item)
		{
			...
			foreach($num_services as $id => $value){
				if(isset($item[$value['code']])){ 
					$this->group_function_params[$value['id']] += System::calculate_number($item[$value['code']]);
				}
			}
		}

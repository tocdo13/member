		...
		<span id="input_group_#xxxx#">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="2" bgcolor="#FFFFFF" id="table_#xxxx#">
			  <tr id="tr_#xxxx#">
				...
				<td class="multi-input"><input  name="mi_traveller[#xxxx#][room_level]" type="text" style="width:60px;font-size:10px;" class="select-room" id="room_level_#xxxx#" readonly="readonly"/></td>
                <td class="multi-input"><input  name="mi_traveller[#xxxx#][mi_traveller_room_name]" type="text" id="mi_traveller_room_name_#xxxx#" readonly="readonly" class="select-room" style="width:55px;" onclick="if(checkRoomOut($('traveller_room_id_#xxxx#').value) && $('id_#xxxx#').value){alert('Phong da check out. Ban khong duoc quyen sua');}else{<?php if(!Url::get('rr_id')){?>display_room_table(this,'#xxxx#');<?php }?>}"></td>
				...
				<td class="multi-input"><select  name="mi_traveller[#xxxx#][gender]" id="gender_#xxxx#" style="width:75px; text-transform:uppercase;" class="gender" >
                    <option value="2" <?php (isset($_REQUEST['mi_traveller']['#xxxx#']['gender']) and $_REQUEST['mi_traveller']['#xxxx#']['gender']==2)?'selected="selected"':''?>>[[.male.]]/[[.female.]]</option>
                ...
                <td class="multi-input"><select  name="mi_traveller[#xxxx#][traveller_level_id]" style="width:84px; text-transform:uppercase;" id="traveller_level_id_#xxxx#" class="reservation_type" >[[|traveller_level_options|]]</select></td>
                <td class="multi-input"><input name="mi_traveller[#xxxx#][is_child]" type="checkbox" id="is_child_#xxxx#" style="width:15px;" /></td>
                <td class="multi-input"><!--IF:right_cond(User::can_delete(false,ANY_CATEGORY))--><img src="packages/core/skins/default/images/buttons/delete.gif" onclick="if(confirm('[[.are_you_sure.]]')){ check_is_folio(#xxxx#);}" style="cursor:pointer;"/>
				<!--/IF:right_cond--></td >
			   ...
			   <tr>
                <td class="multi-input" colspan="12" style="padding-left:60px;" id="extra_guest_#xxxx#">
                	...
                	<table cellpadding="3" cellspacing="0" width="100%">
                    	<tr>
                        	<td>[[.phone.]]: </td>
                        	<td><input  name="mi_traveller[#xxxx#][phone]" style="width:120px;" type="text" id="phone_#xxxx#" /></td>
                        	<td>[[.email.]]: </td>
                        	<td><input  name="mi_traveller[#xxxx#][email]" style="width:120px;"  id="email_#xxxx#" /></td>
                        	<td>[[.address.]]: </td>
                            <td><input  name="mi_traveller[#xxxx#][address]" style="width:140px;"  id="address_#xxxx#" /></td>
                            <td><a href="#" onclick="var conf = confirm('[[.are_you_ok.]]'); if(conf){window.location='<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/'.Url::$root.'form.php?block_id='.Module::block_id().'&cmd=checkout_guest';?>&r_id=<?php echo Url::get('r_id');?>&rr_id='+jQuery('#reservation_room_id_#xxxx#').val()+'&traveller_id='+jQuery('#reservation_traveller_id_#xxxx#').val()+'<?php echo PORTAL_ID;?>';}"><input  name="mi_traveller[#xxxx#][checkout_guest]" type="button" id="checkout_guest_#xxxx#" value="[[.checkout_guest.]]" style="width:90px; height:23px;"/></a> </td>
                        </tr>
                    	<tr>  
                        	<td>[[.arrival_time.]]</td>
                            ...
                            <td></td>-->
                            <td>[[.is_vn.]]</td>
                            <!--<td><select name="is_vn" id="is_vn_#xxxx#" style="width: 140px; "></select></td>-->
                            <td style="width:140px;">
                                <select name="mi_traveller[#xxxx#][is_vn]" id="is_vn_#xxxx#" style="float: left;">
                                <option id="0"></option>
                                <!--
                                <option value="0" <?php (isset($_REQUEST['mi_traveller']['#xxxx#']['is_vn']) and $_REQUEST['mi_traveller']['#xxxx#']['is_vn']==0)?'selected="selected"':''?>>[[.Alien.]]</option>
                                <option value="1" <?php (isset($_REQUEST['mi_traveller']['#xxxx#']['is_vn']) and $_REQUEST['mi_traveller']['#xxxx#']['is_vn']==1)?'selected="selected"':''?>>[[.Overseas_Vietnamese.]]</option>
                                -->
                                <option value="2" <?php (isset($_REQUEST['mi_traveller']['#xxxx#']['is_vn']) and $_REQUEST['mi_traveller']['#xxxx#']['is_vn']==2)?'selected="selected"':''?>>[[.Viet_nam.]]</option>
                                <option value="3" <?php (isset($_REQUEST['mi_traveller']['#xxxx#']['is_vn']) and $_REQUEST['mi_traveller']['#xxxx#']['is_vn']==3)?'selected="selected"':''?>>[[.Viet_nam_in_foreign.]]</option>
                                 </select>
                            </td>
                            <!-- End Oanh -->
                            <td><input  name="mi_traveller[#xxxx#][change_guest]" type="button" id="change_guest_#xxxx#" value="[[.change_room.]]" style="width:90px; height:23px;" onclick="window.location='<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/'.Url::$root.'form.php?block_id='.Module::block_id().'&cmd=change_guest';?>&rr_id='+jQuery('#reservation_room_id_#xxxx#').val()+'&traveller_id='+jQuery('#reservation_traveller_id_#xxxx#').val()+'&portal_id=<?php echo PORTAL_ID;?>';" /> </td>                            
                        </tr>
                        <tr>
                            <td>[[.to_judge.]]</td>
                            <td>
                                <select name="mi_traveller[#xxxx#][to_judge]" id="to_judge_#xxxx#" style="float: left;">
                                <option id="0"></option>
                            ...
                            <td>[[.note.]]: </td>
                            <td colspan="5"><textarea  name="mi_traveller[#xxxx#][note]" style="width:479px; text-transform:uppercase;"  id="note_#xxxx#"></textarea></td>
                        </tr>
                    	<tr>
                        	<td>[[.flight_code.]]: </td>
                        	<td><input  name="mi_traveller[#xxxx#][flight_code]" style="width:120px; text-transform:uppercase;" type="text" id="flight_code_#xxxx#" /></td>
							...
							<td>[[.car_note_arrival.]]: </td>
                            <td><input  name="mi_traveller[#xxxx#][car_note_arrival]" style="width:140px; text-transform:uppercase;"  id="car_note_arrival_#xxxx#" /></td>
                            <td>
                                <div>
							         <input  name="mi_traveller[#xxxx#][pickup]" type="checkbox" id="pickup_#xxxx#" value="1" /><label for="pickup_#xxxx#">[[.pick_up.]]</label>
							    </div>
							    <div>
							         <input  name="mi_traveller[#xxxx#][pickup_foc]" type="checkbox" id="pickup_foc_#xxxx#" value="1" /><label for="pickup_foc_#xxxx#">[[.pickup_foc.]]</label>
	          					</div>
							</td>
                        </tr>
                    	<tr>
                        	<td>[[.flight_code_departure.]]: </td>
                        	<td><input  name="mi_traveller[#xxxx#][flight_code_departure]" style="width:120px; text-transform:uppercase;" type="text" id="flight_code_departure_#xxxx#" /></td>
                        	...
                        	<td>[[.car_note_departure.]]: </td>
                            <td><input  name="mi_traveller[#xxxx#][car_note_departure]" style="width:140px; text-transform:uppercase;"  id="car_note_departure_#xxxx#" /></td>
                            <td>
							<div>
							<input  name="mi_traveller[#xxxx#][see_off]" type="checkbox" id="see_off_#xxxx#" value="1" /><label for="see_off_#xxxx#">[[.see_off.]]</label>
							</div>
							<div>
							<input  name="mi_traveller[#xxxx#][see_off_foc]" type="checkbox" id="see_off_foc_#xxxx#" value="1" /><label for="see_off_foc_#xxxx#">[[.see_off_foc.]]</label>
							</div>
							</td>
                        </tr>
                        <tr>
                        	<td>[[.province.]]: </td>
                        	<td class="multi-input">
                                <select  name="mi_traveller[#xxxx#][province_id]" style="width:150px;" id="province_id_#xxxx#">
                                    <option value=""></option>
                                    [[|province_options|]]
                                </select>
                            </td>
                            <td>[[.member_code.]]:</td>
                            <td class="multi-input"><input  name="mi_traveller[#xxxx#][member_code]" style="width:120px;" type="text" id="member_code_#xxxx#" /></td>
                            <td><input name="create_member_#xxxx#" type="checkbox" id="create_member_#xxxx#" onchange="fun_check_create_member(this);" onclick="fun_check_create_member(this);" /> [[.create_member.]]</td>
                            <td>[[.competence.]]</td>
                            <td><input  name="mi_traveller[#xxxx#][competence]" style="width:120px;" type="text" id="competence_#xxxx#" /></td>
                        </tr>
                    </table>
                </td>
			  </tr>
			</table>
			<span id="detail_link_#xxxx#" style="font-size:10px;"></span>
            <span id="history_link_#xxxx#" style="font-size:10px; margin-left:30px;"></span>
			<br/>
        </span>
    </span>
</span>
<form name="UpdateTravellerForm" method="post">
<div style="text-align:center">  
<div style="width:1030px;margin-right:auto;margin-left:auto;">
	...
	<tr valign="top">
		<td align="left">
			<table width="100%" cellpadding="0" cellspacing="0">
                ...
                <div>
					<td width="90%" nowrap  class="form-title">[[.update_traveller.]]</td>
                   ...
                   <a target="_blank" class="button-medium-save" href="<?php echo $hef;?>" >[[.import_from_file_excel.]]</a>
                    </td>
                    <!--Oanhbtk them export file excel -->                         
                    <td><input name="export_file_excel" type="submit" onclick="exportExcel()" value="[[.export_file_excel.]]" class="button-medium-save" style="white-100px;"/></td>
                    <!-- End Oanhbtk -->
				  	<?php if(User::can_add(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap"><input name="save" type="submit" value="[[.Save.]]" class="button-medium-save" onclick="jQuery('#mask').show();jQuery('.button-medium-save').css('display','none');"/></td><?php }?>
					<?php if(User::can_edit(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap"><input name="update" type="submit" value="[[.Save_and_stay.]]" class="button-medium-save stay" onclick="jQuery('#mask').show();jQuery('.button-medium-save').css('display','none');"/></td><?php }?>
					<?php if(User::can_delete(false,ANY_CATEGORY)){?><td width="1%" nowrap="nowrap"><a href="#" onclick="onBackEvent();"  class="button-medium-back">[[.back.]]</a></td><?php }?>
				</div>
                </form>
                </tr>
		  </table>
   ...
   <table id="myTable" class="tablesorter" width="100%" cellpadding="2" cellspacing="0" border="1">
	...
	<td><?php if(Form::$current->is_error()){?><div><br><?php echo Form::$current->error_messages();?></div><br clear="all"><?php }?>
    	<fieldset style="background:#f2f4ce;margin-bottom:5px;">
			<legend class="legend-title">[[.guest_in_house.]]&nbsp;(<span id="count_number_of_guest"></span> [[.guest.]])</legend>
            <div style="padding:3px 0px;">[[.Change_all_traveller_level.]]: <select id="change_all_traveller_id" onchange="change_all_traveller_level();">
            	[[|traveller_level_options|]]
            </select>
             [[.change_all_national.]] <input name="changeAllNational" type="text" id="changeAllNational" onchange="updateAllNationality();" onblur="updateAllNationality();" style="width:60px; text-transform:uppercase;">
           	 </div>
			<span id="mi_traveller_all_elems">
                    <span class="multi-input-header" style="width:35px;">[[.index.]]</span>
                    <span class="multi-input-header" style="width:55px;">[[.room_type.]]</span>
                    <span class="multi-input-header" style="width:50px;">[[.room.]]</span>
                    <span class="multi-input-header" style="width:96px;">[[.first_name.]] (*)</span>  
                    <span class="multi-input-header" style="width:75px;">[[.last_name.]] (*)</span>
                    <span class="multi-input-header" style="width:77px;">[[.passport.]] </span>
                    <span class="multi-input-header" style="width:15px;" title="[[.transit.]]">&nbsp; </span>
                    <span class="multi-input-header" style="width:75px;">[[.visa.]]</span>
                    <span class="multi-input-header" style="width:79px;">[[.visa_expired.]]</span>
                    <span class="multi-input-header" style="width:67px;">[[.gender.]]</span>
                    <span class="multi-input-header" style="width:57px;">[[.birth_date.]]</span>
                    <span class="multi-input-header" style="width:123px;">[[.nationality.]]</span>
                    <span class="multi-input-header" style="width:84px;">[[.traveller_level.]] (*)</span>
                    <span class="multi-input-header" style="width:10px;"><img src="packages/core/skins/default/images/buttons/child.png" alt="Trẻ em" /></span>
                    <span class="multi-input-header" style="border:0px;background:none;"></span>
                    <br clear="all">
			  </span>
              <input name="traveller_delete" type="hidden" id="traveller_delete" />
              <input type="button" value="[[.add_traveller.]]" class="button-medium-add" onclick="addnew_traveller();">
            [[.copy_traveller.]] <input  name="traveller_indexs" id="traveller_indexs" value="index1,index2,index3" onclick="if(this.value=='index1,index2,index3'){this.value='';}" onblur="if(this.value==''){this.value='index1,index2,index3';}" type ="text" value="<?php echo String::html_normalize(URL::get('traveller_indexs'));?>"> [[.quantity.]] <input  name="traveller_quantity" id="traveller_quantity" style="width:30px;" size="100" maxlength="3" type ="text"><input type="button" value="[[.copy.]]" onclick="copyTraveller($('traveller_indexs').value,$('traveller_quantity').value)">
         </fieldset>
	<table width="100%">
	<tr><td align="left">
		</td></tr>
			...
			if(mi_traveller_arr[i]['check_out']==1){
				if(jQuery('#id_'+j).val()==mi_traveller_arr[i]['reservation_traveller_id'] && jQuery('#traveller_room_id_'+j).val()!= 'undefined'){
					...
					if(mi_traveller_arr[i]['status']=='CHANGE'){
						jQuery('#table_'+j+' input[type=button]').val('[[.has_changed_room.]]');
						jQuery('#checkout_guest_'+j).css('display','none');							
					}else if(mi_traveller_arr[i]['status']=='CHECKOUT'){
						jQuery('#table_'+j+' input[type=button]').val('[[.checkout.]]');						
						jQuery('#checkout_guest_'+j).css('display','none');	   
					}
				}
			}else{
...
function updateAll(){
	for(var i=101;i<=input_count;i++){
		...
		if($("traveller_id__"+i)){
			...
			$('detail_link_'+i).innerHTML = '<a target="blank" style="font-size:11px;" href="<?php echo 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root;?>?page=traveller&cmd=edit&id='+jQuery('#traveller_id__'+i).val()+'">'+viewDetailLink+'#'+jQuery('#traveller_id__'+i).val()+'</a>';
		
            var viewHistoryLink = '<?php echo Portal::language("view_history"); ?>';
			$('history_link_'+i).innerHTML = '<a target="blank" style="font-size:11px;" href="<?php echo 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/'.Url::$root;?>?page=traveller&id='+jQuery('#traveller_id__'+i).val()+'">'+viewHistoryLink+'#'+jQuery('#traveller_id__'+i).val()+'</a>';
        }
		...
		if(jQuery('#check_out_'+i).val()==1){
				...
				if(jQuery('#status_'+i).val()=='CHANGE'){
					jQuery('#table_'+i+' input[type=button]').val('[[.has_changed_room.]]');
					jQuery('#checkout_guest_'+i).css('display','none');							
				}else if(jQuery('#status_'+i).val()=='CHECKOUT'){
					jQuery('#table_'+i+' input[type=button]').val('[[.checkout.]]');						
					jQuery('#checkout_guest_'+i).css('display','none');	   
				}
		}else{
			for(var k in count_guest){
...
function addnew_traveller()
{
    ...
    $('mi_traveller_room_name_'+input_count).value = '<?php if(Url::get('rr_id') && isset([[=room_name=]])){ echo [[=room_name=]];}else{ echo '';}?>';
    $('room_level_'+input_count).value = '<?php if(Url::get('rr_id') && isset([[=room_level=]])){ echo [[=room_level=]];}else{ echo '';}?>';
    $('traveller_room_id_'+input_count).value = '<?php if(Url::get('rr_id') && isset([[=room_id=]])){ echo ([[=room_id=]].'-'.[[=departure_time=]]);}else{ echo '';}?>';
...
{      
    ...
    jQuery.ajax({
					...
					success:function(html)
                    {
                        if(html==1)
                        {
                            alert('[[.guest_have_folio_can_not_delete.]]');
                            return false;
                        }
                        else
                        {

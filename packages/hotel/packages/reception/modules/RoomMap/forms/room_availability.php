...
class RoomAvailabilityForm extends Form
{
			...
			if(User::can_edit(false,ANY_CATEGORY))
			{
				...
				if(Url::sget('in_date'))
                {
					foreach($room_ids as $room_id)
                    {
						...
						while($time<=$end_time)
                        {
							$cond = 'status<>\'CANCEL\' and status<>\'NOSHOW\'
	...
	function draw()
	{
	    set_time_limit(-1);
        /**
        * tieu binh them th n?u khách hàng h? nh?p giá và ch?n phòng khách trong cùng h?ng phòng thì gi? 
        * nguyên giá n?u không thì l?y giá m?c d?nh dã du?c khai báo ? room_level
        * m?i khi kích kính lúp dã truy?n tên h?ng phòng và giá nhung do trong ph?n ch?n phòng 
        * có th? ngu?i ta kích vào ch?n h?ng phòng lúc dó trang load l?i làm m?t 
        * tên và giá nên ch? này luu vào session.
        **/
        if(Url::get('room_level_name')){
            $_SESSION['room_level_name'] = Url::get('room_level_name');
        }
        if(Url::get('price')){
            $_SESSION['price'] = Url::get('price');
        }
        if(Url::get('usd_price')){
            $_SESSION['usd_price'] = Url::get('usd_price');
        }
        /**
        end tieubinh
        **/
		...
		$this->map['ebs'] = $ebs;
        $layout = 'room_availability';
        if(Url::get('type_check') AND Url::get('type_check')=='group')
        {
            $layout = 'room_availability_group';
        }
	...
	function get_books_without_room_return_arr()
    {
		...
		if(Url::get('r_r_id') && Url::get('r_r_id')!=''){
		    if(Url::get('type_check') AND Url::get('type_check')=='group')
            {
                $group = explode("_",Url::get('r_r_id'));
                
                for($ij=0;$ij<sizeof($group);$ij++)
                {
                    $cond .= 'AND reservation_room.id<>'.$group[$ij].' '; 
                }
            }
            else
	...
	function get_rooms()
	{
        ...
        // Manh: them xu ly de lay lich su phong
        $cond_history = '';
        $start_date_his = '';
        $end_date_his = '';
        if( $his_in_date = DB::fetch("select max(in_date) as in_date from room_history where in_date<='".$from_date."' AND portal_id='".PORTAL_ID."'","in_date") )
        {
            $start_date_his = $his_in_date;
        }
        elseif( $his_in_date = DB::fetch("select min(in_date) as in_date from room_history where in_date>'".$from_date."' AND portal_id='".PORTAL_ID."'","in_date") )
        {
            $start_date_his = $his_in_date;
        }
        if( $his_in_date = DB::fetch("select max(in_date) as in_date from room_history where in_date<='".$to_date."' AND portal_id='".PORTAL_ID."'","in_date") )
        {
            $end_date_his = $his_in_date;
        }
        elseif( $his_in_date = DB::fetch("select min(in_date) as in_date from room_history where in_date>'".$to_date."' AND portal_id='".PORTAL_ID."'","in_date") )
        {
            $end_date_his = $his_in_date;
        }
        ...
        $sql = '
    			...
    			from
    				...
    				inner join room_type on room_type.id = room_type_id
                                     left outer join room_status on room.id = room_status.room_id 
    				left outer join minibar on room.id = minibar.room_id 
    			where
    				room.portal_id = \''.PORTAL_ID.'\' 
                    '.$cond_area.'
                    and (room_status.room_id not in ( 
                        select room_status.room_id from room_status where  \''.$from_date.'\'<=room_status.in_date 
                        and room_status.in_date<=\''.$to_date.'\' and (room_status.house_status=\'REPAIR\' OR room_status.house_status=\'CLOSE\')
                        ) OR room_status.room_id is null)
    			order by 
    				floor, 
    				room.position
    		';
		...
		{
		    // manh them de lay lich su phong
            if($start_date_his!='' AND $end_date_his!='')
            {
                for($i=$start_date_his;$i<=$end_date_his;$i+=24*3600)
                {
                    if(isset($rooms[$key]))
                    {
                        if(!DB::exists('SELECT rhd.room_id from room_history_detail rhd inner join room_history rh on rh.id=rhd.room_history_id where rh.in_date=\''.Date_Time::to_orc_date(date('d/m/Y',$i)).'\' AND rh.portal_id=\''.PORTAL_ID.'\' AND rhd.close_room=1 AND rhd.room_id='.$key))
                        {
                            unset($rooms[$key]);
                        }
                    }
            ...
            if(isset($rooms[$key]))
            {
                ...
                $rooms[$key]['usd_price'] = System::display_number($room['price']/$exchange_rate);
            }
            // end manh  
		}
		return $rooms;
	...
	function get_room_statuses()
	{
		...
		where
			reservation.portal_id = \''.PORTAL_ID.'\'
			and room_status.status <> \'CANCEL\' and room_status.status <> \'NOSHOW\' AND room_status.status <> \'AVAILABLE\' 
	...
	function get_room_level()
    {
		...
		order by rl.is_virtual,rl.price
	');
        $currency_id = (HOTEL_CURRENCY == 'VND')?'USD':'VND';
    	$exchange_rate = DB::fetch('select id,exchange from currency where id=\''.$currency_id.'\'','exchange');
        foreach($room_level as $key=>$value){
            $room_level[$key]['usd_price'] = System::display_number($value['price']/$exchange_rate);
        }
        return 	$room_level;
	}
	function cancel_booking_expired()
    {
        ...
        {// 5h sau gi? checkin m?c d?nh th� m?i x�a
			foreach($rrs as $k => $rr)
            {

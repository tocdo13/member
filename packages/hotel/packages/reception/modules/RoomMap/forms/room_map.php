...
class RoomMapForm extends Form
{
	function RoomMapForm()
	{
        ...
        if(Url::get('view_layout')=='simple'){
            $this->link_css(Portal::template('hotel').'/css/roomsimple.css');//Important
        }else{
	...
	function on_submit()
	{
		if($this->check())
		{
				...
				if(Url::sget('in_date') AND isset($_REQUEST['check_submit_date']))
                {
					foreach($room_ids as $room_id)
                    {
						...
						while($time<=$end_time){
                            ...
                            /** END thêm thông tin tên phòng cho phần log đổi trạng thái phòng**/
							{ 
							 /** Minh them user_repair **/
                                $user_repair=(Url::get('house_status')=='REPAIR')?User::id():'';
                                DB::update('room_status',
									array(
									'note'=>Url::get('note'),
                        ...
                        if(Url::get('house_status') =='' || Url::get('house_status') =='CLEAN' || Url::get('house_status') =='DIRTY')
                        {
            ...
            if(Url::get('view_layout')=='simple')
                Url::redirect_current(array('in_date','view_layout'=>'simple'));
            else
	...
	function draw()
	{
		...
		$rooms = $this->get_rooms(Url::get('in_date'));

		$this->map = array();
		$room_statuses = $this->get_room_statuses();
		$this->map['total_booked_room'] = 0; /** phong book **/
        $this->map['total_overdue_booked_room'] = 0; /** phong book qua han **/
		$this->map['total_checkin_room'] = 0; /** phong luu **/
		$this->map['total_available_room'] = 0; /** phong trong **/
        $this->map['total_checkin_today'] = 0; /** phong ci trong ngay (tinh ca dayused) **/
		$this->map['total_overdue_room'] = 0; /** phong o qua han **/
        $this->map['autual_checkin'] = 0; /** phong thuc te CI **/
        $this->map['autual_checkout'] = 0; /** phong thuc te CO **/
		$this->map['total_checkout_today_room'] = 0; /** phong se tra **/
        $this->map['total_repaire_room'] = 0; /** phong hong **/
        /** Mạnh fix trường hợp đổi phòng đã ở qua đêm **/
        $this->map['total_change_room_today']=0;
        /** end manh **/
        /** Kimtan them 12/01/18 de dem so luong phong den,di,luu **/	
        $room_status_arr = $this->get_status_new();
        $this->map['arr_room'] = $room_status_arr['arr_room'];
        $this->map['autual_checkin_room'] = $room_status_arr['autual_checkin_room'];
        $this->map['autual_checkout_room'] = $room_status_arr['autual_checkout_room'];
        $this->map['occ_room'] = $room_status_arr['occ_room'];
        $this->map['dep_room'] = $room_status_arr['dep_room'];
        $this->map['dayused_room'] = $room_status_arr['dayused_room'];
        
        //$total_room = DB::fetch('Select count(room.id) as total from room inner join room_level on room_level.id = room.room_level_id Where room_level.is_virtual = 0 and room.portal_id = \''.PORTAL_ID.'\'  ','total');
                
        $this->map['total_available_room'] = sizeof($this->get_rooms(Url::get('in_date'),true));
        /** end Kimtan them 12/01/18 de dem so luong phong den,di,luu **/		
				...
				}
				{
					...
					if($room_status['room_id']!=$room_id)
					{
						...
						if(isset($room_status['start_date']) && $room_status['start_date']!=''){
							...
							$rooms[$room_id]['time_out'] = ($room_status['house_status']=='REPAIR')?(Date_Time::to_time($room_status['end_date'])+86399):$room_status['time_out'];
						}
						$rooms[$room_id]['price'] = System::display_number($room_status['price']);
						$rooms[$room_id]['can_book'] = 1;
                        $rooms[$room_id]['change_room_from_rr'] = 0;
						switch($room_status['status'])
                        {
                            ...
                            case 'DAYUSED':
                                ...
                                if($room_status['change_room_from_rr']!=''){
                                    $rooms[$room_id]['change_room_from_rr']=$room_status['change_room_from_rr'];
                                }
		...
		foreach($room_overdues as $room_overdue)
		{
			...
			{
				if($room_overdue['status']=='CHECKIN')
				{
		...
		{	
			/** Kimtan tinh toan trang thai phong phai de o day ms chinh xac **/
            if($room['house_status']=='REPAIR'){
				if($room['is_virtual'] == 0)
                {
            ...
            switch($room['status'])
            {
                ...
                case 'OCCUPIED':
					if($room['is_virtual'] == 0)
                    {
                        ...
                        $this->map['total_available_room']--;
                        if(Date_Time::to_time(date('d/m/Y',$room['time_in'])) == Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year))
                        {
                            if($room['change_room_from_rr']!=''){
                                $this->map['total_change_room_today']++;
                            }else{
                       ...
                       if(date('d/m/Y',$room['time_out']) == ($this->day.'/'.$this->month.'/'.$this->year))
                       {
  						    if($room['is_virtual'] == 0)
                            {
                                $this->map['total_checkout_today_room']++;
            ...
            /** end Kimtan tinh toan trang thai phong phai de o day ms chinh xac **/
            if(isset($room['reservations'])){
				...
				foreach($room['old_reservations'] as $k=> $value){
					if($k == 0 && $value['status']=='CHECKOUT'){
                        ...
                        if($room['is_virtual'] == 0)
                            $this->map['total_checkout_today_room'] ++; /** Tan them 15/01/18 **/
						//unset($room['old_reservations'][$k]);
					}
                    /** Tan them 15/01/18 **/
                    if($k != 0){ /** th cong them cac cham nho ben duoi phong **/
						if($room['is_virtual'] == 0)
                        {
                            if($value['status']=='BOOKED')
                            $this->map['total_booked_room']++;
                            elseif($value['status']=='OVERDUE_BOOKED')
                                $this->map['total_overdue_booked_room'] ++;
                            elseif($value['status']=='EXPECTED_CHECKOUT')
                                $this->map['total_checkout_today_room'] ++;
	...
	function get_rooms($in_date = false,$is_virtual = false)
	{
	    if($is_virtual){
	    ...
	    /** Manh: them de check phan khu so do phong **/
        $cond_area = '';
        $cond_history = '';
  ...
  order by 
    ...
    // end
    /** Tan them ham dem trang thai 12/01/18 **/
    function get_status_new()
    {
		$room_status = array();
        $room_status['arr_room'] = 0; /** phong den **/
        $room_status['autual_checkin_room'] = 0; /** phong thuc te CI **/
        $room_status['autual_checkout_room'] = 0; /** phong thuc te CO **/
        $room_status['occ_room'] = 0; /** phong luu **/
        $room_status['dep_room'] = 0; /** phong di **/
        $room_status['dayused_room'] = 0; /** phong day_used **/
		...
		$sql2='
				...
				WHERE 
					rs.in_date >= \''.Date_Time::to_orc_date(($this->day.'/'.$this->month.'/'.$this->year)).'\'
                    AND rs.in_date <=\''.Date_Time::to_orc_date(($this->day.'/'.$this->month.'/'.$this->year)).'\'
                    and reservation.portal_id = \''.PORTAL_ID.'\'
    ...
    /** end Tan them ham dem trang thai 12/01/18 **/
}
?>

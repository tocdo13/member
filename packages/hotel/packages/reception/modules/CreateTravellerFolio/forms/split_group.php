...
/*--------------------------send mail--------------------------*/      
	...
	{	
        ...
        /** tach dat coc nhom theo tung lan dat coc **/
        $sql_dps_group = 'Select id, amount, time, exchange_rate from payment where type_dps=\'GROUP\' and reservation_id='.$id;
        $deposits_group = DB::fetch_all($sql_dps_group);
        //System::debug($deposits_group);
        if($deposits_group)
        {
            foreach($deposits_group as $d_key=>$d_value)
            {
                $percent = 100;
                $status = 0;
                $amount = $d_value['amount']*$d_value['exchange_rate'];
                $items['DEPOSIT_GROUP_'.$d_key]['net_amount'] = System::display_number($amount);
                if(isset($folio_other['DEPOSIT_GROUP_'.$d_key]))
                {
					if($folio_other['DEPOSIT_GROUP_'.$d_key]['percent'] == 100 and $folio_other['DEPOSIT_GROUP_'.$d_key]['total_amount'] >= round($amount,2))
                    {
						$status = 1;
					}
                    else
                    {
						$percent = 100 - $folio_other['DEPOSIT_GROUP_'.$d_key]['percent'];
						$amount = $amount - $folio_other['DEPOSIT_GROUP_'.$d_key]['amount'];
					}
				}
                $items['DEPOSIT_GROUP_'.$d_key]['id'] = $d_key;
				$items['DEPOSIT_GROUP_'.$d_key]['type'] = 'DEPOSIT_GROUP';
				$items['DEPOSIT_GROUP_'.$d_key]['service_rate'] = 0;
				$items['DEPOSIT_GROUP_'.$d_key]['tax_rate'] = 0;
				$items['DEPOSIT_GROUP_'.$d_key]['date'] = Date('d/m/Y',$d_value['time']);
				$items['DEPOSIT_GROUP_'.$d_key]['rr_id'] = $id;
				$items['DEPOSIT_GROUP_'.$d_key]['percent'] = $percent;
				$items['DEPOSIT_GROUP_'.$d_key]['status'] = $status;
				$items['DEPOSIT_GROUP_'.$d_key]['amount'] = number_format($amount,2);
				$items['DEPOSIT_GROUP_'.$d_key]['description'] = Portal::language('deposit_for_group');
                ...
                $reservation_rooms['GROUP']['change_room_from_rr'] = '';
                $reservation_rooms['GROUP']['change_room_to_rr'] = '';
                $reservation_rooms['GROUP']['items'] = $items;
            }
        }
        /** and tach dat coc nhom theo tung lan dat coc **/
      //tieu binh                     
        $all_service = array();      
        ...
        { 
			...
			if(Url::get('folio_id'))
            {
				...
				foreach($order_splits as $i => $order)
                {
                    ...
                    /** start:KID them de truong hop khong co ngay thi khong hien thi cho th DEPOSIT **/
                    if($order['date_use']!='')
                    {
                    ...
                    {
                        $order_splits[$i]['date'] = '';
                        $order_splits[$i]['full_date'] ='';
                    }
                    /** end:KID them de truong hop khong co ngay thi khong hien thi cho th DEPOSIT **/
                }
                /** THL **/

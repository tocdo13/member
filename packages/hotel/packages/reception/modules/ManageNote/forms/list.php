<?php
class ListNoteForm extends Form
{
    function ListNoteForm()
    {
        Form::Form('ListNoteForm');
    ...
    function draw()
    {
        ...
        if(Url::get('from_date'))
        {
            ...
            $cond .= ' and reservation_note.create_time >= '.$time_from.' ';
        }
        if(Url::get('to_date'))
        {
            ...
            $cond .= ' and reservation_note.create_time <= '.$time_to.' ';
        }
        $item_per_page = 50;
		$sql = 'SELECT count(*) AS acount FROM reservation_note where '.$cond.' ';
        ...
        {
            $items[$k]['stt'] = $i++;
            $items[$k]['confirm'] = ($v['confirm']?Portal::language('Yes'):Portal::language('No'));

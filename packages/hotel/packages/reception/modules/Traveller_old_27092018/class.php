	...
	function Traveller($row)
	{
		...
		if(User::can_view(false,ANY_CATEGORY))
		{
			...
			if(
				...
				or !URL::check('cmd')
                or
                (URL::check(array('cmd'=>'export')) and User::can_add(false,ANY_CATEGORY))
				or !URL::check('cmd')
                or
                (URL::check(array('cmd'=>'export_xml')) and User::can_add(false,ANY_CATEGORY))
				or !URL::check('cmd')
			)
			{
            ...
            elseif((URL::get('cmd')) AND (URL::get('cmd')=='import_excel')){
                require_once 'forms/import_excel.php';
			     $this->add_form(new ImportExcelTravellerForm());
            }
			else
			{

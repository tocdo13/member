...
class ReportKeyForm extends Form
{
    ...
    function on_submit()
	{
        if(isset($_REQUEST['get_delete']) && $_REQUEST['get_delete']!='')
        {
            if(!empty($_REQUEST['check_list']))
            {
                ...
                DB::delete('manage_key_salto',$conditions);
            }
        }
	}
	function draw()
	{	   
        ...
        //xoa di tat ca nhung key co thoi gian tao the la 90 ngay va phong da duoc checkout
        ...
        /** Thuc hien chuc nang Phan trang cho report 
        B1: include thuc hien paging 
        B2: dem tong so dong de phan trang $count['count']& khai bao so dong hien thi  $item_per_page=100
	   B3: goi ham paging
	   B4: hien thi sang layout: $this->map['paging'] = $paging;
	   **/
        ...
        $sql ="
                ...
                INNER JOIN manage_key_salto ON reservation_room.id=manage_key_salto.reservation_room_id
                WHERE $cond
                ORDER BY manage_key_salto.create_time desc
                ";
    ...
    function list_key_r_r($r_r_id,$start=0,$end=0)
    {
        ...
        if($start!=0 && $end!=0)
        {
            $cond .=' AND manage_key_salto.create_time >='.$start.' AND manage_key_salto.create_time <='.($end + 86400);
        }
        
        $sql = 'SELECT manage_key_salto.id,manage_key_salto.begin_time,
                    manage_key_salto.end_time,manage_key_salto.create_time,
                    manage_key_salto.create_user,
                    manage_key_salto.number_keys,
                    manage_key_salto.type
                FROM manage_key_salto where manage_key_salto.reservation_room_id='.$r_r_id.$cond .'
                ORDER BY manage_key_salto.create_time desc';
        ...
        if(!empty($items_key))
        {
            ...
            foreach($items_key as $key=>$value)
            {
                ...
                $items_key[$key]['type'] = $items_key[$key]['type']=='CN'?'first card':'late card';          
            } 
        }
        return $items_key;
    }
    ...
    {
        $sql ="SELECT manage_key_salto.id
                FROM manage_key_salto
                LEFT JOIN reservation_room ON manage_key_salto.reservation_room_id=reservation_room.id WHERE reservation_room.id is null";
        ...
        if(trim($conditions)!='')
            DB::query("DELETE FROM manage_key_salto WHERE ".$conditions);
    }
    function remove_key_expiry_date()
    {
        //lay ra nhung key co create qua 90 ngay so voi ngay hien tai va trang thai phong = checkout 
        $now =  mktime(0,0,0,date('m'),date('d'),date('Y'));
        $past = $now - (90*24*60*60);
        $sql ="SELECT manage_key_salto.id
            FROM manage_key_salto 
            INNER JOIN reservation_room ON manage_key_salto.reservation_room_id=reservation_room.id AND (reservation_room.status='CHECKOUT' OR reservation_room.status='CANCEL')
            WHERE manage_key_salto.create_time<=$past";
        $ids = DB::fetch_all($sql);
        $conditions ='';
        foreach($ids as $key=>$value)
        {

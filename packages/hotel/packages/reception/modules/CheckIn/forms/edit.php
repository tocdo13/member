...
class EditReservationForm extends Form
{
	function EditReservationForm()
	{
		...
		$this->add('reservation_room.adult',new IntType(false,'invalid_adult','1','1000')); 
		...
		if($this->check())
		{
			...
			$sql = '
				SELECT
					...
					,traveller.passport ,traveller.visa ,traveller.note 
	...
	{	
		...
		if(!isset($_REQUEST['mi_reservation_room']))
		{
			$sql = '
				select
					...
					,reservation_room.room_id ,reservation_room.room_level_id 
			...
			$room_status = DB::fetch_all('
				SELECT
					...
					change_price,reservation_room_id,in_date,room_id
				FROM 
					room_status 
				WHERE 
			...
			$reservation_arr = get_reservation(Url::iget('id'));
			...
			{		
				...
				$mi_reservation_room[$key]['deposit'] = System::display_number($value['deposit']);
				$change_price_arr = array();
		...
		{		
			$sql = '
				select
					...
					,country.name_2 as nationality_id
					,reservation_room.reservation_id
					,reservation_traveller.reservation_room_id
					,CASE WHEN room.id is not null THEN room.name ELSE reservation_traveller.temp_room END as mi_traveller_room_name
					,CASE WHEN room.id is not null THEN concat(room.id,concat(\'-\',to_char(reservation_room.departure_time,\'DD/MM/YYYY\'))) ELSE \'\' END as traveller_room_id
				...
				where
					reservation_traveller.reservation_id='.Url::iget('id')
					.(URL::get('reservation_room_id')?' and reservation_room.id=\''.URL::get('reservation_room_id').'\'':'').'
				order by
		...
		$this->map +=array(
			...
			'nationalities'=>DB::fetch_all('select id, name_2 as name from country where 1=1 order by name_2'),

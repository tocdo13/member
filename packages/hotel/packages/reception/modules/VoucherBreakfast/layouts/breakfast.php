...
<div>
    <form name="searchForm" method="POST" onsubmit="return checkSubmit();" style="width: 100%;">
        <div style="margin: 50px 50px; width:100%;">
            <div style="float: left; margin-right: 10px;">
                <label for="re_id">[[.recode.]] : </label>
                <input name="re_id" type="number" id="re_id" />        
            </div>
            <div  style="float: left; margin-right: 10px;">
                <label for="re_id">[[.room_status.]] : </label>
                <select name="status" id="status"  >
                    <option value="all">[[.all.]]</option>
                    <option value="new_traveller">Khách đến trong ngày</option>
                    <option value="old_traveller">Khách ở lưu</option>    
                </select>     
            </div>
            <div  style="float: left; margin-right: 10px;">
                <label for="re_id">[[.in_date.]] : </label>
                <input name="in_date" type="text" id="in_date" readonly=""  value="<?php if(!isset([[=in_date=]])) echo date("d/m/Y"); else echo [[=in_date=]]; ?>"/>        
            </div>
            <div>
...
<div id="button_print" style="float: right; margin-bottom:20px; margin-right:20px">
    ...
    <button type="button" onclick="print_group();" style="margin-left: 10px;">[[.print.]]</button>
</div>
<div style="clear: both;"></div>
<div id="single_print_div">
      <!--LIST:list_customer-->  
        <?php
            if((date("H")*3600+date("i")*60+date("s"))>([[=end_time_breakfast=]])) //|| (date("H",[[=list_customer.time_in=]])*3600+date("i",[[=list_customer.time_in=]])*60+date("s",[[=list_customer.time_in=]]))>[[=end_time_breakfast=]])
            {
                [[=list_customer.time_in=]] = Date_Time::to_time(date("d/m/Y",time()+86400));
            }
            // Truong hop ngay in toi thieu la ngay hien tai
            
        ?>
         <div style="page-break-after: always;"> 
           <table class="print_table" width="25%" align="left">
            <tr class="no-border">
                <td width="100%">
                      <div style="font-size: 13px; padding:10px;">
                            <div>
                                ...
                                <div style="text-align: right; width:100%" class="no_seri">No.#<span class="no_[[|list_customer.reservation_room_id|]]"></span></div>
                                <div class="h_v">
                                    <div> 
                                ...
                                <div>
                                    <!--<div class="center" style="margin-top: 16px;">
                                        <label>Nguồn khách/ Agent: </label><span style="font-weight: bold;">[[|customer_name|]]</span>
                                    </div>-->
                                    <div style="margin-top:5px;" class="center">
                                        <label>Tên khách/ GuestName : </label><span class="guest_name" style="word-break: break-all; font-weight: bold; width:100%;">[[|list_customer.last_name|]]</span>
                                    </div>
                                    <div style="margin-top:5px; width:100%;" class="center">
                                        <?php
                                            if(isset([[=list_customer.is_child=]]) && [[=list_customer.is_child=]]!=1)
                                            {
                                        ?>
                                    ...
                                    <div  style="margin-top:5px;" class="center">
                                        ...
                                        <?php if(!isset([[=is_group=]])){
                                         ?>
                                         <span style="font-weight: bold;">[[|room_name|]]</span>
                                         <?php   
                                        } 
                                        else{
                                        ?>
                                          <span style="font-weight: bold;">[[|list_customer.room_name|]]</span>  
                                        <?php    
                                        }
                                        ?>                               
                                        </div>
                                ...
                                <div class="center c2" style="margin-top: 5px;">
                                    <div>
                                        ...
                                        <input id="get_time_print_<?php echo $i; ?>" name="get_time_print_<?php echo $i; ?>" type="text" readonly=""  class="date-select none1" style="width: 120px; float:left;"/>
                                        <p class="input_time_<?php echo $i; ?> none" style="font-weight: bold;"></p>
                                        <p style="display: none;" class="time_in"><?php echo date('d/m/Y',[[=list_customer.time_in=]]); ?></p>
                                        <p style="display: none;" class="time_in_timestamp">[[|list_customer.time_in|]]</p>
                                        <p style="display: none;" class="time_out"><?php echo date('d/m/Y',[[=list_customer.time_out=]]); ?></p>
                                        <p style="display: none;" class="time_out_timestamp">[[|list_customer.time_out|]]</p>
                                    </div>
                                    <div style="clear: both;"></div>
                                </div>
                                ...
                                <div class="reservation_room_[[|list_customer.reservation_room_id|]]" stt="<?php echo $i; ?>" guest_name="[[|list_customer.last_name|]]" is_child="[[|list_customer.is_child|]]" reservation_room_id="[[|list_customer.reservation_room_id|]]" style="width: 100%; text-align:center;">
                                    <svg id="barcode_<?php echo $i; ?>" style="width:150px;height:75px;"></svg>
                                </div>
                            </div>
                        </div> 
                    </td> 
...
?>
    <p style="color: red; font-weight: bold; padding-left: 40px;">Phòng chưa có khách hoặc chưa nhập số lượng người lớn, trẻ em hoặc không có xác nhận ăn sáng. Xin vui lòng kiểm tra lại !</p>
    <script>
...
<?php
    // Truong hop ngay in toi thieu la ngay hien tai
    if((date("H")*3600+date("i")*60+date("s"))>([[=end_time_breakfast=]]))
    {
        [[=time_in=]] = Date_Time::to_time(date("d/m/Y",time()+86400));
    }
?>
var miny= <?php echo isset([[=time_in=]])?date('Y',[[=time_in=]]):"''";?>;
var minm= <?php echo isset([[=time_in=]])?(intval(date('m',[[=time_in=]]))-1):"''";?>;
var mind = <?php echo isset([[=time_in=]])?date('d',[[=time_in=]]):"''";?>;
var maxy = <?php echo isset([[=time_out=]])?date('Y',[[=time_out=]]):"''";?>;
var maxm= <?php echo isset([[=time_out=]])?(intval(date('m',[[=time_out=]]))-1):"''";?>;
var maxd = <?php echo isset([[=time_out=]])?(date('d',[[=time_out=]])):"''";?>;
var count = <?php if(isset($i)) echo $i; else echo "0"; ?>;
jQuery("#in_date").datepicker({minDate: new Date(<?php echo date("Y"); ?>,<?php echo intval(date("m")-1); ?>,<?php echo date("d"); ?>)});
   if(count!=0){ 
    for(var i=1; i<count; i++){
     ...
     jQuery('#time_group').change(function(){
        ...
        if(date.getDate()==7){ // Ngay chu nhat thoi gian an sang tu 6h00 - 10h00;
            start_time_breakfast = date_timestamp+[[|start_time_breakfast|]];
            end_time_breakfast = date_timestamp+[[|end_time_breakfast|]];
        }
        else{ // Ngay binh thuong thoi gian an sang tu 6h00 - 9h00;
            start_time_breakfast = date_timestamp+[[|start_time_breakfast|]];
            end_time_breakfast = date_timestamp+[[|end_time_breakfast|]];
        }
        for(var i=1; i<count; i++){
        ...
        if(confirm('[[.Are_you_sure.]]?'))
        {
            ...
            jQuery.ajax({
                ...
                success : function(data){
                    ...
                    count_2 = <?php if(isset($i)) echo $i; else echo "0"; ?>;
                    ...
                    jQuery("table.warrap_2").css("border",'none');
                     jQuery(".print_table").each(function(){
                        ...
                        jQuery(this).removeAttr("width");
                        jQuery(this).removeAttr("align");
                        //jQuery(this).find(".no_seri span").html(stt);
                        //stt++;
                    });
...
function checkSubmit(){
    var date = jQuery("#in_date").val();
    if(!date){
        alert("Bạn phải chọn ngày in phiếu !");
        return false;
    }
    return true;
}

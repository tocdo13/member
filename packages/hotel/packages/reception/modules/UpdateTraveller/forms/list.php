...
class UpdateTravellerForm extends Form{
    ...
    {	
        ...
        $entry_target=DB::fetch_all('select code as id,CONCAT(entry_purposes.code,CONCAT(\' - \',entry_purposes.name)) as name from entry_purposes order by code ASC');
        $entry_target_options = '';
        foreach($entry_target as $k=>$v)
        {
            $entry_target_options .= '<option value="'.$v['id'].'">'.$v['name'].'</option>';
        }
        $this->map['entry_target_options'] = $entry_target_options;

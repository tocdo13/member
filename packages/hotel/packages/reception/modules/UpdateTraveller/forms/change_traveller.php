...
class UpdateTravellerChangeForm extends Form{
			...
			if($this->check()){
				...
				foreach($reservation_room_old as $key => $reservation_room){
					if($rr_id_to==0){
                        ...
                        unset($reservation_room['note']);
                        unset($reservation_room['note_change_room']);
                        unset($reservation_room['change_room_from_rr']);
                        unset($reservation_room['change_room_to_rr']);
                        unset($reservation_room['old_arrival_time']);
	...
	function get_rooms($id,$room_id){
        ...
        $sql = "SELECT room_status.id, room_status.house_status,room_status.room_id,room.name FROM room_status INNER JOIN room ON room_status.room_id = room.id WHERE in_date='".Date_Time::to_orc_date(date("d/m/Y"))."' ORDER BY room_status.id";
        $room_house_status = DB::fetch_all($sql);
		//System::debug($room_house_status);
        foreach($rooms as $k=>$room){
            ...
            foreach($room_house_status as $key=>$value)
            {
                if($value['room_id'] == $room['id']){
                    if($value['house_status']=='DIRTY' && strpos($rooms[$k]['name'],"Dirty")===FALSE)
                    {
                         $rooms[$k]['name'] .= '---Dirty';   
                    }
                    else if($value['house_status']=='CLEAN' && strpos($rooms[$k]['name'],"Dirty")!==FALSE)
                    {
                        $rooms[$k]['name'] = str_replace('---Dirty',"",$rooms[$k]['name']);
                        //System::debug($rooms[$k]['name']);
                    }
                    if(strpos($rooms[$k]['name'],"Dirty")==true)
                    {
                        unset($rooms[$k]);
                    }
                }

...
<p><?php
    if(count([[=room_double=]])>0){
        ...
        echo '<p style="color:red">Các phòng dưới đây bắt buộc phải nhập thời gian checkin checkout</p><table class="table_error">
        ...
        foreach([[=room_double=]] as $key=>$val){
            echo '<tr>
                    <td rowspan="2" style="padding:5px 10px;">'.$key.'</td>';
            $i=0;        
            foreach($val as $v){
                    $i++;
                  echo '

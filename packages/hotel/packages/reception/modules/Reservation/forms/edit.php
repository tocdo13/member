...
class EditReservationForm extends Form
{
	...
	function on_submit()
	{
       ...
       {
            if($value['room_id'] !='' AND $value['status'] =='CHECKIN' and ( $value['id']=='' or ($value['id']!='' and DB::exists('select id from reservation_room where id='.$value['id'].' and status!=\'CHECKIN\'')) ) )
            {
        ...
        if(!isset($_REQUEST['mi_reservation_room']))
		{
			...
			foreach($mi_reservation_room as $key=>$value)
			{
                ...
                **/
                ...
                $mi_reservation_room[$key]['confirm'] = $value['confirm'];
                ...
                $mi_reservation_room[$key]['cancel_note'] = $value['cancel_note'];

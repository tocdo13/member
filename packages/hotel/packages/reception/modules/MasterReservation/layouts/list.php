        ...
        <table style="width: 100%;" cellpadding="5" border="0" bordercolor="#EEEEEEE">
            <tr>
                <td style="text-align: center;">
                    <b style="text-transform: uppercase; color: #171717; font-size: 30px; font-weight: normal; line-height: 20px;">RESERVATION</b><a href="#" class="icon-button w3-text-white" style="float: right; text-transform: uppercase; text-decoration: none;" onclick="FunSearch();"><i class="fa fa-fw fa-search"></i>[[.search.]]</a>
                </td>
            </tr>
            <tr style="border: 1px solid lightgray;">
                <td>
                    <div style="text-align: center;">
                        ...
                        <input name="from_date" type="text" id="from_date" onchange="CheckDate('from_date');" onkeypress="if(event.keyCode==13){FunSearch();}" readonly="readonly" style="padding: 5px; border: 1px solid #DDDDDD; width: 72px;" />
                        ...
                        <input name="to_date" type="text" id="to_date" onchange="CheckDate('to_date');" onkeypress="if(event.keyCode==13){FunSearch();}" readonly="readonly" style="padding: 5px; border: 1px solid #DDDDDD; width: 72px;" />
                        ...
                        <input name="reservation_id" type="text" id="reservation_id" autocomplete="OFF" onfocus="Autocomplete('RESERVATION_ID');" onkeypress="if(event.keyCode==13){FunSearch();}" style="padding: 5px; border: 1px solid #DDDDDD; width: 60px;" />
                        ...
                        <input name="room_name" type="text" id="room_name" onfocus="Autocomplete('ROOM_NAME');" onkeypress="if(event.keyCode==13){FunSearch();}" autocomplete="OFF" style="padding: 5px; border: 1px solid #DDDDDD; width: 50px;" />
                        ...
                        <select id="status" name="status" style="padding: 5px; border: 1px solid #DDDDDD; width: 80px;"><option value="">[[.select.]]</option><option value="BOOKED">BOOKED</option><option value="CHECKIN">CHECKIN</option><option value="CHECKOUT">CHECKOUT</option></select>
                        <label for="traveller_name">[[.traveller.]]:</label> 
                        <input name="traveller_name" type="text" id="traveller_name" onfocus="Autocomplete('TRAVELLER_NAME');" onkeypress="if(event.keyCode==13){FunSearch();}" autocomplete="OFF" style="padding: 5px; border: 1px solid #DDDDDD; width: 10%;" />
                        ...
                        <input name="passport" type="text" id="passport" onfocus="Autocomplete('PASSPORT');" onkeypress="if(event.keyCode==13){FunSearch();}" autocomplete="OFF" style="padding: 5px; border: 1px solid #DDDDDD; width: 7%;" />
...
<script>
    ...
    function CheckDate($id){
        if(jQuery("#from_date").val()!='' && jQuery("#to_date").val()!='' && count_date(jQuery("#from_date").val(),jQuery("#to_date").val())<0){
            if($id){
                alert('[[.start_date_must_be_less_than_end_date.]] !');
                if($id=='from_date')
                                ...
                                content += '<div id="TIME_'+html['time']+'_'+count+'">';
                                    content += '<table cellpadding="3" border="1" bordercolor="#EEEEEEE" style="width: 100% !important;">';
                                        content += '<tr class="w3-light-gray" style="text-align: center; height: 30px; text-transform: uppercase;">';
                                            ...
                                            content += '<th class="w3-border">[[.recode.]]</th>';
                                            content += '<th class="w3-border">[[.customer_name.]]</th>';
                                            content += '<th class="w3-border">[[.room.]]</th>';
                                            content += '<th class="w3-border">[[.room_level.]]</th>';
                                            content += '<th class="w3-border">[[.time_in.]]</th>';
                                            content += '<th class="w3-border">[[.time_out.]]</th>';
                                            content += '<th class="w3-border">[[.foc.]]</th>';
                                            content += '<th class="w3-border">[[.foc_all.]]</th>';
                                            content += '<th class="w3-border">[[.description.]]</th>';                                         
                                            content += '<th class="w3-border">[[.status.]]</th>';
                                            content += '<th class="w3-border">[[.create_folio.]]</th>';
                                            content += '<th class="w3-border">[[.add_service.]]</th>';
                                            content += '<th class="w3-border">[[.edit.]]</th>';
                                        content += '</tr>';
                                        for(var i in html['data']){
                                            content += '<tr class="DataItems">';
                                                ...
                                                content += '<td style="text-align: right; color: red;">'+html['data'][i]['status']+'<select id="fast_status_'+html['data'][i]['id']+'_'+html['time']+'_'+count+'" onchange="if(this.value!=\'\' && confirm(\'[[.are_you_sure.]] \')){FastOperation('+html['data'][i]['id']+','+html['time']+','+count+','+html['data'][i]['reservation_id']+','+html['data'][i]['customer_id']+',\'STATUS\');}else{this.value=\'\';}" style="padding: 3px; border: 1px solid #DDDDDD; margin-left: 5px; width: 75px; fload: right;"><option value="">[[.select.]]</option>'+html['data'][i]['fast_status_option']+'</select></td>';
                                                content += '<td style="text-align: center;"><select id="fast_operation_'+html['data'][i]['id']+'_'+html['time']+'_'+count+'" onchange="FastOperation('+html['data'][i]['id']+','+html['time']+','+count+','+html['data'][i]['reservation_id']+','+html['data'][i]['customer_id']+',\'FOLIO\','+(html['data'][i]['mice']==null?'\'\'':html['data'][i]['mice'])+');" style="padding: 3px; width: 110px; border: 1px solid #DDDDDD;"><option value="">[[.select.]]</option>'+html['data'][i]['fast_operation_option']+'</select></td>';
                                                content += '<td style="text-align: center;"><select id="add_service_'+html['data'][i]['id']+'_'+html['time']+'_'+count+'" onchange="AddService('+html['data'][i]['id']+','+html['time']+','+count+','+html['data'][i]['reservation_id']+','+html['data'][i]['customer_id']+');" style="padding: 3px; width: 110px; border: 1px solid #DDDDDD;"><option value="">[[.select.]]</option>'+html['data'][i]['add_service_option']+'</select></td>';
                                            ...
                                            if(to_numeric(html['data'][i]['count_child'])>0){
                                                content += '<tr class="w3-light-gray w3-text-orange">';
                                                    content += '<th colspan="4" style="text-align: center;">[[.traveller.]]</th>';
                                                    content += '<th>[[.nationality.]]</th>';
                                                    content += '<th>[[.time_in.]]</th>';
                                                    content += '<th>[[.time_out.]]</th>';
                                                    content += '<th colspan="2">[[.passport.]]</th>';
                                                    content += '<th colspan="4">[[.address.]]</th>';                                                                                                        
                                                    content += '<th>[[.history_of_stay.]]</th>';
                                ...
                                if(!$reload){
                                    if(last_time_search.length>0){
                                        ...
                                        history += '<fieldset style="border: none; border-top: 1px solid #EEEEEE;">';
                                            history += '<legend style="color: #555555;">[[.history_search.]] <i class="fa fa-fw fa-remove" style="cursor: pointer; color: #fdc5c5;" onclick="jQuery(\'.history_items\').remove(); jQuery(\'#data_search_old\').css(\'display\',\'none\');"></i></legend>';

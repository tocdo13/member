	...
	function draw()
	{
        ...
        $check_autual_checkin = DB::fetch_all
                ...
                where
                    ...
                    and reservation_room.time_in <= \''.(Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year)+ 86400 + (0)).'\'
			...
			{	
				...
				}
				{
					...
					if($room_status['room_id']!=$room_id)
					{
                        ...
                        {
                            if($room_status['change_room_from_rr'] == '' && $room_status['change_room_to_rr'] == '' && $room_status['time_in'] >= (Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year)+(0)))
                               {
                                    $this->map['total_checkin_today_room']++;
                               }
                        if(
                            ($room_status['reservation_status']=='CHECKIN' and((int)$room_status['time_in'] - (int)(Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year)+(0)) > 0)) 
                             or
                             (
                                 $room_status['reservation_status']=='CHECKOUT' 
                                 and $room_status['departure_time']!=$this->day.'/'.$this->month.'/'.$this->year and( (int)$room_status['time_in'] - (int)(Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year)+(0)) > 0) 
                             )
                           )
                                {
						 ...
						 if ($room_status['change_room_to_rr'] != '' and $room_status['change_room_from_rr'] == '')
                            {
                                if($room_status['time_in'] >= (Date_Time::to_time($this->day.'/'.$this->month.'/'.$this->year)+(0)))
                                {
        ...
        //luu nguyen giap add room can booked and start date booked, end date booked
            
           $sql_booked = 'SELECT room.name as id,min(room_status.in_date) as min_date,max(room_status.in_date) as max_date 
                        FROM room_status,room
                        WHERE room_status.room_id=room.id and room.portal_id=\''.PORTAL_ID.'\' and room.name=\''.$room['name'].'\'
                        AND status=\'BOOKED\' and in_date >=to_char(sysdate, \'DD-MON-YY\')  
                        GROUP BY room.name';
            $result = DB::fetch_all($sql_booked);
            if(count($result)>0)
            {
                //is_booked = 1 va start_booked, end_booked
                $rooms[$key]['is_booked'] =1;
                 $rooms[$key]['start_booked'] =Date_Time::convert_orc_date_to_date($result[$room['name']]['min_date'],"/");
                 $rooms[$key]['end_booked'] =Date_Time::convert_orc_date_to_date($result[$room['name']]['max_date'],"/");
            }
            else
            {

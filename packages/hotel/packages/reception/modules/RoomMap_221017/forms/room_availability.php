...
class RoomAvailabilityForm extends Form
{
	function RoomAvailabilityForm()
	{
		...
		$this->link_js('packages/hotel/packages/reception/modules/RoomMapNew/room_map.js');
	...
	function get_rooms()
	{
		...
		$sql = '
			...
			where
				room.portal_id = \''.PORTAL_ID.'\' and (room_status.room_id not in ( 
                    select room_status.room_id from room_status where  \''.$from_date.'\'<=room_status.in_date 
                    and room_status.in_date<=\''.$to_date.'\' and room_status.house_status=\'REPAIR\'

...
class ImportTravellerForm extends Form{
    ...
    function parse_excel($excel,$room_double)
	{
        ...
        foreach($row_content as $key=>$val){
            ...
            $time_checkin='';
            $time_checkout='';
             //so sanh cac truong province,cmnd,hang khach,quootich voi csdl
            $province = DB::fetch_all('select UPPER(code) as id,id as province_id from province');
            //System::debug($province);die;
            ...
            $room_id =DB::fetch(' select id,name from room where name = \''.$val['mi_traveller_room_name'].'\' and portal_id = \'#default\'');

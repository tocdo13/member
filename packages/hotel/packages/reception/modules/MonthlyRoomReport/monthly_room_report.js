				...
				if(to_numeric(items_js[key]['days'][k]['start_time']) <= to_day && items_js[key]['days'][k]['reservation_status'] != 'REPAIR' && items_js[key]['days'][k]['reservation_status'] != 'HOUSEUSE' && items_js[key]['days'][k]['reservation_status'] != 'BOOKED' && items_js[key]['days'][k]['reservation_status'] != 'LT' && to_numeric(items_js[key]['days'][k]['end_time'])>to_day)
                {
						...
						jQuery(this).draggable(
                        {
							...
							,stop: function()
                            {
                                ...
                                var get_arr_id = this.id.split('_')[1];
                                //console.log(get_arr_id);
                                if(to_numeric(rooms_js[get_arr_id]['days'][start_time]['do_not_move'])==1)
                                {
                                    alert('phong dang duoc giu boi TK: '+rooms_js[get_arr_id]['days'][start_time]['user_do_not_move']+''+((rooms_js[get_arr_id]['days'][start_time]['note_do_not_move']!=null)?' \n Ly do: '+rooms_js[get_arr_id]['days'][start_time]['note_do_not_move']:'')+' \n, khong the thuc hien thao tac nay!');
                                    returnOldPosition();HideDialog('dialog');jQuery('#mask').hide();
                                    return false;
                                }
...
{
    jQuery('#option_select').show(300);
    
    if(event.ctrlKey || event.metaKey || jQuery('#option_selected').val() == 1)
    {
        if(orgDate != -1)// && orgRoom_id == room_id
        {
            var count_n = 0;
            for(var item_n in items_org)
            {
                var cur_room_id = to_numeric(items_org[item_n]['id']);
                if(cur_room_id  == orgRoom_id || cur_room_id == room_id)
                {
                    count_n++;
                }   
                
                if(count_n >=1 && count_n <= 2)
                {
                    //deSelectRow(cur_room_id);
                    var minD,maxD;
                    if(orgDate < day)
                    {
                    ...
                    {
                        if(flag[cur_room_id + '_'+i] == 0)
                        {
                            bgr_room(cur_room_id,i,false);
                        }
                    }
                    if(flag[cur_room_id + '_'+day] == 0)
                    {
                        bgr_room(cur_room_id,day,false);
                    }
                }
                if(orgRoom_id == room_id && count_n == 1)
                    break;
                if(count_n >= 2)
                    break; 
            }  
...
function check_invisible()
{
	for(var j in reser_act)
    {
            ...
            if(reser_act[j]=='CHECKOUT')
            {
                ...
                jQuery('#repair').css({'display':'none'});
                jQuery('#un_repair').css({'display':'none'});
				//jQuery('#add td').css({'background':'#D6D6D6','cursor':'text','pointer-events':'auto','color':'#6E6E6E'});
				jQuery('#cancel td').css({'background':'#D6D6D6','cursor':'text','pointer-events':'none','color':'#6E6E6E'});
				jQuery('#checkin td').css({'background':'#D6D6D6','cursor':'text','pointer-events':'none','color':'#6E6E6E'});
                jQuery('#extra_service td').css({'background':'#D6D6D6','cursor':'text','pointer-events':'none','color':'#6E6E6E'});
                jQuery('#assign').css({'display':'none'});
...
function bgr_room(room_id,day,kt)
{
	if(flag[room_id+'_'+day] == 1 && kt==false)
    {
		...
		if(rooms[room_id] != undefined)
        {
            //n?u khÃ´ng cÃ²n Ã´ nÃ o c?a phÃ²ng dc ch?n thÃ¬ xÃ³a phÃ²ng kh?i m?ng
...
function save(rr_id,room_id, start, nights,action,div_id,reservation_status,original_start_time)
{
    ...
    if(reason_change != '')
    {
        /** START GÃ¡n phÃ²ng trÃªn THSD phÃ²ng, doi phong , chon gan phong Sau khi nh?n OK-> load l?i thÃ¬ phÃ²ng du?c gÃ¡n tr? v? v? trÃ­ cu**/
        ...
        /** END GÃ¡n phÃ²ng trÃªn THSD phÃ²ng, doi phong , chon gan phong Sau khi nh?n OK-> load l?i thÃ¬ phÃ²ng du?c gÃ¡n tr? v? v? trÃ­ cu**/

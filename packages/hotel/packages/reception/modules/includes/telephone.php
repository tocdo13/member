...
class TelephoneLib
{
	...
	{
		$path = 'telephone/';
		if(!is_dir($path.substr(PORTAL_ID,1)))
		{
			mkdir($path.substr(PORTAL_ID,1));
		...
		{
			$local_file = date('m').'_'.date('Y').'.dat';
		}
	...
	function get_time_call($start_time,$duration){
		$start_time = str_replace(array(';','´'),array('','0'),$start_time);
	...
	static function update_telephone_daily($file_name)
	{
		if(file_exists($file_name))
		{
			...
			// so thuc
		  	$pattern = '/([0-9\.]+) ([0-9\:]+) ([0-9]+) ([0-9]+)([\s]+)([0-9\:]+) ([0-9]+)([\s]+)([0-9]+)([\s]+)([0-9\.]+)(.*)/';

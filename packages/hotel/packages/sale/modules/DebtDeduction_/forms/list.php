...
class DebtDeductionListForm extends Form
{
    ...
    function draw()
    {
        ...
        $sql = '
                    ...
                    WHERE CUSTOMER_REVIEW_DEBT.PORTAL_ID=\''.PORTAL_ID.'\'
                   
                ';
        $this->map['total'] =  DB::fetch($sql,'count');   
        $sql = 'SELECT * FROM
                (
                    SELECT CUSTOMER_REVIEW_DEBT.*,customer.id as cid, customer.name, party.name_1 as name_1,
                    ...
                    WHERE CUSTOMER_REVIEW_DEBT.PORTAL_ID=\''.PORTAL_ID.'\'
                    ORDER BY CUSTOMER_REVIEW_DEBT.id DESC
                 )
                 WHERE

...
class Wedding extends Form
{
    function Wedding()
    {
        ...
        //$this->add('num_people',new IntType(true,'miss_num_people','0','100000000000')); 
        //$this->add('num_table',new IntType(true,'miss_num_table','0','100000000000'));       		 
        //$this->add('table_reserve',new IntType(true,'miss_table_reserve','0','100000000000'));
        //$this->add('product.product_id',new TextType(true,'miss_product_id',0,255));
        //$this->add('service.product_id',new TextType(true,'miss_service_product_id',0,255));
        //$this->add('eating_product.product_id',new TextType(true,'miss_eating_product_id',0,255));
        //$this->add('banquet_room.banquet_room_id',new TextType(true,'miss_banquet_room_id',0,255));
    ...
    function on_submit()
	{
	   if(Url::get('act')=='payment'){ // Khi chọn nút thanh toán thì Save món trước khi gọi module thanh toán
					//Url::redirect_current(array('bar_id'=>$_SESSION['bar_id'],'cmd','arrival_time','id','act'=>'payment','total_amount'=>System::calculate_number(Url::get('total_payment'))));
					echo '<script src="packages/core/includes/js/jquery/jquery.min.1.4.2.js"></script>';
					...
					$tt = 'form.php?block_id='.BLOCK_PAYMENT.'&cmd=payment&id='.Url::get('id').'&type=BAR&portal_id='.PORTAL_ID.'&total_amount=100000000000';
					echo '<script>window.open(\''.$tt.'\',Array(\'payment\',\'payment_for\',80,210,950,500));</script>';
	...
	function draw()
	{
		...
		if(Url::get('action')=='edit')	
		{
			...
			//============================== product ===============================
			$product = BanquetOrderDB::get_reservation_product(1||2||3);
			$i=0;
            ...
            $eating_product_items = BanquetOrderDB::get_reservation_product(2);
            $service_items = BanquetOrderDB::get_reservation_product(3);
			$category_items = BanquetOrderDB::get_reservation_category();
			$banquet_room_items = BanquetOrderDB::get_reservation_banquet_room(2);
			$total_before_tax = 0;

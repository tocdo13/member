...
class Wedding extends Form
{
    ...
    function on_submit()
	{
		if($this->check())
		{
		    //$total = System::calculate_number(Url::get('sum_total'));  
			...
			$party_info = array(
                    ...
                    'cancel_charge_percent'=>Url::get('cancel_charge_percent'),
                    'cancel_charge_amount'=>System::calculate_number(Url::get('cancel_charge_amount')),
                    'table_reserve',
					'party_category',
					'home_phone',
					'party_type'=>1,
                    ...
                    'cut_of_time'=>Url::get('cut_of_time'),
                    'confirm'=>Url::get('confirm')?1:0,
                    'portal_id'=>PORTAL_ID,
			...
			if(Url::get('action')=='edit' and $row = DB::select_id('party_reservation',Url::iget('id')))
			{
					...
					foreach($_REQUEST['mi_product'] as $row=>$row_data)
					{
					    if(isset($row_data['product_id']))
                        {
    						...
    						$list_product .= 'Product: '.$product['product_id'].', Quantity: '.$product['quantity'].', Price: '.$product['price'].'<br>';
    					       //oanh
                        	$product['discount_percent'] = $row_data['discount_percent'];
				...
				{
				    //System::debug($_REQUEST['mi_eating_product']);
//                    exit();
				...
				if(isset($_REQUEST['mi_banquet_room']))
				{
					...
					foreach($_REQUEST['mi_banquet_room'] as $row=>$row_data)
					{
                        ...
                        $banquet_room['discount_percent'] = $row_data['discount_percent'];
			...
			{ 
				...
				if(isset($_REQUEST['mi_product']))
				{
					foreach($_REQUEST['mi_product'] as $row=>$row_data)
					{
                        ...
                        $product['type'] = 1;
                        $product['discount_percent'] = $party_info['discount_percent'];
						$list_product .= 'Product: '.$product['product_id'].', Quantity: '.$product['quantity'].', Price: '.$product['price'].'<br>';
						$id = DB::insert('party_reservation_detail',$product);
					}
				}
	...
	function draw()
	{
		...
		if(Url::get('action')=='edit')	
		{
			...
			foreach($product_items as $key=>$value)
			{
                ...
                $_REQUEST['mi_product'][$key]['quantity'] = System::display_number($value['quantity']);
				//$ttl = $value['price']*($value['quantity']);
                /** oanh **/
                $_REQUEST['mi_product'][$key]['discount_percent'] = $value['discount_percent'];
                $ttl = $value['price']*($value['quantity']) - $value['price']*($value['quantity'])*$value['discount_percent']/100;
                /** End  **/
            ...
            foreach($vegetarian_product_items as $key=>$value)
			{
				...
				//$ttl = $value['price']*($value['quantity']);
                /** oanh **/
                $_REQUEST['mi_vegetarian'][$key]['discount_percent'] = $value['discount_percent'];
                $ttl = $value['price']*($value['quantity']) - $value['price']*($value['quantity'])*($value['discount_percent'])/100;
                /** End  **/
            ...
            foreach($service_items as $key=>$value)
			{
                ...
                $_REQUEST['mi_service'][$key]['quantity'] = System::display_number($value['quantity']);
				//$ttl = $value['price']*($value['quantity']);
                /** oanh **/
                $_REQUEST['mi_service'][$key]['discount_percent'] = $value['discount_percent'];
                $ttl = $value['price']*($value['quantity']) - $value['price']*($value['quantity'])*$value['discount_percent']/100;
                /** End  **/
            ...
            foreach($eating_product_items as $key=>$value)
			{
                ...
                $_REQUEST['mi_eating_product'][$key]['quantity'] = System::display_number($value['quantity']);
				/** oanh **/
                $_REQUEST['mi_eating_product'][$key]['discount_percent'] = $value['discount_percent'];
                $ttl = $value['price']*($value['quantity']) - $value['price']*($value['quantity'])*$value['discount_percent']/100;
                /** End  **/
			...
			foreach($banquet_room_items as $key=>$value)
			{
				...
				$banquet_room_items[$key]['price'] = System::display_number($value['price']);
                $banquet_room_items[$key]['discount_percent'] = $value['discount_percent'];
            ...
            $row['cancel_charge_amount'] = System::display_number($row['cancel_charge_amount']);
			$row['creater'] = DB::fetch('select name_1 as name from party where user_id=\''.$row['user_id'].'\'','name');
		...
		{  
            ...
            $user_data['full_name'];
            $this->map['creater'] = $user_data['full_name'];
            $this->map['customer_name']='';
            $this->map['night_audit_date']='';
            if(time()<=(Date_time::to_time(date('d/m/Y'))+(12*3600)))
            {
                $time_type = 'MORNING';
            }
            elseif(time()<=(Date_time::to_time(date('d/m/Y'))+(18*3600)))
            {
                $time_type = 'AFTERNOON';
            }
            else
		...
		$this->map['time_type_list'] = array(
                ...
                'NIGHT'=>Portal::language('Night')
    		);
        $promotions = DB::select_all('party_promotions','party_type_id = 1'); 
        $this->map['promotions'] = $promotions; 
        /** MICE **/
        $this->map['close_mice'] = 0;
        if(Url::get('action')=='edit' AND $this->map['mice_reservation_id']!='' AND DB::exists('SELECT id FROM mice_reservation WHERE close=1 AND id='.$this->map['mice_reservation_id']))
        {
            $this->map['close_mice'] = 1;
        }
        /** end MICE **/ 
        $this->parse_layout('wedding',$this->map);
	}

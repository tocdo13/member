...
//                    exit();
                ...
                if($_REQUEST['status']=='CHECKOUT')
                {
                    ...
                    $warehouse = DB::fetch('select warehouse_id, warehouse_id_2 
                                                        ...
                                                        where portal_department.portal_id = \''.PORTAL_ID.'\' and portal_department.department_code = \'BANQUET\' '); 
			...
			//if(URL::get('status')=='')
            echo '<script>window.setTimeout("location=\''.Url::redirect_current(array('status'=>URL::get('status'),'just_edited_id'=>$party_id)).'\'",1000);</script>';
			exit();
		}
	}	
	function draw()
	{
		...
		if(Url::get('action')=='edit')	
		{
            ...
            if($row['mice_reservation_id']){
                $row['full_name']=DB::fetch('select customer.name as id from mice_reservation inner join customer on mice_reservation.customer_id=customer.id where mice_reservation.id='.$row['mice_reservation_id'].'','id');
            }
			...
			$this->map = $row;
            /** check mice da tao bill */
            $check_bill_mice = DB::fetch('
                                        SELECT 
                                            count(mice_invoice.id) as count
                                        FROM 
                                            mice_invoice
                                            inner join mice_reservation on mice_reservation.id = mice_invoice.mice_reservation_id
                                            inner join party_reservation on party_reservation.mice_reservation_id = mice_reservation.id
                                        WHERE 
                                            party_reservation.id =\''.Url::get('id').'\'
            ');
            $this->map += $check_bill_mice;
		}
		else
		{  
			$this->map = array();
        ...
        $this->map['use_mice'] = DB::fetch('select mice_use from department where code=\'BANQUET\'','mice_use');
        /** end MICE **/

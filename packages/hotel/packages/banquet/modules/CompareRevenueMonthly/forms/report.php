<?php
class CompareRevenueMonthlyForm extends Form
{
	function CompareRevenueMonthlyForm()
	{
		Form::Form('CompareRevenueMonthlyForm');
	...
	function draw()
	{
        ...
        $prev_cond = $cond . ' '
            				...
            				.(URL::get('from_date')?' and party_reservation.checkin_time>=\''.(Date_Time::to_time(URL::get('from_date')) - 365*24*60*60).'\'':'')
            				.(URL::get('to_date')?' and party_reservation.checkin_time<\''.(Date_Time::to_time(URL::get('to_date'))+86400 - 365*24*60*60).'\'':'')
		...
		;
        //System::debug($rooms);
        $items = $this -> get_items($cond);
        $prev_items = $this -> get_items($prev_cond);
        $all_items = array();
        foreach($items as $key=>$value)
        {
            $all_items[$key]['current'] = $value; 
        }
        foreach($prev_items as $key=>$value)
        {
            $items[$key]['pre'] = $value;
        }
        //System::debug($all_items);
    ...
    function print_all_pages($items)
	{
		...
		if(sizeof($pages)>0)
		{
			$this->group_function_params = array(
                    ...
                    'pre_total_num_meeting'=>0,
                    'pre_total_num_party'=>0,
                    'pre_total_num_party_table'=>0,
                    'pre_total_num_meeting_table'=>0,
                    'pre_total_meeting_num_people'=>0,
                    'pre_total_num_people'=>0,
                    'pre_total_eating'=>0,
                    'pre_total_drinking'=>0,
                    'pre_total_meeting_service'=>0,
                    'pre_total_meeting_eating'=>0,
                    'pre_total_meeting_drinking'=>0,
                    'pre_total_service'=>0,
                    'pre_total_meeting'=>0,
                    'pre_total_party'=>0,
				);
    ...
    function print_page($items, $page_no, $total_page)
	{
		...
		foreach($items as $item)
		{
            ...
            if($temp=System::calculate_number($item['pre']['num_meeting']))
			{
				$this->group_function_params['pre_total_num_meeting'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['num_party']))
			{
				$this->group_function_params['pre_total_num_party'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['num_party_table']))
			{
				$this->group_function_params['pre_total_num_party_table'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['meeting_num_people']))
			{
				$this->group_function_params['pre_total_meeting_num_people'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['num_people']))
			{
				$this->group_function_params['pre_total_num_people'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['eating']))
			{
				$this->group_function_params['pre_total_eating'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['drinking']))
			{
				$this->group_function_params['pre_total_drinking'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['service']))
			{
				$this->group_function_params['pre_total_service'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['meeting_eating']))
			{
				$this->group_function_params['pre_total_meeting_eating'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['meeting_drinking']))
			{
				$this->group_function_params['pre_total_meeting_drinking'] += $temp;
			}
            if($temp=System::calculate_number($item['pre']['meeting_service']))
			{
				$this->group_function_params['pre_total_meeting_service'] += $temp;
			}
			$this->group_function_params['pre_total_meeting'] += System::calculate_number($item['pre']['meeting_service'] + $item['pre']['meeting_drinking'] + $item['pre']['meeting_eating']);
            $this->group_function_params['pre_total_party'] += System::calculate_number($item['pre']['service'] + $item['pre']['drinking'] + $item['pre']['eating']);
		}	

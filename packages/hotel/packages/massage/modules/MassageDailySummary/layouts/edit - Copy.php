	...
	<span id="mi_product_group_sample">
		<div id="mi_product_group_#xxxx#" style="width:100%;text-align:left;" >
            ...
            <span class="multi-input" style="display: none;">
                <input  name="mi_product_group[#xxxx#][time]" style="width:110px;" type="text" id="time_#xxxx#" readonly="readonly" class="readonly"/>
            </span>
			<span class="multi-input">
			...
			<span class="multi-input">
                ...
                <input  name="mi_product_group[#xxxx#][time_in_date]" style="width:81px;" type="text" id="time_in_date_#xxxx#" readonly="readonly" value="<?php echo Url::get('date')?Url::get('date'):'';?>" onchange="check_validate_time(#xxxx#);"/>
            </span>
			<span class="multi-input">
                ...
                <input  name="mi_product_group[#xxxx#][time_out_date]" style="width:81px;" type="text" id="time_out_date_#xxxx#" readonly="readonly" value="<?php echo Url::get('date')?Url::get('date'):'';?>" onchange="check_validate_time(#xxxx#);"/>
            </span>
			<span class="multi-input DeleteInput_#xxxx#">
                <img src="<?php echo Portal::template('core');?>/images/buttons/delete.gif" onclick="if(!confirm('[[.are_you_sure.]]')){return false};DeleteInput(#xxxx#);mi_delete_row($('mi_product_group_'+#xxxx#),'mi_product_group','#xxxx#','product_');delete_staff(#xxxx#);if(document.all)event.returnValue=false; else return false;" style="cursor:pointer;"/>
            </span>
            <br clear="all" /><!---->
		</div>
	</span> 
	...
	<table cellpadding="15" cellspacing="0" width="100%" border="0" bordercolor="#CCCCCC" class="table-bound">
		<tr>
            ...
            <td align="right" nowrap="nowrap" style="width: 40%; padding-right: 30px;">
            	<?php if(User::can_add(false,ANY_CATEGORY)){?><input name="save" type="submit" value="[[.Save_and_close.]]" class="w3-btn w3-orange w3-text-white" onclick="jQuery('.button-medium-save').css('display','none');" style="text-transform: uppercase; margin-right: 5px;"/><?php }?>
                <?php if(User::can_add(false,ANY_CATEGORY)){?><input name="save_stay" type="submit" value="[[.Save_stay.]]" class="w3-btn w3-blue w3-text-white" onclick="jQuery('.button-medium-save').css('display','none');" style="text-transform: uppercase; margin-right: 5px;"/><?php }?>
        ...
        <fieldset>
        	...
        	<table style="width: 65%;" border="0" cellspacing="0" cellpadding="2">
                ...
                <tr valign="middle">
                    ...
                    <td>
                        <select name="hotel_reservation_room_id" id="hotel_reservation_room_id" style="display: block; float:left;" onchange="updateRoomTraveller(this.value)"></select>
                        &nbsp;<span id="room_traveller_name" style="display: block; float:left; margin-left: 5px;" >[[|room_traveller_name|]]</span><span style="margin-left: 7px;display: block; float:left;"><input name="pay_with_room" type="checkbox" id="pay_with_room" onclick="check_pay_with_room(this);" style="display:block; padding:10px;" <?php if([[=pay_with_room=]]==1){ echo 'checked';}?> title="Trả tiền về phòng" value="1" /></span>
                    ...
                    <td align="left">
                        <input name="discount_amount" type="text" id="discount_amount" onchange="updateTotalPayment();" style="width:80px;" class="input_number"/>
                        <input name="discount" type="text" id="discount" onchange="updateTotalPayment();" style="width:40px;" class="input_number"/>% 
...
//        }
	...
	function countProductAmount(id){
        ...
        $('use_time_'+id).value = number_format(stringToNumber($('time_'+id).value)*$('quantity_'+id).value);
	}
	...
	function getProductFromCode(id,value){	 	   
	    if(typeof(product_array[value])=='object'){
            ...
            $('time_'+id).value = product_array[value]['use_time'];
			$('name_'+id).className = '';
...
function DeleteInput(xxxx)
{
    ...
    var amount = jQuery('#amount_'+xxxx).val();
    jQuery('#total_amount').val(number_format(to_numeric(total_amount) - to_numeric(amount)));
}

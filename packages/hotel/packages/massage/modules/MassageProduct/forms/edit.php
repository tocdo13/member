<?php
class EditMassageProductForm extends Form
{
	function EditMassageProductForm()
	{
		Form::Form('EditMassageProductForm');
		$this->add('massage_product.id',new TextType(false,'invalid_code',0,20)); 
		if(URL::get('type')!='MATERIAL' and URL::get('type')!='EQUIPMENT' and URL::get('type')!='TOOL')
		{
			$this->add('massage_product.price',new FloatType(true,'invalid_price','0','100000000000')); 
		}
		$this->add('massage_product'.'.type',new SelectType(false,'invalid_type',array('GOODS'=>'GOODS','PRODUCT'=>'PRODUCT','MATERIAL'=>'MATERIAL','EQUIPMENT'=>'EQUIPMENT','SERVICE'=>'SERVICE','TOOL'=>'TOOL'))); 
		$this->add('massage_product'.'.category_id',new IDType(true,'invalid_category_id','product_category')); 
		$this->add('massage_product'.'.unit_id',new IDType(true,'invalid_unit_id','unit')); 

		$languages = DB::select_all('language');
		foreach($languages as $language)
		{
			$this->add('massage_product'.'.name_'.$language['id'],new TextType(true,'invalid_name',0,2000)); 
		}
	}
	function on_submit()
	{
		if($this->check() and !is_array(URL::get('selected_ids')))
		{
			if(URL::get('selected_ids'))
			{
				...
				{
					$old_items = DB::select_all('massage_product','id in (\''.str_replace(',','\',\'',URL::get('selected_ids')).'\')');
				}
				else
				{
					$old_item = DB::select('massage_product','id=\''.URL::get('selected_ids').'\'');
			...
			if(isset($_REQUEST['mi_product']))
			{
				foreach($_REQUEST['mi_product'] as $key=>$record)
				{
					...
					$row = DB::select('massage_product','id=\''.$record['id'].'\'');
					if($record['id'] and isset($old_items[$record['id']]) and $row )
					{
						DB::update('massage_product',$record,'id=\''.$record['id'].'\'');
					...
					{
						$ids[] = DB::insert('massage_product',$record);  
					}
				}
			...
			foreach($old_items as $item)
			{
				...
				{
					DB::delete('massage_product','id=\''.$item['id'].'\'');
				}
			}

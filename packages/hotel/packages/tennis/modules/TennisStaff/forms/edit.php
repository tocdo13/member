<?php
class EditTennisStaffForm extends Form
{
	function EditTennisStaffForm()
	{
		Form::Form();
	...
	function on_submit(){
		if($this->check()){
			...
			if(Url::get('cmd')=='edit'){
				$id = Url::iget('id');
				DB::update('tennis_staff',$array,'id='.Url::iget('id'));
			}else{
				$id = DB::insert('tennis_staff',$array);
			} 
	...
	function draw()
	{
		...
		$item = TennisStaff::$item;
		if($item){
			//$item['birth_date'] = Date_Time::convert_orc_date_to_date($item['birth_date'],'/');
		...
		$this->map['title'] = (Url::get('cmd')=='add')?Portal::language('add_TennisStaff'):Portal::language('edit_TennisStaff');

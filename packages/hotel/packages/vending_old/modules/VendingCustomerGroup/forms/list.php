...
class ListCustomerGroupForm extends Form
{
	...
	{
		$cond = '1=1'
			.(URL::get('name')?' and vending_customer_group.name LIKE \'%'.URL::get('name').'%\'':'') 
			.((URL::get('cmd')=='delete' and is_array(URL::get('selected_ids')))?' and vending_customer_group.id in (\''.join(URL::get('selected_ids'),'\',\'').'\')':'')
		;
		DB::query('
			select 
			...
			where '.$cond.'
			order by vending_customer_group.structure_id
		');
		$items = DB::fetch_all();

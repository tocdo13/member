		...
		if($this->check())
        {
			...
			if(URl::get('group_deleted_ids')){
				...
				foreach($group_deleted_ids as $delete_id)
				{
					...
					DB::delete_id('vending_customer_contact',$delete_id);
				}
			}
				...
				foreach($_REQUEST['mi_contact_group'] as $key=>$record)
				{
					...
					if(!$empty)
					{
						...
						if($record['id'])
						{
							...
							DB::update('vending_customer_contact',$record,'id=\''.$id.'\'');
						}
						else
						{
							if(isset($record['id'])){
							...
							$description .= 'Edit [Contact name: '.$record['contact_name'].', Mobile: '.$record['contact_phone'].', Mobile: '.$record['contact_mobile'].', Email: '.$record['contact_email'].']<br>';
							DB::insert('vending_customer_contact',$record);
						}
					}
				}
			}			
	...
	function draw()
	{
		...
		if($item){
			...
			if(!isset($_REQUEST['mi_contact_group']))
			{
				$sql = '
					SELECT
						vending_customer_contact.*
					FROM
						vending_customer_contact
						inner join vending_customer on vending_customer.id = vending_customer_contact.customer_id
					WHERE 1>0
						AND vending_customer_contact.customer_id=\''.$item['id'].'\' 
					ORDER BY
						vending_customer_contact.id
				';
		...
		$groups = DB::fetch_all('SELECT ID,NAME as name FROM vending_CUSTOMER_GROUP WHERE '.IDStructure::child_cond(ID_ROOT,1).'');

	   ...
	   //System::debug($_REQUEST);exit();
		if(Url::get('act') != '' || Url::get('acction') == 1 || Url::get('cancel'))
        {
    		...
    		if($this->check())
            {
                ...
                //echo AutomaticVend::$department_id;
                $array += array(						
                ...
                $department_code_new = ($array['department_code']!='')?$array['department_code']:Url::get('department_code');
                $warehouse_id = DB::fetch('Select * from portal_department where department_code = \''.$department_code_new.'\' and portal_id = \''.PORTAL_ID.'\' ','warehouse_id');
                DeliveryOrders::get_delivery_orders($this->ve_res_id,'VENDING',$warehouse_id);      	
                $cave = DB::fetch('SELECT total from ve_reservation where id='.$ve_id); 
  		    }
            if(Url::get('act')=='payment')
            {
				...
				$tt = 'form.php?block_id='.BLOCK_PAYMENT.'&cmd=payment&id='.$ve_id.'&type=VEND&total_amount='.$cave['total'].'';
            ...
            elseif(Url::get('act')=='deposit')
            {
				...
				echo '<script src="packages/hotel/packages/reception/modules/includes/common01.js"></script>';
				echo '<LINK rel="stylesheet" href="packages/hotel/skins/default/css/jquery.windows-engine.css" type="text/css">';
				$tt = 'form.php?block_id='.BLOCK_PAYMENT.'&cmd=deposit&id='.$ve_id.'&type=VEND&total_amount='.$cave['total'].'';
                echo '<script>window.location.href = \''.$tt.'\'</script>'; 
            }
            else
            {

...
if($dept_id)
    ...
    $bars = DB::fetch_all('
    	...
    	WHERE
    		'.$cond.' AND department.id = (select id from department where code = \'VENDING\')
                    --Oanh comment: department.id = (select id from department where code = \'VENDING\')
            
            ');
            //);   //Oanh comment do thua );
            //System::debug($bars);    
    return $bars;
}
...
{ 
	$areas = DB::fetch_all('    SELECT 
                                ...
                                WHERE 
                                    --Oanh comment:department.parent_id = (Select id from department where code = \'VENDING\'
                                    department.id = (Select id from department where code = \'VENDING\')
                           ' );
                           //System::debug($areas);
    if(!User::is_admin())
    {

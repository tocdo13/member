...
{	
	...
	{
		return DB::fetch_all('select vending_customer_group.* from vending_customer_group where structure_id<>'.ID_ROOT.' order by structure_id and portal_id = \''.PORTAL_ID.'\'');
	}
    /**
     * Tu dong sinh module va category dung de phan quyen
     * prefix : ti?n t? c?a module
     * code : ket hop voi prefix de tao module name
     * name : sinh ra name cua category
     * package_name : ten package de gan module
     * category_name : ten category cha (vn or english do php design loi~ font ko viet dc TV =)) ), de gan cac category moi tao ra vao la con
     */
    static function update_module($prefix, $code, $name, $package_name, $category_name=false)
    {
     ...
     */
    static function delete_module($table_name,$id = false,$code = false)
    {
        if($id)
        {
            $cond = ' id =  '.$id;
            ...
            if($row = DB::fetch('Select * from '.$table_name.' where '.$cond))
            {
                ...
                if(isset($row['module_name']))
                {
                    ...
                    */        
                    ...
                    $count_module = DB::fetch('Select count(*) as acount from module where name = \''.$row['module_name'].'\' ','acount');
                    if($count_module==1)
                    {
                        //Lay ra id cua module can xoa
                        $module_id = DB::fetch('Select id from module where name = \''.$row['module_name'].'\' ','id');

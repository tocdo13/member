...
<div class="body">
	...
	<table width="100%" cellpadding="2" cellspacing="0" bordercolor="#C6E2FF" border="1" style="border-collapse:collapse">
		<tr class="table-header">
            ...
            <th nowrap width="150" align="center">[[.note.]]</th>
			...
			<th nowrap width="150" align="left">
		...
		<tr bgcolor="<?php if(URL::get('just_edited_id',0)==[[=items.id=]]){ echo '#EFFFDF';} else {echo 'white';}?>" valign="middle" <?php Draw::hover('#EFEEEE');?>>
			...
			<td nowrap align="left" onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=[[|items.id|]]';}else{just_click=false;}" style="cursor:pointer;">MN_[[|items.id|]]</td>
            <td nowrap align="left" onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=[[|items.id|]]';}else{just_click=false;}" style="cursor:pointer;">[[|items.code|]]</td>
            <td align="left" onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=[[|items.id|]]';}else{just_click=false;}" style="cursor:pointer;">
			  [[|items.room_name|]] ([[.date.]]:[[|items.arrival_time|]] - [[|items.departure_time|]] / [[.status.]]: [[|items.status|]])</td>
              <td nowrap align="center" onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=[[|items.id|]]';}else{just_click=false;}">[[|items.note|]]</td>
...
<td nowrap align="left"onclick="if(typeof(just_click)=='undefined' || !just_click){location='<?php echo URL::build_current();?>&id=[[|items.id|]]';}else{just_click=false;}" style="cursor:pointer;">
			...
			<?php if(User::can_edit(false,ANY_CATEGORY) && ([[=items.Status=]] !='CHECKOUT') )
			{
			?><td nowrap align="center">

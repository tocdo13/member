<?php
class EditHKWarehouseInvoiceForm extends Form
{
	function EditHKWarehouseInvoiceForm()
	{
		Form::Form('EditHKWarehouseInvoiceForm');
		$this->link_css(Portal::template('warehousing').'/css/invoice.css');
		$this->link_css('packages/core/skins/default/css/jquery.autocomplete.css');
		$this->link_js('packages/core/includes/js/jquery/jquery.autocomplete.js');
		if(Url::get('cmd')=='add'){
			$this->add('bill_number',new UniqueType(true,'invalid_bill_number','hk_wh_invoice','bill_number'));
		}
		...
		$this->add('product_group.num',new FloatType(true,'invalid_product_number',0,9999));		
	}
	function on_submit(){
		if($this->check()){
			if(isset($_REQUEST['mi_product_group']))
			{
				...
				if(Url::get('cmd')=='edit'){
					$id = Url::iget('id');
					DB::update('hk_wh_invoice',$array+array('last_modify_user_id'=>Session::get('user_id'),'last_modify_time'=>time()),'id='.Url::iget('id'));
				}else{
					$id = DB::insert('hk_wh_invoice',$array+array('user_id'=>Session::get('user_id'),'time'=>time()));
				}
				if(URl::get('group_deleted_ids'))
				{
					...
					{
						DB::delete_id('hk_wh_invoice_detail',$delete_id);
					}
				}	
				if(isset($_REQUEST['mi_product_group']))
				{	
					foreach($_REQUEST['mi_product_group'] as $key=>$record)
					{
						...
						if(!$empty)
						{
							...
							if($record['id'])
							{
								DB::update('hk_wh_invoice_detail',$record,'id=\''.$record['id'].'\'');
							}
							else
							{
								unset($record['id']);
								if(DB::exists('SELECT id FROM wh_product WHERE id=\''.$record['product_id'].'\'')){
									DB::insert('hk_wh_invoice_detail',$record);
								}
							}
						}
					}
	...
	function draw()
	{
		...
		if($item){
			...
			if(!isset($_REQUEST['mi_product_group']))
			{
				$sql = '
					SELECT
						...
						(hk_wh_invoice_detail.price*hk_wh_invoice_detail.num) as payment_price,
					...
					WHERE
						hk_wh_invoice_detail.invoice_id=\''.$item['id'].'\'
				';
			...
			if(!Url::get('bill_number')){
				$lastest_item = DB::fetch('SELECT id,bill_number FROM hk_wh_invoice where type=\''.Url::get('type').'\' ORDER BY bill_number DESC');

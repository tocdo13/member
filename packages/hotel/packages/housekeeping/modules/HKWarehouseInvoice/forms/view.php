<?php
class ViewHKWarehouseInvoiceForm extends Form
{
	function ViewHKWarehouseInvoiceForm()
	{
		Form::Form('ViewHKWarehouseInvoiceForm');
		...
		if($item){
			...
			$sql1 = '
				SELECT 
					...
					SUM(hk_wh_invoice_detail.price*hk_wh_invoice_detail.num) as total_amount
				FROM
					hk_wh_invoice_detail
					INNER JOIN warehouse_invoice ON warehouse_invoice.id = hk_wh_invoice_detail.invoice_id
				WHERE
					'.$cond.'
				GROUP BY 
					'.$group_by.'
			...
			$products = DB::fetch_all('
				SELECT
					hk_wh_invoice_detail.*,hk_wh_invoice_detail.price*hk_wh_invoice_detail.num as payment_price,wh_product.name_'.Portal::language().' as name
				FROM
					hk_wh_invoice_detail

<?php 
class AmenitiesInvoice extends Module
{
	function AmenitiesInvoice($row)
	{
		Module::Module($row);
		...
		if(User::can_view(false,ANY_CATEGORY))
		{
			if(URL::check('selected_ids') and is_array(URL::get('selected_ids')) and sizeof(URL::get('selected_ids'))>0 and User::can_delete(false,ANY_CATEGORY))
			{
				...
				if(sizeof(URL::get('selected_ids'))>1)
				{
					...
					$this->add_form(new DeleteSelectedAmenitiesInvoiceForm());
				}
				else
				{
					...
					$this->add_form(new DeleteAmenitiesInvoiceForm());
				}
			}
			else
    			if(URL::check(array('cmd'=>'checkout','id')) and DB::exists_id('housekeeping_invoice',URL::get('id')))
    			{
        			...
        			)
        			{
        				...
        				case 'delete':
        					require_once 'forms/delete.php';
        					$this->add_form(new DeleteAmenitiesInvoiceForm());break;
        				case 'edit':
        					require_once 'forms/edit.php';
        					$this->add_form(new EditAmenitiesInvoiceForm());break;
        				case 'add':
        					require_once 'forms/add.php';
        					$this->add_form(new AddAmenitiesInvoiceForm());break;
        				default: 
        					if(URL::check('id') and DB::exists_id('housekeeping_invoice',$_REQUEST['id']))
        					{
        						require_once 'forms/detail.php';
        						$this->add_form(new DetailAmenitiesInvoiceForm());
        					}
        					else
        					{
        						require_once 'forms/list.php';
        						$this->add_form(new ListAmenitiesInvoiceForm());
        					}
        					break;
        				}
        			}

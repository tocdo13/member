...
class EditEquipmentInvoiceForm extends Form
{
	...
	function on_submit()
	{
		if($this->check())
		{
				...
				foreach($_REQUEST['equipment'] as $key=>$record)
				{
					...
					if($old = DB::select('housekeeping_invoice_detail','invoice_id=\''.$invoice_id.'\' and product_id=\''.$record['product_id'].'\''))
					{
						...
						DB::query('
							...
							set quantity=quantity+'.$old['quantity'].'-'.intval($record['quantity']).'
							where 
						...
						DB::query('
							update housekeeping_equipment 
							set quantity=quantity-'.intval($record['quantity']).'
							where 
								room_id=\''.Url::get('room_id').'\' and
								product_id=\''.$record['product_id'].'\'
						');
			...
			System::log('edit','Edit compensation invoice #'.$id.' at room '.DB::fetch('select name from room where id=\''.Url::get('room_id').'\'','name'),
						'Code: <a href="?page=equipment_invoice&id='.$id.'">'.$id.'</a><br>
	...
	function draw()
	{
		...
		$sql = '
			select
				...
				COALESCE(housekeeping_invoice_detail.quantity,0) as quantity,
		...
		/*$rooms = DB::fetch_all('
			...
			order by
				room.name
		');*/
		//$this->map['room_id_list'] = String::get_list($rooms);

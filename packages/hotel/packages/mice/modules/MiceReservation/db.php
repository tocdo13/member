﻿<?php 
	class MiceReservationDB
    {
        static function get_total_room($id)
        {
            $sql='
                select 
    				reservation_room.*,
    		...
    		$row['time_arrival']=date('d/m/Y',$fromtime);
    	...
    	//----------------------------------------Tien phong`----------------------------------------------------
    			...
    			$row['total_amount'] = 0;
    			$row['room_price'] = Url::get('price')?(Url::get('price')):($row['price']);
    			if($row['show_price'] == 0)
                {
    			...
    			$sql = '
    				SELECT 
                        ...
                        ,room_status.change_price as total_amount
    					,room.name as room_name
    					,room_status.in_date
    			...
    			foreach($room_statuses as $k=>$v)
                {
                    if($c>$count_package_room)
                    {
                        if($row['net_price'] == '0')
                        {
                            $room_statuses[$k]['total_amount'] = ($v['change_price']/100*(100+$v['service_rate']))/100*(100+$v['tax_rate']);                     
                        }
                        if($row['net_price']==1)
                        {
                        ...
                        if($row['reduce_balance'] > 0)
                        {
                            $discount = $room_statuses[$k]['total_amount']*($row['reduce_balance']/100);                  
                        }else
                        {
                            $discount = 0;              
                        }
                        $total_amount = $room_statuses[$k]['total_amount'] - $discount;
                        
                        $items['ROOM_'.$v['room_status_id']]['net_amount'] = ($amount);
                        ...
                        $items['ROOM_'.$v['room_status_id']]['total_amount'] = ($total_amount);
                        ...
                        {
        					$items['ROOM_'.$v['room_status_id']]['description'] .= ' + '.(EXTRA_CHARGE_ON_SATURDAY).' '.HOTEL_CURRENCY.' '.Portal::language('friday');
        				}
        				$day[$n]['saturday_charge'] = '';
        				if(date('w',Date_Time::to_time($v['convert_date'])) == 6 and EXTRA_CHARGE_ON_SUNDAY > 0)
                        {
        					$items['ROOM_'.$v['room_status_id']]['description'] .= ' + '.(EXTRA_CHARGE_ON_SUNDAY).' '.HOTEL_CURRENCY.' '.Portal::language('saturday');
        				}
        				if(isset($holiday[date('d/m/Y',$d)]))
                        {
        					$items['ROOM_'.$v['room_status_id']]['description'] .= ' + '.($holiday[date('d/m/Y',$d)]['charge']).' '.HOTEL_CURRENCY.' '.$holiday[date('d/m/Y',$d)]['name'];
        				}
        				if($row['foc'] !='')
                        {
                ...
                //System::debug($row['extra_services']);exit();
    			if(!empty($row['extra_services']))
                {
    				foreach($row['extra_services'] as $s_key=>$s_value)
                    {
                        if($s_value['package_sale_detail_id']==0)
                        {
                            if($row['net_price'] == '0')
                            {
                                $s_value['total_amount'] = ($s_value['total_amount']/100*(100+$s_value['service_rate']))/100*(100+$s_value['tax_rate']);                     
                            }
                            ...
                            $total_amount = $s_value['total_amount'];
                            
    						$items['EXTRA_SERVICE_'.$s_key]['net_amount'] = ($amount);
                            ...
                            $items['EXTRA_SERVICE_'.$s_key]['total_amount'] = ($total_amount);
    				...
    				foreach($minibars as $k=>$minibar)
                    {
    					...
    					$minibar_details = DB::fetch_all('
                                						SELECT 
                                                            ...
                                                            (HK_I_D.price * HK_I_D.quantity) AS total_amount
                                						FROM 
                        ...
                        if($package_amount_minibar<=0)
                        {
                            ...
                            $total_amount = $minibar['total'];
                            
    						$items['MINIBAR_'.$k]['net_amount'] = ($amount);
                            ...
                            $items['MINIBAR_'.$k]['total_amount'] = ($total_amount);
    			...
    			if($laundrys = DB::fetch_all())
                {
                    ...
                    {	
    					...
    					if($package_amount_laundry<=0)
                        {
                            ...
                            $total_amount = $laundry['total'];
                            
    						$items['LAUNDRY_'.$k]['net_amount'] = ($amount);
                            ...
                            $items['LAUNDRY_'.$k]['total_amount'] = ($total_amount);
    	   ...
    	   //--------------------------------------------/laundry--------------------------------------------------------
    			...
    			if($compensated_items = DB::fetch_all($sql_compensated_item))
                {
    				foreach($compensated_items as $k=>$compensated_item)
                    {
                        ...
                        $total_amount = $compensated_item['total'];
                        
    					$items['EQUIPMENT_'.$k]['net_amount'] = ($amount);
    					...
    					$items['EQUIPMENT_'.$k]['total_amount'] = ($total_amount);
            ...
            //-----------------------ticket--------------------------------------------------
    			...
    			if($ticket_resrs = DB::fetch_all($sql))
                {
    				foreach($ticket_resrs as $bk=>$reser)
                    {
    					...
    					$items['TICKET_'.$bk]['net_amount'] = ($amount);
            ...
            //-----------------------package--------------------------------------------------
    			...
    			if($package = DB::fetch_all($sql))
                {
    				foreach($package as $bk=>$reser)
                    {
    					...
    					$items['PACKAGE_'.$bk]['net_amount'] = ($amount);
    		...
    		//---------------------------------VEND-----------------------------------------
                ...
                if($ve_resrs = DB::fetch_all($sql))
                {
    				foreach($ve_resrs as $bk=>$reser)
                    {
    					...
    					$items['VE_'.$bk]['net_amount'] = ($amount);
            ...
            //---------------------------------/VEND----------------------------------------
    			...
    			if($bar_resrs = DB::fetch_all($sql))
                {
    				foreach($bar_resrs as $bk=>$reser)
                    {
                        ...
                        $items['BAR_'.$bk]['net_amount'] = ($amount);
    					...
    					$items['BAR_'.$bk]['total_amount'] = ($amount);
            ...
            //----------------------------------------------------KARAOKE-------------------------------------------------------			
    			...
    			if($karaoke_resrs = DB::fetch_all($sql))
                {
    				foreach($karaoke_resrs as $bk=>$reser)
                    {
    					...
    					$items['KARAOKE_'.$bk]['net_amount'] = ($amount);
    	   ...
    	   //-------------------------------------------------Other services----------------------------------------
    			...
    			foreach($row['services'] as $s_key=>$s_value)
                {
    				if($s_value['type']=='SERVICE')
                    {
    					$percent = 100;$status = 0;
    					$amount = $s_value['amount'];
    					$items['SERVICE_'.$s_key]['net_amount'] = ($amount);
    			...
    			foreach($row['services'] as $s_key=>$s_value)
                {
    				if($s_value['type']=='ROOM')
                    {
    					$percent = 100;$status = 0;
    					$amount = $s_value['amount'];
    					$items['ROOM_SERVICE_'.$s_key]['net_amount'] = ($amount);
    	...
    	//----------------------------------------/Other services------------------------------------------------		
    			...
    			if($row['total_massage_amount'] = DB::fetch_all($sql_massage))
                {
    			    foreach($row['total_massage_amount'] as $mg=>$mgr)
                    {
        					...
        					$items['MASSAGE_'.$mg]['net_amount'] = ($amount);
    			...
    			if(URL::get('tennis_invoice'))
                {
    				...
    				if($row['total_tennis_amount'] = DB::fetch($sql_tennis,'total_amount') and HAVE_TENNIS)
                    {
    					...
    					$items['TENNIS_'.$id]['net_amount'] = ($amount);
    			...
    			if(URL::get('swimming_pool_invoice'))
                {
    				...
    				if($row['total_swimming_pool_amount'] = DB::fetch($sql_swimming_pool,'total_amount') and HAVE_SWIMMING)
                    {
    					...
    					$items['SWIMMING_POOL_'.$id]['net_amount'] = ($amount);
    			...
    			if(URL::get('shop_invoice'))
                {
    				...
    				if($row['total_shop_amount'] = DB::fetch($sql_shop,'total_amount'))
                    {
    					...
    					$items['SHOP_'.$id]['net_amount'] = ($amount);
    			...
    			if($row['room_id'])
                {
    				...
    				if($phones = DB::fetch_all($sql_p))
                    {
    					foreach($phones as $t_key=>$t_value)
                        {
    						...
    						$items['TELEPHONE_'.$t_key]['net_amount'] = ($amount);
                            ...
                            $items['TELEPHONE_'.$t_key]['total_amount'] = ($amount);
    			...
    			if($row['reduce_amount'] != '' && $row['reduce_amount']>0){
    				...
    				$items['DISCOUNT_'.$id]['net_amount'] = ($amount);
                    ...
                    $items['DISCOUNT_'.$id]['total_amount'] = ($amount);
    			...
    			if($row['deposit']>0)
                {
                    ...
                    $status = 0;
    				$amount =$row['deposit'];
    				$items['DEPOSIT_'.$id]['net_amount'] = ($amount);
                    ...
                    $items['DEPOSIT_'.$id]['total_amount'] = ($amount);

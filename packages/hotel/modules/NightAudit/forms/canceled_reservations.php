<?php
class CanceledReservationsForm extends Form
{
	function CanceledReservationsForm()
	{
		Form::Form('CanceledReservationsForm');
	...
	function draw()
	{
		...
		$cond = '
			...
			AND (reservation_room.lastest_edited_time >= '.Date_Time::to_time($_SESSION['night_audit_date']).' AND reservation_room.lastest_edited_time < '.(Date_Time::to_time($_SESSION['night_audit_date']) + 24*3600).')
		';
		...
		foreach($items as $key=>$value){
            ...
            $items[$key]['lastest_edited_time'] = date('d/m/Y H:i\'',$value['lastest_edited_time']);			
		}
		$this->map['items'] = $items;
		$this->parse_layout('canceled_reservations',$this->map);
	}
}
?>
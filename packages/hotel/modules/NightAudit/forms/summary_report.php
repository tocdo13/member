...
class SummaryReportForm extends Form
{
	...
	{ 
		...
		$this->cond = '
			room_status.in_date = \''.Date_Time::to_orc_date($_SESSION['night_audit_date']).'\'
		';
	}
	function on_submit(){
		$this->update_night_audit();
	...
	function draw()
	{
		...
		$sql = '
			...
			WHERE
                reservation_room.status =\'CHECKOUT\' and
		...
		foreach($items as $value){
			...
			if($value['r_status'] == 'OCCUPIED' and Date_Time::convert_orc_date_to_date($value['arrival_time'],'/') == $_SESSION['night_audit_date']){
				$this->map['today_check_ins']++;
			}
			if($value['status'] == 'CHECKOUT' and Date_Time::convert_orc_date_to_date($value['departure_time'],'/') == $_SESSION['night_audit_date']){
				$this->map['today_check_outs']++;
				if($value['house_status']=='DIRTY'){
					$this->map['checked_out_rooms_marked_dirty']++;
				}
			}
			if($value['status'] == 'BOOKED' and Date_Time::convert_orc_date_to_date($value['arrival_time'],'/') < $_SESSION['night_audit_date']){
		...
		////////////////////////////////////////////BOOKED////////////////////////////////////////////////////
			$sql = '
				...
				WHERE
					...
					AND (	reservation_room.time>='.Date_Time::to_time($_SESSION['night_audit_date']).'
							and reservation_room.time<'.(Date_Time::to_time($_SESSION['night_audit_date'])+24*3600).')
			';
			...
			$sql = '
				...
				WHERE
					...
					AND (	reservation_room.lastest_edited_time>='.Date_Time::to_time($_SESSION['night_audit_date']).'
							and reservation_room.lastest_edited_time<'.(Date_Time::to_time($_SESSION['night_audit_date'])+24*3600).')
			';
		...
		$sql = '
			...
			WHERE 
				reservation.portal_id = \''.PORTAL_ID.'\' AND room_status.in_date = \''.Date_Time::to_orc_date($_SESSION['night_audit_date']).'\' AND house_status=\'REPAIR\'';
		...
		$sql = '
			...
			where 
                ...
                reservation.portal_id = \''.PORTAL_ID.'\'
				AND reservation_room.time_out <= \''.Date_Time::to_time($_SESSION['night_audit_date']).'\'
                AND reservation_room.time_out > \''.(Date_Time::to_time($_SESSION['night_audit_date']) -86400).'\'
		...
		$sql = '
			...
			WHERE
				...
				AND reservation_room.arrival_time = \''.Date_Time::to_orc_date($_SESSION['night_audit_date']).'\'
		';
		if($item = DB::fetch($sql)){
		...
		$sql = '
			SELECT
				SUM(housekeeping_invoice.total_before_tax) AS id
			FROM
			...
			WHERE
				...
				AND housekeeping_invoice.time >= '.Date_Time::to_time($_SESSION['night_audit_date']).'
				AND housekeeping_invoice.time <= '.(Date_Time::to_time($_SESSION['night_audit_date'])+24*3600).'
		';
		if($item = DB::fetch($sql)){
			$this->map['total_revenue'] += $item['id'];
		...
		$sql = '
			SELECT 
				sum(bar_reservation.total_before_tax) as id
			FROM 
				bar_reservation
			WHERE 
				 ('.Date_Time::to_time($_SESSION['night_audit_date']).'<= departure_time AND departure_time <'.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).')
		...
		if(HAVE_KARAOKE){
			$sql = '
				SELECT 
					sum(ka_reservation.total_before_tax) as id
				  FROM 
					ka_reservation
				  ...
				  WHERE 
					reservation.portal_id = \''.PORTAL_ID.'\'
					AND ka_reservation.time_out>='.Date_Time::to_time($_SESSION['night_audit_date']).'
					AND ka_reservation.time_out < '.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).'
			';
			if($item = DB::fetch($sql)){
				$this->map['total_revenue'] += $item['id'];
			...
			$sql = '
				SELECT 
				  ...
				  WHERE 
					...
					AND massage_product_consumed.time_out>='.Date_Time::to_time($_SESSION['night_audit_date']).'
					AND massage_product_consumed.time_out < '.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).'
			';
			if($item = DB::fetch($sql)){
				$this->map['total_revenue'] += $item['id'];
        ...
        $this->map['telephone_revenue'] = 0;	
		if(TELEPHONE_CONFIG){
            $sql = '
					SELECT
						SUM(telephone_report_daily.total_before_tax) AS id
					FROM
					...
					WHERE
                        telephone_report_daily.portal_id = \''.PORTAL_ID.'\' and 
						telephone_report_daily.hdate >='.Date_Time::to_time($_SESSION['night_audit_date']).' and telephone_report_daily.hdate <= '.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).'
				';
			if($item = DB::fetch($sql)){
				$this->map['telephone_revenue'] += $item['id'];
				$this->map['telephone_revenue'] = System::display_number($item['id']);
			}
		}
...
//				  WHERE 
//				   reservation.portal_id = \''.PORTAL_ID.'\'	
//				   AND tennis_reservation_court.time_out>='.Date_Time::to_time($_SESSION['night_audit_date']).'
//				   AND tennis_reservation_court.time_out < '.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).'
//			';
//			if($item = DB::fetch($sql)){
//				$this->map['total_revenue'] += $item['id'];
...
//				  WHERE 
//					reservation.portal_id = \''.PORTAL_ID.'\'
//					AND swimming_pool_reservation_pool.time_out>='.Date_Time::to_time($_SESSION['night_audit_date']).'
//					AND swimming_pool_reservation_pool.time_out < '.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).'
//			';			
//			if($item = DB::fetch($sql)){
//				$this->map['total_revenue'] += $item['id'];
...
//		}
		$sql = 	'	
			select 
				...
				where 
					...
					AND extra_service_invoice_detail.in_date = \''.Date_Time::to_orc_date($_SESSION['night_audit_date']).'\'
					AND extra_service_invoice_detail.used = 1
					AND (extra_service.code <> \'C.I.EARLY\' AND extra_service.code <> \'C.O.LATE\')
		';
        ...
        $sql = 	'	
			select 
					sum(ticket_invoice.total) as id
				from 
					ticket_invoice
				...
				where 
					...
					AND ticket_invoice.time = \''.Date_Time::to_time($_SESSION['night_audit_date']).'\'
		';
        
        $this->map['ticket_revenue'] = 0;
		if($item = DB::fetch($sql)){
			$this->map['total_revenue'] += $item['id'];
			$this->map['ticket_revenue'] = System::display_number($item['id']);
		}
        ...
        $sql = '
			SELECT 
				sum(ve_reservation.total_before_tax) as id
			FROM 
				ve_reservation
			WHERE 
				 ('.Date_Time::to_time($_SESSION['night_audit_date']).'<= time AND time <'.(Date_Time::to_time($_SESSION['night_audit_date'])+(24*3600)).')
				 AND ve_reservation.portal_id = \''.PORTAL_ID.'\'
				 AND ve_reservation.status <> \'CANCEL\'
			';
        $this->map['ve_revenue'] = 0;
		if($item = DB::fetch($sql)){
			$this->map['total_revenue'] += $item['id'];
			$this->map['ve_revenue'] = System::display_number($item['id']);
		}
		/*$sql = '
			select 
	...
	function update_night_audit(){
		...
		DB::update('night_audit',array('time'=>$closed_time,'status'=>'CHECKED'),'in_date=\''.Date_Time::to_orc_date($_SESSION['night_audit_date']).'\'');
		$this->create_next_date($_SESSION['night_audit_date']);
		if(isset($_SESSION['night_audit_date'])){
			unset($_SESSION['night_audit_date']);
		}
	}
	function create_next_date($date){
		$date = Date_Time::to_time($date) + (24*3600);
		$date = Date_Time::to_orc_date(date('d/m/Y',$date));
		if(!DB::exists('SELECT id FROM night_audit WHERE in_date = \''.$date.'\' AND portal_id = \''.PORTAL_ID.'\'')){
			DB::insert('night_audit',array('in_date'=>$date,'status'=>'UNCHECK','portal_id'=>PORTAL_ID));
		}
	}
}
?>
<?php
class LateCheckinForm extends Form
{
	function LateCheckinForm()
	{
		Form::Form('LateCheckinForm');
	...
	function draw()
	{
		$this->map = array();
		$cond = '
			reservation_room.status = \'CHECKIN\' AND reservation_room.confirm = 1 AND extra_service_invoice.late_checkin is not null
		...
		$sql = '
			...
			(
				...
				from 
					...
					inner join reservation on reservation.id=reservation_room.reservation_id
					inner join room_status on room_status.reservation_room_id=reservation_room.id
                    inner join extra_service_invoice on extra_service_invoice.reservation_room_id=reservation_room.id					left outer join tour on tour.id=reservation.tour_id
					left outer join room on room.id=reservation_room.room_id
					left outer join room_level on room_level.id=room.room_level_id 
					left outer join reservation_traveller on reservation_room.id=reservation_traveller.reservation_room_id
					left outer join traveller on reservation_traveller.traveller_id=traveller.id
		...
		$this->parse_layout('late_checkin',$this->map);
	}
}
?>
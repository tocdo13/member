<?php
class BarCancelForm extends Form
{
	function BarCancelForm()
	{
		Form::Form('BarCancelForm');
	...
	function draw()
	{
		$cond = '
                ...
                and bar_reservation.cancel_time > \''.(Date_Time::to_time($_SESSION['night_audit_date']) -86400).'\'
                and bar_reservation.cancel_time <= \''.Date_Time::to_time($_SESSION['night_audit_date']).'\'
                ';
		$item_per_page = 200;
		DB::query('
		...
		$this->parse_layout('bar_cancel',array(
				'items'=>$items,
				'paging'=>$paging,
			));
	}

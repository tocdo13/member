...
class ProductPriceDB
{
    ...
    static function export_cache()
	{
        ...
        $department = DB::fetch_all('Select portal_department.department_code as id, department.parent_id as dpm_parent_id from portal_department LEFT JOIN department ON portal_department.department_code = department.code where portal_department.portal_id = \''.$portal_id.'\'');
        //System::debug($department);
        $parent_res_department = DB::fetch('SELECT id FROM department WHERE code=\'RES\'');
        $is_child_restaurant = 0;
		require_once 'packages/hotel/includes/php/product.php';
        foreach($department as $key => $value)
        {
            if($value['dpm_parent_id']==$parent_res_department['id']){
                $is_child_restaurant = 1;
            }
            else{
                $is_child_restaurant = 0;
            }
            $product_price_list = Product::get_product_price($portal_id,$key,$is_child_restaurant);
            $str = "product_array={'':''";
            ...
            $name = 'cache/data/'.str_replace('#','',$portal_id).'/'.$key.'_'.str_replace('#','',$portal_id).'.js';
            if(User::is_admin())
            {
                echo $name;
            }
            //ProductPriceDB::save_file($name,$str);
    ...
    static function save_file($name,$content)
	{
        $name = ProductPriceDB::make_dir($name);		
		$hand = fopen($name,'w+');
		fwrite($hand,$content);
		fclose($hand);
	}

...
<div class="setting-bound1">
	<form name="TableRestore" method="post">
        <br/>
        <div align="right">
            <div align="center" style="font-size:24px;font-weight:bold">[[.Recovery_data_at.]] <?php echo date('H:i d/m/Y',time());?></div>
        </div>
        <br />

    	<fieldset>
            <select name="folders" id="folders" onchange="TableRestore.submit();"></select>
            <input  name="Restore" type="submit" id="Restore" value=" [[.Restore.]] "/>
            <input  name="backup" type="button" id="backup" value=" [[.backup.]] " onclick="window.location='<?php echo Url::build_current(array());?>';"/>
        </fieldset>
        <br />
		...
		<table width="100%" cellpadding="5" cellspacing="0" border="1" bordercolor="#CCCCCC" style="border-collapse:collapse">
		  ...
		  <?php $i=0;?>
			<?php foreach([[=tables=]] as $key=>$value){?>
			  <?php $i++;?>
			<tr bgcolor="<?php {echo 'white';}?>" valign="middle" style="cursor:hand;">

...
class ModeratorDB
{
	...
	function get_privilege($account_id,$portal_id)
	{
		$privileges = DB::fetch_all('
			SELECT
				privilege.*,
				privilege_module.*,
		...
		$privilege_groups = DB::fetch_all('
			...
			WHERE
				account_privilege_group.account_id=\''.$account_id.'\'
				AND account_privilege_group.portal_id = \''.$portal_id.'\'
		');
		foreach($privilege_groups as $key=>$value)
		{

...
jQuery.Window = (function()  {
	...
	// static constants
	var VERSION = "4.04";           // the version of current plugin
	...
	function selectWindow(wnd) {
		...
		if( typeof wnd == 'object' ) { // it's window instance
			...
			lastSelectedWindow = jQuery("#"+wnd).get(0).windowInstance;
		}
	...
	function pushWindow(wnd) {
		if( typeof wnd == 'object' ) { // it's window instance
			...
			windowStorage(jQuery("#"+wnd).get(0).windowInstance);
		}
	}
	...
	function pushMinWindow(windowId) {
		minWindowStorage.push(jQuery("#"+windowId).get(0).windowInstance);
	}
	...
	function constructor(caller, options) {
		...
		var options = jQuery.extend({
			...
			resizable: false,              // [boolean:true] to control window resizable
		...
		function initialize(instance) {
			...
			realCaller.append("<div id='"+windowId+"' class='window_panel "+options.containerClass+"'></div>");
			...
			// build header html
			var headerHtml = "<div class='window_header window_header_normal ui-widget-header no-resizable "+options.headerClass+"'>"+
			...
			if( options.showFooter ) {
				container.append("<div class='window_footer ui-widget-content no-draggable "+options.footerClass+"'><div></div></div>");
		...
		function maximize(bImmediately, bNoSaveDisplay) {
			if( !jQuery.browser.msie ) { // in IE, must do hide scrollbar routine after animation finished
...
jQuery.window.setAnimationSpeed = jQuery.Window.setAnimationSpeed;

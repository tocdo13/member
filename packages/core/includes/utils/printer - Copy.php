...
class Printer 
{
    ...
    function write_r( $items, $group, $table, $department, $bill_title = 'ORDER ' ) {
		...
		printer_draw_text(Printer::$handler,'Nhan vien:',$left,$top);	
		printer_draw_text(Printer::$handler,Printer::convert(Session::get('user_id')),($left+130),$top);
		$top += (2*$font_width_title);
		printer_select_font(Printer::$handler, $font_title);		
		...
		printer_draw_text(Printer::$handler,'Thoi gian '.' :'.date('H:i\' d/m/Y'),$left,$top);
		...
		printer_draw_text(Printer::$handler,'Ten mon (Ghi chu)',$left+55,$top);	
		$top+=(3.5*$font_width_nomarl);
		printer_draw_line(Printer::$handler, $left,$top,$right,$top);
		...
		foreach($items as $key=>$value)
		{
			if(isset($value['quantity']))
			{
                ...
                foreach($arr_name as $kn => $vn)
                {
                    if(!$bol)
                        ...
                        $top += (3*$font_width_nomarl);
                    printer_draw_text(Printer::$handler,Printer::convert($vn),$left+55,$top);
                }
                
                /*if($value['note'])
				{
					$top += (3*$font_width_nomarl);
					printer_select_font(Printer::$handler,$font_nomarl_italic);
                    $arr_note = break_string($value['note']);
                    $bol = 0;
                    foreach($arr_note as $kn => $vn)
                    {
                        if(!$bol)
                            $bol = 1;
                        else
			...
			{    
                $top += (3*$font_width_nomarl);
				printer_select_font(Printer::$handler, $font_nomarl);		
				printer_draw_text(Printer::$handler,'HUY '.$value['remain'],($left+6),$top);
                ...
                foreach($arr_name as $kn => $vn)
                {
                    if(!$bol)
                        ...
                        $top += (3*$font_width_nomarl);
                    printer_draw_text(Printer::$handler,Printer::convert($vn),$left+95,$top);
                }			

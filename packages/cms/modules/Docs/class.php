...
class Docs extends Module
{
	...
	{
		Module::Module($row);
        System::set_page_title("Documents");
		require_once 'db.php';
		...
		if(User::can_view(false,ANY_CATEGORY))
		{
			...
			case 'view':
				$this->view_cmd();
				break;
            /** giap.ln them truong hop hien thi chuc nang quan ly cong viec cho moi thanh vien **/
            case 'view_module':
				$this->view_module();
				break;
            case 'edit_module':
    ...
    function view_module()
    {
        if(User::can_view(false,ANY_CATEGORY))
        {
            require_once 'forms/view_module.php';
            $this->add_form(new ViewModuleForm());
        }
        else
        {
            Url::access_denied();
    ...
    function edit_module()
    {
        ...
        {
            require_once 'forms/edit_module.php';
            $this->add_form(new EditModule());
        }
        else
        {
            Url::access_denied();
    ...
    function edit_form()
    {
        if(User::can_edit(false,ANY_CATEGORY))
        {
            ...
            $this->add_form(new EditModuleForm());
        }
        else
        {
            Url::access_denied();

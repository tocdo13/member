	...
	<!--LIST:categories-->
		<li class="menumain">
			...
			<ul>
				...
				<li><a href="<?php echo Url::build('home',array('category_id'=>[[=categories.id=]])); ?>"> [[.Home_page.]] [[|categories.name_1|]]</a></li>
				<li></li>
				<!--IF:cond2([[=categories.child=]])-->
					<?php $group = false;?>
					<!--LIST:categories.child-->
						<?php	if($group != [[=categories.child.group_name=]]){if($group) echo '<li></li>';$group = [[=categories.child.group_name=]];}if(![[=categories.child.url=]]) [[=categories.child.url=]] = 'javascript:void(0)';?>
